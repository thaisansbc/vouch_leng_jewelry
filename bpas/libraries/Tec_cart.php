...
class Tec_cart
{
    ...
    protected function _save_cart()
    {
        ...
        foreach ($this->_cart_contents as $key => $val) {
            ...
            $this->_cart_contents['cart_total']         += $this->bpas->formatDecimal(($val['price'] * $val['qty']), 4);
            $this->_cart_contents['total_item_tax']     += $this->bpas->formatDecimal(($val['tax'] * $val['qty']), 4);
            $this->_cart_contents[$key]['row_tax']  = $this->bpas->formatDecimal(($this->_cart_contents[$key]['tax'] * $this->_cart_contents[$key]['qty']), 4);
            $this->_cart_contents[$key]['subtotal'] = $this->bpas->formatDecimal(($this->_cart_contents[$key]['price'] * $this->_cart_contents[$key]['qty']), 4);
        }
    ...
    public function cart_data($re = false)
    {
        ...
        foreach ($citems as &$value) {
            $value['price']    = $this->bpas->convertMoney($value['price']);
            $value['subtotal'] = $this->bpas->convertMoney($value['subtotal']);
            if ($this->has_options($value['rowid'])) {
                $value['options'] = $this->product_options($value['rowid']);
                foreach ($value['options'] as &$opt_value) {
                    $opt_value['price'] = $this->bpas->convertMoney($opt_value['price']);
                }
            }
        }
        $total     = $this->bpas->convertMoney($this->total(), false, false);
        $shipping  = $this->bpas->convertMoney($this->shipping(), false, false);
        $order_tax = $this->bpas->convertMoney($this->order_tax(), false, false);
        $cart      = [
            'total_items'        => $this->total_items(),
            'total_unique_items' => $this->total_items(true),
            'contents'           => $citems,
            'total_item_tax'     => $this->bpas->convertMoney($this->total_item_tax()),
            'subtotal'           => $this->bpas->convertMoney($this->total() - $this->total_item_tax()),
            'total'              => $this->bpas->formatMoney($total, $this->selected_currency->symbol),
            'shipping'           => $this->bpas->formatMoney($shipping, $this->selected_currency->symbol),
            'order_tax'          => $this->bpas->formatMoney($order_tax, $this->selected_currency->symbol),
            'grand_total'        => $this->bpas->formatMoney(($this->bpas->formatDecimal($total) + $this->bpas->formatDecimal($order_tax) + $this->bpas->formatDecimal($shipping)), $this->selected_currency->symbol),
        ];
        ...
        if (!empty($this->Settings->tax2)) {
            if ($order_tax_details = $this->site->getTaxRateByID($this->Settings->default_tax_rate2)) {
                if ($order_tax_details->type == 2 || $order_tax_details->rate == 0) {
                    $order_tax = $this->bpas->formatDecimal($order_tax_details->rate, 4);
                } elseif ($order_tax_details->type == 1) {
                    $order_tax = $this->bpas->formatDecimal(((($this->total()) * $order_tax_details->rate) / 100), 4);
                }
                return $order_tax;
            }
        }
    ...
    {
        return $this->bpas->formatDecimal($this->shop_settings->shipping, 4);
    }

    public function total()
    {
        return $this->bpas->formatDecimal($this->_cart_contents['cart_total'], 4);
    }

    public function total_item_tax()
    {
        return $this->bpas->formatDecimal($this->_cart_contents['total_item_tax'], 4);
    }
