...
class MY_DB_mysqli_driver extends CI_DB_mysqli_driver 
{
    ...
    */
    ...
    public function update($table = '', $set = NULL, $where = NULL, $limit = NULL)
    {
        ...
        $previous_values = null; // Read current values as previous values
        if(empty($where)){
    ...
    public function trail($status, $event, $table, $set = NULL, $previous_values = NULL)
    {
        ...
        if($event == 'update') {
            ...
            if((empty($previous_values) && empty($set)) || empty($set)) return 1;
        }
        ...
        return parent::insert('user_audit_trails' ,
            [
                ...
                // 'user_id'    => !empty($this->CI->config->item('auth')['user_id']) ? $this->CI->config->item('auth')['user_id'] : 0,
                'event'      => $event,
                'table_name' => $table,
