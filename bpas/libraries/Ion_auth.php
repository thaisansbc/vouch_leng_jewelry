...
class Ion_auth
{
        ...
        if ($this->auth_model->forgotten_password($identity)) {   //changed
            ...
            if ($user) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
                    ...
                    try {
                        if ($this->bpas->send_email($user->email, $subject, $message)) {
                            $this->set_message('forgot_password_successful');
                            return true;
                        } else {
    ...
    public function forgotten_password_complete($code)
    {
        ...
        if ($new_password) {
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                try {
                    if ($this->bpas->send_email($profile->email, $subject, $message)) {
                        $this->set_message('password_change_successful');
                        $this->auth_model->trigger_events(['post_password_change', 'password_change_successful']);
                        return true;
                    } else {
        ...
        if (!$email_activation || $active == '1') {
            ...
            if ($id !== false) {
                if ($notify) {
                    ...
                    try {
                        $this->bpas->send_email($email, $subject, $message);
                    } catch (Exception $e) {
                        $this->set_error($e->getMessage());
                    }
                }
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                try {
                    if ($this->bpas->send_email($email, $subject, $message)) {
                        $this->auth_model->trigger_events(['post_account_creation', 'post_account_creation_successful', 'activation_email_successful']);
                        $this->set_message('activation_email_successful');
                        return $id;
                    }
    ...
    public function EmployeeRegister($username, $password, $email, $additional_data = [], $active = false, $notify = false)
    {
        $this->auth_model->trigger_events('pre_account_creation');
        ...
        if (!$email_activation || $active == '1') {
            $id = $this->auth_model->EmployeeRegister($username, $password, $email, $additional_data, $active);
            if ($id !== false) {
                if ($notify) {
                    $this->load->library('parser');
                    $parse_data = [
