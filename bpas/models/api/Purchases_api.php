...
class Purchases_api extends CI_Model
{
    ...
    {
        if ($filters['suppplier']) {
            $this->db->where('suppplier', $filters['suppplier']);
        }
        if ($filters['suppplier_id']) {
            $this->db->where('suppplier_id', $filters['suppplier_id']);
        }
        if ($filters['start_date']) {
            $this->db->where('date >=', $filters['start_date']);
        }
    ...
    public function getExpense($filters)
    {
        if (!empty($expenses = $this->getQuotes($filters))) {
            return array_values($expenses)[0];
        }
        return false;
    }
     public function getExpenseItems($expense_id)
    {
        return $this->db->get_where('expense_items', ['expense_id' => $expense_id])->result();
    }

     public function countExpenses($filters = [], $ref = null)
    {
        if ($filters['suppplier']) {
            $this->db->where('suppplier', $filters['suppplier']);
        }
