...
class Sales_order_store_model extends CI_Model
{
    ...
    public function addSale($data = [], $items = [], $payment = [], $si_return = [])
    {
        ...
        if ($this->db->insert('sales_order', $data)) {
            ...
            if ($this->site->getReference('spr') == $data['reference_no']) {
                $this->site->updateReference('spr');
            }
    ...
    public function deleteSale($id)
    {
        ...
        if ($this->db->delete('sale_order_items', ['sale_id' => $id]) && 
    ...
    {
        $this->db->select('sale_order_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.image, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code, products.second_name as second_name, products.unit as base_unit_id, products.type as product_type, units.code as base_unit_code')
    ...
    public function getAllInvoiceItemsWithDetails($sale_id)
    {
        ...
        $q = $this->db->get_where('sale_order_items', ['sale_id' => $sale_id]);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function updateSale($id, $data, $items = [])
    {
        ...
        if ($this->db->update('sales_order', $data, ['id' => $id]) && 
            $this->db->delete('sale_order_items', ['sale_id' => $id])) {
            foreach ($items as $item) {
                $item['sale_id'] = $id;
                $this->db->insert('sale_order_items', $item);
    ...
    public function updateStatus($id, $status, $note)
    {
        ...
        if ($this->db->update('sales_order', ['sale_status' => $status, 'note' => $note], ['id' => $id]) && $this->db->delete('costing', ['sale_id' => $id])) {
            if ($status == 'completed' && $sale->sale_status != 'completed') {
                foreach ($items as $item) {
                    $item = (array) $item;
