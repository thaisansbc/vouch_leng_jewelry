...
class Transfers_model extends CI_Model
{
    ...
    public function addTransfer($data = [], $items = [], $stockmoves = [], $accTrans=[])
    {
        $this->db->trans_start();
        ...
        if ($this->db->trans_status() === false) {
            log_message('error', 'An errors has been occurred while adding the transfer (Add:Transfers_model.php)');
        } else {
            return true;
        }
        return false;
    ...
    public function updateTransfer($id, $data = [], $items = [], $stockmoves = [], $accTrans = [])
    {
        $this->db->trans_start();
        ...
        if ($this->db->update('transfers', $data, ['id' => $id])) {
            $tbl = $otransfer->status == 'completed' ? 'purchase_items' : 'transfer_items';
            $this->db->delete($tbl, ['transfer_id' => $id]);
            $this->site->delete_stock_movement('Transfer', $id);
            //======= Add accounting =======//
        ...
        $this->db->trans_complete();
        if ($this->db->trans_status() === false) {
            log_message('error', 'An errors has been occurred while adding the transfer (Update:Transfers_model.php)');
        } else {
            return true;
        }
        return false;
    ...
    public function resetTransferActions($id, $delete = null)
    {
        ...
        if ($ostatus == 'sent' || $ostatus == 'completed') {
            foreach ($oitems as $item) {
                ...
                $clause    = ['purchase_id' => null, 'transfer_id' => null, 'product_id' => $item->product_id, 'warehouse_id' => $otransfer->from_warehouse_id, 'option_id' => $option_id, 'expiry' => $item->expiry];
                $this->site->setPurchaseItem($clause, $item->quantity);
                if ($delete) {
                    $option_id = (isset($item->option_id) && !empty($item->option_id)) ? $item->option_id : null;
                    $clause    = ['purchase_id' => null, 'transfer_id' => null, 'product_id' => $item->product_id, 'warehouse_id' => $otransfer->to_warehouse_id, 'option_id' => $option_id, 'expiry' => $item->expiry];
    ...
    public function syncTransderdItem($product_id, $warehouse_id, $quantity, $option_id = null, $expiry = null)
    {
        if ($pis = $this->site->getPurchasedItems($product_id, $warehouse_id, $option_id, null, $expiry)) {
            $balance_qty = $quantity;
            foreach ($pis as $pi) {
                if ($balance_qty <= $quantity && $quantity > 0) {
                    if ($pi->quantity_balance >= $quantity) {
                        $balance_qty = $pi->quantity_balance - $quantity;
                        $this->db->update('purchase_items', ['quantity_balance' => $balance_qty], ['expiry' => $expiry, 'id' => $pi->id]);
                        $quantity = 0;
                    } elseif ($quantity > 0) {
                        $quantity    = $quantity - $pi->quantity_balance;
                        $balance_qty = $quantity;
                        $this->db->update('purchase_items', ['quantity_balance' => 0], ['expiry' => $expiry, 'id' => $pi->id]);
                    }
                }
                if ($quantity == 0) {
                    break;
            ...
            $clause = ['purchase_id' => null, 'transfer_id' => null, 'product_id' => $product_id, 'warehouse_id' => $warehouse_id, 'option_id' => $option_id, 'expiry' => $expiry];
        ...
        if ($this->db->update('transfers', ['status' => $status, 'note' => $note], ['id' => $id])) {
            ...
            foreach ($items as $item) {
                ...
                if ($status == 'sent' || $status == 'completed') {
                    $this->syncTransderdItem($item['product_id'], $transfer->from_warehouse_id, $item['quantity'], $item['option_id'], $item['expiry']);
                } else {
