...
class Returns_model extends CI_Model
{
    ...
    public function addReturn($data = [], $items = [], $stockmoves = [], $accTrans = [])
    {
        $this->db->trans_start();
            ...
            if ($stockmoves) {
                foreach ($stockmoves as $stockmove) {
                    if ($stockmove['product_type'] != 'combo') {
                        $stockmove['transaction_id'] = $return_id;
    ...
    {
        $this->db->trans_start();
        if ($this->db->delete('return_items', ['return_id' => $id]) && $this->db->delete('returns', ['id' => $id])) {
            $this->site->deleteAccTran('Return', $id);
            $this->site->deleteStockmoves('Return', $id);
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() === false) {
    ...
    public function updateReturn($id, $data = [], $items = [], $stockmoves = [], $accTrans = array())
    {
        $this->db->trans_start();
