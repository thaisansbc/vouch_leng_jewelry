...
class Returns_request_model extends CI_Model
{
    ...
    {
        $this->db->trans_start();
        if ($this->db->insert('returns_request', $data)) {
            ...
            foreach ($items as $item) {
                ...
                $this->db->insert('return_request_items', $item);
    ...
    public function deleteReturn($id)
    {
        ...
        $this->db->delete('return_request_items', ['return_id' => $id]);
        $this->db->delete('returns_request', ['id' => $id]);
        $this->db->trans_complete();
        if ($this->db->trans_status() === false) {
    ...
    {
        $q = $this->db->get_where('returns_request', ['id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    {
        $this->db->select('return_request_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.image, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code, products.second_name as second_name')
            ->join('products', 'products.id=return_request_items.product_id', 'left')
            ->join('product_variants', 'product_variants.id=return_request_items.option_id', 'left')
            ->join('tax_rates', 'tax_rates.id=return_request_items.tax_rate_id', 'left')
            ->where('return_id', $return_id)
            ->group_by('return_request_items.id')
            ->order_by('id', 'asc');

        $q = $this->db->get('return_request_items');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function updateReturn($id, $data = [], $items = [])
    {
        $this->db->trans_start();
        $this->resetSaleActions($id);
        if ($this->db->update('returns_request', $data, ['id' => $id]) && 
            $this->db->delete('return_request_items', ['return_id' => $id])) {
            // $return_id = $id;
            foreach ($items as $item) {
                // $item['return_id'] = $return_id;
