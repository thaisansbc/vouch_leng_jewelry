...
class Cart_ajax extends MY_Shop_Controller
{
    ...
    public function add($product_id)
    {
        if ($this->input->is_ajax_request() || $this->input->post('quantity')) {
            ...
            $price   = $this->bpas->setCustomerGroupPrice((isset($product->special_price) && !empty($product->special_price) ? $product->special_price : $product->price), $this->customer_group);
            $price   = $this->bpas->isPromo($product) ? $product->promo_price : $price;
            $option  = false;
            if (!empty($options)) {
            ...
            if (!$this->Settings->overselling && $this->checkProductStock($product, 1, $selected)) {
                if ($this->input->is_ajax_request()) {
                    $this->bpas->send_json(['error' => 1, 'message' => lang('item_out_of_stock')]);
                } else {
            ...
            $tax        = $this->bpas->formatDecimal($ctax['amount']);
            $price      = $this->bpas->formatDecimal($price);
            $unit_price = $this->bpas->formatDecimal($product->tax_method ? $price + $tax : $price);
    ...
    public function add_wishlist($product_id)
    {
        ...
        if (!$this->loggedIn) {
            $this->bpas->send_json(['redirect' => site_url('login')]);
        }
        if ($this->shop_model->getWishlist(true) >= 10) {
            $this->bpas->send_json(['status' => lang('warning'), 'message' => lang('max_wishlist'), 'level' => 'warning']);
        }
        if ($this->shop_model->addWishlist($product_id)) {
            $total = $this->shop_model->getWishlist(true);
            $this->bpas->send_json(['status' => lang('success'), 'message' => lang('added_wishlist'), 'total' => $total]);
        } else {
            $this->bpas->send_json(['status' => lang('info'), 'message' => lang('product_exists_in_wishlist'), 'level' => 'info']);
        }
    }
    ...
    public function scanpay()
    {
        ...
        $this->data['addresses']  = $this->loggedIn ? $this->shop_model->getAddresses() : false;
        $this->data['page_title'] = lang('checkout');
        $this->page_construct('pages/ScanPay', $this->data);
    }
    public function destroy()
    {
        if ($this->input->is_ajax_request()) {
            if ($this->cart->destroy()) {
                ...
                $this->bpas->send_json(['redirect' => base_url()]);
            } else {
                $this->bpas->send_json(['status' => lang('error'), 'message' => lang('error_occured')]);
            }
        }
    }
        ...
        if ($this->input->is_ajax_request()) {
            if ($rowid = $this->input->post('rowid', true)) {
                if ($this->cart->remove($rowid)) {
                    $this->bpas->send_json(['cart' => $this->cart->cart_data(true), 'status' => lang('success'), 'message' => lang('cart_item_deleted')]);
                }
            }
        }
    }
    ...
    public function remove_wishlist($product_id)
    {
        ...
        if ($this->shop_model->removeWishlist($product_id)) {
            $total = $this->shop_model->getWishlist(true);
            $this->bpas->send_json(['status' => lang('success'), 'message' => lang('removed_wishlist'), 'total' => $total]);
        } else {
            $this->bpas->send_json(['status' => lang('error'), 'message' => lang('error_occured'), 'level' => 'error']);
        }
    }
    ...
    public function update($data = null)
    {
        ...
        if ($this->input->is_ajax_request()) {
            if ($rowid = $this->input->post('rowid', true)) {
                ...
                $price   = $this->bpas->setCustomerGroupPrice((isset($product->special_price) ? $product->special_price : $product->price), $this->customer_group);
                $price   = $this->bpas->isPromo($product) ? $product->promo_price : $price;
                // $price = $this->bpas->isPromo($product) ? $product->promo_price : $product->price;
                ...
                if ($this->checkProductStock($product, $this->input->post('qty', true), $selected)) {
                    if ($this->input->is_ajax_request()) {
                        $this->bpas->send_json(['error' => 1, 'message' => lang('item_stock_is_less_then_order_qty')]);
                    } else {
                ...
                if ($this->cart->update($data)) {
                    $this->bpas->send_json(['cart' => $this->cart->cart_data(true), 'status' => lang('success'), 'message' => lang('cart_updated')]);
                }
            }
        }
    }
