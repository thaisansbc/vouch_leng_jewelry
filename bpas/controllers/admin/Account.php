   ...
   //     $this->bpas->checkPermissions('payments', true);
	...
	public function getCreditNote($biller_id = false)
	{
		...
		. lang('actions') . ' <span class="caret"></span></button>
			<ul class="dropdown-menu pull-right" role="menu">
				<li>' . $detail_link . '</li>
				<li class="">' . $payments_link . '</li>
				<li class="">' . $add_payment_link . '</li>
		...
		$this->datatables->select("{$this->db->dbprefix('credit_note')}.id as id,
			DATE_FORMAT({$this->db->dbprefix('credit_note')}.date, '%Y-%m-%d %T') as date,
			{$this->db->dbprefix('companies')}.company, 
			reference_no,
			{$this->db->dbprefix('credit_note')}.subject as subject,
			customer,
			{$this->db->dbprefix('credit_note')}.sale_status,
			grand_total as grand_total,
			{$this->db->dbprefix('credit_note')}.paid as paid,
			(IFNULL({$this->db->dbprefix('credit_note')}.grand_total, 0) - IFNULL({$this->db->dbprefix('credit_note')}.paid, 0)) as balance ,
			{$this->db->dbprefix('credit_note')}.payment_status,
			{$this->db->dbprefix('credit_note')}.attachment,
			sale_id")
			->from('credit_note')
	...
	{   
		...
		if ($this->form_validation->run() == true) {
			...
			for ($r = 0; $r < $i; $r++) {
				$item_description    = $_POST['item_note'][$r];
                $item_amount      	 = $_POST['amount'][$r];

				$product = [
					'description' => $this->bpas->decode_html($item_description),
					'unit_price'   => $item_amount
				];
				$products[] = $product;
				$total += $this->bpas->formatDecimal(($item_amount));
			}
		...
		if ($this->form_validation->run() == true && $this->accounts_model->addCreditNote($data, $products,$accTrans)) {
			...
			//$this->data['document_required']     = $this->settings_model->getDocumentsRequireds();
	...
	public function edit_credit_note($id = null)
	{
		...
		if ($this->form_validation->run() == true) {
			...
			$grand_total    = $this->bpas->formatDecimal(($total), 3);
		...
		if ($this->form_validation->run() == true && $this->accounts_model->EditCreditNote($id, $data, $products,$accTrans)) {
			$this->session->set_userdata('remove_slls', 1);
			...
			$this->data['count']        = explode(',', $this->session->userdata('warehouse_id'));
			$this->data['projects']     = $this->site->getAllProject();
			$this->data['inv_items']    = json_encode(false);
			$this->data['id']           = $id;
			$this->data['agencies']     = $this->site->getAllUsers();
			$this->data['billers']      = $this->site->getAllCompanies('biller');
			$this->data['units']        = $this->site->getAllBaseUnits();
			...
			$this->data['document_required']    = null;//$this->settings_model->getDocumentsRequireds();
			$this->data['credit_note_items'] 	= $this->accounts_model->getCreditNoteItems($id);
			$user = $this->site->getUser($this->session->userdata('user_id'));
			if ($this->Settings->multi_biller) {
				$this->data['user_billers'] = $user->multi_biller ? explode(',', $user->multi_biller) : null;
			} else {
	...
	public function view_credit_note($id = null)
	{
		...
		$this->data['sale'] 		= $this->sales_model->getInvoiceByID($inv->sale_id);
	  	$this->data['getBankInfo']  = null;//$this->accounts_model->getBankInfo($inv->bank_info);
	...
	public function add_payment_credit_note($id = null)
	{
		...
		if ($sale->payment_status == 'paid' && $sale->grand_total == $sale->paid) {
			$this->session->set_flashdata('error', lang("credit_already_paid"));
			$this->bpas->md();
		}
		$this->form_validation->set_rules('amount-paid', lang("amount"), 'required');
    ...
    function view_creditNote_payment($id = NULL){
    	...
    	$this->data['inv'] = $inv;
    	$this->data['payment'] = $payment;
    	$this->data['page_title'] = $this->lang->line("view_creditnote_payment");

    	$this->data['modal_js'] = $this->site->modal_js();
    	$this->load->view($this->theme . 'accounts/view_creditnote_payment', $this->data);
    }
	...
	public function getDebitNote($biller_id = null){
	    ...
	    $payment_status = $this->input->get('payment_status') ? $this->input->get('payment_status') : NULL;
			...
			$add_payment_link   = anchor('admin/account/add_debit_payment__/$1', '<i class="fa fa-money"></i> ' . lang('add_payment'), 'data-toggle="modal" data-backdrop="static" data-target="#myModal"');
			$add_payment_links   = anchor('admin/account/add_debit_payment/$1', '<i class="fa fa-money"></i> ' . lang('add_payment'), 'data-toggle="modal" data-backdrop="static" data-target="#myModal"');
			...
			$this->datatables
				->select("debit_note.id, 
					...
					{$this->db->dbprefix('debit_note')}.subject,
					{$this->db->dbprefix('debit_note')}.supplier, 
					{$this->db->dbprefix('debit_note')}.status as status,
					abs({$this->db->dbprefix('debit_note')}.grand_total) as grand_total, 
					{$this->db->dbprefix('debit_note')}.paid as paid,
					(IFNULL({$this->db->dbprefix('debit_note')}.grand_total, 0) - IFNULL({$this->db->dbprefix('debit_note')}.paid, 0)) as balance ,
					{$this->db->dbprefix('debit_note')}.payment_status, 
		...
		if ($start_date) {
		 ...
		 if ($payment_status) {
            $get_status = explode('_', $payment_status);
            $this->datatables->where_in('debit_note.payment_status', $get_status);
        }
	...
	public function add_debit_note($purchase_id = null){
	    ...
	    if ($this->form_validation->run() == true) {
	        $reference = $this->input->post('reference_no') ?: $this->site->getReference('deb');
	        $date = ($this->Owner || $this->Admin || $this->GP['change_date']) ? $this->bpas->fld(trim($this->input->post('date'))) : date('Y-m-d H:i:s');
	        $biller_id = $this->input->post('biller') ?: $this->Settings->default_biller;
	        $supplier_id = $this->input->post('supplier');
	        ...
	        // Save via model (now returns id)
	        $debit_note_id = $this->accounts_model->addDebitNotes($data, $products);
	    ...
	    $this->data['warehouses'] = $this->site->getAllWarehouses();
		$this->data['ponumber']   = $this->site->getReference('deb');
	    $this->load->helper('string');
	    $this->session->set_userdata('user_csrf', random_string('alnum', 20));
	    $this->data['csrf'] = $this->session->userdata('user_csrf');

	    $bc   = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('account/debit_note'), 'page' => lang('debit_notes')], ['link' => '#', 'page' => lang('add_debit_note')]];
	...
	public function edit_debit_note($id = null)
	{
		...
		$inv = $this->accounts_model->getDebitNoteByID($id);
		 $this->data['debit_note_items'] = $this->accounts_model->getDebitNoteItems($id);
		...
		if ($this->form_validation->run() == true) {
			...
			$total = $this->bpas->formatDecimal($this->input->post('amount'));
		...
		if ($this->form_validation->run() == true && $this->accounts_model->updateDebitNotes($id, $data, $products)) {
	...
	public function debit_payments($id = null,$ex_id = null)
	{
		...
		$this->data['payments'] = $this->accounts_model->getDebitNotePayments($id);
	...
	public function add_debit_payment($id = null){
	    $this->bpas->checkPermissions('debit_payments', true);
	    $this->load->helper('security');
	    ...
	    if (!$debit_note) {
	        $this->session->set_flashdata('error', lang('debit_note_not_found'));
	        admin_redirect('account/debit_note');
	    }

	    $balance = $debit_note->grand_total - $debit_note->paid;
	    if ($debit_note->payment_status == 'paid' && $balance <= 0) {
	        $this->session->set_flashdata('error', lang('debit_note_already_paid'));
	        $this->bpas->md();
	    }
	    ...
	    if ($this->form_validation->run() == true) {
	        $paid_by_account = $this->input->post('paid_by');

	        $date = ($this->Owner || $this->Admin || $this->GP['change_date'])
	            ? $this->bpas->fld(trim($this->input->post('date')))
	            : date('Y-m-d H:i:s');
	        ...
	        if ($_FILES['userfile']['size'] > 0) {
	            ...
	            $payment['attachment'] = $this->upload->file_name;
	        }

	        // Accounting Entries (GL)
	        $accTranPayments = [];
	        if ($this->Settings->module_account == 1) {
	            // Debit Accounts Payable
	            $accTranPayments[] = [
	                'tran_no'       => $id,
	                'tran_type'     => 'Debit Payment',
	                'tran_date'     => $date,
	                'reference_no'  => $reference_no,
	                'account_code'  => $this->accounting_setting->default_payable,
	                'amount'        => -($this->input->post('amount_paid')),
	                'narrative'     => 'Payment for Debit Note ' . $reference_no,
	                'description'   => $this->input->post('note'),
	                'created_by'    => $this->session->userdata('user_id'),
	                'biller_id'     => $debit_note->biller_id,
	            ...
	            $accTranPayments[] = [
	                ...
	                'amount'        => $this->input->post('amount_paid'),
	                'narrative'     => 'Payment via ' . $this->site->getAccountName($paid_by_account),
	                'description'   => $this->input->post('note'),
	                'created_by'    => $this->session->userdata('user_id'),
	                'biller_id'     => $debit_note->biller_id,
	                'activity_type' => 1,
	        ...
	        if ($this->accounts_model->addDebitPayment($payment, $accTranPayments)) {
	            $this->session->set_flashdata('message', lang('payment_added'));
	            admin_redirect('account/debit_note');
	        } else {
			...
			$this->data['inv']         = $debit_note;
					$this->data['payment_ref'] = $this->site->getReference('ppay');
					$this->data['modal_js']    = $this->site->modal_js();
					$this->data['currencies'] = $this->site->getAllCurrencies();
	        // $this->data['bank_accounts'] = $this->site->getCashBankAccounts();
	...
	public function edit_debit_payment($id = null)
	{
		...
		if ($this->form_validation->run() == true && $this->accounts_model->EditDebitNotPayment($id, $payment,$accTranPayments)) {
			$this->session->set_flashdata('message', lang('payment_updated'));
			admin_redirect('account/debit_note'); 
		} else {
			$get_payment = $this->site->getPaymentByID($id);

			$sale = $this->accounts_model->getDebitNoteByID($get_payment->debit_note_id);
	...
	public function delete_debit_payment($id = null)
	{
		$this->bpas->checkPermissions('delete');
		if ($this->input->get('id')) {
			$id = $this->input->get('id');
		}
		if ($this->accounts_model->deleteDebitNotePayment($id)) {
			$this->session->set_flashdata('message', lang('payment_deleted'));
			redirect($_SERVER['HTTP_REFERER']);
		}
	}
	function view_debitNote_payment($id = NULL){

    	$payment = $this->site->getPaymentByID($id);
    	$inv = $this->accounts_model->getDebitNoteByID($payment->debit_note_id);
    	$this->data['biller'] = $this->site->getCompanyByID($inv->biller_id);
    	$this->data['customer'] = $this->site->getCompanyByID($inv->supplier_id);
    	$this->data['inv'] = $inv;
    	$this->data['payment'] = $payment;
    	$this->data['page_title'] = $this->lang->line("view_debitnote_payment");
