...
class Purchases_order extends MY_Controller
{
    ...
    public function getPurchasesOrder($biller_id = null)
    {
        ...
        $this->datatables
            ->select("purchases_order.id, DATE_FORMAT({$this->db->dbprefix('purchases_order')}.date, '%Y-%m-%d %T') as date, project_name, reference_no, purchase_ref, supplier, eta, order_status, grand_total, purchases_order.status")
