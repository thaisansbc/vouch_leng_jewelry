...
class Promos extends MY_Controller
{
    ...
    //             'start_date'  => $this->input->post('start_date') ? $this->bpas->fldl(trim($this->input->post('start_date'))) : null,
    //             'end_date'    => $this->input->post('end_date') ? $this->bpas->fldl(trim($this->input->post('end_date'))) : null,
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data_buy = $this->input->post('product2buy[]');
            $i = sizeof($data_buy);
            for ($r = 0; $r < $i; $r++) {
        ...
        if ($this->form_validation->run() == true && $this->promos_model->addPromo($data, $data_buy, $data_get)) {
            $this->session->set_flashdata('message', $this->lang->line('promo_added'));
            admin_redirect('promos');
        } else {
            $this->data['items'] = $this->site->getAllProducts();
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $bc                  = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('add_promo')]];
            $meta                = ['page_title' => lang('add_promo'), 'bc' => $bc];
            $this->page_construct('promos/add', $meta, $this->data);
    ...
    public function delete($id = null)
    {
        ...
        if ($this->promos_model->deletePromo($id)) {
            $this->bpas->send_json(['error' => 0, 'msg' => lang('promo_deleted')]);
        }
    }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->promos_model->updatePromo($id, $data, $data_buy, $data_get)) {
            ...
            $this->data['bproducts'] = $this->site->getPromosItemByID($id, 'product2buy');
            $this->data['gproducts'] = $this->site->getPromosItemByID($id, 'product2get');
    ...
    public function getPromos()
    {
        ...
        $this->datatables
            ->select("promos.id as id, promos.name, promos.start_date, promos.end_date")
            ->from('promos')
    ...
    public function modal_view($id = null)
    {
        ...
        $pr_details = $this->site->getPromosByID($id);
        $data_buy = $this->site->getPromosItemByID($id,"product2buy");
        $data_get = $this->site->getPromosItemByID($id,"product2get");
        if (!$id ) {
            $this->session->set_flashdata('error', lang('promos_not_found'));
        }
        $this->data['product2buy']     = $data_buy;
        $this->data['product2get']     = $data_get;
        $this->data['promos']     = $pr_details;
       
        $this->load->view($this->theme .'promos/modal_view', $this->data);
    }
    // public function getPromos()
    // {
    //     $this->bpas->checkPermissions('index');
