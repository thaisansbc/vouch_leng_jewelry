...
class Payments extends MY_Controller
{
    ...
    public function paypalipn()
    {
        ...
        $this->bpas->log_payment('Paypal IPN called');
        ...
        if (!$fp) {
            $this->bpas->log_payment('Paypal Payment Failed (IPN HTTP ERROR)', $errstr);
            ...
            while (!feof($fp)) {
                ...
                if (stripos($res, 'VERIFIED') !== false) {
                    $this->bpas->log_payment('Paypal IPN - VERIFIED');
                    ...
                    if (($_POST['payment_status'] == 'Completed' || $_POST['payment_status'] == 'Processed' || $_POST['payment_status'] == 'Pending') && ($_POST['receiver_email'] == $paypal->account_email) && ($_POST['mc_gross'] == ($custom[1] + $custom[2]))
                        ...
                        if ($inv = $this->sales_model->getInvoiceByID($invoice_no)) {
                            ...
                            if ($this->sales_model->addPayment($payment)) {
                                ...
                                $this->bpas->log_payment('Payment has been made for Sale Reference #' . $_POST['item_name'] . ' via Paypal (' . $_POST['txn_id'] . ').', print_r($_POST, ture));
                                try {
                                    $this->bpas->send_email($paypal->account_email, 'Payment has been made via Paypal', $message);
                                } catch (Exception $e) {
                        ...
                        $this->bpas->log_payment('Payment failed for Sale Reference #' . $reference . ' via Paypal (' . $_POST['txn_id'] . ').', print_r($_POST, ture));
                        $this->session->set_flashdata('error', lang('payment_failed'));
                    }
                } elseif (stripos($res, 'INVALID') !== false) {
                    $this->bpas->log_payment('INVALID response from Paypal. Payment failed via Paypal.', print_r($_POST, ture));
                    $this->session->set_flashdata('error', lang('payment_failed'));
                }
            }
            fclose($fp);
    ...
    public function skrillipn()
    {
        ...
        $this->bpas->log_payment('Skrill IPN called');
        ...
        if (strtoupper(md5($concatFields)) == $_POST['md5sig'] && $_POST['status'] == 2 && $_POST['pay_to_email'] == $skrill->account_email) {
            ...
            if ($inv = $this->sales_model->getInvoiceByID($invoice_no)) {
                ...
                if ($this->sales_model->addPayment($payment)) {
                    ...
                    $msg       = file_get_contents('./themes/' . $theme . '/admin/views/email_templates/payment.html');
                    $message   = $this->parser->parse_string($msg, $parse_data);
                    $this->bpas->log_payment('Payment has been made for Sale Reference #' . $_POST['item_name'] . ' via Skrill (' . $_POST['mb_transaction_id'] . ').', print_r($_POST, ture));
                    try {
                        $this->bpas->send_email($skrill->account_email, 'Payment has been made via Skrill', $message);
                    } catch (Exception $e) {
                        $this->bpas->log_payment('Email Notification Failed: ' . $e->getMessage());
                    }
                    $this->session->set_flashdata('message', lang('payment_added'));
            ...
            $this->bpas->log_payment('Payment failed for via Skrill.', print_r($_POST, ture));
            $this->session->set_flashdata('error', lang('payment_failed'));
        }
        redirect('/');
        exit();
