...
// 	}
   ...
   //     $this->db->join('suspended_items','suspended_bills.id = suspended_items.suspend_id');
    ...
    public function index_($sid = NULL)
    {	
    ...
    {   
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateTicket($id, $data, $products,$accTrans,$accTranPayments, $commission_product)) {
            ...
            admin_redirect('tickets/list_ticket');
        } else {
            ...
            $this->page_construct('tickets/edit_ticket', $meta, $this->data);
        }
    }
    ...
    public function view_ticket($sale_id = null, $modal = null)
    {
        ...
        $this->load->view($this->theme . 'tickets/view_ticket', $this->data);
    ...
    public function modal_view($id = null,$logo=null)
    {
        ...
        $this->load->view($this->theme . 'tickets/modal_view', $this->data);
    }


    public function view_($id = null , $issue_inv=null)
    {
        $this->bpas->checkPermissions('index');
    ...
    public function getTicketSales($warehouse_id = null){
        ...
        $view_logo        = anchor('admin/tickets/modal_view/$1/logo', '<i class="fa fa-money"></i> ' . lang('print_with_logo'), 'data-toggle="modal" data-backdrop="static" data-target="#myModal"');
        $detail_link          = anchor('admin/tickets/view_ticket/$1', '<i class="fa fa-file-text-o"></i> ' . lang('view_ticket'));
   	...
   	public function index()
    {
        ...
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('tickets'), 'page' => lang('tickets')), array('link' => '#', 'page' => lang('list_tickets')));
        $meta = array('page_title' => lang('list_tickets'), 'bc' => $bc);
        $this->page_construct('tickets/index', $meta, $this->data);
    }
    ...
    public function list_ticket($warehouse_id = null)
    {
        ...
        $this->page_construct('tickets/index_ticket', $meta, $this->data);
    }
    ...
    public function ticket_booking($sale_order_id = null, $quote_id = null, $tickets = [])
    {
        ...
        $this->form_validation->set_rules('from', lang('from'), 'required');
        if ($this->form_validation->run() == true && $this->input->post('add_booking') != NULL) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_tax_rate      = isset($_POST['product_tax'][$r]) ? $_POST['product_tax'][$r] : null;
                $item_discount      = isset($_POST['product_discount'][$r]) ? $_POST['product_discount'][$r] : null;
                $item_timeout_id    = isset($_POST['item_timeout_id'][$r]) ? $_POST['item_timeout_id'][$r] : null;
                $item_timeout_name  = isset($_POST['item_timeout_name'][$r]) ? $_POST['item_timeout_name'][$r] : null;
                $item_unit          = $_POST['product_unit'][$r];
                $item_quantity      = $_POST['product_base_quantity'][$r];
                $item_warranty      = isset($_POST['warranty'][$r]) ? $_POST['warranty'][$r] : '';
                $item_weight        = 0;
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $product = [
                        ...
                        'timeout_id'         => $item_timeout_id,
                        'timeout_name'       => $item_timeout_name,
        ...
        if ($this->form_validation->run() == true && $this->input->post('add_booking') != NULL && $this->sales_model->addTicket($data, $products, $payment, $si_return = array(), $accTrans, $accTranPayments, null, $commission_product)) {
            $this->session->set_userdata('remove_slls', 1);
            ...
            if ($quote_id || $sale_id || $tickets) {
                if ($quote_id) {
                    ...
                    $this->data['quote'] = $this->sales_order_model->getInvoiceByID($sale_id);
                    $items               = $this->sales_order_model->getTicket($arr_tickets); 
                    $this->data['inv']   = $this->data['quote'];
                    $sale_items          = $this->site->getSaleItemsBySaleOrderID($sale_id);
                }
                ...
                if($items){
                    ...
                    foreach ($items as $row) {
                        ...
                        $timeout = $this->site->getcustomfieldById($row->timeout_id); 
                            // }
                        // }
            ...
            $this->data['payment_ref']   = ''; //$this->site->getReference('pay');
            $bc                          = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('tickets'), 'page' => lang('tickets')], ['link' => '#', 'page' => lang('booking')]];
            $meta                        = ['page_title' => lang('booking'), 'bc' => $bc];
            $this->page_construct('tickets/add_booking', $meta, $this->data);
        }
    }
