...
class Commission extends MY_Controller
{
    ...
    public function index($warehouse_id = null)
    {
        ...
        $this->page_construct('commission/index', $meta, $this->data);
    }
    public function commission_payable()
    {
        ...
        $bc                         = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('reports'), 'page' => lang('reports')], ['link' => '#', 'page' => lang('commission_payable')]];
        $meta                       = ['page_title' => lang('commission_payable'), 'bc' => $bc];
        $this->page_construct('commission/commission_payable', $meta, $this->data);
    }
    public function commissions_payable()
    {
        $this->bpas->checkPermissions('payments');
        $this->load->admin_model('reports_model');
        $Settings = $this->site->getSettings();
        ...
        $bc                         = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('reports'), 'page' => lang('reports')], ['link' => '#', 'page' => lang('commission_payable')]];
        $meta                       = ['page_title' => lang('commission_payable'), 'bc' => $bc];
        $this->page_construct('commission/commissions_payable', $meta, $this->data);
    }
     public function getPaymentsCommissionReport($pdf = null, $xls = null)
    {
        ...
        $add_payments_link = anchor('admin/commission/updatePaymentCommissionStatus/$1', '<i class="fa fa-money"></i> ' . lang('add_payments'));
        $action = '<div class="text-center"><div class="btn-group text-left">'
        . '<button type="button" class="btn btn-default btn-xs btn-primary dropdown-toggle" data-toggle="dropdown">'
        . lang('actions') . ' <span class="caret"></span></button>
        <ul class="dropdown-menu pull-right" role="menu">
            <li>' . $add_payments_link . '</li>
        </ul>
    </div></div>';
        if ($start_date) {
        ...
        if ($pdf || $xls) {
            ...
            redirect($_SERVER['HTTP_REFERER']);
             ...
             $si = "( SELECT id as sale_id, SUM(FORMAT(({$this->db->dbprefix('sales')}.paid*100/{$this->db->dbprefix('sales')}.grand_total),2)) as rpercent
            from {$this->db->dbprefix('sales')} ";
            $si .= " GROUP BY {$this->db->dbprefix('sales')}.id ) FSI";
            $this->load->library('datatables');
            $this->datatables
                ->select("{$this->db->dbprefix('commissions')}.id as id, DATE_FORMAT({$this->db->dbprefix('commissions')}.date, '%Y-%m-%d %T') as date,CONCAT({$this->db->dbprefix('users')}.first_name,' ',{$this->db->dbprefix('users')}.last_name),{$this->db->dbprefix('sales')}.reference_no as sale_refer, {$this->db->dbprefix('commissions')}.amount, CONCAT({$this->db->dbprefix('commissions')}.percent,'%'), {$this->db->dbprefix('commissions')}.status, {$this->db->dbprefix('commissions')}.note")
                ->from('commissions')
                ->join('sales','sales.id = commissions.sale_id')
                ->join('users','users.id = commissions.created_by')
                ->join($si, 'FSI.sale_id = sales.id', 'left');
            $this->datatables->where('commissions.status!=', 'paid');
            $this->datatables->where("commissions.percent <= FSI.rpercent");
                // ->where("{$this->db->dbprefix('commissions')}.percent ","FORMAT(({$this->db->dbprefix('sales')}.paid*100/{$this->db->dbprefix('sales')}.grand_total),2)");
            if ($user) {
    ...
    public function getPaymentsReport($pdf = null, $xls = null)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("{$this->db->dbprefix('payments')}.id as id, DATE_FORMAT({$this->db->dbprefix('payments')}.date, '%Y-%m-%d %T') as date, " . $this->db->dbprefix('payments') . '.reference_no as payment_ref, ' . $this->db->dbprefix('sales') . '.reference_no as sale_ref, ' . $this->db->dbprefix('purchases') . ".reference_no as purchase_ref, paid_by,amount, {$this->db->dbprefix('payments')}.commission_amount, {$this->db->dbprefix('payments')}.payment_commission_status , type")
                ->from('payments')
                ->join('sales', 'payments.sale_id=sales.id', 'left')
                // ->join('users', 'users.id=sales.saleman_by', 'left')
                ->join('purchases', 'payments.purchase_id = purchases.id', 'left')
                ->group_by('payments.id')
                ->where('payments.commission_amount > ',0)->where('payments.payment_commission_status','due');
     ...
     public function commissions($id = null)
    {
        ...
        $this->data['commissions'] = $this->sales_model->getInvoiceCommissions($id);
        $this->data['inv']      = $this->sales_model->getInvoiceByID($id);
        $this->load->view($this->theme . 'property/sales/commissions', $this->data);
    }
    public function delete_commission($id = null)
    {
        ...
        if ($this->input->get('id')) {
         ...
         $commission = $this->sales_model->getCommissionByID($id);
         if ($commission->status == 'paid') {
            $this->session->set_flashdata('error', lang('x_delete_commission'));
            $this->bpas->md();
        }
         if ($this->Owner || $this->Admin){
           if ($this->sales_model->deleteCommission($id)) {
            $this->session->set_flashdata('message', lang("deleted_successful"));
            redirect($_SERVER["HTTP_REFERER"]);
        }
        }else{
     ...
     public function edit_commission($id = null)
    {
        ...
        $commission = $this->sales_model->getCommissionByID($id);
        $sale = $this->sales_model->getInvoiceByID($commission->sale_id);
        if ($commission->status == 'paid') {
            $this->session->set_flashdata('error', lang('x_edit_commission'));
            $this->bpas->md();
        }
        $this->form_validation->set_rules('commission_amount', lang("commission_amount"), 'required');
        ...
        } elseif ($this->input->post('edit_commission')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('sale_property');
        }
        if ($this->form_validation->run() == true && $this->sales_model->updateCommission($id, $data)) {
            $this->session->set_flashdata('message', lang("commission_updated"));
            redirect($_SERVER["HTTP_REFERER"]);
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['commissions'] = $commission;
            $this->data['inv'] = $sale;
            $saleman = $this->site->getUser($sale->saleman_by);
            $this->data['commission'] = $sale->grand_total * $saleman->commission / 100;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'property/sales/edit_commission', $this->data);
        }
    }
     public function commission_note($id = null)
    {
        $this->bpas->checkPermissions('payments', true);
        ...
        $this->data['page_title'] = lang('commission_note');

        $this->load->view($this->theme . 'property/sales/commission_note', $this->data);
    }
    public function commission_receipt($id = null)
    {
        ...
        $this->load->view($this->theme . 'property/sales/commission_receipt', $this->data);
    }
    public function add_commission($id = null)
    {
        $this->bpas->checkPermissions('payments', true);
        ...
        $this->form_validation->set_rules('commission_amount', lang("amount"), 'required|is_numeric');
        $this->form_validation->set_rules('percent', lang("percent"), 'required');
        $this->form_validation->set_rules('userfile', lang("attachment"), 'xss_clean');
        if ($this->form_validation->run() == true) {
            if ($this->Owner || $this->Admin){
                $commission_amount = $this->bpas->formatDecimal($this->input->post('commission_amount'));
            }else{
                $this->session->set_flashdata('error', lang('access_denied'));
                $this->bpas->md();
            }
            ...
            $commission = array(
                ...
                'amount' => $saleman ? ($commission_amount == 0 ? ($this->input->post('commission_amount') * $saleman->commission) / 100 : $commission_amount) : 0,
                'percent' => $percent,
                'note' => $this->input->post('note'),
                'created_by' => $this->session->userdata('user_id'),

                // 'payment_commission_status' => $this->input->post('payment_commission_status')
            );
            ...
            //     }
            //     $photo = $this->upload->file_name;
            //     $commission['attachment'] = $photo;
            // }
            //$this->bpas->print_arrays($commission);

        } elseif ($this->input->post('add_commission')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER["HTTP_REFERER"]);
        }

        if ($this->form_validation->run() == true && $this->sales_model->addCommission($commission)) {
            ...
            if ($sale->sale_status == 'returned' && $sale->paid == $sale->grand_total) {
                $this->session->set_flashdata('warning', lang('commission_was_returned'));
                $this->bpas->md();
            }
            if ($sale->payment_commission_status == 'paid') {
                $this->session->set_flashdata('warning', lang('commission_was_paymentd'));
                $this->bpas->md();
            }
            $saleman = $this->site->getUser($sale->saleman_by);
            if (empty($saleman)) {
                $this->session->set_flashdata('warning', lang('do_not_has_saleman'));
                $this->bpas->md();
            }
            $this->data['inv'] = $sale;
            $saleman = $this->site->getUser($sale->saleman_by);
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'property/sales/add_commission', $this->data);
        }
    }
    public function updatePaymentCommissionStatus($id = null){
        $data = array(
                        'status'    => 'paid',
        ...
        if($id != NULL){
            if($this->sales_model->updatePaymentCommissionStatus($id, $data)){
                  $this->session->set_flashdata('message', lang("payments_added"));
                  admin_redirect("commission/commissions_payable");
            }
        }else{
     ...
     public function commissions_actions()
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'combine_to_paid') {
                    $payment = array(
                        'status'    => 'paid',
                        'paid_by'   => $this->session->userdata('user_id'),
                        'paid_date' => date('Y-m-d H:i:s'),
                    ...
                    foreach ($_POST['val'] as $id) {
                       $return = $this->sales_model->updatePaymentCommissionStatus($id, $payment);
                    }
                   if($return){
                        $this->session->set_flashdata('message', lang("payments_added"));
                        admin_redirect("commission/commissions_payable");
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $project->project_name);
                    ...
                    $this->excel->getDefaultStyle()->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    $filename = 'payment_' . date('Y_m_d_H_i_s');
                    $this->load->helper('excel');
                    create_excel($this->excel, $filename);
                }
                } else {
    ...
    public function commission_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'combine_to_paid') {
                    $payment = array(
                        'payment_commission_status'=>'paid' 
                    );
                    foreach ($_POST['val'] as $id) {
                       $return = $this->sales_model->updatePaymentCommission($id, $payment);
                    }
                   if($return){
                        $this->session->set_flashdata('message', lang("payments_added"));
                        admin_redirect("commission/commission_payable");
                    }else{
                        $this->session->set_flashdata('error', lang('something_wrong'));
                        redirect($_SERVER['HTTP_REFERER']);
                    }
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        $payments_link = anchor('admin/sale_property/payments_commission/$1', '<i class="fa fa-money"></i> ' . lang('view_payments'), 'data-toggle="modal" data-backdrop="static" data-target="#myModal"');
        $add_payment_link = anchor('admin/sale_property/add_payment/$1', '<i class="fa fa-money"></i> ' . lang('add_commission'), 'data-toggle="modal" data-backdrop="static" data-target="#myModal"');
       $request_payment_link = anchor('admin/sale_property/add_payment/$1', '<i class="fa fa-money"></i> ' . lang('request_commission'), 'data-toggle="modal" data-backdrop="static" data-target="#myModal"');
     ...
     $pp = "( SELECT sale_id, SUM(STL) as stl, SUM(SA) as sa from {$this->db->dbprefix('payments')} pi GROUP BY pi.sale_id ) Ppayment";
        ...
        if ($warehouse_id) {
            if($this->SaleAgent){
                $this->datatables
                    ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, biller, 
                        ...
                        0 as stl,
                        Ppayment.sa as sa,
                        {$this->db->dbprefix('sales')}.attachment, return_id")
                    ->from('sales')
                     ->join($pp, 'sales.id = Ppayment.sale_id', 'left')
                    ->where(array('warehouse_id' => $warehouse_id, 'return_sale_ref'=>NULL));
            }elseif($this->SaleLeader) {
                $this->datatables
                    ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, biller, 
                        {$this->db->dbprefix('sales')}.customer, 
                        grand_total, paid, (grand_total-paid) as balance, 
            ...
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, biller, {$this->db->dbprefix('sales')}.customer, grand_total, paid, (grand_total-paid) as balance, 
                    Ppayment.stl as stl,
                    Ppayment.sa as sa, 
                    {$this->db->dbprefix('sales')}.attachment, return_id")
                ->from('sales')
             ...
             //   $this->datatables->join('product_name', 'deliveries.sale_id=sales.id', 'left')
        }
    ...
    public function view($id = null)
    {
        ...
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $inv = $this->sales_model->getInvoiceByID($id);
        $this->data['sales_USD'] = $this->sales_model->getSalesTotals_usd($inv->customer_id,'USD');
        $this->data['sales_KHM'] = $this->sales_model->getSalesTotals_khm($inv->customer_id,'KHM');
        $this->data['sales_BAT'] = $this->sales_model->getSalesTotals_bat($inv->customer_id,'BAT');
    ...
    public function view_kh($id = null)
    {
        $this->bpas->checkPermissions('index');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('sales'), 'page' => lang('sales')), array('link' => '#', 'page' => lang('view')));
        $meta = array('page_title' => lang('view_sales_details'), 'bc' => $bc);
        $this->page_construct('sales/view_kh', $meta, $this->data);
    }
    public function view_baht($id = null)
    {
        $this->bpas->checkPermissions('index');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('sales'), 'page' => lang('sales')), array('link' => '#', 'page' => lang('view')));
        $meta = array('page_title' => lang('view_sales_details'), 'bc' => $bc);
        $this->page_construct('sales/view_baht', $meta, $this->data);
    }
	//-------------loan--------------
	public function loan_payments($id = null)
    {
        $this->bpas->checkPermissions('index');
        if ($this->input->get('id')) {
            $id = $this->input->get('id');
		...
		//-----
		$this->data['payments'] = $this->sales_model->getInvoicePayments_loan($id);
        $this->data['inv'] = $this->sales_model->getInvoiceByID($id);
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('sales'), 'page' => lang('sales')), array('link' => '#', 'page' => lang('view')));
        $meta = array('page_title' => lang('view_sales_details'), 'bc' => $bc);
        $this->page_construct('loans/payments_loan', $meta, $this->data);
    }
	public function view_loan_payments($id = null)
    {			
        $this->bpas->checkPermissions('index');
        if ($this->input->get('id')) {
            $id = $this->input->get('id');
		...
		$this->data['biller'] = $this->site->getCompanyByID($inv->biller_id);
		 $this->data['warehouse'] = $this->site->getWarehouseByID($inv->warehouse_id);
		$this->data['payments'] = $this->loan_model->getInvoicePayments_loan($id);
        $this->data['inv'] = $this->sales_model->getInvoiceByID($id);
		$this->data['rows'] = $this->sales_model->getAllInvoiceItems($id);
		$this->data['return_sale'] = $inv->return_id ? $this->sales_model->getInvoiceByID($inv->return_id) : NULL;
        $this->data['return_rows'] = $inv->return_id ? $this->sales_model->getAllInvoiceItems($inv->return_id) : NULL;
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('sales'), 'page' => lang('sales')), array('link' => '#', 'page' => lang('view')));
        $meta = array('page_title' => lang('view_sales_details'), 'bc' => $bc);
        $this->page_construct('loans/view_loan', $meta, $this->data);
    }
	...
	public function view_order($id = null, $view = null, $save_bufffer = null)
    {
    //    $this->bpas->checkPermissions();
		...
		$this->load->view($this->theme . 'sales/sale_order', $this->data);
    }
    ...
    {
        $this->bpas->checkPermissions();
        $getexchange_khm = $this->bpas->getExchange_rate('KHM');
        $getexchange_bat = $this->bpas->getExchange_rate('BAT');
        $exchange_khm = $getexchange_khm->rate;
        $exchange_bat = $getexchange_bat->rate;
	...
	//	$this->form_validation->set_rules('project', $this->lang->line("project"), 'required');
        if ($this->form_validation->run() == true) {
            ...
            $data = array('date' => $date,
                ...
                'currency_invoice' => $this->input->post('currency')
            );
            ...
            if ($payment_status == 'booking' || $payment_status == 'partial' || $payment_status == 'paid') {
                ...
                if ($this->input->post('paid_by') == 'gift_card') {
                    ...
                    $payment = array(
                        ...
                        'percent' => $this->input->post('percent'),
                        'month' => $this->input->post('month'),
                        'paid_amount' => $paid_usd.','.$paid_khm.','.$paid_bat,
        ...
        if ($this->form_validation->run() == true) {
            $add_sale=$this->sales_model->addSale($data, $products, $payment);
            if($add_sale){
                $get_curr= $this->sales_model->getInvoiceByID($add_sale);
                if($get_curr->currency_invoice =='KHM'){
                    admin_redirect('sales/view_kh/'.$add_sale.'');
                }elseif ($get_curr->currency_invoice =='BAT') {
                    admin_redirect('sales/view_baht/'.$add_sale.'');
                }else{
                    admin_redirect('sales/view/'.$add_sale.'');
                }
                $sales = $this->sales_model->getSalesTotals($customer_id);
                $due_balance=(isset($sales->total_amount) || isset($sales->paid)) ? ($sales->total_amount - $sales->paid) : '0.00';
                $this->db->update('sales', array('due_balance' => $due_balance), array('id' => $add_sale));
            }
            ...
            if ($quote_id || $sale_id) {
                ...
                foreach ($items as $item) {
                    ...
                    $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")",
                            'row' => $row,'ca_name' => $row->code, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options);
                    $c++;
                }
	...
	//	$this->form_validation->set_rules('project', $this->lang->line("project"), 'required');
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products)) {
            ...
            foreach ($inv_items as $item) {
                ...
                $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => '2222',
    ...
    public function delete($id = null)
    {
        ...
        if ($inv->sale_status == 'returned') {
            $this->bpas->send_json(array('error' => 1, 'msg' => lang("sale_x_action")));
        }

        if ($this->sales_model->deleteSale($id)) {
            if ($this->input->is_ajax_request()) {
                $this->bpas->send_json(array('error' => 0, 'msg' => lang("sale_deleted")));
            }
    ...
    public function delete_return($id = null)
    {
        ...
        if ($this->sales_model->deleteReturn($id)) {
            if ($this->input->is_ajax_request()) {
                $this->bpas->send_json(array('error' => 0, 'msg' => lang("return_sale_deleted")));
            }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('G1', lang('stl'));
                    $this->excel->getActiveSheet()->SetCellValue('H1', lang('sa'));
                    ...
                    foreach ($_POST['val'] as $id) {
						...
						$sale = $this->db->select("{$this->db->dbprefix('sales')}.id as id, 
							DATE_FORMAT({$this->db->dbprefix('sales')}.date, 
							'%Y-%m-%d %T') as date, reference_no, biller, 
							{$this->db->dbprefix('sales')}.customer, grand_total, paid, 
							(grand_total-paid) as balance, Ppayment.stl as stl,Ppayment.sa as sa, 
							{$this->db->dbprefix('sales')}.attachment, return_id")
							->join($pp, 'sales.id = Ppayment.sale_id', 'left');
				
						$q = $this->db->get_where('sales', array('sales.id' => $id, 'return_sale_ref'=>NULL));
						if ($q->num_rows() > 0) {
							$sale = $q->row();
							...
							$this->excel->getActiveSheet()->SetCellValue('E' . $row, $this->bpas->formatMoney($sale->grand_total));
							$this->excel->getActiveSheet()->SetCellValue('F' . $row, $this->bpas->formatMoney($sale->paid));
							$this->excel->getActiveSheet()->SetCellValue('G' . $row, $this->bpas->formatMoney($sale->stl));
							$this->excel->getActiveSheet()->SetCellValue('H' . $row, $this->bpas->formatMoney($sale->paid));
							$row++;
						}else{
							return false;
						}
		
                    //    $sale = $this->loan_model->get_commission_ByID($id);
    
                    }
                    $this->excel->getActiveSheet()->getColumnDimension('A')->setWidth(20);
                    $this->excel->getActiveSheet()->getColumnDimension('B')->setWidth(20);
                    $this->excel->getDefaultStyle()->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    $filename = 'Commission_' . date('Y_m_d_H_i_s');
                    $this->load->helper('excel');
                    create_excel($this->excel, $filename);
                }
            } else {
    ...
    public function delete_delivery($id = null)
    {
        ...
        if ($this->sales_model->deleteDelivery($id)) {
            $this->bpas->send_json(array('error' => 0, 'msg' => lang("delivery_deleted")));
        }
    ...
    public function email_payment($id = null)
    {
        ...
        $customer = $this->site->getCompanyByID($inv->customer_id);
        if ( ! $customer->email) {
            $this->bpas->send_json(array('msg' => lang("update_customer_email")));
        }
        $this->data['inv'] = $inv;
        $this->data['payment'] = $payment;
    ...
    public function suggestions()
    {
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                if ($row->promotion && (date('Y-m-d', strtotime('today')) >= $row->start_date && date('Y-m-d', strtotime('today')) < $row->end_date)) {
            ...
            //    $category = $this->site->getCategoryByID($row->category_id);
                ...
                $pr[] = array('id' => ($c + $r), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ") | ".$row->category_name, 'category_name' => $row->category_id,
                    'row' => $row,'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options);
                $r++;
            }
    ...
    public function sell_gift_card()
    {
        ...
        if (!$error) {
            if ($this->sales_model->addGiftCard($data)) {
                $this->bpas->send_json(array('result' => 'success', 'message' => lang("gift_card_added")));
            }
        } else {
            $this->bpas->send_json(array('result' => 'failed', 'message' => $error));
        }
    ...
    public function delete_gift_card($id = null)
    {
        ...
        if ($this->sales_model->deleteGiftCard($id)) {
            $this->bpas->send_json(array('error' => 0, 'msg' => lang("gift_card_deleted")));
        }
    }
    ...
    public function get_award_points($id = null)
    {
        ...
        $this->bpas->send_json(array('sa_points' => $row->award_points));
    }
        ...
        if ($warehouse_id) {
            ...
            $this->datatables
                ->select("sales.id, sales.date as date, sales.reference_no as sale_no, sales.biller, sales.customer, 
							sales.sale_status, COALESCE(bpas_sales.grand_total,0) as amount,
