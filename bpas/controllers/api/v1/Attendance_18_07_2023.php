...
class Attendance extends REST_Controller
{
    ...
    public function get_apply_leave_post()
    {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            $employee_id = $this->post('employee_id') ?? null;
            if ($employee_id == null) {
                $this->response([
                    'status' => FALSE,
                    'message' => 'Employee id is required',
                ...
                $data = [];
                $data = $this->attendance_api->getEmployeeAllTakeLeave($employee_id);
                $this->response([
                    'status'    => TRUE,
                    'message'   => "Success",
                    'data'      => $data
    ...
    public function get_apply_dayoff_post()
    {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            $employee_id = $this->post('employee_id') ?? null;
            if ($employee_id == null) {
                ...
                $data = $this->attendance_api->getEmployeeAllDayOffs($employee_id);
                $this->response([
                    'status'    => TRUE,
                    'message'   => "Success",
                    'data'      => $data
