...
class Sales extends REST_Controller
{
    ...
    public function getpos(){
        $result =  array("pushed_ids"=>1,"pushed_reg_ids"=>3);
                    ...
                    if($product_details){
                        ...
                        for ($p=0; $p < sizeof($addon_items); $p++) { 
                            ...
                            if ($extra_details) {
                                ...
                                $extractQuantity  = isset($_POST['addon_product_qty'][$r]) ? $_POST['addon_product_qty'][$r] : 0;
                        ...
                        foreach ($jdata['payments'] as $pay) {
                            if (isset($pay['amount']) && ($pay['amount'] > 0 || $grand_total == 0) && isset($pay['paid_by']) && !empty($pay['paid_by'])) {
                                if(isset($pay['balance_amount'])){
                                    ...
                                    $amount = $this->bpas->formatDecimal(isset($pay['balance_amount']) ? $pay['amount'] - $pay['balance_amount'] : $pay['amount']);
                                } 
                                    $payment[] = array(
                                        'date'         => $date,
                                        'amount'       => $amount,
                                        'paid_amount'  => isset($pay['paid_amount']) ? implode(',', array($pay['paid_amount'], $pay['paid_amount_kh'], $pay['paid_amount_bat'])) : implode(',', array($pay['paid_amount_kh'], $pay['paid_amount_bat'])),
                                        'currency_rate'=> $pay['currency_rate'],
                                        'paid_by'      => isset($pay['paid_by']) ? $pay['paid_by'] : null,
                                        'cheque_no'    => isset($pay['cheque_no']) ? $pay['cheque_no'] : null,
                                        'cc_no'        => isset($pay['cc_no']) ? $pay['cc_no'] : null,
                                        'cc_holder'    => isset($pay['cc_holder']) ? $pay['cc_holder'] : null,
                                        'cc_month'     => isset($pay['cc_month']) ? $pay['cc_month'] : null,
                                        'cc_year'      => isset($pay['cc_year']) ? $pay['cc_year'] : null,
                                        'cc_type'      => isset($pay['cc_type']) ? $pay['cc_type'] : null,
                                        'cc_cvv2'      => isset($pay['cc_cvv2']) ? $pay['cc_cvv2'] : null,
                                        'created_by'   => 1,//$this->session->userdata('user_id')) ? 1,//$this->session->userdata('user_id') : null,
                                        'type'         => 'received',
                                        'note'         => isset($pay['pay_note']) ? $pay['pay_note'] : null,
                                        'pos_paid'     => isset($pay['amount']) ? $pay['amount'] : null,
                                        'pos_balance'  => isset($pay['balance_amount']) ? $pay['balance_amount'] : null,
                                        // 'currency'       => $this->input->post('kh_currenncy') ?   => $this->input->post('kh_currenncy' : null,
                                    );
