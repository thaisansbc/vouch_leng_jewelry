...
$('#posdiscount').focus(function () {
     ...
     * ---------------------- */
     ...
     $(document).on('click', '.posdel', function () {
        ...
        var check_bill = $("#check_bill").val();
		if(check_bill !=""){
			var boxd = bootbox.dialog({
				title: "<i class='fa fa-key'></i> Why delete ?",
				message: '<input id="pos_pin" name="pos_pin" type="text" placeholder="Enter Reason" class="form-control"> ',
				buttons: {
					success: {
     ...
     * Sell Gift Card modal
     ...
     $('.date').datetimepicker({format: site.dateFormats.js_sdate, fontAwesome: true, language: 'bpas', todayBtn: 1, autoclose: 1, minView: 2 });
    ...
    $(document).on("focus", '.rquantity', function () {
        ...
        if(check_bill !=""){
            var boxd = bootbox.dialog({
                title: "<i class='fa fa-key'></i> Why Edit QTY ?",
                message: '<input id="pos_pin" name="pos_pin" type="text" placeholder="Enter Reason" class="form-control"> ',
                buttons: {
                    success: {
                        label: "<i class='fa fa-tick'></i> Submit",
...
function loadItems() {
    ...
    if (localStorage.getItem('positems')) {
        ...
        if (pos_settings.remote_printing != 1) {
            ...
            var styles = '<style>table, th, td { border-collapse:collapse; border-bottom: 1px solid #CCC; }.bold { font-weight: bold; }</style>';
         //   var pos_head1 = '<p>'+site.settings.logo+'</p>';
			...
			var pos_head2 = '<table width="100%" class="no-border">'+
								'<tr>'+
            ...
            $("#bill_span").prepend(styles + pos_head1+'<h4>'+lang.bill+'</h4>'+pos_head2);
        ...
        $.each(sortedItems, function () {
            ...
            //order symbol
            var symbol = '', old_item = formatDecimal(item.order.new_qty);
            if (formatDecimal(item.order.new_qty) < formatDecimal(item_qty)) {
                symbol = '+';
            } else if (formatDecimal(item.order.new_qty) > formatDecimal(item_qty)) {
                symbol = '-';
            }
			...
			if(currency_code =='BAT'){
				tr_html += '<input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(parseFloat(item_price * def_currency) + parseFloat(pr_tax_val)) + '</span></td>';
				if(item_code =="Time"){
					var hour=formatQuantity2(item_qty);
					var seconds=hour * 3600, sec_num = parseInt(seconds);
				...
				tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_price * def_currency) + parseFloat(pr_tax_val)) * parseFloat(item_qty))) + '</span></td>';
            ...
            //console.log(item.row.category_name);
            ...
            if (pos_settings.remote_printing != 1) {
				...
				if(currency_code =="BAT"){
					...
					if(item_price >0){
						var bprTr = '<tr>'+
								'<td>'+(an-1)+'</td>'+
								'<td>'+ item_name + '</td>'+
								'<td>'+formatDecimal(item_qty)+'</td>'+
								'<td style="text-align:right;">B ' + (item_discount != 0 ? '<del>'+formatMoney(parseFloat(item_price) + parseFloat(pr_tax_val) + item_discount)+'</del>' : '') + formatMoney(parseFloat(item_price*def_currency) + parseFloat(pr_tax_val*def_currency))+ '</td>'+
								'<td style="text-align:right;">B '+ formatMoney(((parseFloat(item_price*def_currency) + parseFloat(pr_tax_val*def_currency)) * parseFloat(item_qty))) +'</td>'+
					...
					if(item_code !="Time"){
						var bprTr = '<tr>'+
								...
								'<td>'+ item_name + '</td>'+
								'<td>'+formatDecimal(item_qty)+'</td>'+
								'<td style="text-align:right;">$ ' + (item_discount != 0 ? '<del>'+formatMoney(parseFloat(item_price) + parseFloat(pr_tax_val) + item_discount)+'</del>' : '') + formatMoney(parseFloat(item_price) + parseFloat(pr_tax_val))+ '</td>'+
				...
				if(item_code !="Time"){
    				if(item_price >0){
    					...
    					oprTr += '</td>'+
    							'<td>[ ' + old_item +' ]</td>'+
    							'<td>[ ' + ((formatDecimal(item_qty) - old_item) > 0 ? '+ '+(formatDecimal(item_qty) - old_item):(formatDecimal(item_qty) - old_item)) +' ]</td>'+
    						'</tr>';
    				}
				}
		...
		if(currency_code =='BAT'){
			$('#total').text('B '+formatMoney(total * def_currency));
		}else{
        ...
        if(gtotal_kh.toFixed(2) % 100 >= 50){ //alert("ceil: " + gtotal.toFixed(2));
			$('#gtotal_en').text('KHM '+ money_comma(Math.ceil(gtotal_kh.toFixed(2) / 100) * 100));
		}else{//alert("floor: " + gtotal_kh);
			$('#gtotal_en').text('KHM '+ money_comma(Math.floor(gtotal_kh.toFixed(2) / 100) * 100));
		}
		...
		*/
        if (pos_settings.remote_printing != 1) {
			...
			if(currency_code =="BAT"){
				var total_='B '+formatMoney(total*def_currency);
				var grand_total_='B '+formatMoney(gtotal*def_currency);
			}else{
				var total_=formatMoney(total);
				var grand_total_=formatMoney(gtotal);
			}
            var bill_totals = '';
		bill_totals += '<tr>'
				...
				+'<td>'+lang.total+'</td>'
				+'<td>:</td>'
				+'<td style="text-align:right;">'+total_+'</td>'
				+'</tr>';

            if(order_discount > 0 || product_discount > 0) {
				if(currency_code =="BAT"){
					var discount_=formatMoney((order_discount*def_currency)+(product_discount*def_currency));
				}else{
					var discount_=formatMoney(total);
				}
                bill_totals += '<tr>'+
				'<td></td>'+
				'<td>បញះតំលៃ</td>'+
				'<td>'+lang.discount+'</td>'+
				'<td>:</td>'+
				'<td style="text-align:right;">'+discount_+'</td>'
				+'</tr>';
            }
            if (site.settings.tax2 != 0 && invoice_tax != 0) {
				if(currency_code =="BAT"){
					var invoice_tax_=formatMoney(invoice_tax*def_currency);
				}else{
					var invoice_tax_=formatMoney(invoice_tax);
				}
                bill_totals += '<tr>'+
				'<td></td>'+
				...
				'<td>'+lang.order_tax+'</td>'+
				'<td>:</td>'+
				'<td style="text-align:right;">'+invoice_tax_+'</td></tr>';
            }
            bill_totals += '<tr>'+
								'<td></td>'+
								'<td>សរុបចុងក្រោយ</td>'+
								...
								'<td>:</td>'+
								'<td style="text-align:right;">'+grand_total_+'</td>'+
								'</tr>';
            if(pos_settings.rounding != 0) {
                round_total = roundNumber(gtotal, parseInt(pos_settings.rounding));
                var rounding = formatDecimal(round_total - gtotal);
                bill_totals += '<tr class="bold">'+
									'<td>'+lang.rounding+'</td>'+
									'<td style="text-align:right;">'+formatMoney(rounding)+'</td>'+
								'</tr>';
                bill_totals += '<tr class="bold">'+
									'<td>'+lang.total_payable+'</td>'+
									'<td style="text-align:right;">$ '+formatMoney(round_total)+'</td></tr>';
            }
         /*   bill_totals += '<tr>'+
								'<td></td>'+
								'<td>ចំនួនទំនិញ</td>'+
