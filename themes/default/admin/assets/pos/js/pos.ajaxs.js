...
function loadItems() {
    ...
    if (localStorage.getItem('positems')) {
        ...
        $.each(sortedItems, function () {
            ...
            //    note tr_html += '<td><div class="input-group" style="display:inline-flex;"><span class="minus" style="margin:0 3px;font-size:20px;cursor:pointer;"><i class="fa fa-minus"></i></span><input class="kb-pad text-center rquantity" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" type="text" value="' + formatQuantity2(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();" style="width:0.4in; text-align: center;" /><span class = "plus" style="margin:0 3px;font-size:20px;cursor:pointer;"><i class="fa fa-plus"></i></span></div><input name="product_unit[]" type="hidden" class="runit" value="' + product_unit + '"><input name="product_base_quantity[]" type="hidden" class="rbase_quantity" value="' + base_quantity + '"></td>';
                // tr_html += '<td><input class="form-control input-sm kb-pad text-center rquantity" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" type="text" value="' + formatQuantity2(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"><input name="product_unit[]" type="hidden" class="runit" value="' + product_unit + '"><input name="product_base_quantity[]" type="hidden" class="rbase_quantity" value="' + base_quantity + '"></td>';
                ...
                $(document).ready(function () {
                    $('.minus').click(function () {
                        // var $input = $(this).parent().find('.rquantity');
                        // var $input = $(this).parent().parent('.minus').children('input');
            ...
            var ssubtotalkh = formatMoney(parseFloat(kh_rate) * ((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)));
            if (item_currency != 'KHM') {
        ...
        if (currency != "KHM") {
            ...
            if (gtotal.toFixed(2) % 100 >= 50) { //alert("ceil: " + gtotal.toFixed(2));
                $('#gtotal').text('KHM ' + money_comma(Math.ceil(gtotal.toFixed(2) / 100) * 100));
            } else { //alert("floor: " + gtotal_kh);
                $('#gtotal').text('KHM ' + money_comma(Math.floor(gtotal.toFixed(2) / 100) * 100));
            }
        ...
        */
        if (pos_settings.remote_printing != 1) {
            ...
            bill_totals += '<tr>' +
                '<td colspan="4" style="text-align:right;">' + lang.grand_total + '(KHM):</td>' +
                '<td width="70" style="text-align:left;"> ' + formatMoney(gtotal) + '៛</td>' +
                '</tr>';
            bill_totals += '<tr>' +
