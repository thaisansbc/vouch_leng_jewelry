    ...
    if (localStorage.getItem('positems')) {
        ...
        $.each(sortedItems, function () {
            ...
            if (item_code == "Time") {
                ...
                // <input class="form-control input-sm kb-pad text-center rquantity" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" type="hidden" value="' + formatQuantity2(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></input>
                var time = hours + ':' + minutes + ':' + seconds;
                tr_html += '<td>' + time + '<div class="input-group"><span class="input-group-btn"><button class="btn btn-default btn-number" disabled="disabled" data-type="minus" data-field="quant[1]"><span class="glyphicon glyphicon-minus"></span></button> </span><input type="text" name="quant[1]" class="form-control input-number" value="1" min="1" max="10"><span class="input-group-btn"><button type="button" class="btn btn-default btn-number" data-type="plus" data-field="quant[1]"> <span class="glyphicon glyphicon-plus"></span></button></span></div><input class="form-control input-sm kb-pad text-center rquantity" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" type="hidden" value="' + formatQuantity2(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"><input name="product_unit[]" type="hidden" class="runit" value="' + product_unit + '"><input name="product_base_quantity[]" type="hidden" class="rbase_quantity" value="' + base_quantity + '"></td>';
            } else {
                alert('text')
                // tr_html += '<td><a class="btn btn-default btn-number" id="minus" href="#">-</a><span id = "value" >1</span><a id = "plus" href = "#" class="btn btn-default btn-number"> + </a></td> ';
                // tr_html += '<td><div class="group-input"><span class="minus"><i class="fa fa-minus"></i></span><input class="kb-pad text-center rquantity"  name="quantity[]" style="width:60%;text-align: center;" type ="number" value="1" disabled/><span class = "plus"><i class="fa fa-plus"></i></span></div</td>';
                // tr_html += '<td><div class="group-input"> <span class="minus"><i class="fa fa-minus fa-x"></i></span><input class="kb-pad text-center rquantity"  tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" style="width:60%;text-align: center;" type ="text" value="' + formatQuantity2(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();" /><span class = "plus"><i class="fa fa-plus fa-x"></i></span></div><input name="product_unit[]" type="hidden" class="runit" value="' + product_unit + '"><input name="product_base_quantity[]" type="hidden" class="rbase_quantity" value="' + base_quantity + '"> </td>';
                 tr_html += '<td><div class="input-group" style="display:inline-flex;"><span class="minus" style="margin:0 3px;font-size:20px;cursor:pointer;"><i class="fa fa-minus"></i></span><input class="kb-pad text-center rquantity" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" name="quantity[]" style="width:0.4in; text-align: center;" type ="text" value="' + formatQuantity2(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();" disabled/><span class = "plus" style="margin:0 3px;font-size:20px;cursor:pointer;"><i class="fa fa-plus"></i></span></div><input name="product_unit[]" type="hidden" class="runit" value="' + product_unit + '"><input name="product_base_quantity[]" type="hidden" class="rbase_quantity" value="' + base_quantity + '"></td>';
...
// $('quantity_' + row_no + "").on('change', function(){
//   if($(this).val() <= 0)
//     $(this).val(0);
//   checkAndDisable();
// });
...
//   if ($('quantity_' + row_no + "").val() == 0) {
//     $(".minus").attr('disabled', true);
//   }
// }
            }

            var ssubtotal = formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)));
            tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + (ssubtotal != 0 ? '$' + ssubtotal : 'Free') + '</span></td>';
            tr_html += '<td class="text-center"><i class="fa fa-times fa-2x tip pointer posdel" id="' + row_no + '" title="Remove" style="cursor:pointer;"></i></td>';
            ...
            if (pos_settings.remote_printing != 1) {
                ...
                if (item_code != "Time") {
                    ...
                    bprTr += '</td>' +
                        '<td width="10">' + formatDecimal(item_qty) + '</td>' +
                    ...
                    oprTr += '</td>' +
                        //	'<td>[ ' + (item_ordered != 0 ? 'xxxx' : formatDecimal(item_qty)) +' ]</td>'+
        ...
        */
        if (pos_settings.remote_printing != 1) {
            ...
            /*   bill_totals += '<tr>'+
								...
								'</tr>';*/
            bill_totals += '<tr>' +
                '<td colspan="5"><center>Thank you, Please come again</center></td>' +
                '</tr>';
