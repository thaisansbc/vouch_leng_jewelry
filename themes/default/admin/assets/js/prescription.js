$(document).ready(function(e) {
    ...
    $(document).on('click', '#editItem', function() {
        ...
        slitems[item_id].row.serial_no = $('#pserial').val();
    ...
    $(document).on('change', '.rserial', function() {
        var row = $(this).closest('tr');
        var item_id = row.attr('data-item-id');
        slitems[item_id].row.serial_no = $(this).val();
        localStorage.setItem('slitems', JSON.stringify(slitems));
        loadItems();
        return;
    });
      $(document).on('change', '.rmorning', function() {
        var row = $(this).closest('tr');
        var item_id = row.attr('data-item-id');
        slitems[item_id].row.morning_no = $(this).val();
        localStorage.setItem('slitems', JSON.stringify(slitems));
        loadItems();
        return;
    });  $(document).on('change', '.revening', function() {
        var row = $(this).closest('tr');
        var item_id = row.attr('data-item-id');
        slitems[item_id].row.evening_no = $(this).val();
        localStorage.setItem('slitems', JSON.stringify(slitems));
        loadItems();
        return;
    }); 
    $(document).on('change', '.rnight', function() {
        var row = $(this).closest('tr');
        var item_id = row.attr('data-item-id');
        slitems[item_id].row.night_no = $(this).val();
        localStorage.setItem('slitems', JSON.stringify(slitems));
        loadItems();
        return;
    });
     $(document).on('change', '.rafteroon', function() {
        var row = $(this).closest('tr');
        var item_id = row.attr('data-item-id');
        slitems[item_id].row.afteroon_no = $(this).val();
        localStorage.setItem('slitems', JSON.stringify(slitems));
        loadItems();
        return;
    });
    $(document).on('change', '.mserial', function() {
        var row = $(this).closest('tr');
        var item_id = row.attr('data-item-id');
        slitems[item_id].row.max_serial = $(this).val();
        localStorage.setItem('slitems', JSON.stringify(slitems));
        loadItems();
        return;
    });
    $(document).on('change', '.pdetail', function() {
        var row = $(this).closest('tr');
        var item_id = row.attr('data-item-id');
        slitems[item_id].row.details = $(this).val();
        localStorage.setItem('slitems', JSON.stringify(slitems));
        loadItems();
        return;
    });
    ...
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function() {
            ...
            var product_id = item.row.id,
                ...
                item_morning = item.row.morning_no ? item.row.morning_no : '',
                item_afteroon = item.row.afteroon_no ? item.row.afteroon_no : '',
                item_evening = item.row.evening_no ? item.row.evening_no : '',
                item_night = item.row.night_no ? item.row.night_no : '',
            ...
            if (item.row.code == 'sub_qty') {
                ...
                tr_html +=
                    ...
                    '"></td>';
                     ...
                     tr_html +=
                    '<td><input class="form-control text-center rmorning" tabindex="' +
                    (site.settings.set_focus == 1 ? an : an + 1) +
                    '" name="morning[]" type="text" value="' + item_morning +'" data-id="' +
                    row_no +
                    '" data-item="' +
                    item_id +
                    '" id="morning_' +
                    ...
                    tr_html +=
                    '<td><input class="form-control text-center rafteroon" tabindex="' +
                    (site.settings.set_focus == 1 ? an : an + 1) +
                    '" name="afteroon[]" type="text" value="' + item_afteroon +'" data-id="' +
                    row_no +
                    '" data-item="' +
                    item_id +
                    '" id="afteroon_' +
                    ...
                    '" onClick="this.select();"></td>';
                     ...
                     tr_html +=
                    '<td><input class="form-control text-center revening" tabindex="' +
                    (site.settings.set_focus == 1 ? an : an + 1) +
                    '" name="evening[]" type="text"  value="' + item_evening +'"  data-id="' +
                    row_no +
                    '" data-item="' +
                    item_id +
                    '" id="evening_' +
                    ...
                    '" onClick="this.select();"></td>';
                    tr_html +=
                        '<td><input class="form-control text-center rnight" tabindex="' +
                        (site.settings.set_focus == 1 ? an : an + 1) +
                        '" name="night[]" type="text"  value="'+ item_night +'"  data-id="' +
                        row_no +
                        '" data-item="' +
                        item_id +
                        '" id="night_' +
                ...
                //if (site.settings.product_serial == 1) {
                    tr_html +=
                        ...
                        row_no +'" value="' +item_serial +'">'+
                            '<input type="hidden" name="dosage" value="morning"> '+
                            '<input type="hidden" name="dosage" value="morning"> '+
                            '<input type="hidden" name="dosage" value="morning"> '+
                        '</td>';
                    tr_html +=
                        '<td class="text-right"><input class="form-control input-sm mserial" name="max_serial[]" type="text" id="max_serial_' +
                        row_no +'" value="' +item_max_serial +'"></td>';
                    // tr_html +=
                    //     '<td class="text-right"><input class="form-control input-sm pdetail" name="product_detail[]" type="text" id="max_serial_' +
                    //     row_no +'" value="' +(item_detail != '' ? item_detail: "") +'"></td>';
               // }
                tr_html +=
                    '<td class="text-center" style="width:30px;"><i class="fa fa-times tip pointer sldel" id="' +
                    row_no +
        ...
        tfoot +=
            ...
            '<th class="text-center"><i class="fa fa-trash-o" style="opacity:0.5; filter:alpha(opacity=50);"></i></th></tr>';
