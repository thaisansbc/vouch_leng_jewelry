$(document).ready(function(e) {
    ...
    $('#slpayment_status').change(function(e) {
        ...
        if (ps == 'booking' || ps == 'partial' || ps == 'paid') {
            if (ps == 'paid') {
                $('#amount_1').val(formatDecimal(parseFloat(total + invoice_tax - order_discount)));
    ...
    $(document).on('focus', '.amount', function() {
        ...
        $.ajax({
            url: site.base_url + 'money/get_exchange_rate',
            type: 'GET',
            dataType: 'json',
            data: {
                from_currency: slitems[item_id].row.from_currency_code,
                to_currency  : slitems[item_id].row.to_currency_code,
                amount       : new_qty
            },
            success: function(data) {
                slitems[item_id].row.exchange_rate = data.exchange_rate;
        ...
        //slitems[item_id].row.from_currency_code;
        //slitems[item_id].row.to_currency_code;
        //slitems[item_id].row.qty;
    });

    
    $(document).on('change', '.from_currency', function() {
            
        var from_currency_code = $(this).val();
        ...
        $.ajax({
            ...
            data: {
                ...
                to_currency  : slitems[item_id].row.to_currency_code,
                amount       : slitems[item_id].row.qty
            },
            success: function(data) {
                slitems[item_id].row.exchange_rate = data.exchange_rate;
                slitems[item_id].row.from_currency_code = from_currency_code;
                localStorage.setItem('slitems', JSON.stringify(slitems));
                loadItems();
            }
        });
    ...
    $(document).on('change', '.to_currency', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                slitems[item_id].row.exchange_rate = data.exchange_rate;
                slitems[item_id].row.to_currency_code = to_currency_code;
                localStorage.setItem('slitems', JSON.stringify(slitems));
                loadItems();
            }
        });
...
function loadItems() {
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function() {
            ...
            var exchange_rate   = item.row.exchange_rate ? item.row.exchange_rate : '';
            var currencies      = item.currencies ? item.currencies : '';
            var item_from_currency_code = item.row.from_currency_code;
            var item_to_currency_code = item.row.from_currency_code;
            ...
            if (item.row.code == 'sub_qty') {
                ...
                tr_html =
                    ...
                    ' - ' + item_name + ((item_expiry   != null && site.settings.product_expiry == 1) ? ' (' + item_expiry + ')' : '') + (sel_opt != '' ? ' (' + sel_opt + ')' : '') + '</span>'
                        + '<i class="pull-right fa fa-edit tip pointer edit hide" id="' +row_no +'" data-item="' +item_id +'" title="Edit" style="cursor:pointer;' + ((item.free) ? "display:none;" : "") + '"></i>'+ button_combo; 
                tr_html += (item_detail != '' ? '[' + item_detail + ']' : "") + '</td>';
            
           
              
                var opt_cur = $("<select name=\"from_currency\[\]\" class=\"form-control select from_currency\" />");
                if(item.currencies !== false) {
                    $.each(item.currencies, function () {
                        if (item.row.from_currency_code == this.code)
                            $("<option />", {value: this.code, text: this.name, selected: 'selected'}).appendTo(opt_cur);
                        else
                            $("<option />", {value: this.code, text: this.name}).appendTo(opt_cur);
                    });
                } else {
                    $("<option />", {value: 0, text: 'n/a'}).appendTo(opt_cur);
                    opt_cur = opt_cur.hide();
                }
                /*---------to currency--------*/
                var opt_to_currency = $("<select name=\"to_currency\[\]\" class=\"form-control select to_currency\" />");
                if(item.to_currency !== false) {
                    $.each(item.to_currency, function () {
                        if (item.row.to_currency_code == this.code)
                            $("<option />", {value: this.code, text: this.name, selected: 'selected'}).appendTo(opt_to_currency);
                        else
                            $("<option />", {value: this.code, text: this.name}).appendTo(opt_to_currency);
                    });
                } else {
                    $("<option />", {value: 0, text: 'n/a'}).appendTo(opt_to_currency);
                    opt_to_currency = opt_to_currency.hide();
                }
                ...
                tr_html +=
                    ...
                    row_no +
                    '">' + (opt_cur.get(0).outerHTML)  +
                    '</span></td>';
                tr_html += '<td>'+(opt_to_currency.get(0).outerHTML) +'</td>';
                tr_html +=
                    '<td>'+product_combo+'<input class="form-control text-center amount" ' + ((item.free) ? "disabled" : "") + ' tabindex="' +
                ...
                tr_html +=
                        '<td class="text-right"><input class="form-control input-sm " name="exchange_rate[]" type="hidden" id="exchange_rate_' +
                        row_no +
                        '" value="' +
                        ...
                        row_no +
                        '">' +
                        formatMoney(parseFloat(exchange_rate))+
                        '</span></td>';
                
                tr_html +=
                    '<td class="text-right"><span class="text-right subtotal" id="subtotal_' +
                    row_no +
                    '">' +
                    formatMoney(parseFloat(exchange_rate) * parseFloat(item_qty)) +
                    '</span></td>';
                tr_html +=
