$(document).ready(function () {
	...
	$('#stlform_type').change(function(e) {
		console.log($(this).val());
        localStorage.setItem('stlform_type', $(this).val());
    });
       if ((stlform_type = localStorage.getItem('stlform_type'))) {
        $('#stlform_type').select2('val', stlform_type);  
    }
...
function loadItems() {
    if (localStorage.getItem('stlitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            tr_html = '<td class="hidden"><input name="student_id[]" type="hidden" class="rid" value="' + student_id + '">' + item.row.firstname + '</td>';
			...
			tr_html += '<td><div class="col-md-12"> <div class="form-group"> <label for="document">Attach Document</label><span class="file-input file-input-new"><div class="input-group "><div tabindex="-1" class="form-control file-caption  kv-fileinput-caption"><div class="file-caption-name"></div></div><div class="input-group-btn"><button type="button" class="btn btn-default btn-danger fileinput-remove fileinput-remove-button" tabindex="-1"><i class="fa fa-ban-circle"></i> Remove</button><div class="btn btn-primary btn-file"> <i class="fa fa-folder-open"></i> &nbsp;Browse ... <input id="document" type="file" data-browse-label="Browse ..." name="document" data-show-upload="false" data-show-preview="false" class="form-control file"></div></div></div></span></div></div></td>';
        ...
        if (count > 1) {
            ...
            $('#stlform_type').select2('readonly', true);
        }
        set_page_focus();
    }
}
