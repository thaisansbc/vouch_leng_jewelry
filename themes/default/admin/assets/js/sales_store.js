$(document).ready(function(e) {
    ...
    if ((slcustomer = localStorage.getItem('slcustomer'))) {
        $customer.select2('val', slcustomer);
    }    
    ...
    //         },
    //         ajax: {
    //             url: site.base_url + 'customers/suggestions_biller',
    //             dataType: 'json',
    //             quietMillis: 15,
    ...
    $('#reset').click(function(e) {
        bootbox.confirm(lang.r_u_sure, function(result) {
            if (result) {
                ...
                if (localStorage.getItem('slfrom_warehouse')) {
                    localStorage.removeItem('slfrom_warehouse');
                }
                if (localStorage.getItem('slto_warehouse')) {
                    localStorage.removeItem('slto_warehouse');
                }
    ...
    if ($('#slfrom_warehouse').val()) {
        var wh_id = $('#slfrom_warehouse').val();
        $.ajax({
    ...
    $('#slfrom_warehouse').change(function(e) {
        localStorage.setItem('slfrom_warehouse', $(this).val());
        $.ajax({
            url: site.base_url + 'sales/getWarehouseByID_Ajax',
            type: 'GET',
            dataType: 'Json',
    ...
    if ((slfrom_warehouse = localStorage.getItem('slfrom_warehouse'))) {
        $('#slfrom_warehouse').select2('val', slfrom_warehouse);  
    }

    $('#slto_warehouse').change(function(e) {
        localStorage.setItem('slto_warehouse', $(this).val());
    });
    if ((slto_warehouse = localStorage.getItem('slto_warehouse'))) {
        if (!localStorage.getItem('slcustomer')) {
            localStorage.removeItem('slto_warehouse');
        } else {
            $.ajax({
                ...
                type: 'GET',
                dataType: 'Json',
                data: {'warehouse_id':slto_warehouse},
                success: function(data) {
                    if(data != null) {
                        $('#slto_warehouse').empty('');
                        $('#slto_warehouse').append('<option value="' + data.id + '" selected>' + data.name + '</option>');
                        $('#slto_warehouse').val(data.id);
                        $('#slto_warehouse').select2('val', data.id);
                    }
                }
            });
        }
    ...
    $(document).on('click', '#sellGiftCard', function(e) {
        if (count == 1) {
            slitems = {};
            if ($('#slfrom_warehouse').val() && $('#slcustomer').val()) {
                $('#slbiller').select2('readonly', true);
                $('#slfrom_warehouse').select2('readonly', true);
                $('#slcustomer').select2('readonly', true);
                $('#slto_warehouse').select2('readonly', true);
            } else {
...
function loadItems() {
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function() {
            ...
            var item_oqty    = (item.row.new_entry != 1 ? item.row.oqty : item_qty),
                item_bqty    = (item.row.new_entry != 1 ? item.row.quantity_balance : item_qty),
            ...
            if (item.row.code == 'sub_qty') {
                ...
                tr_html =
                    ...
                    (sel_opt != '' ? ' (' + sel_opt + ')' : '') + '</span><i class="pull-right fa fa-edit tip pointer edit" id="' +
                    row_no +
                    '" data-item="' +
                    item_id +
                ...
                if (site.settings.warranty == 1) {
                    tr_html +=
                        '<td class="text-right"><input class="form-control input-sm warranty" placeholder="Number of Day" name="warranty[]" type="text" id="warranty' +
                        row_no +
                        '" value="' +
                        item_warranty +
                        '"></td>';
                ...
                tr_html +=
                    ...
                    '<input class="form-control text-center rquantity" ' + ((item.free) ? "disabled" : "") + ' tabindex="' +
            ...
            if (item.name == 'Fiber') {
                $('#addition_type').show();
                add_col = 1;
                if (parseFloat(item.qty) < parseFloat(base_quantity)) {
                    console.log(item.qty + ' / ' + base_quantity)
                    $('#row_' + row_no).addClass('danger');
                    if (site.settings.overselling != 1 || (site.settings.overselling == 1 && whOverselling != 1)) {
                        $('#add_sale, #edit_sale').attr('disabled', true);
                        bootbox.alert('purchase quantity not enough !');
                        return;
                    }
                }
            }
