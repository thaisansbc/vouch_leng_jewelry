$(document).ready(function(e) {
    ...
    $(document).on('click', '.edit', function() {
        ...
        $('#item_id').val(item_id);
        $('#pserial').val(
            row.children().children('.rserial').val()
        );
    ...
    $(document).on('click', '#editItem', function() {
        ...
        slitems[item_id].row.option = poption; //$('#poption').val() ? $('#poption').val() : '';
            ...
            if (site.settings.tax1 == 1) {
                if (pr_tax !== false && pr_tax != 0 && pr_tax.rate != 0) {
                    if (pr_tax.type == 1) {
                        ...
                        price = item.row.tax_method == 0 ? formatDecimal(price - parseFloat(pr_tax.rate), 4) : formatDecimal(price + parseFloat(pr_tax.rate), 4);
                    }
                }
            }
    ...
    var old_row_set_qty;
    $(document)
        .on('focus', '.rset_quantity', function() {
            old_row_set_qty = $(this).val();
        })
        .on('change', '.rset_quantity', function() {
            var row = $(this).closest('tr');
            if (!is_numeric($(this).val())) {
                $(this).val(old_row_set_qty);
                bootbox.alert(lang.unexpected_value);
                return;
            }
            var new_set_qty = parseFloat($(this).val()),
                item_id = row.attr('data-item-id');
            console.log(slitems);
            console.log(item_id);
            // slitems[item_id].row.base_quantity = (slitems[item_id].reward.exchange_quantity * new_set_qty);
            slitems[item_id].row.base_quantity = (slitems[item_id].reward.exchange_quantity * new_set_qty);
            if (slitems[item_id].row.unit != slitems[item_id].row.base_unit) {
                $.each(slitems[item_id].units, function() {
                    if (this.id == slitems[item_id].row.unit) {
                        slitems[item_id].row.base_quantity = unitToBaseQty((slitems[item_id].reward.exchange_quantity * new_set_qty), this);
                    }
                });
            }
            slitems[item_id].row.set_quantity      = new_set_qty;
            slitems[item_id].row.qty               = (slitems[item_id].reward.exchange_quantity * new_set_qty);
            slitems[item_id].row.receive_qty       = (slitems[item_id].reward.receive_quantity * new_set_qty);
            localStorage.setItem('slitems', JSON.stringify(slitems));
            loadItems();
        });
...
function loadItems() {
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function() {
            ...
            var exchange_product     = item.exchange_product;
            var receive_product      = item.receive_product;
            ...
            var item_reward_set      = item.row.set_quantity;
            var item_interest        = item.row.interest;
            if (type == 'product') {
                receive_product_id   = item.row.receive_product_id,
                receive_product_unit = item.row.receive_unit,
                item_receive_qty     = item.row.receive_qty;
                item_receive_base_quantity = item.row.receive_base_quantity;
            } 
            ...
            if (item.units && item.row.fup != 1 && product_unit != item.row.base_unit) {
                if (site.settings.select_price == 1) {
                    if(item.row.units_edit_price != 1){
                        $.each(item.set_price, function () {
            ...
            var product_unit_code = '', f_type = item.type_id ? item.type_id : '';
            var f_option = '', f_sel = '';
            if (item.units) {
            ...
            if (item.row.code == 'sub_qty') {
                ...
                tr_html =
                    ...
                    product_id +
                    '"><input name="reward_id[]" type="hidden" class="rtype" value="' +
                    reward_id +
                    ...
                    item_detail +'"><span class="sname" id="name_' + row_no +'">' + exchange_product.code + ' - ' + exchange_product.name + '</span>'
                ...
                if (site.settings.product_serial == 1) {
                    tr_html += '<td class="text-right hide"><input class="form-control input-sm rserial" name="serial[]" type="text" id="serial_' + row_no + '" value="' + item_serial + '"></td>';
                }
                if (site.settings.product_serial == 1) {
                    tr_html += '<td class="text-right hide"><input class="form-control input-sm maxserial" name="max_serial[]" type="text" id="max_serial' + row_no + '" value="' + item_max_serial + '"></td>';
                }
                if (site.settings.warranty == 1) {
                    tr_html += '<td class="text-right hide"><input class="form-control input-sm warranty" placeholder="Number of Day" name="warranty[]" type="text" id="warranty' + row_no + '" value="' + item_warranty + '"></td>';
                }
                if (type == 'product') {
                    tr_html += '<td><input class="form-control text-center rquantity" ' + ((item.free) ? "disabled" : "") + ' tabindex="' + (site.settings.set_focus == 1 ? an : an + 1) + '" name="quantity[]" type="text" value="' + formatQuantity2(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();" readonly><input name="product_unit[]" type="hidden" class="runit" value="' + product_unit + '"><input name="product_base_quantity[]" type="hidden" class="rbase_quantity" value="' + base_quantity + '"><input name="product_weight[]" type="hidden" class="rproduct_weight" value="' + item_weight + '"></td>';
                    ...
                    tr_html += '<td class="hide"><span id="product_unit_code" class="text-right"> (' + product_unit_code + ') </span></td>';
                    tr_html += '<td><input id="receive_product_id_' + row_no + '" name="receive_product_id[]" type="hidden" class="form-control text-center rreceive_product_id" value="' + receive_product_id + '"><span class="sname" id="receive_name_' + row_no +'">' + receive_product.code + ' - ' + receive_product.name + '</span></td>';
                    tr_html += '<td>' + 
                                    '<input name="receive_product_unit[]" type="hidden" class="rreceive_unit" value="' + receive_product_unit + '">' + 
                                    '<input name="receive_base_quantity[]" type="hidden" class="rreceive_base_quantity" value="' + item_receive_qty + '">' + 
                                    '<input id="receive_quantity_' + row_no + '" name="receive_quantity[]" type="text" class="form-control text-center rset_quantity" value="' + formatQuantity2(item_receive_qty) + '" readonly>' + 
                                '</td>';
                } else {
                    tr_html += '<td class="hide"><input class="form-control text-center rquantity" ' + ((item.free) ? "disabled" : "") + ' tabindex="' + (site.settings.set_focus == 1 ? an : an + 1) + '" name="quantity[]" type="text" value="' + formatQuantity2(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();" readonly><input name="product_unit[]" type="hidden" class="runit" value="' + product_unit + '"><input name="product_base_quantity[]" type="hidden" class="rbase_quantity" value="' + base_quantity + '"><input name="product_weight[]" type="hidden" class="rproduct_weight" value="' + item_weight + '"></td>';
                    tr_html += '<td class="text-right"><input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + item_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(item_price) + '</span></td>';
                    tr_html += '<td class="hide"><span id="product_unit_code" class="text-right"> (' + product_unit_code + ') </span></td>';
                    tr_html += '<td class="text-center"><input class="form-control input-sm text-right rinterest" name="interest[]" type="hidden" id="interest_' + row_no + '" value="' + item_interest + '">' + formatMoney(item_interest) + '</td>';
                }
                if ((site.settings.product_discount == 1) || item_discount) {
                    tr_html += '<td class="text-right hide"><input class="form-control input-sm rdiscount" name="product_discount[]" type="hidden" id="discount_' + row_no + '" value="' + item_ds + '"><span class="text-right sdiscount text-danger" id="sdiscount_' + row_no + '">' + formatMoney(0 - item_discount * item_qty) + '</span></td>';
                }
                if (site.settings.tax1 == 1) {
                    tr_html += '<td class="text-right hide"><input class="form-control input-sm text-right rproduct_tax" name="product_tax[]" type="hidden" id="product_tax_' + row_no + '" value="' + pr_tax.id + '"><span class="text-right sproduct_tax" id="sproduct_tax_' + row_no + '">' + (parseFloat(pr_tax_rate) != 0 ? '(' + formatDecimal(pr_tax_rate) + ')' : '') + ' ' + formatMoney(pr_tax_val * item_qty) + '</span></td>';
                }
                tr_html += '<td><input id="set_quantity_' + row_no + '" name="set_quantity[]" type="text" class="form-control text-center rset_quantity" value="' + formatQuantity2(item_reward_set) + '"></td>';
                var subtotal = (parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_reward_set);
                if (type == 'money' && parseFloat(item_interest) > 0) {
                    var interest = parseFloat((item_price * item_interest) / 100);
                    subtotal = ((parseFloat(item_price) + parseFloat(interest)) + parseFloat(pr_tax_val)) * parseFloat(item_reward_set);
                } 
                ...
                count          += parseFloat(item_reward_set);
                count_exchange += parseFloat(item_qty);
                if (type == 'product') {
                    count_receive  += parseFloat(item_receive_qty);
                }
                an++;
            }
        ...
        var col = ((site.settings.product_serial == 1) ? 2 : (type == 'product' ? 1 : 3)) + add_col;
        if (site.settings.product_serial == 1) {
            col++;
        }
        var tfoot = '<tr id="tfoot" class="tfoot active">';
        if (site.settings.warranty == 1) {
            tfoot += '<th class="text-right hide"></th>';
        }
        tfoot += '<th colspan="' + col + '">Total</th>';
        if (type == 'product') {
            tfoot += '<th class="text-center">' + formatQty(parseFloat(count_exchange) - 1) + '</th><th></th><th></th><th class="text-center">' + formatQty(parseFloat(count_receive) - 1) + '</th>';
        }
        if ((site.settings.product_discount == 1) || product_discount) {
            tfoot += '<th class="text-right hide">' + formatMoney(product_discount) + '</th>';
        }
        if (site.settings.tax1 == 1) {
            tfoot += '<th class="text-right hide">' + formatMoney(product_tax) + '</th>';
        }
