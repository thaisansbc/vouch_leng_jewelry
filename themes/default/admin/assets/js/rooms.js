$(document).ready(function(e) {
    ...
    $(document).on('change', '.rroom_option', function(e) { 
        var row = $(this).closest('tr');
        var item_id = row.attr('data-item-id');
        slitems[item_id].row.room_option = $(this).select2('val');
        localStorage.setItem('slitems', JSON.stringify(slitems));
        loadItems();
    });
    ...
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function() {
            ...
            var product_id = item.row.id,
                ...
                item_name     = item.row.name != null ? item.row.name.replace(/"/g, '&#034;').replace(/'/g, '&#039;') : '',
            ...
            var item_room_option = item.row.room_option;
            ...
            //     });
            // }
            if (item.room_options !== false) {
                $.each(item.room_options, function() {
                    if (this.id == item_room_option) {
                        if (this.price != 0 && this.price != '' && this.price != null) {
            ...
            if (item.row.code == 'sub_qty') {
                ...
                tr_html +=
                    ...
                    '</span></td>';
                tr_html += '<td><span id="product_unit_code" class="text-right"> ' + item.row.unit + ' Bed </span></td>';

                var sel_room_options = "";
                if (item.room_options != false && item.room_options != null) {
                    sel_room_options = '<select id="room_option_' + row_no + '" name="room_option[]" class="form-control rroom_option">';
                    $.each(item.room_options, function () {
                        if (this.id == item_room_option) {
                            sel_room_options += '<option value="' + this.id + '" selected="selected">' + this.custom_field_name + '</option>';
                        } else {
                            sel_room_options += '<option value="' + this.id + '">' + this.custom_field_name + '</option>';
                        }
                    });
                    sel_room_options += '</select>';
                }
                tr_html += '<td>' + sel_room_options + '</td>';
                tr_html += '<td><input name="checkin_date[]" type="text" class="form-control datetime" value="' + $("#arrival").val() + '"></td>';
                tr_html +=
        ...
        tfoot += '<th colspan="' + col + '">Total</th><th></th><th class="text-center">' + formatQty(parseFloat(count) - 1) + '</th>';
