$(document).ready(function(e) {
    ...
    $(document).on('click', '.edit', function() {
        ...
        if (item.units) {
            ...
            uopt = $('<select id="punit" name="punit" class="form-control select" />');
            $('<option />', { value: item.row.unit, text: "Seat", selected: true }).appendTo(uopt);
        }
        //--------product option-----------
    ...
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function() {
            ...
            var product_id = item.row.id,
                ...
                item_timeout_id = item.row.timeout_id,
                item_timeout_name = item.row.timeout_name,
                item_timeout_description = item.row.timeout_description,
                item_discount = 0,
            ...
            if (item.row.code == 'sub_qty') {
                ...
                tr_html =
                    ...
                    item_detail +
                    '"><input name="item_timeout_name[]" type="hidden" class="ritemoutname" value="' +
                item_timeout_name +
                    '"><input name="item_timeout_id[]" type="hidden" class="ritemoutid" value="' +
                item_timeout_id +
                ...
                (sel_opt != '' ? ' (' + sel_opt + ')' : '') + '</span><span class="item_timeout_description"> ('+
                item_timeout_description + ')</span><i class="pull-right fa fa-edit tip pointer edit" id="' +
                    row_no +
                    '" data-item="' +
                    item_id +
                ...
                tr_html += '<td><span id="product_unit_code" class="text-right"> ' + item.row.unit + 'Seat </span></td>';
                var check_in = $("#arrival").val();
                tr_html += '<td class="hide"><input name="checkin_date[]" type="text" class="form-control datetime" value="'+check_in+'"></td>';
        ...
        tfoot += '<th colspan="' +
            col +
            '">Total</th><th></th><th class="text-center">' +
