...
function loadItems() {
    if (localStorage.getItem('stlitems')) {
        ...
        $.each(sortedItems, function () {
			...
			var study_info_id = item.row.study_info_id;
			var class_id      = '';
			var class_name    = '';
			if (item.row.class_id > 0){
				class_id   = item.row.class_id;
				class_name = item.row.class_name;
			}
            var newTr = $('<tr id="row_' + row_no + '" class="row_' + item_id + '" data-item-id="' + item_id + '"></tr>');
            tr_html  = '<td><input name="student_id[]" type="hidden" class="rid" value="' + student_id + '"><input name="study_info_id[]" type="hidden" class="stdid" value="' + study_info_id + '">'+item.row.code+'</td>';
			tr_html += '<td><input name="s_academic_year[]" type="hidden" value="' + item.row.academic_year+'"/>'+item.row.lastname+' '+item.row.firstname+'</td>';
			tr_html += '<td>'+item.row.lastname_other+' '+item.row.firstname_other+'</td>';
			tr_html += '<td>'+item.row.gender+'</td>';
			tr_html += '<td><input type="hidden" name="class_id[]" value="'+class_id+'"/>' + class_name + '</td>';
...
$(document).ready(function() {
	...
	$(document).on('change', '#stlprogram, #stlskill, #stlgrade', function() {
		...
		var skill   = $('#stlskill').val();
		var grade   = $('#stlgrade').val();
		setSections(program, skill, grade);
	});
	...
	$(document).on('change', '#stlacademic_year, #stlbiller, #stlprogram, #stlskill, #stlgrade, #stltimeshift', function() {
		...
		var grade         = $('#stlgrade').val();
		var timeshift     = $('#stltimeshift').val();
		setClasses(academic_year, biller, program, skill, grade, timeshift);
	});
});
			...
			success: function (data) {
				...
				if (data) {
					$(data).each(function (element, value){
						$('#stlsection').append('<option value="' + value.id + '">' + (value.name + ' (' + value.code + ')') + '</option>');
					});
				}
...
function setRooms(biller_id, df = null) {
	if (biller_id != null) {
		$.ajax({
			type: 'GET',
			url: site.base_url + 'schools/getRoomByBiller/' + biller_id,
			dataType: 'json',
			success: function (data) {
				$('#stlroom').find('option').remove().end();
				$('#stlroom').append('<option value="" selected="selected">Please select room</option>');
				if (data) {
					$(data).each(function (element, value){
						$('#stlroom').append('<option value="' + value.id + '">' + value.name + '</option>');
					});
				}
				$('#stlroom').select2('val', df);
			}, 
		});
	}
}