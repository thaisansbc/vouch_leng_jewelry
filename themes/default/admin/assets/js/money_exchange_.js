$(document).ready(function(e) {
    ...
    $(document).on('change', '.from_currency', function() {
        ...
        slitems[item_id].row.from_currency_code = $(this).val();
        localStorage.setItem('slitems', JSON.stringify(slitems));
        calculateExchangeRate(this);
    });
    $(document).on('change', '.to_currency', function() {
        var item_id = $(this).closest('tr').attr('data-item-id');
        slitems[item_id].row.to_currency_code = $(this).val();
        localStorage.setItem('slitems', JSON.stringify(slitems));
        calculateExchangeRate(this);
    });
   /* $(document).on('change', '.amount', function() {
        var item_id = $(this).closest('tr').attr('data-item-id');
        slitems[item_id].row.qty = $(this).val();
        localStorage.setItem('slitems', JSON.stringify(slitems));
        
    });*/

    function calculateExchangeRate(parm) {
        var new_qty = $(this).val();

        var row = $(parm).closest('tr');
    
            var new_qty = parseFloat($(parm).val()),
                item_id = row.attr('data-item-id');
            //slitems[item_id].row.from_currency_code;
            //slitems[item_id].row.to_currency_code;
            //slitems[item_id].row.qty;
