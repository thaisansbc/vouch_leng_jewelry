$(document).ready(function(e) {
    ...
    $(document).on('change', '#sppayment_term', function () {
        var id = $(this).val() ? $(this).val() : '';
        $.ajax({
            type: "get", async: false,
            url: site.base_url + "sale_property/getcustomfield/" + id,
            dataType: "json",
            success: function (data) {
                if (data.discount != null) {
                    $('#sldiscount').val(data.discount);
                    localStorage.setItem('sldiscount', data.discount);
                    loadItems();
                    return;
                } else {
                    $('#sldiscount').val(0);
                    localStorage.setItem('sldiscount', 0);
                    loadItems();
                    return;
                }
            }
...
function loadItems() {
    if (localStorage.getItem('slitems')) {
        ...
        console.log(item);
            ...
            var row_no = item.id;
            var newTr = $('<tr style ="background-color:#FFFFFF" id="row_' + row_no + '" class="row_' + item_id + '" item-id="' + item.row.id + '"data-item-id="' + item_id + '"></tr>');
            if (item.row.code == 'sub_qty') {
                ...
                if (site.settings.product_serial == 1) {
                    tr_html +=
                        '<td class="text-right"><input class="form-control input-sm rserial" name="max_serial[]" type="text" id="max_serial' +
                        row_no +
                        '" value="' +
                        item_max_serial +
                        '"></td>';
                ...
                tr_html +=
                    '<td class="text-center">1<input class="form-control text-center rquantity hide" tabindex="' +
