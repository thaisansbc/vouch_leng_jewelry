$(document).ready(function () {
    ...
    $('#reset').click(function (e) {
        bootbox.confirm(lang.r_u_sure, function (result) {
            if (result) {
                ...
                if (localStorage.getItem('csmoverselling')) {
                    localStorage.removeItem('csmoverselling');
                }
                $('#modal-loading').show();
                location.reload();
            }
    ...
    $('#csmwarehouse').change(function (e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if(data != null) {
                    localStorage.setItem('csmoverselling', data.overselling);
                }
            }
        });
    });
    if (csmwarehouse = localStorage.getItem('csmwarehouse')) {
        ...
        $.ajax({
            ...
            type: 'GET',
            dataType: 'Json',
            data: {'warehouse_id':csmwarehouse},
            success: function(data) {
                if (data != null) {
                    localStorage.setItem('csmoverselling', data.overselling);
                }
    ...
    $(document).on('change', '#punit', function () {
        ...
        if (site.settings.select_price == 1 && item.set_price != "") {
            ...
            if (item.units && unit != csmitems[item_id].row.base_unit) {
                $.each(item.units, function () {
                    if (this.id == unit) {
                        base_quantity = unitToBaseQty($('#pquantity').val(), this);
	...
	$(document).on('change', '.sunit', function () {
        ...
        if (site.settings.select_price == 1 && item.set_price != "") {
            if (item.set_price) {
                ...
                unit_price = formatDecimal(item.row.base_unit_price + aprice);
            }
        } else {
...
function loadItems() {
    if (localStorage.getItem('csmitems')) {
        ...
        $.each(sortedItems, function () {
				...
				$.each(item.pitems, function () {
					if ((this.quantity -0) > 0) {
						if (item.row.expired == this.expiry) {
							expiry_option += '<option selected value="'+this.expiry+'">'+(this.expiry ? this.expiry : 'N/A')+'</option>';
							qoh = formatDecimalRaw(this.quantity);
						} else {
							expiry_option += '<option value="'+this.expiry+'">'+(this.expiry ? this.expiry : 'N/A')+'</option>';
						}
					}
                });
            ...
            if (item_type == 'standard' && item.options !== false) {
            ...
            } else if (item_type == 'standard' && site.settings.product_expiry == 1 && item.pitems) {
				...
				if (base_quantity > qoh || base_quantity > item_aqty) {
                    ...
                    //     $('#add_consignment, #add_consignment_next, #edit_consignment').attr('disabled', true); 
                    // }
				}
