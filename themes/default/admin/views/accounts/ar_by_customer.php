...
<?php //if ($Owner || $Admin) {
    echo admin_form_open('account/arByCustomer_actions', 'id="action-form"');
    //}
?>
<style>
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-star"></i><?=lang('ar_by_customer') . ' (' . lang('All_Customer') . ')';?></h2>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
        ...
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                    <a href="#" id="ar_statement" class="tip" title="<?= lang('ar_statements') ?>">
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    <?php echo admin_form_open("account/ar_by_customer"); ?>
                    <div class="row">
                        <div class="col-sm-4">
                ...
                <div class="table-responsive">
                    <table id="POData" cellpadding="0" cellspacing="0" border="0" class="table table-condensed table-bordered table-hover table-striped">
                            <tr class="active">
                                ...
                                <th class="text-center"><?php echo $this->lang->line("biller"); ?></th>
                        ...
                        <?php 
							...
							<?php
								...
								$total_am = 0;
									foreach($items as $row){
										...
										$am = $sale->grand_total - $sale->paid;
							?>
								<tr>
									<td>&nbsp;&nbsp;&nbsp;&nbsp;<?=$row->fullname?></td>
									<td><?=$sale->biller?></td>
									<td><?= lang($sale->sale_status)?></td>
									<td><?=$this->bpas->hrsd($sale->date)?></td>
									<td><?=$sale->reference_no?></td>
									<td class="text-right"><?=$this->bpas->formatMoney($sale->grand_total)?></td>
									<td  class="text-right"></td>
									<td  class="text-right"><?=$this->bpas->formatMoney($sale->paid)?></td>
									<td  class="text-right"></td>
									<td  class="text-right"><?=$this->bpas->formatMoney($am)?></td>
								</tr>
								
								<?php
								$total_return = 0;
								$return_sale = $this->accounts_model->getReturnBySID($row->id);
                                if (is_array($return_sale)){
								...
								<tr>
									...
									<td><?=$return->biller?></td>
									<td>Return</td>
									<td><?=$this->bpas->hrsd($return->date)?></td>
									<td><?=$return->reference_no?></td>
									<td></td>
									<td class="text-right"><?=$this->bpas->formatMoney($return->grand_total)?></td>
									<td class="text-right"></td>
									<td class="text-right"></td>
									<td class="text-right"><?php if($am<0){?>(<?=$this->bpas->formatMoney(abs($am))?>) <?php }else{ echo $this->bpas->formatMoney($am);}?></td>
								</tr>
							<?php
							...
							<tr>
									...
									<td class="text-right"><b><?=$this->bpas->formatMoney($total_sale)?></b></td>
									<td class="text-right"><b><?=$this->bpas->formatMoney($total_return_amoun)?></b></td>
									<td class="text-right"><b><?=$this->bpas->formatMoney($total_pay_amoun)?></b></td>
									<td class="text-right"><b></b></td>
									<td class="text-right"><b>
                                        <?php 
                                            if($total_am<0){
                                                echo '('.$this->bpas->formatMoney(abs($total_am)).')'; 
                                            }else{ 
                                                echo $this->bpas->formatMoney($total_am);
                                            }?></b>
                                    </td>
								</tr>
							<?php
							...
							<tr class="foot">
								...
								<td class="text-right"><b><?=$this->bpas->formatMoney($total_sale2)?></b></td>
								<td class="text-right"><b><?=$this->bpas->formatMoney($total_return_amoun2)?></b></td>
								<td class="text-right"><b><?=$this->bpas->formatMoney($total_pay_amoun2)?></b></td>
								<td class="text-right"><b></b></td>
								<td class="text-right"><b>
                                    <?php 
                                    if($total_am2<0){
                                        echo '('.$this->bpas->formatMoney(abs($total_am2)).')';
                                    }else{ 
                                        echo $this->bpas->formatMoney($total_am2);
                                    }?></b></td>
							</tr>
                    </table>
                </div>
...
<script>
    $(document).ready(function(){
        $("#excel").click(function(e){
            e.preventDefault();
            window.location.href = "<?=admin_url('Account/arByCustomer/0/xls/0/'.$customer2.'/'.$start_date2.'/'.$end_date2)?>";
            return false;
        });
        /*$('#pdf').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('Account/arByCustomer/pdf/0/0/'.$customer2.'/'.$start_date2.'/'.$end_date2)?>";
            return false;
        });*/
        $('#ar_statement').click(function (event) {
            event.preventDefault();
            window.open('<?= admin_url('Account/arByCustomer/0/0/ar_statement/'.$customer2.'/'.$start_date2.'/'.$end_date2) ?>', '_blank');
            return false;
        });
    });
</script>