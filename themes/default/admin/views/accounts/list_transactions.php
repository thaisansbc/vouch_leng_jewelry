...
<script>
    $(document).ready(function () {
        var oTable = $('#SupData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('account/getTransactions').'/?v=1'.$v ?>',
			"bAutoWidth": false ,
			...
			"fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                ...
                for (var i = 0; i < aaData.length; i++) {
					if(isNaN(parseFloat(aaData[aiDisplay[i]][11]))){
						...
						total_debit += parseFloat(aaData[aiDisplay[i]][11]);
					}
					
					if(isNaN(parseFloat(aaData[aiDisplay[i]][12]))){
						total_credit += parseFloat(0);
					}else{
						total_credit += parseFloat(aaData[aiDisplay[i]][12]);
					}
                }
               var nCells = nRow.getElementsByTagName('th');
                nCells[11].innerHTML = currencyFormat(parseFloat(total_debit));
                nCells[12].innerHTML = currencyFormat(parseFloat(total_credit));
            },
            ...
            {column_number: 11, filter_default_label: "[<?=lang('debit');?>]", filter_type: "text", data: []},
            {column_number: 12, filter_default_label: "[<?=lang('credit');?>]", filter_type: "text", data: []},
			{column_number: 13, filter_default_label: "[<?=lang('created_by');?>]", filter_type: "text", data: []},
        ], "footer");
    });
</script>
...
<div class="breadcrumb-header">
    <h2 class="blue"><i class="fa-fw fa fa-users"></i><?= lang('journal').' '.lang('transactions'); ?></h2>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo admin_form_open("account/transactions"); ?>
                    <div class="row">
                        <div class="col-sm-4">
