    ...
    <div class="well well-sm">
        <div class="row bold">
            ...
            <div class="col-xs-6 pull-right text-right">
                <?php $br = $this->bpas->save_barcode($transfer->transfer_no, 'code39', 35, false, $transfer->id); ?>
                <img src="<?= admin_url() ?>assets/uploads/barcode<?= $this->session->userdata('user_id').$transfer->id; ?>.png"
                     alt="<?= $transfer->transfer_no ?>"/>
                <?php $this->bpas->qrcode('link', urlencode(admin_url('transfers/view/' . $transfer->id)), 1, false, $transfer->id); ?>
                <img src="<?= admin_url() ?>assets/uploads/qrcode<?= $this->session->userdata('user_id').$transfer->id; ?>.png"
                     alt="<?= $transfer->transfer_no ?>"/>
            </div>
        ...
        <table class="table table-bordered table-hover table-striped">
            ...
            foreach ($rows as $row):
                ...
                <tr>
                    ...
                    <td style="text-align:center; width:80px; "><?php echo $this->bpas->formatQuantity($row->quantity); ?></td>
                    <td style="width: 100px; text-align:right; padding-right:10px; vertical-align:middle;"><?php echo $this->bpas->formatMoney($row->net_unit_cost); ?></td>
                    <?php
                    if ($this->Settings->tax1) {
                        echo '<td style="width: 80px; text-align:right; vertical-align:middle;"><!--<small>(' . $row->tax . ')</small>--> ' . $this->bpas->formatMoney($row->item_tax) . '</td>';
                    }
                    ?>
                    <td style="width: 100px; text-align:right; padding-right:10px; vertical-align:middle;"><?php echo $this->bpas->formatMoney($row->subtotal); ?></td>
                </tr>
                <?php $r++;
            endforeach;
            ?>
            ...
            <?php if ($this->Settings->tax1) { ?>
                <tr>
                    ...
                    <td style="text-align:right; padding-right:10px;"><?php echo $this->bpas->formatMoney($transfer->total); ?></td>
                </tr>
                <?php echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . $this->lang->line("product_tax") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->bpas->formatMoney($transfer->total_tax) . '</td></tr>';
            }
            ?>
            <tr>
                <td colspan="<?php echo $col; ?>"
                ...
                <td style="text-align:right; padding-right:10px; font-weight:bold;"><?php echo $this->bpas->formatMoney($transfer->grand_total); ?></td>
            </tr>
            </tfoot>
        </table>
    </div>
