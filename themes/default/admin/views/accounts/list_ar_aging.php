...
<div class="tab-content">
					...
					<div class="box-icon">
						<ul class="btn-tasks">
							<?php if ($Owner || $Admin) { ?>
							    ...
							    <li class="dropdown"><a href="#" id="ar_aging_form" data-action="ar_aging_form" class="tip" title="<?= lang('ar_aging_form') ?>"><i class="icon fa fa-file-text"></i></a></li>
					   		<?php } else { ?>
	    	...
	    	<div class="box-content">
				<div class="row">
					<div class="col-lg-12">
						...
						<div id="form">
							<?php echo admin_form_open("account/list_ar_aging".$user_id); ?>
							<div class="row">
								<div class="col-sm-4">
						...
						<script>
							$(document).ready(function () {
								var oTable = $('#SLData').dataTable({
									...
									'sAjaxSource': "<?= admin_url('account/getSales_pending' . ($warehouse_id ? '/' . $warehouse_id : '')) . '/?v=1' . $v ?>",
									...
									"fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
										...
										for (var i = 0; i < aaData.length; i++) {
											...
											ar_n += parseFloat(aaData[aiDisplay[i]][6]);
										}
										...
										nCells[6].innerHTML = currencyFormat(parseFloat(ar_n));
									}
						...
						<?php echo admin_form_open('account/list_ar_aging_actions/'.(isset($warehouse_id) ? $warehouse_id : ''), 'id="action-form"'); ?>
			...
			<div class="box-header">
				<div class="box-icon">
					<ul class="btn-tasks">
						...
						<?php }?>	
						<!-- <li class="dropdown">
							<a href="#" id="image2" class="tip" title="<?= lang('save_image') ?>"><i class="icon fa fa-file-picture-o"></i></a>
						</li> -->
					</ul>
				</div>	
		    </div>
			<div class="box-content">
				<div class="row">
					<div class="col-lg-12">
						...
						<script>
							$(document).ready(function () {
								var oTable = $('#SLData0_30').dataTable({
									...
									"iDisplayLength": <?=$Settings->rows_per_page?>,
									'bProcessing': true, 'bServerSide': true,
									'sAjaxSource': '<?= admin_url('account/list_ar_aging_0_30' . ($warehouse_id ? '/' . $warehouse_id : '')).'/?v=1'.$v ?>',
									'fnServerData': function (sSource, aoData, fnCallback) {
										aoData.push({
											"name": "<?=$this->security->get_csrf_token_name()?>",
											"value": "<?=$this->security->get_csrf_hash()?>"
						...
						<?php echo admin_form_open('account/list_ar_aging_actions2', 'id="action-form2"'); ?>
						<div class="table-responsive">
			...
			<div class="box-header">
				<div class="box-icon">
					<ul class="btn-tasks">
						...
						<?php }?>	
						<!-- <li class="dropdown">
							<a href="#" id="image3" class="tip" title="<?= lang('save_image') ?>"><i class="icon fa fa-file-picture-o"></i></a>
						</li> -->
					</ul>
				</div>
			</div>
			<div class="box-content">
				<div class="row">
					<div class="col-lg-12">
						...
						<script>
							$(document).ready(function () {
								var oTable = $('#SLData30_60').dataTable({
									...
									"iDisplayLength": <?=$Settings->rows_per_page?>,
									'bProcessing': true, 'bServerSide': true,
									'sAjaxSource': '<?= admin_url('account/list_ar_aging_30_60' . ($warehouse_id ? '/' . $warehouse_id : '')).'/?v=1'.$v ?>',
									'fnServerData': function (sSource, aoData, fnCallback) {
										aoData.push({
											"name": "<?=$this->security->get_csrf_token_name()?>",
											"value": "<?=$this->security->get_csrf_hash()?>"
						...
						<?php echo admin_form_open('account/list_ar_aging_actions3', 'id="action-form3"'); ?>
						<div class="table-responsive">
		 ...
		 <div class="box-header">
			<div class="box-icon">
			  <ul class="btn-tasks">
					...
					<?php }?>	
					<!-- <li class="dropdown">
						<a href="#" id="image4" class="tip" title="<?= lang('save_image') ?>"><i class="icon fa fa-file-picture-o"></i></a>
					</li> -->
				</ul>
			</div>
		</div>
			  ...
			  <script>
				$(document).ready(function () {
					var oTable = $('#SLData60_90').dataTable({
						...
						"iDisplayLength": <?=$Settings->rows_per_page?>,
						'bProcessing': true, 'bServerSide': true,
						'sAjaxSource': '<?= admin_url('account/list_ar_aging_60_90' . ($warehouse_id ? '/' . $warehouse_id : '')).'/?v=1'.$v ?>',
						'fnServerData': function (sSource, aoData, fnCallback) {
							aoData.push({
								"name": "<?=$this->security->get_csrf_token_name()?>",
								"value": "<?=$this->security->get_csrf_hash()?>"
	...
	<?php echo admin_form_open('account/list_ar_aging_actions4', 'id="action-form4"'); ?>
	<div class="table-responsive">
		...
		<script>
			$(document).ready(function () {
				var oTable = $('#SLData_over_90').dataTable({
					...
					"iDisplayLength": <?=$Settings->rows_per_page?>,
					'bProcessing': true, 'bServerSide': true,
					'sAjaxSource': '<?= admin_url('account/list_ar_aging_over_90' . ($warehouse_id ? '/' . $warehouse_id : '')).'/?v=1'.$v ?>',
					'fnServerData': function (sSource, aoData, fnCallback) {
						aoData.push({
							"name": "<?=$this->security->get_csrf_token_name()?>",
							"value": "<?=$this->security->get_csrf_hash()?>"
		...
		<div class="box">
			<div class="box-header">
				<div class="box-icon">
					...
					<?php }?>	
					<!-- <li class="dropdown">
						<a href="#" id="image5" class="tip" title="<?= lang('save_image') ?>"><i class="icon fa fa-file-picture-o"></i></a>
					</li> -->
				</ul>
				</div>
			</div>
			<div class="box-content">
				<div class="row">
					<div class="col-lg-12">
						...
						<?php echo admin_form_open('account/list_ar_aging_actions5', 'id="action-form5"'); ?>
						<div class="table-responsive">
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#ar_aging_form').click(function (event) {
            event.preventDefault();
            window.open('<?= admin_url('Account/ar_aging_form/?v=1' . $v) ?>', '_blank');
            return false;
        });
	});
</script>