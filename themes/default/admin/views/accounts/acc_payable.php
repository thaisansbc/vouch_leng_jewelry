...
<script>
    $(document).ready(function () {
        var oTable = $('#POData').dataTable({
            ...
            'sAjaxSource': '<?=admin_url('purchases/getpending_Purchases' . ($warehouse_id ? '/' . $warehouse_id : '/0').($dt ? '/' . $dt : '')).'/?v=1'.$v?>',
...
<?php //if ($Owner) {
	    echo admin_form_open('account/payable_actions/'.($warehouse_id ? $warehouse_id : ''), 'id="action-form"');
	//}
?>
<div class="box">
            ...
            <?php } ?>
                <?php if (!empty($warehouses)) {
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right" class="tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?=admin_url('account/list_ac_payable')?>"><i class="fa fa-building-o"></i> <?=lang('all_warehouses')?></a></li>
                            <li class="divider"></li>
                            <?php
                            	foreach ($warehouses as $warehouse) {
                            	        echo '<li ' . ($warehouse_id && $warehouse_id == $warehouse->id ? 'class="active"' : '') . '><a href="' . admin_url('account/list_ac_payable/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
                            	    }
                                ?>
                        </ul>
                    </li>
        ...
        <div class="row">
            <div class="col-lg-12">
				...
				<div id="form">
                    ...
                    <?php echo admin_form_open("account/list_ac_payable"); ?>
                                        <div class="row">
                        <div class="col-sm-4">
...
<script type="text/javascript" charset="utf-8">
        $(document).ready(function() {
            $('body').on('click', '#combine_payable', function() {
                ...
                $('#myModal').modal({remote: '<?=admin_url('purchases/combine_payment_payable');?>?data=' + arrItems + ''});
                $('#myModal').modal('show');
            });
        });
</script>