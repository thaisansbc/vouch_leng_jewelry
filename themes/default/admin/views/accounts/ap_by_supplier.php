...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-star"></i><?=lang('ap_by_supplier') . ' (' . lang('all_suppliers') . ')'; ?></h2>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
        ...
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                    <a href="#" id="ap_statement" class="tip" title="<?= lang('ap_statements') ?>">
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    <?php echo admin_form_open("account/ap_by_supplier"); ?>
                    <div class="row">
                        <div class="col-sm-4">
                    ...
                    <table id="POData" cellpadding="0" cellspacing="0" border="0"
                        ...
                        ?>
                                    <tr>
                                        ...
                                        <td><?= $this->bpas->hrsd($suppData->date) ?></td>
                                        <td><?= 'Purchase' ?></td>
                                        <!-- <td><?= $type ?></td> -->
                                        <td class="numeric"><?= $this->bpas->formatMoney($suppData->grand_total) ?></td>
                                        <td class="numeric"><?= $this->bpas->formatMoney($suppData->amount_return) ?></td>
                                        <td class="numeric"><?= $this->bpas->formatMoney(0) ?></td>
                                        <td class="numeric"><?= $this->bpas->formatMoney($suppData->amount_deposit) ?></td>
                                        <td class="numeric"><?= $this->bpas->formatMoney($suppData->order_discount) ?></td>
                                        <td class="numeric"><?= $this->bpas->formatMoney($sub_balance) ?></td>
                                    </tr>
                            <?php   
                            ...
                            ?>
                                        <tr class="success">
                                            ...
                                            <td><?= $this->bpas->hrsd($supPmt->date) ?></td>
                                            <td><?= 'Payment' ?></td>
                                            <!-- <td><?= $typePV ?></td> -->
                                            <td class="numeric"></td>
                                            <td class="numeric"></td>
                                            <td class="numeric"><?= $this->bpas->formatMoney(abs($supPmt->amount)) ?></td>
                                            <td class="numeric"></td>
                                            <td class="numeric"></td>
                                            <td class="numeric"><?= $this->bpas->formatMoney($sub_balance - abs($supPmt->amount)) ?></td>
                                        </tr>
                            <?php
                            ...
                            ?>
                                    <tr style="font-weight:bold;">
                                        ...
                                        <td class="numeric"><?= $this->bpas->formatMoney($subTotal) ?></td>
                                        <td class="numeric"><?= $this->bpas->formatMoney($subReturn) ?></td>
                                        <td class="numeric"><?= $this->bpas->formatMoney($subPaid) ?></td>
                                        <td class="numeric"><?= $this->bpas->formatMoney($subDeposit) ?></td>
                                        <td class="numeric"><?= $this->bpas->formatMoney($subDiscount) ?></td>
                                        <td class="numeric"><?= $this->bpas->formatMoney($gbalance) ?></td> 
                                    </tr>
                            <?php } ?>
                        <?php } ?>
                    </table>
...
<script>
    $(document).ready(function(){
        $("#excel").click(function(e){
            e.preventDefault();
            window.location.href = "<?=admin_url('Account/apBySupplier/0/xls/0/'.$start_date2.'/'.$end_date2.'/'.$supplier2.'/'.$balance2) ?>";
            return false;
        });
        $('#pdf').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('Account/apBySupplier/pdf/0/0/?v=1'.$v)?>";
            return false;
        });
        $('#ap_statement').click(function (event) {
            event.preventDefault();
            window.open('<?= admin_url('Account/apBySupplier/0/0/ap_statement/'.$start_date2.'/'.$end_date2.'/'.$supplier2.'/'.$balance2) ?>', '_blank');
            return false;
        });
    });
</script>