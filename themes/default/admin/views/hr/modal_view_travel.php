		...
		<div class="modal-body">
						...
						<th>
							<table>
								<tr>
									...
									<td class="text_center" style="width:20%">
										<?= $this->bpas->qrcode('link', urlencode(admin_url('hr/modal_view_travel/' . $travel->id)), 2); ?>
									</td>
								</tr>
							</table>
						</th>
						...
						<th>
							<table>
								<tr>
									...
									<td class="text_center" style="width:20%"><span style="font-size:<?= $font_size+5 ?>px"><b><i><?= lang('travels') ?></i></b></span></td>
									...
									<td style="width:100%">
										<fieldset>
											...
											<table>
												<tr>
													<td><?= lang('purpose') ?></td>
													<td> : <strong><?= $travel->purpose ?></strong></td>
													<td><?= lang('date') ?></td>
													<td style="text-align:left"> : <?= $this->bpas->hrsd($travel->date) ?></td>
												</tr>
												<tr>
													<td><?= lang('place') ?></td>
													<td> : <strong><?= $travel->place ?></strong></td>
													<td><?= lang('from_date') ?></td>
													<td style="text-align:left"> : <?= $this->bpas->hrsd($travel->start_date) ?></td>
												</tr>
												<tr>
													<td><?= lang('budget') ?></td>
													<td> : <strong><?= $this->bpas->formatDecimal($travel->budget) ?></strong></td>
													<td><?= lang('to_date') ?></td>
													<td style="text-align:left"> : <?= $this->bpas->hrsd($travel->end_date) ?></td>
												</tr>
											</table>
										</fieldset>
									</td>
						...
						<td>
							<table class="table_item">
								...
								<tfoot>
									<?php if($travel->note){?>
										<tr>
											<td style="border:0px !important" colspan="5"><b><?= lang('note') ?> : </b> <?= $this->bpas->decode_html($travel->note)  ?></td>
										</tr>
									<?php } ?>
								</tfoot>
							</table>
			...
			<div id="buttons" style="padding-top:10px;" class="no-print">
				...
				<div class="btn-group btn-group-justified">
					...
					<?php if ($travel->attachment) { ?>
						<div class="btn-group">
							<a href="<?= site_url('welcome/download/' . $travel->attachment) ?>" class="tip btn btn-primary" target="_blank" title="<?= lang('attachment') ?>">
					...
					<?php } ?>
					<div class="btn-group">
						<a href="<?= admin_url('hr/edit_travel/' . $travel->id) ?>" class="tip btn btn-warning sledit" title="<?= lang('edit') ?>">
					...
					<div class="btn-group">
						<a href="#" class="tip btn btn-danger bpo" title="<b><?= $this->lang->line("delete") ?></b>"
							data-content="<div style='width:150px;'><p><?= lang('r_u_sure') ?></p><a class='btn btn-danger' href='<?=admin_url('hr/delete_travel/' . $travel->id) ?>'><?= lang('i_m_sure') ?></a> <button class='btn bpo-close'><?= lang('no') ?></button></div>"
