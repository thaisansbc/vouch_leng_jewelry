...
<script>
    $(document).ready(function () {
        oTable = $('#RBA').dataTable({
            "aaSorting": [[0, "desc"]],
            "aLengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "<?= lang('all') ?>"]],
            "iDisplayLength": <?= $Settings->rows_per_page ?>,
            'bProcessing': true, 'bServerSide': true,
            'sAjaxSource': '<?= admin_url('hr/getResignationsReport/?v=1' . $v); ?>',
			...
			'fnRowCallback': function (nRow, aData, iDisplayIndex) {
				...
				nRow.id = aData[10];
				nRow.className = "hr_resination_link";
				return nRow;
			},
			"aoColumns": [
					{"mRender": fld}, null, null, null, null, null, {"mRender": fsd},{"mRender": fsd}, {"mRender": decode_html},
			...
			{column_number: 6, filter_default_label: "[<?=lang('inform_date');?> (yyyy-mm-dd)]", filter_type: "text", data: []},
			{column_number: 7, filter_default_label: "[<?=lang('last_date');?> (yyyy-mm-dd)]", filter_type: "text", data: []},
			{column_number: 8, filter_default_label: "[<?=lang('reason');?>]", filter_type: "text", data: []},
        ], "footer");
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-dollar"></i><?= lang('resignations_report'); ?></h2>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo admin_form_open("hr/resignations_report"); ?>
                    <div class="row">
						<div class="col-sm-4">
						...
						<div class="col-md-4">
							...
							<div class="form-group">
                                <div class="form-group">
                                    <?php echo form_input('from_date', (isset($_POST['from_date']) ? $_POST['from_date'] : ""), 'class="form-control input-tip date" id="date""'); ?>
                                </div>
                            </div>	
						</div>
						<div class="col-md-4">
							<label class="control-label" for="to_date"><?= lang("to_date"); ?></label>
							<div class="form-group">
                                <div class="form-group">
                                    <?php echo form_input('to_date', (isset($_POST['to_date']) ? $_POST['to_date'] : ""), 'class="form-control input-tip date" id="date""'); ?>
                                </div>
                            </div>	
						</div>
						<div class="col-sm-4">
                            <div class="form-group">
                                <label class="control-label" for="suggest_employee_all"><?= lang("employee"); ?></label>
								<input type="text" name="employee_id" id="suggest_employee_all" value="<?= set_value('employee_id') ?>" class="form-control ui-autocomplete-input" />
								<input type="hidden" name="employee" value="<?= set_value('employee') ?>" id="suggest_employee_all_id">
							</div>
                        </div>
                    </div>
                    <div class="form-group">
                ...
                <div class="table-responsive">
                    <table id="RBA" cellpadding="0" cellspacing="0" border="0" class="table table-bordered table-hover table-striped">
                        <thead>
							<tr class="active">
								<th><?= lang("date"); ?></th>	
								<th><?= lang("code"); ?></th>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#xls').click(function (event) {
            ...
            window.location.href = "<?=admin_url('hr/getResignationsReport/xls/?v=1'.$v)?>";
            return false;
        });
