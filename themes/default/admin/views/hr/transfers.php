...
<script>
    $(document).ready(function () {
        oTable = $('#id_cardData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('hr/getTransfers/'.($biller ? $biller->id : '')); ?>',
			...
			'fnRowCallback': function (nRow, aData, iDisplayIndex) {
				...
				if(aData[5] == 'pending'){
					action.find('.unapprove_transfer').remove();
				}else if(aData[5] == 'approved'){
					action.find('.approve_transfer').remove();
					action.find('.edit_transfer').remove();
					action.find('.delete_transfer').remove();
				}
            },
            "aoColumns": [
                {"mRender": checkbox},
...
<?php if ($Owner || $Admin || $GP['bulk_actions']) {
    echo admin_form_open('hr/transfer_actions', 'id="action-form"');
} ?>
<div class="box">
    <div class="box-header">
		<h2 class="blue"><i class="fa-fw fa fa-heart-o"></i><?= lang('transfers').' ('.($biller ? $biller->name : lang('all_billers')).')' ?></h2>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right" class="tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li>
                            <a href="<?= admin_url('hr/add_transfer') ?>"><i class="fa fa-plus-circle"></i> <?= lang('add_transfer') ?>
                            </a>
                        </li>
                        <li>
				...
				<?php if (!empty($billers) && $this->config->item('one_biller')==false) { ?>
					<li class="dropdown">
						...
						<ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
							<li><a href="<?= admin_url('hr/transfers') ?>"><i class="fa fa-industry"></i> <?= lang('all_billers') ?></a></li>
							<li class="divider"></li>
							<?php
							foreach ($billers as $biller) {
								echo '<li><a href="' . admin_url('hr/transfers/'.$biller->id) . '"><i class="fa fa-home"></i>' . $biller->name . '</a></li>';
							}
							?>
						</ul>
					</liv>
