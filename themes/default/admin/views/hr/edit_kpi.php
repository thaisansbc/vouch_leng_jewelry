...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        <?php  if ($kpi_info) { ?>
			localStorage.setItem('kpdate', '<?= $this->bpas->hrsd($kpi_info->date) ?>');
			localStorage.setItem('kpmonth', '<?= $kpi_info->month ?>');
			localStorage.setItem('kpemployee', '<?= $kpi_info->employee_id ?>');
			localStorage.setItem('kpkpi_type', '<?= $kpi_info->kpi_type_id ?>');
			localStorage.setItem('kpmnote', '<?= $this->bpas->remove_tag($kpi_info->manager_note) ?>');
			localStorage.setItem('kpenote', '<?= $this->bpas->remove_tag($kpi_info->employee_note) ?>');
			localStorage.setItem('kpitems', JSON.stringify(<?= $kpi_items ?>));
        <?php } ?>
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-plus"></i><?= lang('edit_kpi'); ?></h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo admin_form_open_multipart("hr/edit_kpi/".$kpi_info->id, $attrib);
                ?>
                <div class="row">
                    <div class="col-lg-12">
						...
						<div class="col-md-4">
							<div class="form-group">
								...
								<?php
								$kpi_p["0"] = lang("select") . ' ' . lang("kpi_type");
								if($kpi_types){
									foreach ($kpi_types as $kpi_type) {
										$kpi_p[$kpi_type->id] = $kpi_type->name;
									}
								}
								echo form_dropdown('kpi_type', $kpi_p, (isset($_POST['kpi_type']) ? $_POST['kpi_type'] : ''), 'id="kpkpi_type" data-placeholder="' . lang("select") . ' ' . lang("kpi_type") . '" required="required" class="form-control input-tip select" style="width:100%;"');
								?>
							</div>
						</div>
						<div class="col-md-4">
                        ...
                        <div class="col-md-12">
                            <div
                                class="fprom-group"><?php echo form_submit('edit_kpi', lang("submit"), 'id="add_cost_adjustment" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;"'); ?>
