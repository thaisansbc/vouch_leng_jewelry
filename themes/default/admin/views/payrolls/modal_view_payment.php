						...
						<th>
							<table>
								<tr>
									...
									<td class="text_center" style="width:20%">
										<?= $this->bpas->qrcode('link', urlencode(admin_url('payrolls/modal_view_payment/' . $payment->id)), 2); ?>
									</td>
								</tr>
							</table>
						</th>
						...
						<th>
							<table>
								<tr>
									...
									<td class="text_center" style="width:15%"><span style="font-size:<?= $font_size+5 ?>px"><b><i><?= lang('payment') ?></i></b></span></td>
									...
									<td style="width:60%">
										<fieldset>
											...
											<table>
												...
												<?php 
													if($payment->position_id > 0){ 
														$position = $this->payrolls_model->getPositionByID($payment->position_id);
														echo "<tr><td>".lang("position")."</td><td> : ".$position->name."</td></tr>";
													} 
													if($payment->department_id > 0){ 
														$department = $this->payrolls_model->getDepartmentByID($payment->department_id);
														echo "<tr><td>".lang("department")."</td><td> : ".$department->name."</td></tr>";
													} 
													if($payment->group_id > 0){ 
														$group = $this->payrolls_model->getGroupByID($payment->group_id);
					...
					<?php
						...
						foreach ($payment_items as $payment_item){
							$tbody .='<tr>
											...
											<td class="text_right">'.$this->bpas->formatMoney($payment_item->tax_paid).'</td>
											<td class="text_right">'.$this->bpas->formatMoney($payment_item->salary_paid).'</td>
											<td class="text_right">'.$this->bpas->formatMoney($payment_item->tax_paid + $payment_item->salary_paid).'</td>
										</tr>';	
							$total_tax_paid += $payment_item->tax_paid;	
							$total_salary_paid += $payment_item->salary_paid;								
							$i++;
						}
						$footer_colspan = 2;
						...
						$tfooter = '<tr>
										...
										<td class="text_right"><b>'.$this->bpas->formatMoney($total_tax_paid).'</b></td>
										<td class="text_right"><b>'.$this->bpas->formatMoney($total_salary_paid).'</b></td>
										<td class="text_right"><b>'.$this->bpas->formatMoney($total_tax_paid + $total_salary_paid).'</b></td>
									</tr>';
					?>
					<tr>
						<td>
