...
<script>
    $(document).ready(function () {
        oTable = $('#RSLD').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('payrolls/getSeverancDetailsReport/?v=1' . $v); ?>',
            ...
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var first_salary = 0,second_salary = 0,third_salary = 0,total_salary = 0,severance = 0, total_usd = 0, total_khr = 0, usd_100 = 0, usd_50 = 0, usd_20 = 0, usd_10 = 0, khr_20000 = 0, khr_10000 = 0, khr_5000 = 0, khr_2000 = 0, khr_1000 = 0, khr_500 = 0, khr_100 = 0;
                for (var i = 0; i < aaData.length; i++) {
					first_salary += parseFloat(aaData[aiDisplay[i]][7]);
					second_salary += parseFloat(aaData[aiDisplay[i]][8]);
					third_salary += parseFloat(aaData[aiDisplay[i]][9]);
					total_salary += parseFloat(aaData[aiDisplay[i]][10]);
					severance += parseFloat(aaData[aiDisplay[i]][11]);
					total_usd += parseFloat(aaData[aiDisplay[i]][12]);
					total_khr += parseFloat(aaData[aiDisplay[i]][13]);
					usd_100 += parseFloat(aaData[aiDisplay[i]][14]);
					usd_50 += parseFloat(aaData[aiDisplay[i]][15]);
					usd_20 += parseFloat(aaData[aiDisplay[i]][16]);
					usd_10 += parseFloat(aaData[aiDisplay[i]][17]);
					khr_20000 += parseFloat(aaData[aiDisplay[i]][18]);
					khr_10000 += parseFloat(aaData[aiDisplay[i]][19]);
					khr_5000 += parseFloat(aaData[aiDisplay[i]][20]);
					khr_2000 += parseFloat(aaData[aiDisplay[i]][21]);
					khr_1000 += parseFloat(aaData[aiDisplay[i]][22]);
					khr_500 += parseFloat(aaData[aiDisplay[i]][23]);
					khr_100 += parseFloat(aaData[aiDisplay[i]][24]);
                }
                var nCells = nRow.getElementsByTagName('th');
				nCells[7].innerHTML = currencyFormat(first_salary);
				nCells[8].innerHTML = currencyFormat(second_salary);
				nCells[9].innerHTML = currencyFormatKH(third_salary);
				nCells[10].innerHTML = currencyFormat(total_salary);
				nCells[11].innerHTML = currencyFormat(severance);
				nCells[12].innerHTML = currencyFormat(total_usd);
				nCells[13].innerHTML = currencyFormatKH(total_khr);
				nCells[14].innerHTML = (usd_100);
				nCells[15].innerHTML = (usd_50);
				nCells[16].innerHTML = (usd_20);
				nCells[17].innerHTML = (usd_10);
				nCells[18].innerHTML = (khr_20000);
				nCells[19].innerHTML = (khr_10000);
				nCells[20].innerHTML = (khr_5000);
				nCells[21].innerHTML = (khr_2000);
				nCells[22].innerHTML = (khr_1000);
				nCells[23].innerHTML = (khr_500);
				nCells[24].innerHTML = (khr_100);
            }
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-dollar"></i><?= lang('severance_details_report'); ?></h2>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo form_open("payrolls/severance_details_report"); ?>
                    <div class="row">
						<div class="col-md-4">
                    ...
                    <table id="RSLD" cellpadding="0" cellspacing="0" border="0" class="table table-bordered table-hover table-striped">
                        <thead>
							<tr class="active">
								...
								<th rowspan="2"><?= $first_month ?></th>
								<th rowspan="2"><?= $second_month ?></th>
								<th rowspan="2"><?= $third_month ?></th>
								<th rowspan="2"><?= lang("total_salary"); ?></th>
								<th rowspan="2"><?= lang("severance"); ?></th>
                        ...
                        <tbody>
							<tr>
								<td colspan="25" class="dataTables_empty"><?= lang('loading_data_from_server'); ?></td>
							</tr>
                        </tbody>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#xls').click(function (event) {
            ...
            window.location.href = "<?=site_url('payrolls/getSeverancDetailsReport/xls/?v=1'.$v)?>";
            return false;
        });
