...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-plus"></i><?= lang('edit_daily_salary'); ?></h2>
    </div>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <?php
					...
					echo admin_form_open_multipart("payrolls/edit_daily_salary/".$salary->id, $attrib);
                ?>
                <div class="row">
					<div class="col-md-12">
                    ...
                    <?php
						...
						if($additions){
							...
							foreach($additions as $addition){
								if(($addition->name != 'Severance') && 
									($addition->name != 'Indemnity') && ($addition->name != 'Seniority')){
									$th_addition_sub .= '<th>'.$addition->name.'</th>';
									$colspan++;
								}
							}
							$th_addition =  '<th colspan="'.$colspan.'">'.lang("addition").'</th>';
						}
						...
						if($deductions){
							...
							$th_deduction =  '<th colspan="'.$colspan.'">'.lang("deduction").'</th>';
						}
                                    ...
                                    <table id="expTable" class="table items table-striped table-bordered table-condensed table-hover sortable_table">
                                        <thead>
											<tr>
												...
												<?= $th_addition ?>
												<?= $th_deduction ?>
												<th colspan="2"><?= lang("seniority") ?></th>
												...
												<th colspan ="2"><?= lang("tax_on_salary") ?></th>
												<th rowspan="2"><?= lang("self_tax") ?></th>
												<th rowspan="2"><?= lang("net_salary") ?></th>
												<th rowspan="2"><?= lang("severance") ?><br>ប្រាក់បំណាច់កិច្ចសន្យា</th>
												<th rowspan="2"><?= lang("indemnity") ?><br>ប្រាក់សំណង</th>
													...
													foreach($salary_items as $salary_item){
														...
														if($additions){
															...
															foreach($additions as $addition){
																...
																if(($addition->name != 'Severance') && 
																	($addition->name != 'Indemnity') && 
																	($addition->name != 'Seniority')){
														...
														$dataEmp .= "<tr>
															...
															<td><input name='approved_att_id[]' value='".$salary_item->approved_att_id."' type='hidden'/>".$salary_item->lastname." ".$salary_item->firstname."</td>
														...
														$dataEmp .= "
															...
															<td class='text-right'><input name='seniority_response_tax[]' value='".$salary_item->seniority_response_tax."' type='hidden'/>".$this->bpas->formatMoney($salary_item->seniority_response_tax)."</td>
														...
														$dataEmp .= "<td class='text-right'><input name='gross_salary[]' value='".$salary_item->gross_salary."' type='hidden'/>".$this->bpas->formatMoney($salary_item->gross_salary)."</td>
															...
															<td class='text-right'><input name='gross_salary_riel[]' value='".$salary_item->gross_salary_riel."' type='hidden'/>".$this->bpas->formatMoneykh($salary_item->gross_salary_riel)."</td>
															...
															<td class='text-right'><input name='tax_payment_riel[]' value='".$salary_item->tax_payment_riel."' type='hidden'/>(".$this->bpas->formatMoney($salary_item->tax_payment_riel).")</td>
															...
															<td class='text-right'><input name='severance[]' value='".$salary_item->severance."' type='hidden'/>".$this->bpas->formatMoney($salary_item->severance)."</td>
															<td class='text-right hide'><input name='severance_riel[]' value='".$salary_item->severance_riel."' type='hidden'/>".$this->bpas->formatMoney($salary_item->severance_riel)."</td>
															<td class='text-right'><input name='indemnity[]' value='".$salary_item->indemnity."' type='hidden'/>".$this->bpas->formatMoney($salary_item->indemnity)."</td>
															<td class='text-right hide'><input name='indemnity_riel[]' value='".$salary_item->indemnity_riel."' type='hidden'/>".$this->bpas->formatMoney($salary_item->indemnity_riel)."</td>
	...
	$(document).ready(function () {
		function getSalaryEmployee(){
			...
			$.ajax({
				type: "get", 
				async: true,
				url: site.base_url + "payrolls/get_salary_daily_employees/",
				data : { 
						biller_id 		: biller_id,
					...
					if (data != false) {
						$.each(data, function () {
							...
							dataEmp += "<tr>";
								...
								dataEmp += "<td><input name='approved_att_id[]' value='"+this.approved_att_id+"' type='hidden'/>"+this.lastname+" "+this.firstname+"</td>";
								...
								dataEmp += "<td class='text-right'><input name='seniority_response_tax[]' value='"+this.seniority_response_tax+"' type='hidden'/>"+(this.seniority_response_tax > 0 ? formatMoney(this.seniority_response_tax) : "-")+"</td>";
								...
								dataEmp += "<td class='text-right'><input name='tax_payment_riel[]' value='"+this.tax_payment_riel+"' type='hidden'/>("+formatMoney(this.tax_payment_riel)+")</td>";
								...
								dataEmp += "<td class='text-right'><input name='severance[]' value='"+this.severance+"' type='hidden'/>"+(this.severance > 0 ? formatMoney(this.severance) : "-")+"</td>";
								dataEmp += "<td class='text-right hide'><input name='severance_riel[]' value='"+this.severance_riel+"' type='hidden'/>"+(this.severance_riel > 0 ? formatMoney(this.severance_riel) : "-")+"</td>";
								dataEmp += "<td class='text-right'><input name='indemnity[]' value='"+this.indemnity+"' type='hidden'/>"+(this.indemnity > 0 ? formatMoney(this.indemnity) : "-")+"</td>";
								dataEmp += "<td class='text-right hide'><input name='indemnity_riel[]' value='"+this.indemnity_riel+"' type='hidden'/>"+(this.indemnity_riel > 0 ? formatMoney(this.indemnity_riel) : "-")+"</td>";
