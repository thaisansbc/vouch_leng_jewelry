...
<script>
    $(document).ready(function () {
        ...
        oTable = $('#RBFD').dataTable({
            "aaSorting": [[0, "desc"]],
            "aLengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "<?= lang('all') ?>"]],
            "iDisplayLength": <?= $Settings->rows_per_page ?>,
            'bProcessing': true, 'bServerSide': true,
            'sAjaxSource': '<?= admin_url('payrolls/getBenefitDetailsReport/?v=1' . $v); ?>',
			...
			"aoColumns": [{"mRender": fld},null,null,null,null,null,null,{"mRender": currencyFormat}, {"mRender": currencyFormat},{"mRender": currencyFormat},{"mRender" : row_status}],
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var total_cash_advanced = 0, total_addition = 0, total_deduction = 0;
                for (var i = 0; i < aaData.length; i++) {
                    total_cash_advanced += parseFloat(aaData[aiDisplay[i]][7]);
					total_addition += parseFloat(aaData[aiDisplay[i]][8]);
					total_deduction += parseFloat(aaData[aiDisplay[i]][9]);
                }
                var nCells = nRow.getElementsByTagName('th');
                nCells[7].innerHTML = currencyFormat(total_cash_advanced);
				nCells[8].innerHTML = currencyFormat(total_addition);
				nCells[9].innerHTML = currencyFormat(total_deduction);
            }
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-dollar"></i><?= lang('benefit_details_report'); ?></h2>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo admin_form_open("payrolls/benefit_details_report"); ?>
                    <div class="row">
						<div class="col-md-4">
                ...
                <div class="table-responsive">
                    <table id="RBFD" cellpadding="0" cellspacing="0" border="0"
...
<script type="text/javascript">
    $(document).ready(function () {
		$('#pdf').click(function (event) {
            ...
            window.location.href = "<?=admin_url('payrolls/getBenefitDetailsReport/pdf/?v=1'.$v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('payrolls/getBenefitDetailsReport/0/xls/?v=1'.$v)?>";
            return false;
        });
