...
<script>
    $(document).ready(function () {
        ...
        oTable = $('#RSLD').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('payrolls/getPreSalaryDetailsReport/?v=1' . $v); ?>',
			...
			"aoColumns": [{"mRender": fld},null,null,null,null,null,null, {"mRender": currencyFormat},{"mRender": currencyFormat},{"mRender": currencyFormat},{"mRender" : row_status},{"mRender" : pay_status}],
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var gross_salary = 0, paid = 0, balance = 0;
                for (var i = 0; i < aaData.length; i++) {
                    gross_salary += parseFloat(aaData[aiDisplay[i]][7]);
					paid += parseFloat(aaData[aiDisplay[i]][8]);
					balance += parseFloat(aaData[aiDisplay[i]][9]);
                }
                var nCells = nRow.getElementsByTagName('th');
                nCells[7].innerHTML = currencyFormat(gross_salary);
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-dollar"></i><?= lang('pre_salary_details_report'); ?></h2>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#xls').click(function (event) {
            ...
            window.location.href = "<?= admin_url('payrolls/getPreSalaryDetailsReport/xls/?v=1'.$v)?>";
            return false;
        });
