...
<script>
    $(document).ready(function () {
        ...
        oTable = $('#RSLD').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('payrolls/getNssfReport/?v=1' . $v); ?>',
            ...
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var total_gross_salary = 0, total_overtime = 0, total_addition = 0, total_cash_advanced = 0, total_tax_payment = 0, total_net_salary = 0, total_net_pay = 0, total_salary_paid = 0, total_tax_paid = 0, total_paid = 0, total_balance = 0,total_deduction=0;
                for (var i = 0; i < aaData.length; i++) {
                    total_gross_salary += parseFloat(aaData[aiDisplay[i]][7]);
					total_overtime += parseFloat(aaData[aiDisplay[i]][8]);
					total_addition += parseFloat(aaData[aiDisplay[i]][9]);
					total_deduction += parseFloat(aaData[aiDisplay[i]][10]);
					total_cash_advanced += parseFloat(aaData[aiDisplay[i]][11]);
		
                }
                var nCells = nRow.getElementsByTagName('th');
                nCells[7].innerHTML = currencyFormat(total_gross_salary);
				nCells[8].innerHTML = currencyFormat(total_overtime);
				nCells[9].innerHTML = currencyFormat(total_addition);
				nCells[10].innerHTML = currencyFormat(total_deduction);
				nCells[11].innerHTML = currencyFormat(total_cash_advanced);
            }
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-dollar"></i><?= lang('nssf_report'); ?></h2>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo admin_form_open("payrolls/nssf_report"); ?>
                    <div class="row">
						<div class="col-md-4">
                    ...
                    <table id="RSLD" cellpadding="0" cellspacing="0" border="0"
                        ...
                        <thead>
							<tr class="active">
								...
								<th><?= lang("group"); ?></th>
								<th><?= lang("contributory_nssf"); ?></th>
								<th><?= lang("pension_by_staff"); ?></th>
								<th><?= lang("pension_by_company"); ?></th>
								<th><?= lang("health_nssf"); ?></th>
								<th><?= lang("accident_nssf"); ?></th>
							</tr>
                        </thead>
                        <tbody>
							<tr>
...
<script type="text/javascript">
    $(document).ready(function () {
		$('#pdf').click(function (event) {
            ...
            window.location.href = "<?=admin_url('payrolls/getNssfReport/pdf/?v=1'.$v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('payrolls/getNssfReport/0/xls/?v=1'.$v)?>";
            return false;
        });
