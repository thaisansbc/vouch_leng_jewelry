...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-plus"></i><?= lang('add_salary_daily'); ?></h2>
    </div>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <?php
					...
					echo admin_form_open_multipart("payrolls/add_salary_daily", $attrib);
                ...
                <div class="row">
								...
								<?php if ($this->Settings->project) { ?>
		                            <div class="col-md-3">
		                                ...
		                                <div class="project_box form-group">
		                                    ...
		                                    echo form_dropdown('project', $pro, (isset($_POST['project']) ? $_POST['project'] : $project_id), 'id="project" data-placeholder="' . lang("select") . ' ' . lang("project") . '" class="form-control input-tip select" style="width:100%;"');
		                                    ?>
		                                </div>
		                            </div>
		                        <?php } ?>
                    ...
                    <?php
						...
						if($additions){
							...
							$th_addition =  '<th colspan="'.$colspan.'">'.lang("addition").' ប្រាក់បន្ថែម</th>';
						}
						...
						if($deductions){
							...
							$th_deduction =  '<th colspan="'.$colspan.'" style="min-width: 80px;">'.lang("deduction").' ប្រាក់កាត់</th>';
						}
                            ...
                            <div class="controls table-controls">
                                <table id="expTable" class="table items table-striped table-bordered table-condensed table-hover sortable_table">
                                   <thead style="font-size: 12px;">
										<tr>
											<th rowspan="2" style="min-width: 80px;"><?= lang("code") ?></th>
											<th rowspan="2" style="min-width: 100px;"><?= lang("name") ?></th>
											<th colspan="7" class="<?= $enable_attendance ? '':'hide'; ?>"><?= lang("attendance") ?> វត្តមាន</th>
											<th rowspan="2" width="80"><?= lang("salary_per_day") ?><br>ប្រាក់គោល</th>
											<th rowspan="2"><?= lang("absent") ?><br>អវត្តមាន</th>
											<th rowspan="2"><?= lang("permission") ?><br>ច្បាប់</th>
											<th rowspan="2" style="min-width: 80px;"><?= lang("late") ?> យឺត / <?= lang("early") ?> ចេញមុន</th>
											<th rowspan="2"><?= lang("overtime") ?><br>ថែមម៉ោង</th>
											<?= $th_addition ?>
											<?= $th_deduction ?>
											<th colspan="2"><?= lang("seniority") ?> ប្រាក់អតីតភាព</th>

											<th colspan="2" class="text-center"><?= lang("salary") ?></th>
											<th colspan="5" class="text-center"><?= lang("NSSF") ?> ប.ស.ស</th>
											<th colspan="2"><?= lang("gross_salary") ?></th>
											<th rowspan="2"><?= lang("spouse") ?>ប្រពន្ធ</th>
											<th rowspan="2"><?= lang("children") ?>កូន</th>
	...
	$(document).ready(function () {
				...
				success: function (data) {
					...
					if (data != false) {
						$.each(data, function () {
							...
							dataEmp += "<tr>";
								dataEmp += "<td width='50px'><input name='employee_id[]' value='"+employee_id+"' type='hidden'/>"+this.empcode+"</td>";
		...
		$(document).on("change", "#biller", function () {	
			...
			$.ajax({
				...
				dataType: "json",
				success: function (data) {
					var project_sel = "<select class='form-control' id='project' name='project'><option value=''><?= lang('select').' '.lang('project') ?></option>";
					if (data != false) {
						$.each(data, function () {
		...
		$(document).on("change", "#group, #position, #month, #project", function () {	
			getSalaryEmployee();
		});
