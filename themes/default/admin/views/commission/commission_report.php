...
<script>
    $(document).ready(function () {
        ...
        oTable = $('#PayRData').dataTable({
            ...
            'bProcessing': true,
            'bServerSide': true,
            'sAjaxSource': '<?= admin_url('reports/getLeasingcommission_Report/?v=1' . $v) ?>',
            ...
            "aoColumns": [{"mRender": fld}, {"mRender": fld}, null, null, null, {"mRender": ref}, {"mRender": ref},
                 null, 
    			{"mRender": currencyFormat},null,{"mRender": row_status}, null, 
    			{"bVisible": false}],
            ...
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var total = 0, stl = 0, sa = 0, commission = 0;
                for (var i = 0; i < aaData.length; i++) {
                    // if (aaData[aiDisplay[i]][8] == 'sent' || aaData[aiDisplay[i]][8] == 'returned'){
                    //     commission -= parseFloat(aaData[aiDisplay[i]][5]);
                    //     total -= parseFloat(aaData[aiDisplay[i]][7]);
                    // }else{
                        commission += parseFloat(aaData[aiDisplay[i]][8]);
                        total += parseFloat(aaData[aiDisplay[i]][9]);
					// }
					// stl += parseFloat(aaData[aiDisplay[i]][9]);
					// sa += parseFloat(aaData[aiDisplay[i]][10]);
                }
                var nCells = nRow.getElementsByTagName('th');
                nCells[8].innerHTML = currencyFormat(parseFloat(commission));
                nCells[9].innerHTML = currencyFormat(parseFloat(total));
                // nCells[9].innerHTML = currencyFormat(parseFloat(stl));
                // nCells[10].innerHTML = currencyFormat(parseFloat(sa));
            }
        }).fnSetFilteringDelay().dtFilter([
            {column_number: 0, filter_default_label: "[<?=lang('date');?> (yyyy-mm-dd)]", filter_type: "text", data: []},
            {column_number: 1, filter_default_label: "[(yyyy-mm-dd)]", filter_type: "text", data: []},
            {column_number: 2, filter_default_label: "[<?=lang('paid_by');?>]", filter_type: "text", data: []},
            {column_number: 3, filter_default_label: "[<?=lang('create_by');?>]", filter_type: "text", data: []},
            {column_number: 4, filter_default_label: "[<?=lang('customer');?>]", filter_type: "text", data: []},
            {column_number: 5, filter_default_label: "[<?=lang('payment_ref');?>]", filter_type: "text", data: []},
            {column_number: 6, filter_default_label: "[<?=lang('sale_ref');?>]", filter_type: "text", data: []},
            {column_number: 7, filter_default_label: "[<?=lang('saleman');?>]", filter_type: "text", data: []},
            {column_number: 8, filter_default_label: "[<?=lang('amount');?>]", filter_type: "text", data: []},
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-money"></i><?= lang('commission_report'); ?> <?php
            if ($this->input->post('start_date')) {
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo admin_form_open("reports/leasing_commission"); ?>
                    <div class="row">
                        <div class="col-sm-4">
                           ...
                           <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                foreach ($salemans as $saleman) {
                                    $sl[$saleman->id] = $saleman->first_name . " " . $saleman->last_name;
                                }
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=admin_url('reports/getLeasingcommission_Report/pdf/?v=1'.$v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/getLeasingcommission_Report/0/xls/?v=1'.$v)?>";
            return false;
        });
