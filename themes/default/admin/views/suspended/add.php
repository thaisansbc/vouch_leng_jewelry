...
<div id="wrapper">
    ...
    <div id="content">
		<div class="c1">
			<h1 class="modal-title text-center" id="ajaxModalLabel">Kitchen Display</h1>
			<div class="pos">
				<?php
				foreach($kitchen_note as $note_order){
					$date = date_create($note_order->date);
					echo '<div class="table_kitchen" val="'.$note_order->id.'">';
						echo '<span>'.date_format($date, 'H:i:s').'</span>';
						echo '<div class="table_name text-center">'.$note_order->suspend_note.'</div>';
						$this->db->select("*");
						$this->db->from("sma_suspended_items");
						$this->db->where("suspend_id",$note_order->id);
						$query=$this->db->get();
						$result= $query->result_array();
						foreach($result as $data){
							echo '<div class="food_name"><i class="fa fa-cutlery"></i> '.$data['product_name'].'</div>';
						}
					echo '</div>';
