    ...
    <div class="container">
                ...
                <div class="header-nav">
                    ...
                    <ul class="nav navbar-nav pull-right">
                        ...
                        <li class="dropdown">
                            <a class="btn bdarkGreen pos-tip" id="add_expense" title="<span><?=lang('add_expense')?></span>" data-placement="bottom" data-html="true" href="<?=admin_url('expenses/add_expense')?>" data-toggle="modal" data-backdrop="static" data-target="#myModal">
...
?>
    ...
    <div id="content">
        ...
        <div class="c1" style="width: 100%;">
            ...
            <div style="background: #fff;text-align: center;font-size: 24px;font-size: bold;">
                <span><?= lang("member_cards_display") ;?></span>
            </div>
            <div style="width: 100%;">
                <div style="width: 20%;float: left;background: #fff;height: 100vh;">
                    <ul class="list-group">
                      <li class="list-group-item active"><?= lang('table')?></li>
                      <?php 
                      foreach($tables as $row) {
                        echo '<li class="list-group-item" style="'.($row->status?'background:red':'').'">'.$row->name.'</li>';
                      }
                      ?>
                    </ul>
                </div>
                <div style="width: 55%;float: left;">
                    <div style="width:200px;padding: 10px;margin: 0 auto;">
                        <input type="text" name="search" class="form-control">
                    </div>
                    <?php
                    $i=1;
                    if(isset($checkin_cards[0])){
                    foreach($checkin_cards as $note_order){
                        ...
                        echo '<div class="table_display">';
                            echo '<div class="'.$class.' sus_card" 
                                    card_id="'.$note_order->id.'" 
                                    room_name="'.$note_order->card_no.'" 
                                    sid="0" 
                                    price="'.$note_order->price.'" 
                                    discount="'.$note_order->price.'
                                ">';
                                if($note_order->suspend_note){
                                ...
                                echo '<div class="table_name text-center">'.$note_order->card_no.'</div>';
                                
                                if($note_order->suspend_note){
                                    echo '<div style="width:100%;height:80px;background:#D82924;"> 
                                            <div class="text-center txt_title">Amount: '.$note_order->price.' </div>
                                   </div>';
                                }else{
                                    echo '<div style="width:100%;height:80px;background:#78CD51;"> '.$note_order->price.'</div>';
                                }
                        echo '</div>';
                            if($note_order->suspend_note){
                                ...
                                if ($this->Owner || $GP->close_table) {
                                    echo '| <a href="#" cu_id="'.$note_order->id.'" broom="'.$note_order->id.'" class="sus-delete"><i class="fa fa-trash-o"></i></a>';
                                }
                ...
                <div style="width: 25%;float: left;background: #fff;height: 100vh;">
                    <a class="btn bblue pos-tip" style="color: #ffffff;" title="<span><?=lang('add_expense')?></span>" data-placement="bottom" data-html="true" href="<?=admin_url('gym/add_booking')?>" data-toggle="modal" data-backdrop="static" data-target="#myModal">booking now
                    </a>

                    <table width="100%" border="0" class="table table-hover table-striped dataTable">
                        <thead>
                            <th width="50%"><?= lang('name');?></th>
                            <th width="30%"><?= lang('phone');?></th>
                            <th width="20%"><?= lang('table');?></th>
                        </thead>
                        <tr>
                            <td>Ly makara</td>
                            <td>01236547</td>
    ...
    $(window).bind("resize", widthFunctions);
        $(document).ready(function () {
            $('.sus_card').on('click', function (e) {
                ...
                window.location.href = "<?=admin_url('pos/index')?>/"+sid+'/'+card_id; 
