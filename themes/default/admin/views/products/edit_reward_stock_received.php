...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-edit"></i><?= lang('edit_reward_stock_received'); ?></h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo admin_form_open_multipart('products/edit_reward_stock_received/' . $stock_received->id, $attrib); ?>
                <div class="row">
                    <div class="col-lg-12">
                        <?php if ($Owner || $Admin || $GP['change_date']) { ?>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <?= lang('date', 'podate'); ?>
                                    <?php echo form_input('date', (isset($_POST['date']) ? $_POST['date'] : $this->bpas->hrld($stock_received->date)), 'class="form-control input-tip datetime" id="podate" required="required"'); ?>
                                </div>
                            </div>
                        <?php } ?>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('received_reference_no', (isset($_POST['received_reference_no']) ? $_POST['received_reference_no'] : $stock_received->reference_no), 'class="form-control input-tip" id="received_poref" required="required"'); ?>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                echo form_dropdown('warehouse', $wh, (isset($_POST['warehouse']) ? $_POST['warehouse'] : $stock_received->warehouse_id), 'id="powarehouse" class="form-control input-tip select" data-placeholder="' . $this->lang->line('select') . ' ' . $this->lang->line('warehouse') . '" required="required" style="width:100%;"');
                                ?>
                            </div>
                        </div>
                                    ...
                                    <table id="rewardTable" class="table items table-striped table-bordered table-condensed table-hover sortable_table">
                                        ...
                                        <tbody>
                                            <?php foreach ($inv_items as $inv_item) {
                                                $quantity_balance = $inv_item->receive_quantity - $inv_item->received_quantity;
                                                foreach ($stock_received_items as $stock_received_item) { 
                                                    if ($inv_item->reward_exchange_item_id == $stock_received_item->reward_exchange_item_id) {
                                                        $quantity_balance += $stock_received_item->quantity;
                                                    }
                                                }
                                            ?>
                                            <tr>
                                                <td>
                                                    ...
                                                    <input type="hidden" class="rex_quantity_balance" value="<?= $quantity_balance; ?>">
                                                ...
                                                <td><?= $this->bpas->formatDecimal($quantity_balance); ?></td>
                                                <th style="width: 30px !important; text-align: center;">
                                                    <?php if ($quantity_balance <= 0) { ?>
                                                        &nbsp;
                                                    <?php } else { ?>
                            ...
                            <div class="control-group table-group">
                                ...
                                <div class="controls table-controls">
                                    <table id="receiveTable" class="table items table-striped table-bordered table-condensed table-hover sortable_table">
                                        ...
                                        <tbody>
                                            <?php foreach ($stock_received_items as $stock_received_item) {
                                                $quantity_balance = $stock_received_item->quantity;
                                                foreach ($inv_items as $inv_item) { 
                                                    if ($stock_received_item->reward_exchange_item_id == $inv_item->reward_exchange_item_id) {
                                                        $quantity_balance += ($inv_item->receive_quantity - $inv_item->received_quantity);
                                                    }
                                                } ?>
                                                <tr id="<?= $stock_received_item->reward_exchange_item_id; ?>">
                                                    <td>
                                                        <input name="reward_exchange_item_id[]" type="hidden" class="rreward_exchange_item_id" value="<?= $stock_received_item->reward_exchange_item_id; ?>">
                                                        <input name="product[]" type="hidden" class="rproduct_id" value="<?= $stock_received_item->product_id; ?>">
                                                        <input name="product_code[]" type="hidden" value="<?= $stock_received_item->product_code; ?>">
                                                        <input name="product_name[]" type="hidden" value="<?= $stock_received_item->product_name; ?>">
                                                        <input name="product_unit_id[]" type="hidden" value="<?= $stock_received_item->unit; ?>">
                                                        <?= $stock_received_item->product_code . ' - ' . $stock_received_item->product_name . ' (' . $stock_received_item->unit_name . ')'; ?>
                                                    </td>
                                                    <?php if ($Settings->product_expiry == 1) { ?>
                                                        <td><input class="form-control date rexpiry" name="expiry[]" type="text" value="<?= ($stock_received_item->expiry ? $this->bpas->hrsd($stock_received_item->expiry) : ''); ?>"></td>
                                                    <?php } ?>
                                                    <td>
                                                        <input type="hidden" class="rquantity_balance" value="<?= $quantity_balance; ?>">
                                                        <input class="form-control rquantity" name="quantity[]" type="text" value="<?= $this->bpas->formatDecimal($stock_received_item->quantity); ?>">
                                                    </td>
                                                    <td class="text-center"><i class="fa fa-times tip podel" title="Remove" style="cursor:pointer;"></i></td>
                                                </tr>
                                            <?php } ?>
                                        </tbody>
                                    </table>
                        ...
                        <div class="col-md-12">
                            <div class="from-group"><?php echo form_submit('edit_stock_receive', $this->lang->line('submit'), 'id="edit_stock_receive" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;"'); ?>
        ...
        function checkReceivedItems() {
            ...
            if ($('#receiveTable tbody tr').length > 0) {
                if (checkOverbalance(gp_by_item) == true) {
                    $('#edit_stock_receive').attr('disabled', true);
                    ...
                    $('#edit_stock_receive').attr('disabled', false);
                }
            } else {
                $('#edit_stock_receive').attr('disabled', true);
            }
