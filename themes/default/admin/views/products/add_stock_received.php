...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-edit"></i><?= lang('add_stock_received'); ?></h2>
    </div>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                $attrib = ['data-toggle' => 'validator', 'role' => 'form', 'class' => 'edit-po-form'];
                    echo admin_form_open_multipart('purchases/add_stock_received/' . $inv->id, $attrib)
                ?>
                <div class="row">
                    <div class="col-lg-12">
                        <?php if ($Owner || $Admin || $GP['change_date']) { ?>
...
<script type="text/javascript">
    ...
    $('#podiscount').focus(function () {
        ...
        $(document).on("focus", '.received', function () {
            ...
            $.each(poitems[item_id].units, function(){
                if (this.id == unit) {
                    x_received = formatDecimal(baseToUnitQty(poitems[item_id].row.x_received, this), 4);
                }
            });
            ...
            if (parseFloat(new_received) > (parseFloat(poitems[item_id].row.qty) - parseFloat(x_received))) {
                $(this).val(old_received);
                bootbox.alert(lang.unexpected_value);
                return;
            }
        ...
        if (localStorage.getItem('poitems')) {
            ...
            $.each(sortedItems, function () {
                ...
                var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_cost = item.row.cost, 
                    ...
                    item_expiry = item.row.expiry, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
                ...
                tr_html += '<td><input name="ordered_quantity[]" type="hidden" class="oqty" value="' + item_oqty + '">' + 
                               '<input class="form-control text-center received" name="received[]" type="text" value="' + formatDecimal(unit_qty_received) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="received_' + row_no + '" onClick="this.select();">' + 
                ...
                count_qty_received += parseFloat(item_x_received);
