...
<script type="text/javascript">
	...
	/*
		$('.multi-select').live('ifChecked',function(){
			...
			var array = [];
			var icheckbox = document.querySelectorAll('input[type="checkbox"]:checked'); 
			console.log(icheckbox);
	
			var ichecklength = icheckbox.length;
			if(icheckbox[0].value == "on"){
				ichecklength = icheckbox.length - 2;
			}
			var type_value = $("#type").val();
			let currentYear = <?= date('y'); ?>;
			var settings = <?= json_encode($this->Settings); ?>;
			$.ajax({
				url: site.base_url + 'taxs/getInvoicesTax',
				type: 'GET',
				dataType: 'Json',
				data: {type: type_value, icheck : ichecklength},
				success: function(data) {
					parent.find('.tax_reference').val(settings.sales_tax_prefix + currentYear + "/" + data); 
				}
        	}); 
			parent.find('.enable_input').prop("disabled", false);
		});
		$('.multi-select').live('ifUnchecked',function(){
			...
			var array = [];
			var icheckbox = document.querySelectorAll('input[type="checkbox"]:checked'); 
			for (var i = 0; i < icheckbox.length; i++) {
				array.push(icheckbox[i]);
			}
			var value = parent.find('.tax_reference').val();
			parent.find('.tax_reference').val(null); 
			parent.find('.enable_input').prop("disabled", true);
		});
*/
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-plus"></i><?= lang('add_tax'); ?></h2>
    </div>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <?php
					...
					echo admin_form_open_multipart("taxs/add_tax", $attrib);
                ?>
                <div class="row">
					<div class="col-md-12">
						<div class="col-md-4">
                                ...
                                <div class="panel-body" style="padding: 5px;">
									...
									<div class="col-md-4">
										<div class="form-group">
											...
											<?php echo form_input('from_date', (isset($_POST['from_date']) ? $_POST['from_date'] : date("d/m/Y")), 'class="form-control input-tip date" id="from_date" required="required"'); ?>
										</div>
									</div>
									<div class="col-md-4">
										<div class="form-group">
											<?= lang("to_date", "to_date"); ?>
											<?php echo form_input('to_date', (isset($_POST['to_date']) ? $_POST['to_date'] : date("d/m/Y")), 'class="form-control input-tip date" id="to_date" required="required"'); ?>
										</div>
									</div>
                                </div>
                            </div>
