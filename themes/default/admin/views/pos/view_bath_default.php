        ...
        <div id="receiptData">
            ...
            <div id="receipt-data">
                <div class="text-center">
					...
					$khm=" ៛";
					$bat=" B";
					$bat_out= $exchange_rate_bat_out->rate;
				...
				<table width="100%">
					...
					<tr>
						...
						if($inv->suspend_note){ 
							$data = $this->bpas->get_room_name($inv->suspend_note);
							echo ': '.$data->name;
						}
						?>
						</td>
                ...
                <table class="table table-condensed">
					...
					<tbody>
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo '<tr>
									...
									<td border-bottom">'.$bat.$this->bpas->formatMoney($row->unit_price*$bat_out).($row->item_tax != 0 ? ' - '.lang('tax').' <small>('.($Settings->indian_gst ? $row->tax : $row->tax_code).')</small> '.$this->bpas->formatMoney($row->item_tax).' ('.lang('hsn_code').': '.$row->hsn_code.')' : '').'</td>
									<td class="no-border border-bottom text-right">'.$bat.$this->bpas->formatMoney($row->subtotal * $bat_out) . '</td>
								</tr>';
							}
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th class="text-right">
							<?php
							$total_bat=$this->bpas->formatMoney($return_sale ? (($inv->total + $inv->product_tax)+($return_sale->total + $return_sale->product_tax)) : ($inv->total + $inv->product_tax));
								echo $bat.$this->bpas->formatMoney($total_bat * $bat_out);
							?>
							</th>
                        </tr>
                        <?php
                        ...
                        if ($pos_settings->rounding || $inv->rounding != 0) {
                            ...
                            <tr>
								...
								$grand_bat=$this->bpas->formatMoney($return_sale ? (($inv->grand_total + $inv->rounding)+$return_sale->grand_total) : ($inv->grand_total + $inv->rounding));
								echo $bat.$this->bpas->formatMoney($grand_bat * $bat_out);
								?></th>
                            </tr>
                            <?php
                        }
								...
								if (($payment->paid_by == 'cash' || $payment->paid_by == 'deposit') && $payment->pos_paid) {
									if($paid_amount_usd){
										...
										//	echo '<td class="text-right">'.$usd.$this->bpas->formatMoney($payment->pos_paid == 0 ? ($payment->amount / $inv->other_cur_paid_rate) : ($payment->pos_paid /$inv->other_cur_paid_rate) ) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td>';
											echo '<td class="border_top border_right text-right">'.$usd.$this->bpas->formatMoney($amount_usd). '</td>';
										echo '</tr>';
									}
									if($paid_amount_kh){
										...
										echo '<tr>';
											...
											echo '<td class="border_top">:</td>';
												echo '<td class="border_top border_right text-right">'.$khm.$this->bpas->formatMoney($amount_kh * $currency_rate_kh).'</td>';
										echo '</tr>';
									}
									if($paid_amount_bat){
                ...
                <?= $biller->invoice_footer ? '<div class="text-center">Developed By <a href="http://sbcsolution.biz/">SBC Solutions</a></div>' : ''; ?>
