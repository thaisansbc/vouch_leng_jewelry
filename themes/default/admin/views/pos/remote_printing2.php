...
if ($pos_settings->remote_printing != 1) {
    ...
    $info = '';
    $info .= lang('date') . ': ' . $this->bpas->hrld($inv->date) . "\n";
    ...
    foreach ($rows as $row) {
        $r = 1;
        $items .= printLine(product_name(addslashes('#' . $r . ' ' . $row->product_code), 18) . ': ' . printLine($this->bpas->formatQuantity($row->unit_quantity) . ' x ' . $this->bpas->formatMoney($row->unit_price) . ':  ' . $this->bpas->formatMoney($row->subtotal), 14), $char_per_line, ' ') . "\n";
        ...
        if ($row->item_tax != 0) {
            $items .= '   ' . lang('tax') . ' (' . ($Settings->indian_gst ? $row->tax : $row->tax_code) . ') ' . $this->bpas->formatMoney($row->item_tax) . ($row->hsn_code ? ' (' . lang('hsn_code') . ': ' . $row->hsn_code . ')' : '') . "\n";
        }
        $r++;
    }
    if ($return_rows) {
    ...
    $totals = '';
    $totals .= printLine(lang('total') . ': ' . $this->bpas->formatMoney($return_sale ? (($inv->total + $inv->product_tax) + ($return_sale->total + $return_sale->product_tax)) : ($inv->total + $inv->product_tax)), $char_per_line) . "\n";
    if ($Settings->indian_gst) {
        if ($inv->cgst > 0) {
            $cgst = $return_sale ? $inv->cgst + $return_sale->cgst : $inv->cgst;
            $totals .= printLine(lang('cgst') . ': ' . ($Settings->format_gst ? $this->bpas->formatMoney($cgst) : $cgst), $char_per_line) . "\n";
        }
        if ($inv->sgst > 0) {
            $sgst = $return_sale ? $inv->sgst + $return_sale->sgst : $inv->sgst;
            $totals .= printLine(lang('sgst') . ': ' . ($Settings->format_gst ? $this->bpas->formatMoney($sgst) : $sgst), $char_per_line) . "\n";
        }
        if ($inv->igst > 0) {
            $igst = $return_sale ? $inv->igst + $return_sale->igst : $inv->igst;
            $totals .= printLine(lang('igst') . ': ' . ($Settings->format_gst ? $this->bpas->formatMoney($igst) : $igst), $char_per_line) . "\n";
        }
    }
    if ($inv->order_tax != 0) {
        $totals .= printLine(lang('tax') . ': ' . $this->bpas->formatMoney($return_sale ? ($inv->order_tax + $return_sale->order_tax) : $inv->order_tax), $char_per_line) . "\n";
    }
    if ($inv->total_discount != 0) {
        $totals .= printLine(lang('discount') . ': (' . $this->bpas->formatMoney($return_sale ? ($inv->product_discount + $return_sale->product_discount) : $inv->product_discount) . ') ' . $this->bpas->formatMoney($return_sale ? ($inv->order_discount + $return_sale->order_discount) : $inv->order_discount), $char_per_line) . "\n";
    }
    if ($inv->shipping != 0) {
        $totals .= printLine(lang('shipping') . ': ' . $this->bpas->formatMoney($inv->shipping), $char_per_line) . "\n";
    }
    if ($pos_settings->rounding || $inv->rounding != 0) {
        $totals .= printLine(lang('rounding') . ': ' . $this->bpas->formatMoney($inv->rounding), $char_per_line) . "\n";
        $totals .= printLine(lang('grand_total') . ': ' . $this->bpas->formatMoney($return_sale ? ($this->bpas->roundMoney($inv->grand_total + $inv->rounding) + $return_sale->grand_total) : $this->bpas->roundMoney($inv->grand_total + $inv->rounding)), $char_per_line) . "\n";
    } else {
        $totals .= printLine(lang('grand_total') . ': ' . $this->bpas->formatMoney($return_sale ? ($inv->grand_total + $return_sale->grand_total) : $inv->grand_total), $char_per_line) . "\n";
    }
    $totals .= printLine(lang('paid_amount') . ': ' . $this->bpas->formatMoney($return_sale ? ($inv->paid + $return_sale->paid) : $inv->paid), $char_per_line) . "\n";
    $totals .= printLine(lang('due_amount') . ': ' . $this->bpas->formatMoney(($return_sale ? ($inv->grand_total + $return_sale->grand_total) : $inv->grand_total) - ($return_sale ? ($inv->paid + $return_sale->paid) : $inv->paid)), $char_per_line) . "\n";
    $totals .= "\n";
    ...
    if ($payments) {
        foreach ($payments as $payment) {
            if ($payment->paid_by == 'cash' || $payment->paid_by == 'deposit' && $payment->pos_paid) {
                ...
                $payments_text .= printLine(lang('amount') . ': ' . $this->bpas->formatMoney($payment->pos_paid), $char_per_line) . "\n";
                $payments_text .= printLine(lang('change') . ': ' . ($payment->pos_balance > 0 ? $this->bpas->formatMoney($payment->pos_balance) : '0.00'), $char_per_line) . "\n";
            }
            ...
            if ($payment->paid_by == 'Cheque' && $payment->cheque_no) {
                ...
                if ($payment->paid_by == 'other' && $payment->amount) {
                    $payments_text .= printLine(lang('paid_by') . ': ' . lang($payment->paid_by), $char_per_line) . "\n";
                    $payments_text .= printLine(lang('amount') . ': ' . $this->bpas->formatMoney($payment->amount), $char_per_line) . "\n";
    ...
    if ($Settings->invoice_view > 0) {
        if (!empty($tax_summary)) {
            ...
            foreach ($tax_summary as $summary) {
                $footer .= taxLine($summary['name'], $summary['code'], $this->bpas->formatQuantity($summary['items']), $this->bpas->formatMoney($summary['amt']), $this->bpas->formatMoney($summary['tax']), $char_per_line) . "\n";
            }
            $footer .= str_replace("\n", '', drawLine($char_per_line)) . "\n";
    ...
    if ($inv->note) {
        $footer .= printText(strip_tags(preg_replace('/\s+/', ' ', $this->bpas->decode_html($inv->note))), $char_per_line) . "\n\n";
    }
