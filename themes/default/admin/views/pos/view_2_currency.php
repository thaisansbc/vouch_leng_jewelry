...
<!doctype html>
    ...
    <head>
        ...
        <style type="text/css" media="all">
			...
			table .border_top{
				border-top:1px solid #847b7b !important;
			}
			table .border_bottom{
				border-bottom:1px solid #847b7b !important;
			}
			table .border_left{
				border-left:1px solid #847b7b !important;
			}
			table .border_right{
				border-right:1px solid #847b7b !important;
			}
            @media print {
				table{
					font-size: 14px !important;
            ...
            <div id="receipt-data">
				...
				<table width="100%">
					...
					<tr>
						...
						<td width="15%"><?php echo lang("tel");?></td>
						<td width="40%">
						<?php
                ...
                <table class="table table-condensed">
					...
					<tbody>
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo '<tr>
									<td>' . $r . '</td>
									<td>'.product_name($row->product_name, ($printer ? $printer->char_per_line : null)) . ($row->variant ? ' (' . $row->variant . ')' : '') . '<span class="pull-right">' . ($row->tax_code ? '*'.$row->tax_code : '') . '</span></td>
									<td border-bottom">'.$this->bpas->formatQuantity($row->unit_quantity).'</td>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th><?=lang("total");?></th>
                            <th>:</th>
                            <th class="text-right"><?= $usd.$this->bpas->formatMoney($return_sale ? (($inv->total + $inv->product_tax)+($return_sale->total + $return_sale->product_tax)) : ($inv->total + $inv->product_tax));?></th>
                        </tr>
                        <?php
                        if ($inv->order_tax != 0) {
                        ...
                        if ($inv->order_discount != 0) {
                            echo '<tr><th>' . lang("order_discount") . '</th><th class="text-right">' . $this->bpas->formatMoney($inv->order_discount) . '</th></tr>';
                        }
                        if ($inv->shipping != 0) {
                        ...
                        if ($pos_settings->rounding || $inv->rounding != 0) {
                            ...
                            <tr>
                                <th></th>
                                <th><?php echo "ប្រាក់សរុបចុងក្រោយ";?></th>
                                <th><?=lang("grand_total");?></th>
								<th>:</th>
                                <th class="text-right"><?= $usd.$this->bpas->formatMoney($return_sale ? ($inv->grand_total+$return_sale->grand_total) : $inv->grand_total);?></th>
                            </tr>
                            <?php
                        }
                        if ($inv->paid < ($inv->grand_total + $inv->rounding)) {
                            ...
                            <tr>
                                ...
                                <th class="border_top"><?=lang("paid_amount").$usd;?></th>
								<th class="border_top">:</th>
                                <th class="border_top border_right text-right">
								<?= $usd.$this->bpas->formatMoney($return_sale ? ($inv->paid+$return_sale->paid) : $inv->paid);?>
								</th>
                            </tr>
							<?php
							}else{
								$paid_kh=$this->bpas->formatMoney($return_sale ? ($inv->paid+$return_sale->paid) : $inv->paid);
							?>
							 <tr>
                                <th class="border_left"></th>
                                <th><?php echo "ប្រាក់បង់";?></th>
                                <th><?=lang("paid_amount").$khm;?></th>
								<th>:</th>
                                <th class="border_right text-right">
								<?= $khm.$this->bpas->formatMoney($paid_kh * $currency_rate_kh);?>
								</th>
                            </tr>
							<?php
							}
                            ...
                            <tr>
                                <th class="border_left"></th>
                                <th><?php echo "ប្រាក់ជំពាក់​ជាដុល្លា";?></th>
                                <th><?=lang("due_amount").$usd;?></th>
								<th>:</th>
                                <th class="border_right text-right">
									<?= $usd.$this->bpas->formatMoney(($return_sale ? (($inv->grand_total + $inv->rounding)+$return_sale->grand_total) : ($inv->grand_total + $inv->rounding)) - ($return_sale ? ($inv->paid+$return_sale->paid) : $inv->paid));?>
								</th>
                            </tr>
                            <?php
							}
							...
							<tr>
                                <th class="border_left"></th>
                                <th><?php echo "ប្រាក់ជំពាក់ជារៀល";?></th>
						...
						if ($payments) {
							foreach ($payments as $payment) {
								if (($payment->paid_by == 'cash' || $payment->paid_by == 'deposit') && $payment->pos_paid) {
									...
									if($paid_amount_kh){
										echo '<tr>';
											echo '<th class="border_left"></th>';
											echo '<td>' . "ប្រាក់ទទួលជារៀល".'</td>';
											echo '<td>' . lang("amount").'('.$khm.')</td>';
											echo '<td>:</td>';
												echo '<td class="border_right text-right">'.$khm.$this->bpas->formatMoney($paid_amount_kh).'</td>';
										echo '</tr>';
									}
									if($paid_amount_usd){
										echo '<tr>';
										...
										echo '<tr>';
											...
											echo '<td class="border_bottom">:</td>';
											echo '<td class="border_bottom border_right text-right">'.$khm.$this->bpas->formatMoney($payment->pos_balance > 0 ? $this->bpas->formatMoney($payment->pos_balance) : 0).'</th>';
										echo '</tr>';
									}
								}
