    ...
    <div id="wrapper">
        <div id="receiptData">
            ...
            <div id="receipt-data">
				...
				<table width="100%">
					<tr>
						<td width="5%"><?php echo lang("Cust");?></td>
                ...
                <table class="table table-condensed">
					...
					<tbody>
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo '<tr>
									...
									<td>'.product_name($row->product_name, ($printer ? $printer->char_per_line : null)) . ($row->variant ? ' (' . $row->variant . ')' : '') .'</td>'; /*'<span class="pull-right">' . ($row->tax_code ? '*'.$row->tax_code : '') . '</span></td>';*/
										if($row->product_code == "Time"){
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td>:</td>
                            <th class="text-right">
							<?= $usd.$this->bpas->formatMoney($return_sale ? (($inv->total + $inv->product_tax)+($return_sale->total + $return_sale->product_tax)) : ($inv->total + $inv->product_tax));?></td>
                        </tr>
                        <?php
						if ($inv->order_discount != 0) {
                            ...
                            echo '<tr>
									...
									<td>បញ្ចុះតំលៃ</td>
									<td>' . lang("discount") . $dp.'</td>
									<td>:</td>
									<th class="text-right">' . $usd.$this->bpas->formatMoney($inv->order_discount) . '</td></tr>';
                        }
                        if ($inv->order_tax != 0) {
						?>
						<td><?php echo "អាករ";?></td>
						<?php
                            echo '<td>' . lang("VAT 10%") . '</td><td>:</td></td><th class="text-right">' . $this->bpas->formatMoney($return_sale ? ($inv->order_tax+$return_sale->order_tax) : $inv->order_tax) . '</td></tr>';
                        }

                        if ($inv->shipping != 0) {
                            echo '<tr><td>ថ្លៃដឹកជញ្ជូន</td><td>' . lang("delivery") . '</td><td>:</td><th class="text-right">' . '-' . $this->bpas->formatMoney($inv->shipping) . '</td></tr>';
                        }
                        if ($return_sale) {
                            if ($return_sale->surcharge != 0) {
                                echo '' . lang("order_discount") . '</td><th class="text-right">' . $this->bpas->formatMoney($return_sale->surcharge) . '</td></tr>';
                            }
                        }
                        ...
                        if ($pos_settings->rounding || $inv->rounding != 0) {
                            ...
                            <tr style="border:1px solid #adabab;">
                                <th style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;"><?php echo "សរុបចុងក្រោយ";?></td>
                                <th style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;"><?=lang("grand_total");?></td>
								<th style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;">:</td>
                                <th style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;" class="text-right"><?= $usd.$this->bpas->formatMoney($return_sale ? ($inv->grand_total+$return_sale->grand_total) : $inv->grand_total);?></td>
                            </tr>
							<tr style="border:1px solid #adabab;">
                                <th style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;"></td>
                                <th style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;"></td>
								...
								?>
								<th style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;" class="text-right"><?= $khm. number_format(floor($gTotalkh/100)*100);?></td>
							</tr>
                            <?php
							endif;
                        }
						...
						?>
							<!--tr style="border:1px solid #adabab;">
                                <th style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;"></td>
                                <th style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;"><?php echo "សរុបការបញ្ចុះតំលៃ";?></td>
                                <th style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;"><?=lang("Total Saved");?></td>
								<th style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;">:</td>
                                <th style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;" class="text-right"><?=$usd.$this->bpas->formatMoney($totalSaved);?></td>
                            </tr-->
						<?php
						endif;
                        if ($inv->paid < ($inv->grand_total + $inv->rounding)) {
                           $original_paid_amount = $this->bpas->formatMoney($return_sale ? ($inv->paid+$return_sale->paid) : $inv->paid);
							...
							?>
								<tr>
									<th class="border_top"><?php echo "ប្រាក់បង់";?></td>
									<th width="110px" class="border_top"><?=lang("paid_amount").$usd;?></td>
							...
							/*else{
								...
								?>
								 <tr>
									<td><?php echo ($paid_amount_usd)? "" : "ប្រាក់បង់";?></td>
									<th width="110px" ><?=lang("paid_amount").$khm;?></td>
									<td>:</td>
								...
								?>
								 <tr>
									<th class="border_top"><?php echo ($paid_amount_usd)? "" : "ប្រាក់បង់";?></td>
									<th class="border_top"><?=lang("paid_amount").$bat;?></td>
									<th class="border_top">:</td>
									<th class="border_top border_right text-right">
								...
								?>
								<tr>
									<td><?php echo "ប្រាក់ជំពាក់​";?></td>
									<td><?=lang("due_amount").$usd;?></td>
									<td>:</td>
									<th class="border_right text-right">
							...
							/*else{
								...
								?>
								<tr>
									<td><?php echo ($paid_amount_usd)? "" : "ប្រាក់ជំពាក់";?></td>
									<td><?=lang("due_amount").$khm;?></td>
									<td>:</td>
									<th class="border_right text-right">
							...
							foreach ($payments as $payment) {
								if (($payment->paid_by == 'cash' || $payment->paid_by == 'deposit') && $payment->pos_paid) {
									if($paid_amount_usd){
										...
										echo '<tr>';
										echo '<td class="border_top">' . "ទទួលជាដុល្លា".'</td>';
									...
									if($paid_amount_usd){
										...
										echo '<tr>';
											echo '<td class="border_bottom"></td>';
											echo '<td class="border_bottom"></td>';
											$decChange = $this->bpas->formatMoney($payment->pos_balance > 0 ? $this->bpas->formatMoney($payment->pos_balance) : 0);
											$decChangeFinal = $decChange - floor($decChange);
										...
										echo '<tr>';
											...
											if($realChanged % 100 >= 50){
												echo '<td class="border_bottom border_right text-right">'.$khm.number_format(ceil(($change_kh * $currency_rate_kh) / 100) * 100) . '</td>';
											}else{
												echo '<td class="border_bottom border_right text-right">'.$khm.number_format(floor(($change_kh * $currency_rate_kh) / 100) * 100) . '</td>';
											}
				...
				<?php // $biller->invoice_footer ? '<div class="text-center"><font style="font-size:11px;">Developed By <a href="http://sbcsolution.biz/">SBC Solutions | sbcsolution.biz</a></font></div>' : ''; ?>


				<!--div class="text-center">
					<?php $this->bpas->qrcode('link', urlencode('http://sbcsolution.biz/'), 2); ?>
					<img
					src="<?= base_url() ?>assets/uploads/qr_code_cfi.jpg"
					alt="<?= $inv->reference_no ?>"/>
				</div-->
				<!--qrcode<?= $this->session->userdata('user_id') ?>.png-->
			</div>
