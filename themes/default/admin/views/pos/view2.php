                ...
                <table class="table table-condensed">
                    ...
                    <tbody>
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo '<tr><td colspan="2" class="no-border">#' . $r . ': &nbsp;&nbsp;' . $row->product_code . '</td><td class="no-border text-center" style="width:120px;">' . $this->bpas->formatQuantity($row->unit_quantity) . ' x ' . $this->bpas->formatMoney($row->unit_price) . '</td><td class="no-border" style="width:80px;"><span class="pull-right">' . $this->bpas->formatMoney($row->subtotal) . '</span></td></tr>';
                            ...
                            echo '<tr><td colspan="4" class="no-border border-bottom">' . ($row->item_tax != 0 ? lang('tax') . ' <small>(' . ($Settings->indian_gst ? $row->tax : $row->tax_code) . ')</small> ' . $this->bpas->formatMoney($row->item_tax) . ($row->hsn_code ? ' (' . lang('hsn_code') . ': ' . $row->hsn_code . ')' : '') : '') . '</td></tr>';
                            $r++;
                        }
                        if ($return_rows) {
                            ...
                            foreach ($return_rows as $row) {
                                ...
                                echo '<tr><td colspan="2" class="no-border">#' . $r . ': &nbsp;&nbsp;' . $row->product_code . '</td><td class="no-border text-center">' . $this->bpas->formatQuantity($row->unit_quantity) . ' x ' . $this->bpas->formatMoney($row->unit_price) . '</td><td class="no-border"><span class="pull-right">' . $this->bpas->formatMoney($row->subtotal) . '</span></td></tr>';
                                echo '<tr><td colspan="4" class="no-border" style="padding-top:0;padding-bottom:0;">' . product_name($row->product_name, ($printer ? $printer->char_per_line : null)) . ($row->variant ? ' (' . $row->variant . ')' : '') . '</td></tr>';
                                if (!empty($row->second_name)) {
                                    echo '<tr><td colspan="4" class="no-border" style="padding-top:0;padding-bottom:0;">' . $row->second_name . '</td></tr>';
                                }
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th class="text-right"><?=$this->bpas->formatMoney($return_sale ? (($inv->total + $inv->product_tax) + ($return_sale->total + $return_sale->product_tax)) : ($inv->total + $inv->product_tax));?></th>
                        </tr>
                        <?php
                        if ($inv->order_tax != 0) {
                            echo '<tr><th colspan="3">' . lang('tax') . '</th><th class="text-right">' . $this->bpas->formatMoney($return_sale ? ($inv->order_tax + $return_sale->order_tax) : $inv->order_tax) . '</th></tr>';
                        }
                        if ($inv->order_discount != 0) {
                            echo '<tr><th colspan="3">' . lang('order_discount') . '</th><th class="text-right">' . $this->bpas->formatMoney($inv->order_discount) . '</th></tr>';
                        }

                        if ($inv->shipping != 0) {
                            echo '<tr><th colspan="3">' . lang('shipping') . '</th><th class="text-right">' . $this->bpas->formatMoney($inv->shipping) . '</th></tr>';
                        }

                        if ($return_sale) {
                            if ($return_sale->surcharge != 0) {
                                echo '<tr><th colspan="3">' . lang('order_discount') . '</th><th class="text-right">' . $this->bpas->formatMoney($return_sale->surcharge) . '</th></tr>';
                            }
                        }
                        ...
                        if ($Settings->indian_gst) {
                            if ($inv->cgst > 0) {
                                ...
                                echo '<tr><td colspan="3">' . lang('cgst') . '</td><td class="text-right">' . ($Settings->format_gst ? $this->bpas->formatMoney($cgst) : $cgst) . '</td></tr>';
                            }
                            if ($inv->sgst > 0) {
                                $sgst = $return_sale ? $inv->sgst + $return_sale->sgst : $inv->sgst;
                                echo '<tr><td colspan="3">' . lang('sgst') . '</td><td class="text-right">' . ($Settings->format_gst ? $this->bpas->formatMoney($sgst) : $sgst) . '</td></tr>';
                            }
                            if ($inv->igst > 0) {
                                $igst = $return_sale ? $inv->igst + $return_sale->igst : $inv->igst;
                                echo '<tr><td colspan="3">' . lang('igst') . '</td><td class="text-right">' . ($Settings->format_gst ? $this->bpas->formatMoney($igst) : $igst) . '</td></tr>';
                            }
                        }
                        ...
                        if ($pos_settings->rounding || $inv->rounding != 0) {
                            ...
                            <tr>
                                <th colspan="3"><?=lang('grand_total'); ?></th>
                                <th class="text-right"><?=$this->bpas->formatMoney($return_sale ? ($inv->grand_total + $return_sale->grand_total) : $inv->grand_total); ?></th>
                            </tr>
                            <?php
                        }
                        if ($inv->paid < ($inv->grand_total + $inv->rounding)) {
                            ...
                            <tr>
                                ...
                                <th class="text-right"><?=$this->bpas->formatMoney($return_sale ? ($inv->paid + $return_sale->paid) : $inv->paid); ?></th>
                            </tr>
                            <tr>
                                <th colspan="3"><?=lang('due_amount'); ?></th>
                                <th class="text-right"><?=$this->bpas->formatMoney(($return_sale ? (($inv->grand_total + $inv->rounding) + $return_sale->grand_total) : ($inv->grand_total + $inv->rounding)) - ($return_sale ? ($inv->paid + $return_sale->paid) : $inv->paid)); ?></th>
                            </tr>
                            <?php
                        } ?>
                    </tfoot>
                ...
                if ($payments) {
                    ...
                    foreach ($payments as $payment) {
                        ...
                        if (($payment->paid_by == 'cash' || $payment->paid_by == 'deposit') && $payment->pos_paid) {
                            ...
                            echo '<td colspan="2">' . lang('amount') . ': ' . $this->bpas->formatMoney($payment->pos_paid == 0 ? $payment->amount : $payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td>';
                            ...
                            echo '<td>' . lang('paid_by') . ': ' . lang($payment->paid_by) . '</td>';
                            echo '<td colspan="2">' . lang('amount') . ': ' . $this->bpas->formatMoney($payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td>';
