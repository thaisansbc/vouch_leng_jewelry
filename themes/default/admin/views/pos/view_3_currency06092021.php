            ...
            <div id="receipt-data">
                ...
                <table width="100%">
                    <tr>
                        ...
                        <td width="40%"><?php echo ': ' . ($customer->company && $customer->company != '-' ? $customer->company : $customer->name).' ['.($inv->customer_qty).']'; ?></td>
                ...
                <table class="table table-condensed">
                    ...
                    <tbody>
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo '<tr>
                                    ...
                                    <td class="border-bottom-">'.product_name($row->product_name, ($printer ? $printer->char_per_line : null)) . ($row->variant ? ' (' . $row->variant . ')' : '') .'</td>'; /*'<span class="pull-right">' . ($row->tax_code ? '*'.$row->tax_code : '') . '</span></td>';*/
                                        if($row->product_code == "Time"){
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td>:</td>
                            <th class="text-right">
                            <?= $detault_currency.$this->bpas->formatMoney($return_sale ? (($inv->total + $inv->product_tax)+($return_sale->total + $return_sale->product_tax)) : ($inv->total + $inv->product_tax));?></td>
                        </tr>
                        <?php
                        ...
                        ?>
                          ...
                          <?php
                            echo '<td class="text-right" colspan="2">' . lang("អាករ") . '/' . lang("VAT 10%") . '</td><td>:</td></td><th class="text-right">' . $this->bpas->formatMoney($return_sale ? ($inv->order_tax+$return_sale->order_tax) : $inv->order_tax) . '</td></tr>';
                        }

                        if ($inv->shipping != 0) {
                            echo '<tr><td colspan="2" class="text-right">ថ្លៃដឹកជញ្ជូន/' . lang("delivery") . '</td><td>:</td><th class="text-right">' . '-' . $this->bpas->formatMoney($inv->shipping) . '</td></tr>';
                        }
                        if ($return_sale) {
                            if ($return_sale->surcharge != 0) {
                                echo '<tr><td colspan="2" class="text-right">' . lang("order_discount") . '</td><th class="text-right">' . $this->bpas->formatMoney($return_sale->surcharge) . '</td></tr>';
                            }
                        }
                        ...
                        if ($pos_settings->rounding || $inv->rounding != 0) {
                            ...
                            <tr><th colspan="2" class="text-right" style="border-top:1px solid #adabab;border-bottom:1px solid #adabab;">អត្រាប្តូរប្រាក់/<?= lang('rate');?></th>
                        ...
                        if ($inv->paid >0) {
                           $original_paid_amount = $this->bpas->formatMoney($return_sale ? ($inv->paid+$return_sale->paid) : $inv->paid);
                            ...
                            ?>
                                <tr>
                                    <th colspan="2" class="border_top text-right"><?php echo "ប្រាក់ទទួល";?>/<?=lang("cash_received").$usd;?></td>
                        ...
                        if (0) {
                            foreach ($payments as $payment) {
                                ...
                                if($Settings->default_currency =='USD'){
                                    $default_received_lang = "ទទួលជាដុល្លា".'/' . lang("cash_received").'('.$usd.')';
                                    $received_lang_kh = "ទទួលជារៀល".'/' . lang("cash_received").'('.$khm.')';
                                }else{
                                    $default_received_lang = "ទទួលជារៀល".'/' . lang("cash_received").'('.$khm.')';
                                    $received_lang_kh = "ទទួលជាដុល្លា".'/' . lang("cash_received").'('.$usd.')';
                                }
                                ...
                                if (($payment->paid_by == 'cash' || $payment->paid_by == 'deposit') && $payment->pos_paid) {
                                    ...
                                    if($paid_amount_kh){
                                        ...
                                        echo '<tr>';
                                            echo '<td colspan="2" class="border_top text-right">'.$received_lang_kh.'</td>';
                                            echo '<td class="border_top">:</td>';
                                                echo '<td class="border_top border_right text-right">
                                                '.(($Settings->default_currency =='USD')? '៛': '$').number_format($paid_amount_kh).'</td>';
                                        echo '</tr>';
                                    }
                                }
                            }
                                        ...
                                        if($Settings->default_currency =='USD'){
                                            ...
                                            if($paid_amount_usd){
                                                ...
                                                echo '<tr>';
                                                    echo '<td colspan="2" class="border_bottom text-right">' ."ប្រាក់អាប់".'/' .lang("change").$khm.'</td>';
                                                    echo '<td class="border_bottom">:</td>';
                                                    echo '<td class="border_bottom border_right text-right">'.$khm.$this->bpas->formatMoney($change_us) . '</td>';
                                                echo '</tr>';
                                                echo '<tr>';
                                                    echo '<td class="border_bottom"></td>';
                                                    echo '<td class="border_bottom"></td>';
