<script>
	$(document).ready(function () {
        CURI = '<?= admin_url('reports/balance_sheet_by_month'); ?>';
	});
</script>
<style>
	@media print {
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-bars"></i><?= lang('balance_sheet_by_month'); ?> >> <?= (isset($start)?$start:""); ?> >> <?= (isset($end)?$end:""); ?></h2>
    ...
    $(document).ready(function () {
		...
		$("#biller-filter").on('click', function(event){
			...
			if(hasCheck == true){
				...
				var url = "<?php echo admin_url('reports/balance_sheet_by_month/'.$start.'/'.$end.'/0/0') ?>" + '/' + encodeURIComponent(billers);
				window.location.href = "<?=admin_url('reports/balance_sheet_by_month/'. $start .'/'.$end.'/0/0/')?>" + '/' + encodedName;
			}
