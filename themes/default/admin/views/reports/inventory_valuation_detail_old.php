    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
				<?php echo admin_form_open('reports/inventory_valuation_detail/', 'id="action-form"'); ?>
					<div class="row">
                        <div class="col-sm-4">
                    ...
                    <table id="PRData" class="table table-hover table-striped table-condensed">
                        ...
                        <tbody>
							...
							foreach($prDetails as $pr)
							{
								...
								if($qa){
									$unit_n = $this->db->get_where('bpas_product_variants',array('id'=> $qa),1)->row();
									$unit_q = $unit_n->qty_unit;
									$unit_name = ' ( '.($this->bpas->formatQuantity(abs($p_qty)/$unit_q)).' '.$unit_n->name.' )';
							...
							<tr>
								...
								<td class="text-right"><?= $this->bpas->formatQuantity($p_qty) ?> <?=$unit_name?></td>
								<td class="text-right"><?= $p_cost ?></td>
								<td class="text-right"><?= $this->bpas->formatQuantity($qty_on_hand) ?></td>
								<td class="text-right"><?= $cost ?></td>
								<td class="text-right"><?= $this->bpas->formatDecimal($asset_value) ?></td>
							</tr>
							<?php 
							...
							<tr class="active">
								...
								<td></td>
								<td class="text-right"><b><?= $this->bpas->formatDecimal($total_asset_val); ?></b></td>
							</tr>
							<?php 
							...
							<tr>
								...
								<td></td>
								<td class="text-right"><b><?= $this->bpas->formatDecimal($total_assetVal_per_warehouse); ?></b></td>
							</tr>							
							<?php
							$gtt +=$total_assetVal_per_warehouse;
								...
								<td></td>
								<td class="text-right"><b><?= $this->bpas->formatDecimal($gtt); ?></b></td>
							</tr>		
                        </tbody>
		...
		*/
		...
		$('#excel').on('click', function (e) {
            ...
            if ($('.checkbox:checked').length <= 0) {
                window.location.href = "<?= admin_url('reports/inventory/0/xls/'.$reference1.'/'.$wahouse_id1.'/'.$product_id1.'/'.$from_date1.'/'.$to_date1.'/'.$stock_in_out1.'/'.$stockType1.'/'.$cate_id1) ?>";
                return false;
            }
        });
        $('#pdf').on('click', function (e) {
            e.preventDefault();
            if ($('.checkbox:checked').length <= 0) {
                window.location.href = "<?= admin_url('reports/inventory/pdf/0'.$reference1.'/'.$wahouse_id1.'/'.$product_id1.'/'.trim($from_date1).'/'.trim($to_date1).'/'.$stock_in_out1.'/'.$stockType1.'/'.$cate_id1) ?>";
                return false;
            }
        });
	});
