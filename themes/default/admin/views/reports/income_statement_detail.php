<script>$(document).ready(function () {
        CURI = '<?= admin_url('reports/income_statement_detail'); ?>';
    });</script>
<style>@media print {
        .fa {
            color: #EEE;
            ...
            <ul class="btn-tasks">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu pull-right" class="tasks-menus" role="menu"
						...
						<li><a href="<?= admin_url('reports/income_statement_detail') ?>"><i
                    ...
                    <table cellpadding="0" cellspacing="0" border="0"class="table table-condensed table-striped">
						...
						<?php
							...
							?>
								<tr>
									...
									<td class="text-center"><div class="fix-text"><?= $this->bpas->hrld($ide->tran_date); ?></div></td>		
									<td><div class="fix-text"><?= $ide->reference_no; ?></div></td>		
									<td><div class="fix-text"><?= $ide->customer; ?></div></td>
									<td><div class="fix-text"><?= $this->bpas->decode_html(strip_tags($ide->note)); ?></div></td>
									...
									foreach ($new_billers as $biG) {
										...
										if($biG_biller == $ide->biller_id){
											...
											if($ide_display < 0){
												$ide_display = '( ' . $this->bpas->formatMoney(abs($ide->amount)) . ' )';
											}else{
												$ide_display = $this->bpas->formatMoney(abs($ide->amount));
											}
									...
									if($ide_display1 < 0){
										$ide_display1 = '( ' . $this->bpas->formatMoney(abs($ide->amount)) . ' )';
									}else{
										$ide_display1 = $this->bpas->formatMoney(abs($ide->amount));
									}
									?>
							...
							?>
								<tr>
									...
									if($itotal_display < 0){
										$itotal_display = '( ' . $this->bpas->formatMoney(abs($itotal_amount)) . ' )';
									}else{
										$itotal_display = $this->bpas->formatMoney(abs($itotal_amount));
									}
									?>
						...
						<?php } ?>
							...
							for($i = 0; $i < count($new_billers); $i++){
								...
								*/
								echo '<td style="font-weight:bold;border-top:2px solid #000"><div class="fix-text text-right">' . $this->bpas->formatMoney(abs($total_amt_inc)) . '</div></td>';
							}
							?>
							
							<td style="border-top:2px solid #000"><div class="fix-text text-right"><b><?php echo $this->bpas->formatMoney((-1)*$total_income);?></b></div></td>
							</tr>
							<tr class="primary">
						...
						<?php 
							...
							<tr>
								...
								<td class="text-center"><?= $this->bpas->hrld($cde->tran_date); ?></td>		
								<td><?= $cde->reference_no; ?></td>		
								<td><?= $cde->customer; ?></td>
								<td><?= $this->bpas->decode_html(strip_tags($cde->note)); ?></td>
								<!--<td><?= ($cde->account_code .' . '. $cde->accountname); ?></td>-->
								
								<?php 
								$cost_display = $cde->amount;
								if($cost_display < 0) {
									$cost_display = '( ' . $this->bpas->formatMoney(abs($cde->amount)) . ' )';
								}else{
									$cost_display = $this->bpas->formatMoney(abs($cde->amount));
								}
							...
							if($t_cost_display < 0) {
								$t_cost_display = '( ' . $this->bpas->formatMoney(abs($ctotal_amount)) . ' )';
							}else{
								$t_cost_display = $this->bpas->formatMoney(abs($ctotal_amount));
							}
							?>
						...
						?>
							...
							<?php 
								echo $this->bpas->formatMoney(abs($total_cost));
							?>
							<?php //echo $this->bpas->formatMoney((-1)*$total_cost);?>
							</div></td>
							-->
							...
							<tr>
								...
								for($i =0; $i < count($sum_total_income); $i++){
									...
									echo '<td style="font-weight:bold;"><div class="fix-text text-right">' . $this->bpas->formatMoney($amount_per_gross) . '</div></td>';
								}
								?>
								...
								<td><div class="fix-text text-right"><b>
									...
									if((-1)*$total_income - $total_cost < 0){
										echo "(".$this->bpas->formatMoney(abs((-1)*$total_income - $total_cost)).")";
									}else{
										echo $this->bpas->formatMoney(abs((-1)*$total_income - $total_cost));
									}
									?>
							...
							if($ex_details->num_rows() > 0){
								...
								$ex_amount = $ex->amount;
								if($ex_amount < 0){
									$ex_amount = '( ' . $this->bpas->formatMoney(abs($ex->amount)) . ' )';
								}else{
									$ex_amount = $this->bpas->formatMoney(abs($ex->amount));
								}
							?>
								...
								<tr>
									...
									<td class="text-center"><div class="fix-text"><?= $this->bpas->hrld($ex->tran_date); ?></div></td>		
									<td><div class="fix-text"><?= $ex->reference_no; ?></div></td>		
									<td><div class="fix-text"><?= $ex->customer; ?></div></td>
									<td><div class="fix-text"><?= $this->bpas->decode_html(strip_tags($ex->note)); ?></div></td>
							...
							if($total_op_per < 0){
								$total_op_per = '( ' . $this->bpas->formatMoney(abs($total_op_per)) . ' )';
							}else{
								$total_op_per = $this->bpas->formatMoney(abs($total_op_per));
							}
							?>
							...
							<tr>				
								...
								foreach ($new_billers as $bi2) {
									...
									if($s_total1 < 0){
										$s_total1 = '( ' . $this->bpas->formatMoney(abs($s_total1)) . ' )';
									}else{
										$s_total1 = $this->bpas->formatMoney(abs($s_total1));
									}
    ...
    $(document).ready(function () {
		...
		$("#biller-filter").on('click', function(event){
			...
			if(hasCheck == true){
				...
				var url = "<?php echo admin_url('reports/income_statement_detail/'.$start.'/'.$end.'/0/0') ?>" + '/' + encodeURIComponent(billers);
				window.location.href = "<?=admin_url('reports/income_statement_detail/'. $start .'/'.$end.'/0/0/')?>" + '/' + encodedName;
			}
        ...
        $('#pdf').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/income_statement_detail/'. $start .'/'.$end.'/pdf/0/'.$biller_id)?>";
            return false;
        });
		
		$('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/income_statement_detail/'. $start .'/'.$end.'/0/xls/'.$biller_id)?>";
            return false;
        });
