    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
				<?php echo admin_form_open('reports/production_report', 'id="action-form"'); ?>
					<div class="row">
                        <div class="col-sm-4">
                    ...
                    <table class="table table-condensed table-bordered table-hover table-striped">
						...
						<?php
							 $this->db->select("
												bpas_convert.id,
												bom_id,
												bpas_convert.reference_no,
												bpas_convert.date,
												bpas_bom.name,
												bpas_warehouses.name as wname")
										->join("bpas_bom","bpas_bom.id=bpas_convert.bom_id","LEFT")
										->join("bpas_warehouses","bpas_warehouses.id=bpas_convert.warehouse_id","LEFT");
										if($reference){
											$this->db->where("bpas_convert.reference_no",$reference);
										}
										if($from_date && $to_date){
											$this->db->where('bpas_convert.date >="'.$from_date.'" AND bpas_convert.date<="'.$to_date.'"');
										}
										$converts = $this->db->get("bpas_convert")->result();
							$bom_id = 0;
						...
						?>
							<tr style="font-weight:bold;">
								...
								<th colspan="5">
									<?=$this->bpas->hrsd($convert->date)?> <i class="fa fa-angle-double-right" aria-hidden="true"></i>
							...
							?>
								<tr>
									...
									<td class="text-right"><?= $this->bpas->formatMoney($convert_item->cost)?></td>
									<td class="text-right"><?= $this->bpas->formatMoney($total_input_cost)?></td>									
								</tr>
							<?php } ?>
								<tr style="font-weight:bold; text-transform: uppercase;">
									<td class="text-right" colspan="6"><?= lang("total") ?> :</td>							
									<td class="text-right"><?= $this->bpas->formatMoney($total_input)?></td>
								</tr>
							...
							?>
								<tr>
									...
									<td class="text-right"><?= $this->bpas->formatMoney($convert_item1->cost)?></td>
									<td class="text-right"><?= $this->bpas->formatMoney($total_output_cost)?></td>
								</tr>
							<?php } ?>
								<tr style="font-weight:bold; text-transform: uppercase;">
									<td class="text-right" colspan="6"><?= lang("total") ?> :</td>							
									<td class="text-right"><?= $this->bpas->formatMoney($total_output)?></td>
								</tr>
							<?php } ?>
                        </tbody>                       
                    </table>
