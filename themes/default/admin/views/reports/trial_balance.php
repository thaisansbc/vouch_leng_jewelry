<script>
	$(document).ready(function () {
        CURI = '<?= admin_url('reports/trial_balance'); ?>';
    });
</script>
            ...
            <div class="form-group choose-date hidden-xs">
                <div class="controls">
                    <div class="input-group">
                        ...
                        <input type="text" value="<?= ($start ? $this->bpas->hrld($start) : '') . ' - ' . ($end ? $this->bpas->hrld($end) : ''); ?>" id="daterange" class="form-control">
            ...
            <ul class="btn-tasks">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu pull-right" class="tasks-menus" role="menu"aria-labelledby="dLabel">
						<li><a href="<?= admin_url('reports/trial_balance') ?>"><i class="fa fa-building-o"></i> <?= lang('billers') ?></a></li>
						<li class="divider"></li>
						<?php
							foreach ($billers as $biller) {
								echo '<li ' . ($biller_id && $biller_id == $biller->id ? 'class="active"' : '') . '><a href="' . admin_url('reports/trial_balance/'.$start.'/'.$end.'/0/0/' . $biller->id) . '"><i class="fa fa-building"></i>' . $biller->company . '</a></li>';
							}
						?>
					</ul>
                </li>
                    ...
                    <table id="SupData" cellpadding="0" cellspacing="0" border="0" class="table table-hover table-striped table-condensed">
						...
						<?php
							...
							foreach($data10->result() as $row10){
								...
								<tr>
									<td  style="padding-left:30px">
										<a href="<?= admin_url('reports/ledger/0/0/'.$biller_id.'?w=1'.$from_st.$to_st.'&account='.$row10->account_code) ?>">
							...
							?>
								<tr>
									<td  style="padding-left:30px">
										<a href="<?= admin_url('reports/ledger/0/0/'.$biller_id.'?w=1'.$from_st.$to_st.'&account='.$row11->account_code) ?>">
						...
						?>
							...
							<tr>
								<td  style="padding-left:30px">
									<a href="<?= admin_url('reports/ledger/0/0/'.$biller_id.'?w=1'.$from_st.$to_st.'&account='.$row20->account_code) ?>">
						...
						?>
							<tr>
								<td style="padding-left:30px">
									<a href="<?= admin_url('reports/ledger/0/0/'.$biller_id.'?w=1'.$from_st.$to_st.'&account='.$row21->account_code) ?>">
						...
						?>
							...
							<tr>
								<td  style="padding-left:30px">
									<a href="<?= admin_url('reports/ledger/0/0/'.$biller_id.'?w=1'.$from_st.$to_st.'&account='.$row30->account_code) ?>">
						...
						?>
							...
							<tr>
								<td  style="padding-left:40px">
									<a href="<?= admin_url('reports/ledger/0/0/'.$biller_id.'?w=1'.$from_st.$to_st.'&account='.$row40->account_code) ?>">
						...
						?>
							...
							<tr>
								<td  style="padding-left:50px">
									<a href="<?= admin_url('reports/ledger/0/0/'.$biller_id.'?w=1'.$from_st.$to_st.'&account='.$row50->account_code) ?>">
						...
						?>
							<tr>
								<td  style="padding-left:60px">
									<a href="<?= admin_url('reports/ledger/0/0/'.$biller_id.'?w=1'.$from_st.$to_st.'&account='.$row60->account_code) ?>">
						...
						?>
							...
							<tr>
								<td style="padding-left:70px">
									<a href="<?= admin_url('reports/ledger/0/0/'.$biller_id.'?w=1'.$from_st.$to_st.'&account='.$row70->account_code) ?>">
						...
						?>
							...
							<tr>
								<td  style="padding-left:80px">
									<a href="<?= admin_url('reports/ledger/0/0/'.$biller_id.'?w=1'.$from_st.$to_st.'&account='.$row80->account_code) ?>">
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=admin_url('reports/trial_balance/'. $start .'/'.$end.'/pdf/0/'.$biller_id)?>";
            return false;
        });
		$('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/trial_balance/'. $start .'/'.$end.'/0/xls/'.$biller_id)?>";
            return false;
        });
