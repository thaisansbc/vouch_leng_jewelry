    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
				<?php echo admin_form_open('reports/expense_report', 'id="action-form" method="GET"'); ?>
					<div class="row">
						...
						<div class="col-sm-4">
						 ...
						 <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('from_date', (isset($_GET['from_date']) ? $_GET['from_date'] : $this->bpas->hrsd($from_date2)), 'class="form-control date" id="from_date"'); ?>
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                <?= lang("to_date", "to_date"); ?>
                                <?php echo form_input('to_date', (isset($_GET['to_date']) ? $_GET['to_date'] : $this->bpas->hrsd($to_date2)), 'class="form-control date" id="to_date"'); ?>
                            </div>
                        </div>		
                ...
                <div class="table-responsive" style="width:100%;overflow:auto;">
                    ...
                    <table id="tbstock" class="table table-condensed table-bordered table-hover table-striped" >
						...
						if(is_array($expense_cat)){
							foreach($expense_cat as $cat){
								...
								if($from_date2 && $to_date2){
									$this->db->where('date_format(bpas_expenses.date,"%Y-%m-%d") >="'.$from_date2.'" AND date_format(bpas_expenses.date,"%Y-%m-%d") <="'.$to_date2.'"');
								}
								$q = $this->db->get();
								if($q->num_rows()>0){
						...
						?>
							...
							<tr>
								<td></td>
								<td style="text-align:center;"><?=$this->bpas->hrld($row->date)?></td>
							...
							<tr>
								...
								<td style="text-align:right;color:green;"><b><?=$this->bpas->formatDecimal($total)?></b></td>
								<td></td>
								<td></td>
							</tr>
						<?php
...
$(document).ready(function () {
    ...
    $('#excel').on('click', function (e) {
		...
		if ($('.checkbox:checked').length <= 0) {
			//window.location.href = "<?= admin_url('reports/expense_report_action/0/xls/'.$reference_no1.'/'.$from_date1.'/'.$to_date1) ?>";
			return false;
		}
	});
	$('#pdf').on('click', function (e) {
		e.preventDefault();
		if ($('.checkbox:checked').length <= 0) {
			//window.location.href = "<?= admin_url('reports/expense_report_action/pdf/0/'.$reference_no1.'/'.$from_date1.'/'.$to_date1) ?>";
			return false;
		}
	});	
	...
	$("#all,#five").click(function(e){
			...
			if(str == "all"){
				window.location.href = '<?= admin_url('reports/expense_report?str=all'); ?>';
			}else{
				window.location.href = '<?= admin_url('reports/expense_report?str=five'); ?>';
			}
	});
