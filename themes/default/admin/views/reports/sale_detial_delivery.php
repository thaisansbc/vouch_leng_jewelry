        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo admin_form_open("reports/sales_detail_delivery",'method="GET"'); ?>
                    <div class="row">
                        <div class="col-sm-3">
                ...
                <div class="table-responsive">
                    <table class="table table-condensed table-striped">
						...
						foreach($deliveries as $delivery){ 
						       $queries="SELECT bpas_delivery_items.product_name,bpas_products.code,   bpas_delivery_items.begining_balance as t_qty, bpas_delivery_items.quantity_received, bpas_delivery_items.ending_balance as balance , bpas_warehouses.name as warehouse, bpas_product_variants.name as variants  From bpas_delivery_items
									   LEFT JOIN bpas_deliveries ON bpas_deliveries.id =bpas_delivery_items.delivery_id
									   LEFT JOIN bpas_products ON bpas_products.id =bpas_delivery_items.product_id 
									   LEFT	JOIN bpas_product_variants ON bpas_product_variants.id = bpas_delivery_items.product_id 
									   LEFT JOIN bpas_warehouses ON bpas_warehouses.id =bpas_delivery_items.warehouse_id where bpas_delivery_items.delivery_id = {$delivery->id}"; 
									   if($this->session->userdata('user_id')){   
									        $user = $this->session->userdata('user_id');
										    $this->db->where("bpas_deliveries.delivery_by",$user);
									   } 
					...
					?>
						       <tr>
								   ...
								   <td><?=$q->warehouse ?></td>
								   <td class="text-center"><?= $this->bpas->formatDecimal($q->t_qty); ?></td> 
								   <td class="text-center"><?=$this->bpas->formatDecimal($q->quantity_received); ?></td> 
								   <td><?=$q->variants ?></td> 
								   <td class="text-center"><?= $this->bpas->formatDecimal($q->balance);?></td>
							   </tr>
