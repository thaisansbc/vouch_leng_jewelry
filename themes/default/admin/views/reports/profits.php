...
<div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel"><?= lang('day_profit').' ('.$this->bpas->hrsd($date).')'; ?></h4>
        </div>
            ...
            <table width="100%" class="stable">
                <tr>
                    ...
                    <td style="text-align:right; border-bottom: 1px solid #EEE;">
						<h4><span id="revenue">(<?= $this->bpas->formatQuantity($revenues->total_items); ?>)<?= $this->bpas->formatMoney($revenues->total); ?></span></h4>
                    </td>
                </tr>
                <tr>
                    ...
                    <td style="text-align:right;border-bottom: 1px solid #DDD;">
						<h4>
                            <span id="discount"><?php $discount = $refunds ? $discount : 0; echo '('.$this->bpas->formatQuantity($count_dis->count_id).')'.$this->bpas->formatMoney($discount); ?></span>
                        </h4>
					</td>
                </tr>
				<tr>
                    ...
                    <td style="text-align:right;border-bottom: 1px solid #DDD;">
						<h4>
                            <span id="shipping"><?= '('.$this->bpas->formatMoney($count_ship->count_id).')';?><?=$this->bpas->formatMoney($shipping); ?></span>
                        </h4>
					</td>
                </tr>
				<tr style="display:none;">
                    ...
                    <td style="text-align:right;border-bottom: 1px solid #DDD;">
						<h4>
                            <span id="order_tax"><?= $this->bpas->formatMoney($order_tax); ?></span>
                        </h4>
					</td>
                </tr>
				<tr>
                    ...
                    <td style="text-align:right;border-bottom: 1px solid #DDD;"><h4>
						<span id="sales_refund">(<?= $this->bpas->formatQuantity($refunds->quantity); ?>)<?= $this->bpas->formatMoney($refunds->paid); ?></span></h4>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td style="text-align:right; border-bottom: 1px solid #EEE;"><h4>
                        <span id="products_cost">(<?= $this->bpas->formatQuantity($costing->total_items); ?>)<?= $this->bpas->formatMoney($costing->cost); ?></span>
                    </h4></td>
                </tr>
				<tr>
                    <td style="border-bottom: 1px solid #DDD;"><h4><?= lang('expenses'); ?>:</h4></td>
                    <td style="text-align:right;border-bottom: 1px solid #DDD;"><h4>
                            <span id="expenses"><?php echo '('.$this->bpas->formatQuantity($expenses->count_ex).')'. $expense = $expenses ? $this->bpas->formatMoney($expenses->total) : 0; ?></span>
                        </h4></td>
                </tr>
                <tr>
                    ...
                    <td style="text-align:right;">
                        <h4>
                            ...
                            ?>
                        <span><strong id="profit"><?= $this->bpas->formatMoney($profit); ?></strong></span>
                        </h4>
                    </td>
                </tr>
				<?php } ?>
...
$(document).ready(function () {
    ...
    $('#filter').click(function() {
        ...
        var end_date    = $('#end_date').val();
         $.ajax({
            type: 'get',
            url: '<?= admin_url('reports/profits_json'); ?>',
            data: {
            ...
            success: function (data) {
                ...
                if (data.revenues.total_items == null) {
                    $('#revenue').html('(<?= $this->bpas->formatQuantity() ?>)'+ '<?= $this->bpas->formatMoney() ?>');
                } else {
                    $('#revenue').html('('+ data.revenues.total_items +')'+ formatMoney(data.revenues.total));
                }

                if (data.count_dis.count_id == null) {
                    $('#discount').html('(<?= $this->bpas->formatQuantity() ?>)'+ '<?= $this->bpas->formatMoney() ?>');
                } else {
	...
	if (email != null) {
		$.ajax({
			type: "post",
			url: "<?= admin_url('reports/email_receipt') ?>",
