        ...
        <script>
			$(document).ready(function () {
				...
				var oTable = $('#SlRData').dataTable({
					...
					'sAjaxSource': '<?= admin_url('reports/getCustomerProductReport/?v=1' .$v) ?>',
	...
	<div id="products-con" class="tab-pane fade in">
		...
		$(document).ready(function () {
			...
			var oTable = $('#PRPData').dataTable({
					...
					"iDisplayLength": <?= $Settings->rows_per_page ?>,
					'bProcessing': true, 'bServerSide': true,
					'sAjaxSource': '<?= admin_url('reports/getProductsReports/?v=1'.$p) ?>',
					'fnServerData': function (sSource, aoData, fnCallback) {
						aoData.push({
							"name": "<?= $this->security->get_csrf_token_name() ?>",
							"value": "<?= $this->security->get_csrf_hash() ?>"
	...
	<div id="top-products-con" class="tab-pane fade in">
		...
		$(document).ready(function () {
			...
			var oTable = $('#TPRData').dataTable({
					...
					"iDisplayLength": <?= $Settings->rows_per_page ?>,
					'bProcessing': true, 'bServerSide': true,
					'sAjaxSource': '<?= admin_url('reports/getProductsReports2/?v=1'.$p) ?>',
					'fnServerData': function (sSource, aoData, fnCallback) {
						aoData.push({
							"name": "<?= $this->security->get_csrf_token_name() ?>",
							"value": "<?= $this->security->get_csrf_hash() ?>"
		...
		<div class="box">
			<div class="box-content">
				<div class="row">
					<div class="col-lg-12">
						<div class="table-responsive">
							<table id="TPRData" class="table table-hover table-striped">
								<thead>
								<tr class="primary">
									<th style="min-width:30px; width: 30px; text-align: center;">
										<input class="checkbox checkth" type="checkbox" name="check"/>
