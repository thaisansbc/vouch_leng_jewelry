...
if (isset($from_warehouse)) {
    $v .= '&from_warehouse=' . $from_warehouse;
}
if (isset($to_warehouse)) {
    $v .= "&to_warehouse=" . $to_warehouse;
}
if (isset($serial)) {
    $v .= '&serial=' . $serial;
}
...
<?php
    echo admin_form_open('reports/transferReport', 'id="action-form"');
?>
<div class="box">
    <div class="box-header">
        ...
        <div class="box-icon">
            <ul class="btn-tasks">
                ...
                <li class="dropdown"><a href="#" id="xls2" data-action="export_excel"  class="tip" title="<?= lang('download_xls2') ?>"><i class="icon fa fa-file-excel-o"></i></a></li>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo admin_form_open("reports/transfers_report",'method="GET"'); ?>
                    <div class="row">
                        <div class="col-sm-3">
                        ...
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                <?php echo form_input('start_date', (isset($_GET['start_date']) ? $_GET['start_date'] : $this->bpas->hrsd($start_date)), 'class="form-control datetime" id="start_date" autocomplete=off'); ?>
                            </div>
                        </div>
                        <div class="col-sm-3">
                            <div class="form-group">
                                <?= lang("end_date", "end_date"); ?>
                                <?php echo form_input('end_date', (isset($_GET['end_date']) ? $_GET['end_date'] : $this->bpas->hrsd($end_date)), 'class="form-control datetime" id="end_date" autocomplete=off'); ?>
                            </div>
                        </div>
                ...
                <div class="table-responsive">
                    <table class="table table-condensed table-striped">
                        ...
                        foreach($transfers as $transfer){ 
                               $query=$this->db->query("
                                  SELECT product_name,
                                    ...
                                    item_tax, 
                                    subtotal,
                                    bpas_purchase_items.quantity,
                                    bpas_product_variants.name as var_name,
                                    bpas_units.name as unit_name
                                  From bpas_purchase_items
                                  LEFT JOIN bpas_products ON bpas_products.id=bpas_purchase_items.product_id 
                                  LEFT JOIN bpas_units ON bpas_units.id =bpas_products.unit
                                  LEFT JOIN bpas_product_variants ON option_id = bpas_product_variants.id
                                  where bpas_purchase_items.transfer_id = '$transfer->id'")->result();
                        ?>
                        <tbody>
                               <tr class="bold" style="">
                     ...
                     ?>
                               <tr>
                                   ...
                                   <td class="text-center"><?= $this->bpas->formatQuantity($q->quantity)?></td> 
                                   <td class="text-center"><?= !empty($q->var_name)?$q->var_name:$q->unit_name ?></td> 
                                   <td class="text-right"style="display:none;"><?= $this->bpas->formatMoney($q->unit_cost)?></td> 
                                   <td class="text-right"style="display:none;"><?= $this->bpas->formatMoney($q->item_tax)?></td>
                                   <td class="text-right"style="display:none;"><?= $this->bpas->formatMoney($q->subtotal)?></td> 
                               </tr> 
                       <?php } ?>
                               <tr> 
                                   <td></td>
                                   ...
                                   <td class="text-right"style="display:none;"><?=$this->bpas->formatMoney($t_cost) ?></td> 
                                   <td class="text-right"style="display:none;"><?=$this->bpas->formatMoney($t_tax) ?></td>
                                   <td class="text-right"style="display:none;"><?=$this->bpas->formatMoney($t_subtotal) ?></td> 
                               </tr>
                       <?php
                           $g_qty += $Tqty;
                           $g_cost += $t_cost;
                    ...
                    <tfoot>
                               <tr style="background:#428BCA; color:white; font-size:16px !important;"> 
                                   ...
                                   <td class="bold text-right"style="display:none;"><?=$this->bpas->formatMoney($g_cost) ?></td> 
                                   <td class="bold text-right"style="display:none;"><?=$this->bpas->formatMoney($g_tax) ?></td>
                                   <td class="bold text-right"style="display:none;"><?=$this->bpas->formatMoney($g_subtotal) ?></td> 
                               </tr>
                    </tfoot> 
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        // $('#pdf').click(function (event) {
            ...
            // window.location.href = "<?=admin_url('reports/getTransfersReport/pdf/?v=1'.$v)?>";
            // return false;
        // });
        // $('#xls_detail').click(function (event) {
        //     event.preventDefault();
        //     window.location.href = "<?=admin_url('reports/getTransfersReport/0/0/excel_deatail/?v=1' . $v)?>";
        //     return false;
        // });

        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/getTransfersReport/0/xls/?v=1'.$v)?>";
            return false;
        });

        $('#xls2').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/getTransfersReport/0/0/xls2/?v=1'.$v)?>";
            return false;
        });
