		...
		<script>
			$(document).ready(function () {
				...
				var oTable = $('#PrData').dataTable({
					...
					'sAjaxSource': '<?= admin_url('reports/getPurchasedSupplierItemsReport/?v=1'.$v) ?>',
				...
				<div class="box-icon">
					<ul class="btn-tasks">
						...
						<ul class="dropdown-menu pull-right" class="tasks-menus" role="menu"
							...
							<li><a href="<?= admin_url('reports/supplier_by_items_report/' . $user_id) ?>"><i
										class="fa fa-building-o"></i> <?= lang('billers') ?></a></li>
							<li class="divider"></li>
							<?php
							foreach ($billers as $biller) {
								echo '<li ' . ($biller_id && $biller_id == $biller->id ? 'class="active"' : '') . '><a href="' . admin_url('reports/supplier_by_items_report/' . $user_id . '/' . $biller->id) . '"><i class="fa fa-building"></i>' . $biller->company . '</a></li>';
							}
							?>
						</ul>
					</li>
				...
				<div class="row">
					<div class="col-lg-12">
						...
						<div id="productform">
							...
							<?php echo admin_form_open("reports/supplier_by_items_report/" . $user_id); ?>
								<div class="row">
									<div class="col-sm-4">
	...
	<script type="text/javascript">
		$(document).ready(function () {
			$('#pdf').click(function (event) {
				...
				window.location.href = "<?=admin_url('reports/getPurchasedSupplierItemsReport/pdf/?v=1'.$v)?>";
				return false;
			});
			$('#xls').click(function (event) {
				event.preventDefault();
				window.location.href = "<?=admin_url('reports/getPurchasedSupplierItemsReport/0/xls/?v=1'.$v)?>";
				return false;
			});
			...
			$('#pdf1').click(function (event) {
				event.preventDefault();
				window.location.href = "<?=admin_url('reports/getPurchasedSupplierItemsReport/pdf/?v=1'.$p)?>";
				return false;
			});
			$('#xls1').click(function (event) {
				event.preventDefault();
				window.location.href = "<?=admin_url('reports/getPurchasedSupplierItemsReport/0/xls/?v=1'.$p)?>";
				return false;
			});
