<script>$(document).ready(function () {
        CURI = '<?= admin_url('reports/income_statement_by_customer'); ?>';
    });</script>
<style>
@media print {
        .fa {
            ...
            <ul class="btn-tasks">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu pull-right" class="tasks-menus" role="menu"
						...
						<li><a href="<?= admin_url('reports/income_statement_by_customer') ?>"><i
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" class="table table-hover table-striped table-condensed">
						...
						for($i = 1; $i <= count($new_customers); $i++){
						  ...
						  $query = $this->db->query("SELECT
                            ...
                            WHERE
								...
								AND bpas_gl_trans.customer_id = '" . $customer_id . "'
                            ...
                            $totalBeforeAyearRows = $query->row();
								...
								?>
									<td>
										<a href="<?= admin_url('reports/ledger_by_customer/?w=1'.$from_st.$to_st.'&account='.$row->account_code) ?>">
											<?php echo $row->account_code;?> - <?php echo $row->accountname;?>
										</a>
									</td>
									<td class="right">
										<a href="<?= admin_url('reports/ledger_by_customer/?w=1'.$from_st.$to_st.'&customer='.$customer_id.'&account='.$row->account_code) ?>">
						...
						?>
							<tr>
								...
								<td>
									<a href="<?= admin_url('reports/ledger_by_customer/?w=1'.$from_st.$to_st.'&account='.$rowcost->account_code) ?>">
										<?php echo $rowcost->account_code;?> - <?php echo $rowcost->accountname;?>
									</a>
								</td>
								
								<td class="right">
									<a href="<?= admin_url('reports/ledger_by_customer/?w=1'.$from_st.$to_st.'&customer='.$customer_id.'&account='.$rowcost->account_code) ?>">
    ...
    $(document).ready(function () {
		$("#customer-filter").on('click', function(event){
			...
			if(hasCheck == true){
				...
				var url = "<?php echo admin_url('reports/income_statement_by_customer/'.$start.'/'.$end.'/0/0') ?>" + '/' + encodeURIComponent(customers);
				window.location.href = "<?=admin_url('reports/income_statement_by_customer/'. $start .'/'.$end.'/0/0/')?>" + '/' + encodedName;
			}
        ...
        $('#pdf').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/income_statement_by_customer/'. $start .'/'.$end.'/pdf/0/'.$customer_id)?>";
            return false;
        });
		
		$('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/income_statement_by_customer/'. $start .'/'.$end.'/0/xls/'.$customer_id)?>";
            return false;
        });
