...
<script>
	$(document).ready(function () {
        CURI = '<?= admin_url('reports/getSalesReportDetail'); ?>';
    });
</script>
<?php
	//$this->bpas->print_arrays($warehouse);
	$v = "";
...
<script>
	...
	$(document).ready(function(){
		...
		$('#pdf').click(function (event) {
            ...
            window.location.href = "<?=admin_url('reports/saleReportDetail_actions/pdf/?v=1'.$v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/saleReportDetail_actions/0/xls/?v=1'.$v)?>";
            return false;
        });
		*/
	});
				...
				<ul class="btn-tasks">
					...
					<li class="dropdown">
						...
						<ul class="dropdown-menu pull-right" class="tasks-menus" role="menu"
							...
							<li><a href="<?= admin_url('reports/getSalesReportDetail/'.$start.'/'.$end) ?>"><i
										class="fa fa-building-o"></i> <?= lang('billers') ?></a></li>
							<li class="divider"></li>
							<?php
							foreach ($billers as $biller) {
								echo '<li ' . ($biller_id && $biller_id == $biller->id ? 'class="active"' : '') . '><a href="' . admin_url('reports/getSalesReportDetail/'.$start.'/'.$end.'/' . $biller->id) . '"><i class="fa fa-building"></i>' . $biller->company . '</a></li>';
							}
							?>
						</ul>
					</li>
	...
	<div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    <?php echo admin_form_open("reports/getSalesReportDetail"); ?>
                    <div class="row">
                        <div class="col-sm-4">
