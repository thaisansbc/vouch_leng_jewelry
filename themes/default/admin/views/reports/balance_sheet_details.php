<script>$(document).ready(function () {
        CURI = '<?= admin_url('reports/balance_sheet_details'); ?>';	
	});
	</script>
<style>
@media print {
            ...
            <ul class="btn-tasks">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu pull-right" class="tasks-menus" role="menu"
						...
						<li><a href="<?= admin_url('reports/balance_sheet_details') ?>"><i
						...
						foreach ($billers as $biller) {
							...
							//echo '<li ' . ($biller_id && $biller_id == $biller->id ? 'class="active"' : '') . '><a href="' . admin_url('reports/balance_sheet/'.$start.'/'.$end.'/0/0/' . $biller->id) . '"><input type="checkbox" class="checkbox biller_checkbox" value="'. $biller->id .'" >&nbsp;&nbsp;' . $biller->company . '</a></li>';
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="table-scroll">
                    <table id="SupData" cellpadding="0" cellspacing="0" border="0" class="table table-hover table-striped table-condensed">
						<thead>
							<tr >                           
					...
					<?php
						...
						if($this->uri->segment(7)){
							...
							foreach($dataAsset->result() as $row){
								...
								foreach($assetDetails->result() as $asD){
									$s_desc = $this->bpas->decode_html(strip_tags($asD->note));
									echo '</tr>';
									echo '<td class="text-center"><div class="fix-text">'. $asD->tran_type .'</div></td>';
									echo '<td><div class="fix-text">'. $this->bpas->hrld($asD->tran_date) .'</div></td>';
									echo '<td><div class="fix-text">'. $asD->reference_no .'</div></td>';
									echo '<td><div class="fix-text">'. $asD->customer .'</div></td>';
									echo '<td class="col-md-2"><div class="fix-text">'. $this->bpas->limit_words($s_desc, 20) .'</div></td>';
									?>
									...
									foreach ($new_billers as $bi2) {
										...
										$b_amt = '';
										if($asD->amount >0){
											$b_amt = $this->bpas->formatMoney(abs($asD->amount));
										}else{
											$b_amt = '( ' . $this->bpas->formatMoney(abs($asD->amount)) . ' )';
										}
										if($bi2_biller == $asD->biller_id){
											...
											echo '<td class="text-right"><div class="fix-text">' . $this->bpas->formatMoney(0) . '</div></td>';
										}
									}
									?>
								...
								if($total_amt >0){
									$total_amt = $this->bpas->formatMoney(abs($total_amt));
								}else{
									$total_amt = '( ' . $this->bpas->formatMoney(abs($total_amt)) . ' )';
								}
								...
								foreach ($new_billers as $bi2) {
									...
									$total_amt += $s_total;
									if($s_total < 0){
										$s_total = '( ' . $this->bpas->formatMoney(abs($s_total)) . ' )';
									}else{
										$s_total = $this->bpas->formatMoney(abs($s_total));
									}
						...
						?>
							<tr>
								...
								for($c= 0; $c < count($new_billers); $c++){
									...
									if($total_asset_amt<0){
										$total_asset_amt = '( '.$this->bpas->formatMoney(abs($total_asset_amt)).' )';
									}else{
										$total_asset_amt = $this->bpas->formatMoney(abs($total_asset_amt));
									}
								...
								$total_asset_display = '';
								if($total_asset<0){
									$total_asset_display = '( '.$this->bpas->formatMoney(abs($total_asset)).' )';
								}else{
									$total_asset_display = $this->bpas->formatMoney(abs($total_asset));
								}
								?>
							...
							foreach($dataLiability->result() as $rowlia){
								...
								foreach($libDetails->result() as $libD){
									...
									echo '<td><div class="fix-text">'. $this->bpas->hrld($libD->tran_date) .'</div></td>';
									echo '<td><div class="fix-text">'. $libD->reference_no .'</div></td>';
									echo '<td>'. $libD->customer .'</td>';
									echo '<td><div class="fix-text">'. $this->bpas->decode_html(strip_tags($libD->note)) .'</div></td>';
									
									$lib_amt_dis_in = '';
									if((-1)*$libD->amount < 0){
										$lib_amt_dis_in = '( ' . $this->bpas->formatMoney(abs($libD->amount)) . ' )';
									}else{
										$lib_amt_dis_in = $this->bpas->formatMoney(abs($libD->amount));
									}
									?>
									...
									<?php foreach ($new_billers as $bi2) {
										...
										if($bi2_biller == $libD->biller_id){
											...
											echo '<td><div class="fix-text text-right">' . $this->bpas->formatMoney(0) . '</div></td>';
										}
									}
								...
								*/
								...
								echo '<td colspan="2"><div class="fix-text text-right"><b>' . $this->bpas->formatMoney(abs($total_amt)) . '</b></div></td>';
								echo '</tr>';
							}
						...
						?>
							<tr>
								...
								for($c= 0; $c < count($new_billers); $c++){
									...
									/*
									if($total_lib_amt<0){
										$total_lib_amt = '( '.$this->bpas->formatMoney(abs($total_lib_amt)).' )';
									}else{
										$total_lib_amt = $this->bpas->formatMoney(abs($total_lib_amt));
									}
									*/
									...
									if($total_lib_amt < 0){
										echo '<td><div class="fix-text text-right"><b>( ' . $this->bpas->formatMoney(abs($total_lib_amt)) .' )</b></div></td>';
									}else{
										echo '<td><div class="fix-text text-right"><b>'. $this->bpas->formatMoney($total_lib_amt) .'</b></div></td>';
									}
								...
								if($total_liability<0){
									$total_lib_display = '( '.$this->bpas->formatMoney(abs($total_liability)).' )';
								}else{
									$total_lib_display = $this->bpas->formatMoney(abs($total_liability));
								}
								*/
								?>
								<td><div class="fix-text text-right"><b><?php echo $this->bpas->formatMoney(abs($total_liability));?></b></div></td>								
							</tr>
							<!--- equities --->
							<tr class="primary">                            
								<th style="text-align:left;" colspan="<?= $num_col ?>">
							...
							<?php
								...
								$queryIncom = $this->db->query("SELECT sum(bpas_gl_trans.amount) AS amount FROM
														...
														WHERE DATE(tran_date) = '$totalBeforeAyear' AND	bpas_gl_trans.sectionid IN ('40,70') GROUP BY bpas_gl_trans.account_code;");
								$total_income_beforeAyear = $queryIncom->amount;

								$queryExpense = $this->db->query("SELECT sum(bpas_gl_trans.amount) AS amount FROM
															bpas_gl_trans
														INNER JOIN bpas_gl_charts ON bpas_gl_charts.accountcode = bpas_gl_trans.account_code
														WHERE DATE(tran_date) = '$totalBeforeAyear' AND	bpas_gl_trans.sectionid IN ('50,60,80,90') GROUP BY bpas_gl_trans.account_code;");
							...
							for($c= 0; $c < count($new_billers); $c++){
								...
								if($total_per_retained<0){
									$total_per_retained = '( '.$this->bpas->formatMoney(abs($total_per_retained)).' )';
								}else{
									$total_per_retained = $this->bpas->formatMoney(abs($total_per_retained));
								}
							...
							<?php if($total_retained<0) { ?>						
								<td><div class="fix-text text-right"><?php echo '( ' . $this->bpas->formatMoney(abs($total_retained)) . ' )';?></div></td>
							<?php } else { ?>
								<td><div class="fix-text text-right"><?php echo $this->bpas->formatMoney(abs($total_retained));?></div></td>
							<?php }	?>							
								</tr>
							...
							foreach($dataEquity->result() as $rowequity){
								...
								foreach($eqDetails->result() as $eqD){
									...
									echo '<td><div class="fix-text">'. $this->bpas->hrld($eqD->tran_date) .'</div></td>';
									echo '<td><div class="fix-text">'. $eqD->reference_no .'</div></td>';
									echo '<td><div class="fix-text">'. $eqD->customer .'</div></td>';
									echo '<td><div class="fix-text">'. $this->bpas->decode_html(strip_tags($eqD->note)) .'</div></td>';
									?>
									...
									<?php foreach ($new_billers as $bi2) {
										...
										if($bi2_biller == $eqD->biller_id){
											...
											$eq_display = 0;
											if((-1)*$eqD->amount < 0){
												$eq_display = '( ' . $this->bpas->formatMoney(abs($eqD->amount)) . ' )';
											}else{
												$eq_display = $this->bpas->formatMoney(abs($eqD->amount));
											}
									...
									$eq_display1 = 0;
									if((-1)*$eqD->amount < 0){
										$eq_display1 = '( ' . $this->bpas->formatMoney(abs($eqD->amount)) . ' )';
									}else{
										$eq_display1 = $this->bpas->formatMoney(abs($eqD->amount));
									}
									?>
								...
								$sum_eq_per_acc = array();
								echo '<td colspan="2"><div class="fix-text text-right"><b>' . $this->bpas->formatMoney(abs($total_amt_eq)) . '</b></div></td>';
								echo '</tr>';
							...
							if($total_eq_sum <0){
								$total_eq_sum = '( ' . $this->bpas->formatMoney(abs($total_equity-$total_retained)) . ' )';
							}else{
								$total_eq_sum = $this->bpas->formatMoney(abs($total_equity-$total_retained));
							}
						?>							
							...
							<?php
							// $this->bpas->print_arrays($total_eq_arr, $total_retained_arr);
							for($c= 0; $c < count($new_billers); $c++){
								$in_bill_id1 = 0;
								if($this->uri->segment(7)){
									$in_bill_id1 = $new_billers[$c]['id'];
								...
								if($total_eq_amt<0){
									$total_eq_amt = '( '.$this->bpas->formatMoney(abs($total_eq_amt)).' )';
								}else{
									$total_eq_amt = $this->bpas->formatMoney(abs($total_eq_amt));
								}
							...
							if($total_eq_sum <0){
								$total_eq_sum = '( ' . $this->bpas->formatMoney(abs($total_eq_sum)) . ' )';
							}else{
								$total_eq_sum = $this->bpas->formatMoney(abs($total_eq_sum));
							}
							?>
							...
							for($c= 0; $c < count($new_billers); $c++){
								...
								if($total_lib_eq<0){
									$total_lib_eq = '( '.$this->bpas->formatMoney(abs($total_lib_eq)).' )';
								}else{
									$total_lib_eq = $this->bpas->formatMoney(abs($total_lib_eq));
								}
							...
							?>
							<td><div class="fix-text text-right"><b><?php echo $this->bpas->formatMoney(abs($end_total_lib) + $end_total_eq);?></b></div></td>
							<!--<td><strong><span class="pull-right blightOrange"><?php echo $this->bpas->formatMoney(abs($total_equity+$total_liability-$total_retained));?></span></strong></td>-->
							</tr>
						</tbody>
						<!--
                        ...
                        <tr class="active">                            
                            ...
                            <th><span class="pull-right"><?php echo $this->bpas->formatMoney(abs($total_equity+$total_liability-$total_retained)-abs($total_asset));?></span></th>
    ...
    $(document).ready(function () {
		...
		$("#biller-filter").on('click', function(event){
			...
			if(hasCheck == true){
				...
				var url = "<?php echo admin_url('reports/balance_sheet_details/'.$start.'/'.$end.'/0/0') ?>" + '/' + encodeURIComponent(billers);
				window.location.href = "<?=admin_url('reports/balance_sheet_details/'. $start .'/'.$end.'/0/0/')?>" + '/' + encodedName;
			}
        ...
        $('#pdf').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/balance_sheet/'. $start .'/'.$end.'/pdf/0/'.$biller_id)?>";
            return false;
        });
		$('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/balance_sheet/'. $start .'/'.$end.'/0/xls/'.$biller_id)?>";
            return false;
        });
