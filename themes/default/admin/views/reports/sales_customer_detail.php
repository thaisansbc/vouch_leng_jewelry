    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
				<?php echo admin_form_open('reports/sales_customer_detail/', 'id="action-form"'); ?>
					<div class="row">
                        <div class="col-sm-4">
                ...
                <div class="table-responsive">
                    <table id="" class="table table-hover table-striped table-condensed data_table">
                        <thead>
                        <tr class="primary">
							...
							<?php
								 $this->db->select("sales.id,
															...
															sale_items.product_name,
																(CASE
															WHEN bpas_return_items.quantity THEN
																(-1) * bpas_return_items.quantity
															ELSE
															...
															sale_items.unit_price,
																...
																(CASE
															WHEN bpas_return_items.quantity THEN
																((-1) * bpas_return_items.quantity) * bpas_sale_items.unit_price
															ELSE
																bpas_sale_items.quantity * bpas_sale_items.unit_price
															END) as amount")
												->from("companies")
								...
								?>
									<tr>
										...
										<td><?= $this->bpas->decode_html(strip_tags($row->note)) ?></td>
										<td><?= $row->name ?></td>
										<td><?= $row->product_name ?></td>
										<td><?= $this->bpas->formatDecimal($row->quantity) ?></td>
										<td><?= $this->bpas->formatDecimal($row->unit_price) ?></td>
										<td><?= $this->bpas->formatDecimal($row->amount) ?></td>
										<td><?= $this->bpas->formatDecimal($balance) ?></td>
									</tr>
								<?php
								}								
							?>
