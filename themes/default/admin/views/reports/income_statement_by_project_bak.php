<script>$(document).ready(function () {
        CURI = '<?= admin_url('reports/income_statement_by_project'); ?>';
    });</script>
<style>@media print {
        .fa {
            color: #EEE;
            ...
            <ul class="btn-tasks">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu pull-right" class="tasks-menus" role="menu"
						...
						<li><a href="<?= admin_url('reports/income_statement_by_project') ?>"><i
        ...
        <div class="row">
            <div class="col-lg-12" style="overflow-x: scroll">
				...
				<div id="form">
                    ...
                    <?php echo admin_form_open("reports/income_statement_by_project"); ?>
                    <div class="row">
                        <div class="col-sm-4">
						...
						<tbody>
								...
								foreach ($new_billers as $biller1) {
									...
									<tr>
										...
										<td><div class="fix-text"><?= $this->bpas->formatDecimal($biller1->amount)?></div></td>
										<td><div class="fix-text"><?=$biller1->period?></div></td>
										<td><div class="fix-text"><?=$this->bpas->hrsd($biller1->start_date)?></div></td>
										<td>
											<div class="fix-text"><?=$this->bpas->hrsd($biller1->end_date)?></div>
										</td>
										<td><div class="fix-text"><?= $this->bpas->formatDecimal(abs($biller1->begining_balance)) ?></div></td>
									<?php
									...
									for($m_i = 0; $m_i < sizeOf($years_arr); $m_i++){
										...
										$first_i = 1;
										$biller_id_m = '';
										//$this->bpas->print_arrays($monthlyIncomesYear->result());
										...
										foreach($monthlyIncomesYear->result() as $project){
											...
											<!--
											<td><div class="fix-text-moth"><b><?= $this->bpas->formatDecimal(abs($total))?></b></div></td>
											<td style="border-right:1px solid #000 !important;"><div class="fix-text-moth"><b><?=$this->bpas->formatDecimal($project->total_amount - abs($total))?></b></div></td>
											-->
									...
									?>
									<td><div class="fix-text-moth"><b><?= $this->bpas->formatDecimal(abs($total_per))?></b></div></td>
											<td style="border-right:1px solid #000 !important;"><div class="fix-text-moth"><b><?=$this->bpas->formatDecimal($project->total_amount - abs($total_per))?></b></div></td>
							...
							<tr class="active">
								...
								<th colspan="3"></th>
								<th><?= $this->bpas->formatDecimal($f_begin) ?></th>
							<?php
    ...
    $(document).ready(function () {
		...
		$("#biller-filter").on('click', function(event){
			...
			if(hasCheck == true){
				...
				var url = "<?php echo admin_url('reports/income_statement_by_project/'.$start.'/'.$end.'/0/0') ?>" + '/' + encodeURIComponent(billers);
				window.location.href = "<?=admin_url('reports/income_statement_by_project/'. $start .'/'.$end.'/0/0/')?>" + '/' + encodedName;
			}
        ...
        $('#pdf').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/income_statement_by_project/'.$start. '/' . $end . '/pdf/0/'.$biller_id_no_sep)?>";
            return false;
        });
		
		$('#xls').click(function (event) {
            event.preventDefault();
			window.location.href = "<?=admin_url('reports/income_statement_by_project/'.$start. '/' . $end . '/0/xls/'.$biller_id_no_sep)?>";
