<script>$(document).ready(function () {
        CURI = '<?= admin_url('reports/income_statement'); ?>';
    });</script>
<style>
@media print {
        .fa {
            ...
            <ul class="btn-tasks">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu pull-right" class="tasks-menus" role="menu"
						...
						<li><a href="<?= admin_url('reports/income_statement') ?>"><i
					...
					<ul class="dropdown-menu pull-right" class="tasks-menus" role="menu"
						...
						foreach ($projects as $project) {
							...
							if($biller_sep[$b_sep] == $project->project_id){
								echo '<li ' . ($biller_id && $biller_id == $project->project_id ? 'class="active"' : '') . '>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="project_checkbox[]" class="checkbox project_checkbox" checked value="'. $project->project_id .'" >&nbsp;&nbsp;' . $project->project_name . '</li>';
								echo '<li class="divider"></li>';
								$b_sep++;
							}else{
								echo '<li ' . ($biller_id && $biller_id == $project->project_id ? 'class="active"' : '') . '>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="project_checkbox[]" class="checkbox project_checkbox" value="'. $project->project_id .'" >&nbsp;&nbsp;' . $project->project_name . '</li>';
								echo '<li class="divider"></li>';
							}							
						}
						?>
						<li class="text-center"><a href="#" id="biller-filter_project" class="btn btn-primary"><?=lang('submit')?></a></li>
					</ul>
                </li>
            </ul>
        </div>
                ...
                <div class="table-scroll">
                    <table cellpadding="0" id="export_contain" cellspacing="0" border="0" class="table table-hover table-striped table-condensed">
						<thead>
							...
							<tr class="hide">
								<td colspan="3" style="font-weight: bold;font-size: 16px;">Income Statement Report Report</td>
                    		</tr>
                    		<tr class="hide">
									...
									if($this->uri->segment(7)){
										...
										for($i=0; $i < count($biller_sep); $i++){
											if($biller_sep[$i] == $b1->id){
												echo '<th style="text-align:right !important">' . $b1->company . '</th>';
					...
					foreach($dataIncome->result() as $row){
						...
						for($i = 1; $i <= count($new_billers); $i++){
						  ...
						  $query = $this->db->query("SELECT
                            ...
                            WHERE
								...
								AND DATE_FORMAT(bpas_gl_trans.tran_date, '%Y-%m-%d %H:%i') BETWEEN '$from_date' AND '$to_date';");
							
                            $totalBeforeAyearRows = $query->row();
								$amount_income = (-1)*$totalBeforeAyearRows->amount;
								if($amount_income < 0){
									$amount_income = '( '.$this->bpas->formatMoney(abs($totalBeforeAyearRows->amount)).' )';
								}else{
									$amount_income = $this->bpas->formatMoney(abs($totalBeforeAyearRows->amount));
								}
						...
						?>
							...
							for($i = 0; $i < count($new_billers); $i++){
								...
								echo '<td class="right" style="font-weight:bold;border-top:2px solid #000">' . $this->bpas->formatMoney(abs($total_amt_inc)) . '</td>';
							}
							?>
							<?php 
							$total_income_display = '';
							if($total_income < 0){
								$total_income_display = '( '.$this->bpas->formatMoney(abs($total_income)).' )';
							}else{
								$total_income_display = $this->bpas->formatMoney(abs($total_income));
							}
							?>
							...
							<tr>
								...
								for($j = 1; $j <= count($new_billers); $j++){
									...
									if($totalBeforeAyearRows->amount < 0){
										$amount_cost = '( '.$this->bpas->formatMoney(abs($totalBeforeAyearRows->amount)).' )';
									}else{
										$amount_cost = $this->bpas->formatMoney(abs($totalBeforeAyearRows->amount));
									}
								...
								<td>
									<a href="<?= admin_url('reports/ledger/?w=1'.$from_st.$to_st.'&account='.$rowcost->account_code) ?>">
										<?php echo $rowcost->account_code;?> - <?php echo $rowcost->accountname;?>
									</a>
								</td>
								
								<td class="right">
									<a href="<?= admin_url('reports/ledger/?w=1'.$from_st.$to_st.'&biller='.$bill_id.'&account='.$rowcost->account_code) ?>">
						...
						<?php
								...
								$total_per_cost += $this->bpas->formatDecimal($totalBeforeAyearRows->amount);
								$index1++;
								}
								if($total_per_cost < 0){
						...
						?>
								...
								for($in = 0; $in < count($new_billers); $in++){
									...
									foreach ($total_cost_array as $val) {
										if($in_bill_id == $val['id']){
											$total_amt_cost += $this->bpas->formatDecimal($val['amount']);
										}
									}
							...
							<tr>
								...
								for($i =0; $i < count($sum_total_income); $i++){
									...
									if($amount_per_gross < 0){
										$total_amount_per_gross = '('.$this->bpas->formatMoney(abs($amount_per_gross)).')';
									}else{
										$total_amount_per_gross = $this->bpas->formatMoney(abs($amount_per_gross));
									}
								...
								<td class="right" style="font-weight:bold;">
									...
									if(($total_income - $total_cost) < 0){
										echo "(".$this->bpas->formatMoney(abs($total_income - $total_cost)).")";
									}else{
										echo $this->bpas->formatMoney(abs($total_income - $total_cost));
									}
									?>
							...
							<tr class="primary">
								<th style="font-weight:bold;width:20%;text-align:left;"  colspan="<?=$num_col?>"><?= lang("operating_expense"); ?></th>
							</tr>
							<?php
							$total_expense = 0;
							...
							<tr>
								...
								for($i = 1; $i <= count($new_billers); $i++){
									...
									$amount_op = 0;
									if($totalBeforeAyearRows->amount < 0){
										$amount_op = '( '.$this->bpas->formatMoney(abs($totalBeforeAyearRows->amount)).' )';
									}else{
										$amount_op = $this->bpas->formatMoney(abs($totalBeforeAyearRows->amount));
									}
							...
							<tr class="primary">
								<th style="font-weight:bold;width:20%;text-align:left;"  colspan="<?=$num_col?>"><?= lang("EBITDA"); ?></th>
							</tr>
							<?php
							$total_ebitda = 0;
                            $totalBeforeAyear_expense = 0;
							foreach($dataEbitda->result() as $row){
							$total_expense += $row->amount;
							?>
							<tr>
								<?php
							...
							?>
							<tr class="primary">
								<th style="font-weight:bold;width:20%;text-align:left;"  colspan="<?=$num_col?>"><?= lang("EBIT"); ?></th>
							</tr>
							<?php
							$total_ebit = 0;
                            $totalBeforeAyear_expense = 0;
							foreach($dataEbit->result() as $row){
							$total_expense += $row->amount;
							?>
							<tr>
								<?php
							...
							<tr class="active">                            
								...
								for($i = 0; $i < count($sum_total_gross); $i++){
									...
									if($total_per_op_ex < 0){
										$total_per_op_ex = '( '.$this->bpas->formatMoney(abs($total_per_op_ex)).' )';
									}else{
										$total_per_op_ex = $this->bpas->formatMoney(abs($total_per_op_ex));
									}
...
<script type="text/javascript">
    $(document).ready(function () {
		$("#biller-filter").on('click', function(event){
			...
			if(hasCheck == true){
				...
				var url = "<?php echo admin_url('reports/income_statement/'.$start.'/'.$end.'/0/0') ?>" + '/' + encodeURIComponent(billers);
				window.location.href = "<?=admin_url('reports/income_statement/'. $start .'/'.$end.'/0/0')?>" + '/' + encodedName;
			}
		...
		$("#biller-filter_project").on('click', function(event){
			...
			var projects = removeSymbolLastString(project_ids, '-');
			

			if(hasCheck == true){
				var encodedName = encodeURIComponent(projects);
				var url = "<?php echo admin_url('reports/income_statement/'.$start.'/'.$end.'/0/0/0') ?>" + '/' + encodeURIComponent(projects);
				window.location.href = "<?=admin_url('reports/income_statement/'. $start .'/'.$end.'/0/0/0')?>" + '/' + encodedName;
        ...
        $('#pdf').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/income_statement/'. $start .'/'.$end.'/pdf/0/'.$biller_id)?>";
            return false;
        });
		
		$('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/income_statement/'. $start .'/'.$end.'/0/xls/'.$biller_id)?>";
            return false;
        });
    ...
    function exportF(elem) {
	  ...
	  elem.setAttribute("download", "Income_statement.xls"); // Choose the file name
	  return false;
	}
