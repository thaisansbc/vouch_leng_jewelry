...
<script>
    $(document).ready(function () {
        ...
        function qnp(x) {
            v = x.split('');
            return '<strong>'+formatMoney(x)+'</strong>';
        }
        function qtyUnit(x) {
            v = x.split('');
            return '<strong>'+formatQuantity2(x)+'</strong>';
        }
        oTable = $('#PrRData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('reports/getCostsReport/?v=1' . $v) ?>',
            ...
            "aoColumns": [null, null, null, {"mRender": qtyUnit}, {"mRender": qtyUnit}, {"mRender": qnp}, {"mRender": qnp}, {"mRender": qtyUnit}, {"mRender": qtyUnit}, {"mRender": qtyUnit}, {"mRender": qnp}, null],
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var pq = 0, sq = 0, bq = 0, pa = 0, sa = 0, ba = 0, pl = 0, sn = 0, so = 0, sb = 0, tec = 0, op = 0;
                for (var i = 0; i < aaData.length; i++) {
                    o = (aaData[aiDisplay[i]][3]).split('__');
                    ...
                    sr = (aaData[aiDisplay[i]][8]).split('__');
                    eq = (aaData[aiDisplay[i]][9]).split('__');
                    ec = (aaData[aiDisplay[i]][10]).split('__');
                    pq += parseFloat(p[0]);
                    pa += parseFloat(p[1]);
                    sn += parseFloat(s[0]);
                    so += parseFloat(a[0]);
                ...
                nCells[3].innerHTML = '<div class="text-right"> '+formatQuantity2(op)+'</div>';
                nCells[4].innerHTML = '<div class="text-right"> '+formatQuantity2(so)+'</div>';
                nCells[5].innerHTML = '<div class="text-right"> '+formatMoney(pq)+'</div>';
                nCells[8].innerHTML = '<div class="text-left">'+formatQuantity2(sb)+'</div>';
                nCells[6].innerHTML = currencyFormat(parseFloat(sn));
                nCells[7].innerHTML = '<div class="text-left">'+formatQuantity2(bq)+'</div>';
                nCells[9].innerHTML = '<div class="text-left">'+formatQuantity2(ba)+'</div>';
                nCells[10].innerHTML = '<div class="text-left">'+formatMoney(tec)+'</div>';
            }
            ...
            {column_number: 3, filter_default_label: "[<?=lang('opening_balance');?>]", filter_type: "text", data: []},
        ], "footer");
    });
</script>
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-barcode"></i><?= lang('cost_report'); ?> <?php
            if ($this->input->post('start_date')) {
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo admin_form_open('reports/cost_report'); ?>
                    <div class="row">
                        <div class="col-sm-4">
                ...
                <div class="table-responsive">
                    <table id="PrRData"
                        ...
                        <tr class="active">
                            ...
                            <th><?= lang('stock_request'); ?></th>
                            <th><?= lang('stock_return'); ?></th>
                            <th><?= lang('ending_qty'); ?></th>
                            <th><?= lang('ending_usa'); ?></th>
                            <th><?= lang('actions'); ?></th>
                        </tr>
                        </thead>
                        <tbody>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=admin_url('reports/getCostsReport/pdf/?v=1' . $v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/getCostsReport/0/xls/?v=1' . $v)?>";
            return false;
        });
