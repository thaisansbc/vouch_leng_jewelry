...
?>
<style>
    #SlRData > tfoot > tr > th:nth-child(8) > div.text-right { text-align: left; }
    #dtFilter-filter--SlRData-9 { text-align: center; }
    #dtFilter-filter-wrapper--SlRData-9 { width: 100px; }
    #dtFilter-filter-wrapper--SlRData-14 { text-align: center; width: 100px; }
    #SlRData > tfoot > tr > th:nth-child(6) > div, #SlRData > tfoot > tr > th:nth-child(7) > div, #SlRData > tfoot > tr > th:nth-child(8) > div, #SlRData > tfoot > tr > th:nth-child(11) > div { width: 120px; }
</style>
<style type="text/css">
    body {
...
<script>
    ...
    if (x != null) {
        ...
        for (index = 0; index < pqc.length; ++index) {
            var pq = pqc[index];
            var v = pq.split('__');
            d += v[0]+ ' (' + formatQuantity2(v[1]) + ')'+ v[2]+'<br>';
        }
        return d;
    } else {
        return '';
    ...
    $(document).ready(function () {
        oTable = $('#SlRData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('reports/getticket_reports/?v=1' . $v) ?>',
            ...
            "aoColumns": [
                ...
                {"bSearchable": false, "mRender": sqFormat },
                {"mRender": row_status}, 
            ...
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                var rgtotal = 0, gtotal = 0, paid = 0, balance = 0, customer_total = 0, cost = 0;
                for (var i = 0; i < aaData.length; i++) {
                    // if(aaData[aiDisplay[i]][8] != null){
                    //     cost += parseFloat(aaData[aiDisplay[i]][8]);
                    // }
                ...
                // nCells[8].innerHTML = currencyFormat(parseFloat(cost));
            ...
            {column_number: 1,filter_default_label:"[<?=lang('from');?>]", filter_type: "text", data: []},
            {column_number: 2,filter_default_label:"[<?=lang('destination');?>]", filter_type: "text", data: []},
            ...
            {column_number: 6,filter_default_label:"[<?=lang('Seat');?>]", filter_type: "text", data: []},
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-heart"></i><?= lang('ticket_report'); ?> <?php
            if ($this->input->post('start_date')) {
        ...
        <div class="box-icon">
            <ul class="btn-tasks">
                ...
                <li class="dropdown">
                    <a href="#" id="xls_detail" class="tip" title="<?= lang('download_xls') ?>">
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    <?php echo admin_form_open('reports/tickets'); ?>
                    <div class="row">
                        <div class="col-sm-4">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <?= lang('from', 'from'); ?>
                                <?php
                                $get_fields = $this->site->getcustomfield('destination');
                                $destination ['']='';
                                if (!empty($get_fields)) {
                                    foreach ($get_fields as $field_id) {
                                        $destination[$field_id->id] = $field_id->description;
                                    }
                                }
                                echo form_dropdown('from', $destination, (isset($_POST['from']) ? $_POST['from'] : ''), 'id="from" class="form-control input-tip select" data-placeholder="' . $this->lang->line('select') . ' ' . $this->lang->line('from_warehouse') . '"  style="width:100%;" '); ?>
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">  
                            <div class="form-group">
                                ...
                                echo form_dropdown('destination', $destination, (isset($_POST['destination']) ? $_POST['destination'] : ''), 'id="destination" class="form-control input-tip select" data-placeholder="' . $this->lang->line('select') . ' ' . $this->lang->line('to_warehouse') . '"  style="width:100%;" ');
                                ?>
                            </div>
                        </div>
                        <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    echo form_dropdown('time_out',$field,(isset($_POST['time_out']) ? $_POST['time_out'] : ''),  'class="form-control select" id="time_out"'); ?>
                                </div>
                            </div>
                ...
                <div class="table-responsive">
                    <table id="SlRData" class="table table-hover table-striped table-condensed reports-table">
                        ...
                        <tr>
                            <th><?= lang('date'); ?></th>
                            <th style="text-align: center !important;"><?= lang('from'); ?></th>
                            <th style="text-align: center !important;"><?= lang('Destination'); ?></th>
                            <th><?= lang('reference_no'); ?></th>
                            <th><?= lang('biller'); ?></th>
                            <th><?= lang('customer'); ?></th>
                            <th><?= lang('Seat').' (Qty)(Time)';?></th>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=admin_url('reports/getticket_reports/pdf/?v=1' . $v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/getticket_reports/0/xls/?v=1' . $v)?>";
            return false;
        });
        $('#xls_detail').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/getticket_reports/0/0/0/excel_deatail/?v=1' . $v)?>";
            return false;
        });
        
        $('#preview').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/getticket_reports/0/0/preview/?v=1' . $v)?>";
            return false;
        });
