...
<div class="box">
            ...
            <ul class="btn-tasks">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu pull-right" class="tasks-menus" role="menu" aria-labelledby="dLabel">
						<li><a href="<?= admin_url('reports/ledger_by_customer') ?>"><i class="fa fa-building-o"></i> <?= lang('customers') ?></a></li>
						<li class="divider"></li>
						<?php
							foreach ($customers as $customer) {
								echo '<li ' . ($customer_id && $customer_id == $customer->id ? 'class="active"' : '') . '>
										<a href="' . admin_url('reports/ledger_by_customer/0/0/' . $customer->id) . '"><i class="fa fa-building"></i>' . $customer->company . '</a></li>';
							}
						?>
					</ul>
				</li>
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                <div id="form">
                    <?php echo admin_form_open("reports/ledger_by_customer/".$v_form); ?>
                    <div class="row">
                            ...
                            foreach($accounts as $account){								
                                ...
                                foreach($glTranLists as $gltran)
                                {
									...
									<tr>
										...
										<td class="right"><?=$this->bpas->formatMoney($endDebitAmount)?></div></td>
										<td class="right"><?=$this->bpas->formatMoney(abs($endCreditAmount))?></div></td>
										<td class="right"><?=$this->bpas->formatMoney($endAccountBalance)?></div></td>
									</tr>
                                <?php
								}
                            }
...
<script type="text/javascript">
    $(document).ready(function () {
		...
		$('#pdf').click(function (event) {
            ...
            window.location.href = "<?=admin_url('reports/ledger_by_customer/pdf/0/'.$customer_id . '?v=1'.$v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/ledger_by_customer/0/xls/'.$customer_id . '?v=1'.$v)?>";
            return false;
        });
