    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
				<?php echo admin_form_open('reports/customer_details', 'id="action-form"'); ?>
					<div class="row">
                        <div class="col-sm-4">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <?php
								$cu = array(""=>"ALL");
								$cupp = $this->db->select("customer_id,customer")->group_by("customer_id")->get("bpas_sales")->result();
                                foreach ($cupp as $cup) {
                ...
                <div class="table-responsive">
                    <table id="PRData" class="table table-hover table-striped table-condensed">
                        ...
                        <tbody>
							...
							$this->db->select("customer_id,customer,SUM(bpas_sale_items.quantity) as qty")
							->join("bpas_sale_items","bpas_sale_items.sale_id=bpas_sales.id","LEFT");
							if($customer){
								$this->db->where("bpas_sales.customer_id",$customer);
							}
							if($reference){
							...
							if($from_date && $to_date){
								$this->db->where('bpas_sales.date >="'.$from_date.' 00.00" AND bpas_sales.date<="'.$to_date.' 23.59"');
							}
							if($warehouse){
								$this->db->where("bpas_sales.warehouse_id",$warehouse);
							}else{
								if($wid){
									$this->db->where("bpas_sales.warehouse_id IN ($wid)");
								}
							}
							if($category_id){
								$this->db->join('bpas_products', 'bpas_products.id = bpas_sale_items.product_id', 'left');
								$this->db->where("bpas_products.category_id", $category_id);
							}
							$this->db->group_by("customer_id");
							$customers = $this->db->get("bpas_sales")->result();
							...
							<tr>
								...
								<?php
									 $this->db->select("product_id,product_name,bpas_sale_items.quantity,net_unit_price,customer_id,reference_no,bpas_sales.date,'SALE' as transaction_type,unit,option_id ")->join("bpas_sales","bpas_sales.id = bpas_sale_items.sale_id","LEFT")->join("bpas_products","bpas_products.id = bpas_sale_items.product_id","LEFT");
									if($reference){
										$this->db->where("reference_no",$reference);
									}
									if($customer){
									...
									if($from_date && $to_date){
										$this->db->where('bpas_sales.date >="'.$from_date.' 00.00" AND bpas_sales.date<="'.$to_date.' 23.59"');
										//$this->db->where("bpas_sales.date BETWEEN '$from_date' AND '$to_date'");
									}
									if($warehouse){
										$this->db->where("bpas_sales.warehouse_id",$warehouse);
									}else{
									...
									$this->db->select('bpas_sale_items.*, bpas_categories.name as cate_name');
									$this->db->join('bpas_categories', 'bpas_categories.id = bpas_products.category_id', 'left');									
									if($category_id){										
										$this->db->where('bpas_products.category_id', $category_id);
									}
									$sale_items = $this->db->get("bpas_sale_items")->result();
									$tqty = 0 ; 
									$amount = 0 ;
									$vqty = 0;
									$unit_name = "";
								...
								?>
									<tr>
										...
										<td ><?=$unit_name?></td>
										<td class="text-right"><?=$this->bpas->formatMoney(abs($row1->net_unit_price))?></td>
										<td class="text-right"><?=$this->bpas->formatMoney(abs($row1->quantity)*abs($row1->net_unit_price))?></td>
									</tr>
	...
	$(document).ready(function(){
		...
		*/
		...
		$('#excel').on('click', function(e){
			e.preventDefault();
				window.location.href = "<?=admin_url('reports/customersReportDetails/0/xls/'.$warehouse1.'/'.$customer1.'/'.$reference1.'/'.$product_id1.'/'.$from_date1.'/'.$to_date1)?>";
				return false;
		});
		$('#pdf').on('click', function(e){
			e.preventDefault();
				window.location.href = "<?=admin_url('reports/customersReportDetails/pdf/0/'.$warehouse1.'/'.$customer1.'/'.$reference1.'/'.$product_id1.'/'.$from_date1.'/'.$to_date1)?>";
				return false;	
		});
	});
</script>
