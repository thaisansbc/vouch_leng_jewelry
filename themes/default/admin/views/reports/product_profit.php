    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
				<?php echo admin_form_open('reports/product_profit/', 'id="action-form"'); ?>
					<div class="row">
                        <div class="col-sm-4">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('from_date', (isset($_POST['from_date']) ? $_POST['from_date'] : $this->bpas->hrsd($from_date1)), 'class="form-control date" id="from_date"'); ?>
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                <?= lang("to_date", "to_date"); ?>
                                <?php echo form_input('to_date', (isset($_POST['to_date']) ? $_POST['to_date'] : $this->bpas->hrsd($to_date1)), 'class="form-control date" id="to_date"'); ?>
                            </div>
                        </div>
                    ...
                    <table id="PRData" class="table table-hover table-striped table-condensed">
                        ...
                        <tbody>
							...
							if(is_array($warehouses)){
								foreach($warehouses as $warehouse){
										...
										foreach($products as $product){
											if(!empty($product->product_id)){
												...
												foreach($prDetails as $pr)
												{
													...
													$p_qty = $this->bpas->formatDecimal($pr->option_id ? ($pr->quantity * $pr->qty_variant) : $pr->quantity);
													$unit_name = $this->bpas->convert_unit_2_string($pr->product_id,$p_qty);
													$unit_cost_amount = $this->bpas->formatMoney($pr->unit_cost * $p_qty);
													$unit_price_amount = $this->bpas->formatMoney($pr->option_id ? ($pr->unit_price * $pr->quantity) : ($pr->unit_price * $p_qty));
													$profit = $this->bpas->formatMoney(($pr->option_id ? ($pr->unit_price * $pr->quantity) : ($pr->unit_price * $p_qty)) - ($pr->unit_cost * $p_qty));
												...
												<tr class="active">
													...
													<td class="text-right"><b><?= $this->bpas->formatDecimal($total_quantity); ?></b></td>
													<td class="text-right"><b><?= $this->bpas->formatMoney($total_price); ?></b></td>
													<td class="text-right"><b><?= $this->bpas->formatMoney($total_cost); ?></b></td>
													<td class="text-right"><b><?= $this->bpas->formatMoney($total_profit); ?></b></td>
												</tr>
												<?php 
									...
									<tr>
										...
										<td class="text-right"><b><?= $this->bpas->formatDecimal($total_quantity_by_warehouse); ?></b></td>
										<td class="text-right"><b><?= $this->bpas->formatMoney($total_price_by_warehouse); ?></b></td>
										<td class="text-right"><b><?= $this->bpas->formatMoney($total_cost_by_warehouse); ?></b></td>
										<td class="text-right"><b><?= $this->bpas->formatMoney($total_profit_by_warehouse); ?></b></td>
									</tr>
							...
							<tr>
								...
								<td class="text-right" style='background-color: #428BCA;color:white;text-align:right;'><span style=" font-size:17px;"><b><?= $this->bpas->formatDecimal($gqty); ?></b></span></td>
								<td class="text-right" style='background-color: #428BCA;color:white;text-align:right;'><span style=" font-size:17px;"><b><?= $this->bpas->formatDecimal($gcost); ?></b></span></td>
								<td class="text-right" style='background-color: #428BCA;color:white;text-align:right;'><span style=" font-size:17px;"><b><?= $this->bpas->formatDecimal($gprice); ?></b></span></td>
								<td class="text-right" style='background-color: #428BCA;color:white;text-align:right;'><span style=" font-size:17px;"><b><?= $this->bpas->formatMoney($gprofit); ?></b></span></td>
							</tr>
	...
	$(document).ready(function(){
		...
		*/
		...
		$('#excel').on('click', function (e) {
            ...
            e.preventDefault();
                window.location.href = "<?= admin_url('reports/inventory_profit/0/xls/'.$reference1.'/'.$wahouse_id1.'/'.$product_id1.'/'.$from_date1.'/'.$to_date1.'/'.$stockType1.'/'.$cate_id1.'/'.$biller1) ?>";
                return false;
        });
        $('#pdf').on('click', function (e) {
            e.preventDefault();
                window.location.href = "<?= admin_url('reports/inventory/pdf/0'.$reference1.'/'.$wahouse_id1.'/'.$product_id1.'/'.trim($from_date1).'/'.trim($to_date1).'/'.$stockType1.'/'.$cate_id1.'/'.$biller1) ?>";
                return false;  
        });
	});
</script>
