<script>
    $(document).ready(function () {
        var oTable = $('#PQData1').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('reports/getPublicChargeAlerts/'.$warehouse_id) ?>',
                ...
                <?php if (!empty($warehouses) && ($Owner || $Admin)) { ?>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right" class="tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li>
                                <a href="<?= admin_url('reports/public_charge_alerts') ?>">
							...
							if(is_array($warehouses)){
								foreach ($warehouses as $warehouse) {
									echo '<li ' . ($warehouse_id && $warehouse_id == $warehouse->id ? 'class="active"' : '') . '><a href="' . admin_url('reports/public_charge_alerts/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
								}
							}
                            ?>
                        </ul>
            ...
            <div class="col-lg-12">
                ...
                <div class="table-responsive">
                    <table id="PQData1" cellpadding="0" cellspacing="0" border="0"
                           class="table table-condensed table-hover table-striped dfTable reports-table">
                        <thead>
                        <tr class="active">
