<script type="text/javascript">
	var test = '<?=$this->session->userdata('remove_polsr');?>';
     if (test == '1') { 
		...
		// $this->bpas->unset_data('remove_pols');
    ...
    $(document).ready(function () {
		...
		$("#add_item").autocomplete({
            ...
            source: function (request, response) {
				...
				if($.isNumeric(test)){
					$.ajax({
						type: 'get',
						url: '<?= admin_url('purchases/suggests'); ?>',
						dataType: "json",
						data: {
							term: request.term,
            ...
            response: function (event, ui) {
                if ($(this).val().length >= 16 && ui.content[0].id == 0) {
                   ...
                   bootbox.alert('<?= lang('no_match_found') ?>', function () {
					...
					//$(location).attr("href", "<?= admin_url('products/add'); ?>");
                    //$(this).removeClass('ui-autocomplete-loading');
                    // $(this).val('');
                }
            },
        ...
        $(document).on('click', '#addItemManually', function (e) {
            ...
            $.ajax({
                ...
                url: site.admin_url + "products/addByAjax",
						...
						<?php } else if($this->session->userdata('warehouse_id')){ ?>
							<div class="col-md-4">
								<div class="form-group">
									...
									echo form_dropdown('warehouse', '', (isset($_POST['warehouse']) ? $_POST['warehouse'] : $this->session->userdata('warehouse_id')), 'id="powarehouse_" class="form-control input-tip select" data-placeholder="' . lang("select") . ' ' . lang("warehouse") . '" required="required" style="width:100%;" ');
									?>
								</div>
							</div>
						<?php } ?>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <?php if ($Owner || $Admin || $GP['suppliers-add']) { ?><div class="input-group"><?php } ?>
                                                ...
                                                <div class="input-group-addon no-print" style="padding: 2px 5px;"><a
                                                        href="<?= admin_url('suppliers/add'); ?>" id="add-supplier"
                                                        class="external" data-toggle="modal" data-backdrop="static" data-target="#myModal"><i
                                ...
                                <div class="form-group" style="margin-bottom:0;">
                                    <div class="input-group wide-tip">
										...
										// if($this->input->get('addpurrquest')){
											...
											$q = $this->db->get_where('bpas_products',array('id'=>$this->input->get('addpurrquest')),1);
                                            $pcode = $q->row();
											// $pcode = $q->row()->code;
                                        ...
                                        <div class="input-group-addon" style="padding-left: 10px; padding-right: 10px;">
											...
											<a href="<?= admin_url('purchases_request/add');?>" class="gos" ></a>
										</div>
                                        <?php } ?>
                                    </div>
                                </div>
					...
					<div class="col-md-5">
						...
						<div class="form-group all">
						  ...
						  <div class="form-group">
							...
							<?php if ($Owner || $Admin) { ?><div class="input-group"><?php } ?>
									...
									?>	
									<div class="input-group-addon no-print" style="padding: 2px 5px;"><a
											href="<?= admin_url('system_settings/add_category'); ?>" id="add_category"
											class="external" data-toggle="modal" data-backdrop="static" data-target="#myModal"><i
												class="fa fa-2x fa-plus-circle" id="addIcon"></i></a></div>
								</div>
								<?php }else{
						 ...
						 <div class="form-group all">
							...
							<?php if ($Owner || $Admin) { ?><div class="input-group"><?php } ?>
								...
								<div class="input-group-addon no-print" style="padding: 2px 5px;"><a
										href="<?= admin_url('system_settings/add_subcategory'); ?>" id="add_subcategory"
										class="external" data-toggle="modal" data-backdrop="static" data-target="#myModal"><i
											class="fa fa-2x fa-plus-circle" id="addIcon"></i></a></div>
							</div>
							<?php }else{
						...
						<div class="form-group all">
						   ...
						   <div class="input-group"> <?php
							...
							?>
									   <div class="input-group-addon no-print" style="padding: 2px 5px;"><a
										href="<?= admin_url('system_settings/add_unit'); ?>" id="add_unit"
										class="external" data-toggle="modal" data-backdrop="static" data-target="#myModal"><i
											class="fa fa-2x fa-plus-circle" id="addIcon"></i></a></div>
							</div>
						...
						<div class="form-group standard">
							...
							<?= form_input('cost', (isset($_POST['cost']) ? $_POST['cost'] : ($product ? $this->bpas->formatPurDecimal($product->cost) : '')), 'class="form-control tip" id="cost" required="required"') ?>
						</div>
						<div class="form-group all">
							<?= lang("product_price", "price") ?>
							<?= form_input('price', (isset($_POST['price']) ? $_POST['price'] : ($product ? $this->bpas->formatPurDecimal($product->price) : '')), 'class="form-control tip" id="price" required="required"') ?>
						</div>
						...
						<div class="form-group standard">
							...
							<div
								class="input-group"> <?= form_input('alert_quantity', (isset($_POST['alert_quantity']) ? $_POST['alert_quantity'] : ($product ? $this->bpas->formatQuantity($product->alert_quantity) : '')), 'class="form-control tip" id="alert_quantity"') ?>
						...
						<div class="standard">
							...
							<div class="well well-sm" id="attr-con"
								...
								<div class="table-responsive">
									<table id="attrTable" class="table table-condensed table-striped"
										...
										if ($this->input->post('attributes')) {
											...
											foreach ($product_options as $option) {
												echo '<tr class="attr"><td><input type="hidden" name="attr_name[]" value="' . $option->name . '"><span>' . $option->name . '</span></td><td class="code text-center"><input type="hidden" name="attr_warehouse[]" value="' . $option->warehouse_id . '"><input type="hidden" name="attr_wh_name[]" value="' . $option->wh_name . '"><span>' . $option->wh_name . '</span></td><td class="quantity_unit text-center"><input type="hidden" name="attr_quantity_unit[]" value="' . $this->bpas->formatQuantity($option->wh_qty) . '"><span>' . $this->bpas->formatQuantity($option->wh_qty) . '</span></td><td class="quantity text-center"><input type="hidden" name="attr_quantity[]" value="' . $this->bpas->formatQuantity($option->wh_qty) . '"><span>' . $this->bpas->formatQuantity($option->wh_qty) . '</span></td><td class="cost text-right"><input type="hidden" name="attr_cost[]" value="' . $this->bpas->formatMoneyPurchase($option->cost) . '"><span>' . $this->bpas->formatMoneyPurchase($option->cost) . '</span></td><td class="price text-right"><input type="hidden" name="attr_price[]" value="' . $this->bpas->formatMoneyPurchase($option->price) . '"><span>' . $this->bpas->formatMoneyPurchase($option->price) . '</span></span></td><td class="text-center"><i class="fa fa-times delAttr"></i></td></tr>';
											}
										}
										?></tbody>
									</table>
...
$(window).load(function(){	
	...
	function billerChange(){
		...
		$.ajax({
            url: '<?= admin_url() ?>sales/getReferenceByProject/pq/'+id,
            dataType: 'json',
            success: function(data){
                $("#poref").val(data);
    ...
    $(document).ready(function () {
        ...
        $('#supplier_price').val("<?= $product->supplier1price == 0 ? '' : $this->bpas->formatPurDecimal($product->supplier1price); ?>");
        <?php } ?>
        <?php if ($product->supplier2) { ?>
        $('#addSupplier').click();
        select_supplier('supplier_2', "<?= $product->supplier2; ?>");
        $('#supplier_2_price').val("<?= $product->supplier2price == 0 ? '' : $this->bpas->formatPurDecimal($product->supplier2price); ?>");
        <?php } ?>
        <?php if ($product->supplier3) { ?>
        $('#addSupplier').click();
        select_supplier('supplier_3', "<?= $product->supplier3; ?>");
        $('#supplier_3_price').val("<?= $product->supplier3price == 0 ? '' : $this->bpas->formatPurDecimal($product->supplier3price); ?>");
        <?php } ?>
        <?php if ($product->supplier4) { ?>
        $('#addSupplier').click();
        select_supplier('supplier_4', "<?= $product->supplier4; ?>");
        $('#supplier_4_price').val("<?= $product->supplier4price == 0 ? '' : $this->bpas->formatPurDecimal($product->supplier4price); ?>");
        <?php } ?>
        <?php if ($product->supplier5) { ?>
        $('#addSupplier').click();
        select_supplier('supplier_5', "<?= $product->supplier5; ?>");
        $('#supplier_5_price').val("<?= $product->supplier5price == 0 ? '' : $this->bpas->formatPurDecimal($product->supplier5price); ?>");
        <?php } ?>
        function select_supplier(id, v) {
            $('#' + id).val(v).select2({
                ...
                ajax: {
                    url: site.admin_url + "suppliers/suggestions",
                    dataType: 'json',
                    quietMillis: 15,
