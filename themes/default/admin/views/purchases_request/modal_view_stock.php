...
<div class="modal-dialog modal-lg no-modal-header">
    <div class="modal-content">
        <div class="modal-body">
            ...
            <?php } 
		//	echo '<center>'.$warehouse->address.'</center>';
			?>
			<center><h2>Purchase Request</h2></center>
            <hr/>

            <div class="row" style="margin-bottom:15px;">
                <div class="col-xs-4" style="display:none;">
					<div>	<strong style="margin-top:10px;"><?php echo $this->lang->line("supplier"); ?> : </strong> 
							<?= $supplier->company ? $supplier->company : $supplier->company; ?>
					</div>
					<div> <strong><?php echo $this->lang->line("contact"); ?> : </strong> 
							<?= $supplier->name ? $supplier->name : $supplier->name; ?>
					</div>
					
                    <?php
					echo '<strong>'.lang("tel") . ": </strong>" . $supplier->phone . "<br />";
               ...
               //     echo $supplier->address . "<br />" . $supplier->city . " " . $supplier->postal_code . " " . $supplier->state . "<br />" . $supplier->country;
                ...
                <div class="col-xs-6">
					<div>	<strong style="margin-top:10px;"><?php echo $this->lang->line("warehouse"); ?> : </strong> 
							<?= $warehouse->name ?>
					</div>
					<div>	<strong><?php echo $this->lang->line("project"); ?> : </strong> 
							<?= $inv->project_name ?>
					</div>
					<div>	<strong><?php echo $this->lang->line("request_date"); ?> : </strong> 
					...
					if($inv->status =="approved"){
						echo '
						<div>	<strong>'.lang("confirm_date").' : </strong> 
								'.$inv->approved_date.'
						</div>';
					}
				...
				<div class="col-xs-6 text-right">
					<p class="bold">
                        ...
                        <?php if (!empty($inv->return_purchase_ref)) {
                            ...
                            if ($inv->return_id) {
                                echo ' <a data-target="#myModal2" data-toggle="modal" href="'.admin_url('purchases_request/modal_view/'.$inv->return_id).'"><i class="fa fa-external-link no-print"></i></a><br>';
                            } else {
                                echo '<br>';
                            }
                        } 
				/*		if($inv->purchase_ref){
							echo lang("Pr_no"); ?>: <?= $inv->purchase_ref;
						}*/
						?>
                ...
                <table class="table table-hover table-striped print-table order-table">
                    ...
                    <?php if ($inv->grand_total != $inv->total) { ?>
                        <tr>
                            ...
                            ?>
                            <td class="text-right"><?= $this->bpas->formatMoney($return_purchase ? (($inv->total + $inv->product_tax)+($return_purchase->total + $return_purchase->product_tax)) : ($inv->total + $inv->product_tax)); ?></td>
                        </tr>
                    <?php } ?>
                    <?php
                    if ($return_purchase) {
                    ...
                    <?php if ($inv->order_discount != 0) {
                        echo '<tr><td colspan="' . $col . '" class="text-right">' . lang("order_discount") . ' (' . $default_currency->code . ')</td><td class="text-right">'.($inv->order_discount_id ? '<small>('.$inv->order_discount_id.')</small> ' : '') . $this->bpas->formatMoney($return_purchase ? ($inv->order_discount+$return_purchase->order_discount) : $inv->order_discount) . '</td></tr>';
                    }
                    ?>
                    ...
                    <?php if ($inv->shipping != 0) {
                        echo '<tr><td colspan="' . $col . '" class="text-right">' . lang("shipping") . ' (' . $default_currency->code . ')</td><td class="text-right">' . $this->bpas->formatMoney($inv->shipping) . '</td></tr>';
                    }
                    ?>
                    <tr>
                        <td colspan="<?= $col; ?>"
            ...
            <div class="row">
				...
				<div class="col-xs-2">
                    <div class="well- well-sm-">
                        <p>
                            <?= lang("check_by"); ?>: <br>
							<?php //= $created_by->first_name . ' ' . $created_by->last_name; ?> <br>
                            <?= lang("date"); ?>: <br>
							<?php //= $this->bpas->hrld($inv->date); ?>
                        </p>
