			...
			<div class="container">
				...
				<table class="tables">
					...
					<tbody>
						...
						//$this->bpas->print_arrays($payment->amount);
												
                        ?>
							<tr>
								<td><?=$r?></td>
								<td style="text-align:left;padding-left:30px;"><?= lang($payment->reference_no); ?></td>
								<td><?$ip = $this->bpas->formatNumber($payment->amount); 
										   $iparr = split ("\.", $ip); 									   
										   echo "$iparr[0]";									
										?>
							   </td>
								<td>
										<?$ip = $this->bpas->formatNumber($payment->amount); 
										   $iparr = split ("\.", $ip); 									   
										   echo "$iparr[1]";									
										?>
								</td>
						...
						<tr>
							...
							<td colspan="2"><?=$this->bpas->formatNumber($total);?></td>
				...
				<div class="row" style="margin-top:10px;">
					<div class="col-xs-12">
						<li>
							...
							<span><?php echo '<b>' .$this->bpas->convert_number_to_words($total) . '</b>'; ?></span>
						</li>
