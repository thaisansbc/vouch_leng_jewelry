...
var test = '<?=$this->session->userdata('remove_polso');?>';
    ...
    localStorage.setItem('ponote', '<?= str_replace(array("\r", "\n"), "", $this->bpas->decode_html($sale_order->note)); ?>');
	...
	<?php if (isset($inv)) { ?>
        ...
        localStorage.setItem('podate', '<?= $this->bpas->hrld($inv->date);?>');
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <?php
					...
					if($quote_id){
						echo admin_form_open_multipart("purchases/add_purchase_order/".$quote_id, $attrib);				
					}else{
						echo admin_form_open_multipart("purchases/add_purchase_order", $attrib);
					}
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <?= lang("date", "podate"); ?>
                                <?php echo form_input('date', (isset($_POST['date']) ? $_POST['date'] : $this->bpas->hrld(date('Y-m-d H:i:s'))), 'class="form-control input-tip datetime" id="podate" required="required"'); ?>
                                ...
                                <div class="form-group" style="margin-bottom:0;">
                                    <div class="input-group wide-tip">
										...
										// if($this->input->get('addpurrquestorder')){
											...
											$q = $this->db->get_where('bpas_products',array('id'=>$this->input->get('addpurrquestorder')),1);
                                            $pcode = $q->row();
											// $pcode = $q->row()->code;
                                        ...
                                        <div class="input-group-addon" style="padding-left: 10px; padding-right: 10px;">
                                            <a href="#" id="addManually"><i
													...
													<a href="<?= admin_url('purchases/add_purchase_order');?>" class="gos" ></a>
													</div>
                                        <?php } ?>
                                    </div>
                                </div>
...
<script type="text/javascript">
	...
	function billerChange(){
		...
		$.ajax({
            url: '<?= admin_url() ?>sales/getReferenceByProject/poa/'+id,
            dataType: 'json',
            success: function(data){
                $("#poref").val(data);
