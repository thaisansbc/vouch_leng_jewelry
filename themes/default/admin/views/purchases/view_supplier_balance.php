...
<script>
        $(document).ready(function () {
            var oTable = $('#PoRData').dataTable({
                ...
                'sAjaxSource': '<?= admin_url('purchases/getViewSupplierBalance/?v=1' .$v) ?>',
		...
		<?php 		
			echo admin_form_open('purchases/getSupplierBalance_action/'.$user_id, 'id="action-form"');
		?>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div id="form">
                            ...
                            <?php echo admin_form_open("purchases/view_supplier_balance/" . $user_id); ?>
                            <div class="row">
		...
		$(document).ready(function () {
			...
			$('body').on('click', '#combine_pay', function() {
			 if($('.checkbox').is(":checked") === false){
                ...
                $('#myModal').modal({remote: '<?=admin_url('sales/combine_payment_supplier');?>?data=' + arrItems + ''});
                $('#myModal').modal('show');
			});
			...
			$('#pdf_return').click(function (event) {
				...
				window.location.href = "<?=admin_url('reports/getPurchaseReturn/pdf/?v=1'.$v)?>";
				return false;
			});
			$('#xls_return').click(function (event) {
				event.preventDefault();
				window.location.href = "<?=admin_url('reports/getPurchaseReturn/0/xls/?v=1'.$v)?>";
				return false;
			});
			...
			$('#pdf_pro').click(function (event) {
				event.preventDefault();
				window.location.href = "<?=admin_url('reports/getPurchasedReport/pdf/?v=1'.$v)?>";
				return false;
			});
			$('#xls_pro').click(function (event) {
				event.preventDefault();
				window.location.href = "<?=admin_url('reports/getPurchasedReport/0/xls/?v=1'.$v)?>";
				return false;
			});
			...
			$('#pdf_purchase').click(function (event) {
				event.preventDefault();
				window.location.href = "<?=admin_url('reports/getPurchasesReport2/pdf/?v=1'.$v)?>";
				return false;
			});
			...
			// $('#xls_purchase_order').click(function (event) {
				// event.preventDefault();
				// window.location.href = "<?=admin_url('reports/getPurchaseOrder/0/xls/?v=1'.$v)?>";
				// return false;
			// });
			// $('#pdf_purchase_order').click(function (event) {
				// event.preventDefault();
				// window.location.href = "<?=admin_url('reports/getPurchaseOrder/pdf/?v=1'.$v)?>";
				// return false;
			// });
