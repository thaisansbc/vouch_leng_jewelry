...
$(".discount_paid").on('keyup',function() {
    ...
    if(val > balance){
		$(this).val(balance);
		$(this).closest('tr').find('.amount_paid').val(0);
		calculateSum();
        $(this).addClass('f-cus');
        return false;
    }
	$(this).closest('tr').find('.amount_paid').val(formatDecimal(balance - val));
    calculateSum();
});
...
<div class="modal-dialog">
    <div class="modal-content">
        ...
        echo admin_form_open_multipart("purchases/combine_payment_purchase", $attrib); ?>
                    ...
                    <?php if (!empty($combine_sales)) {
                        foreach ($combine_sales as $combine_sale) { ?>
                            <tr class="row<?= $combine_sale->id ?>">
								...
								<td class="col-xs-3">
									<input type="hidden" name="purchase_id[]" class="form-control" value="<?= $combine_sale->id; ?>" id="amount_">
                                    <input type="text" name="amount_paid_line[]" class="amount_paid form-control" value="<?= $this->bpas->formatDecimal($combine_sale->balance); ?>">
                                </td>
    ...
    $(document).ready(function () {
		...
		$('#discount').on('keyup change', function() {
			...
			$('.balance').each(function() {
				...
				$(this).parent().find('.amount_paid').val(formatDecimal(paid));
			});
			...
			var totalAmount = 0;
				$('.balance').each(function() {
					...
					if(((amount > 0) && ((amount - paid) > 0)) || ((amount > 0) && ((amount - paid) == 0))){
						$(this).parent().find('.amount_paid').val(paid);
						amount= amount - paid;
					}else if((amount > 0) && ((amount - paid) < 0)){
						$(this).parent().find('.amount_paid').val(amount);
						amount = 0; 
					}else{
						
						if(amount < 0) {
							$(this).parent().find('.amount_paid').val(0);
							$("#amount_1").val(0);
						}else {
							$(this).parent().find('.amount_paid').val(0);
						}
