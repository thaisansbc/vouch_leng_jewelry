...
<script>
    $(document).ready(function () {
        var oTable = $('#RESLData').dataTable({
			...
			"bStateSave": true,
            'sAjaxSource': '<?= admin_url('purchases/getReturns'.($warehouse_id ? '/'.$warehouse_id : '')) ?>',
...
<?php if ($Owner || !$Admin) {
        echo admin_form_open('purchases/return_purchase_actions', 'id="action-form"');
    }
?>
<div class="box">
            ...
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
						<?php if ($Owner || $Admin || $GP['purchases-import_expanse']) { ?>
							<li>
								<a href="<?= admin_url('purchases/expense_by_csv'); ?>">
						...
						<?php if ($Owner || $Admin || $GP['purchases-import']) { ?>
							<li>
								<a href="<?= admin_url('purchases/purchase_by_csv'); ?>">
									<i class="fa fa-file-text-o"></i>
                ...
                <?php if (!empty($warehouses)) { ?>
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                            <li><a href="<?= admin_url('purchases/return_purchases') ?>"><i class="fa fa-building-o"></i> <?= lang('all_warehouses') ?></a></li>
                            <li class="divider"></li>
                            <?php
                            foreach ($warehouses as $warehouse) {
                                echo '<li><a href="' . admin_url('purchases/return_purchases/' . $warehouse->id) . '"><i class="fa fa-building"></i>' . $warehouse->name . '</a></li>';
                            }
                            ?>
                        </ul>
                    </li>
