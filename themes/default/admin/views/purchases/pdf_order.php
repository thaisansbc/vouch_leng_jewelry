    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="well well-sm">
                <div class="row bold">
                    <div class="col-xs-4"><?=lang("date");?>: <?=$this->bpas->hrld($inv->date);?>
                    ...
                    <div class="col-xs-7 pull-right text-right">
                        <?php $br = $this->bpas->save_barcode($inv->reference_no, 'code39', 35, false, $inv->id);?>
                        <img src="<?=admin_url()?>assets/uploads/barcode<?=$this->session->userdata('user_id') . $inv->id;?>.png"
                             alt="<?=$inv->reference_no?>"/>
                        <?php $this->bpas->qrcode('link', urlencode(admin_url('purchases/view/' . $inv->id)), 1, false, $inv->id);?>
                        <img src="<?=admin_url()?>assets/uploads/qrcode<?=$this->session->userdata('user_id') . $inv->id;?>.png"
                             alt="<?=$inv->reference_no?>"/>
                    </div>
                ...
                <table class="table table-hover table-striped">
                    ...
                    ?>
                        <tr>
                            ...
                            <?php
                                ...
                                if ($Settings->tax1) {
                                    echo '<td style="width: 120px; text-align:right; vertical-align:middle;">' . ($row->item_tax != 0 && $row->tax_name ? '<small>('.$row->tax_name.')</small>' : '') . ' ' . $this->bpas->formatMoney($row->item_tax) . '</td>';
                                }
                            ?>
                    ...
                    ?>
                        ...
                        <?php if ($inv->order_discount != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; ">' . lang("order_discount") .'</td><td style="text-align:right;">' . $this->bpas->formatMoney($inv->order_discount) . '</td></tr>';
                        }
                        ?>
                        <?php if ($inv->shipping != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right;">' . lang("shipping") . '</td><td style="text-align:right;">' . $this->bpas->formatMoney($inv->shipping) . '</td></tr>';
                        }
                        ?>
                        <?php if ($Settings->tax2 && $inv->order_tax != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; ">' .$inv->tax_name.'</td><td style="text-align:right;">' . $this->bpas->formatMoney($inv->order_tax) . '</td></tr>';
                        }
                        ?>
                        <tr>
                            <td colspan="<?= $col; ?>"
