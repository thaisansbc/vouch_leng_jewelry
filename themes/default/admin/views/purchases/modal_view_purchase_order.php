            ...
            <div class="well well-sm">
                <div class="row bold">
                    ...
                    <div class="col-xs-7 text-right">
                        <?php $br = $this->bpas->save_barcode($inv->reference_no, 'code39', 70, false); ?>
                        ...
                        <tr>
							...
							<?php
                                if ($Settings->product_discount) {
                                    ...
                                    echo '<td style="width: 100px; text-align:right; vertical-align:middle;">' .($dpos == true ? '<small>('.$discount.')</small>' : '').' '. $this->bpas->formatMoney($row->item_discount) . '</td>';
                                }
                                if ($Settings->tax1) {
                                    echo '<td style="width: 110px; text-align:right; vertical-align:middle;">' . ($row->item_tax != 0 && $row->tax_name ? '<small>('.$row->tax_name.')</small>' : '') . ' ' . $this->bpas->formatMoney($row->item_tax) . '</td>';
                                }
                            ?>
                    ...
                    ?>
                        <?php if ($inv->grand_total != $inv->total) { ?>
                            <tr>
                                ...
                                // }
                                ?>
                                <td style="text-align:right; padding-right:10px;"><?= $this->bpas->formatMoney($inv->total); ?></td>
                            </tr>
                        <?php } ?>

                        <?php if ($inv->order_discount != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("order_discount") . '</td><td style="text-align:right; padding-right:10px;">' . $this->bpas->formatMoney($inv->order_discount) . '</td></tr>';
                        }
                        ?>
    					<?php if ($inv->shipping != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("shipping") . '</td><td style="text-align:right; padding-right:10px;">' . abs($this->bpas->formatMoney($inv->shipping)) . '</td></tr>';
                        }
                        ?>
                        <?php if ($Settings->tax2 && $inv->order_tax != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;">'. $inv->tax_name .'</td><td style="text-align:right; padding-right:10px;">' . $this->bpas->formatMoney($inv->order_tax) . '</td></tr>';
                        }
                        ?>
                        <tr>
                            <td colspan="<?= $col; ?>"
                    ...
                    <div class="btn-group btn-group-justified">
                        <?php if ($Owner || $Admin || $GP['purchase_order-authorize']) { ?>
                           <?php if ($inv->order_status != 'completed' && $inv->status =='approved') { ?>
                            <div class="btn-group">
                                <a href="<?= admin_url('purchases/Unapproved/' . $inv->id) ?>" class="tip btn btn-primary" title="<?= lang('unapproved') ?>">
                            ...
                            <div class="btn-group">
                                <a href="<?= admin_url('purchases/update_purchases_Order/' . $inv->id) ?>" class="tip btn btn-primary" title="<?= lang('approved') ?>">
                                    <i class="fa fa-check"></i>
                        ...
                        <?php } ?>
                        <div class="btn-group">
                            <a href="<?= admin_url('purchases/view_po/' . $inv->id) ?>" class="tip btn btn-primary" title="<?= lang('view') ?>">
                        ...
                        <?php } ?>
                        <div class="btn-group">
                            <a href="<?= admin_url('purchases/tcs_invoice/' . $inv->id) ?>" 
						...
						<div class="btn-group">
                            <a href="<?= admin_url('purchases/phum_meas_purchase_order/' . $inv->id) ?>" 
                           target="_blank" class="tip btn btn-primary" title="<?= lang('email') ?>">
						...
						<div class="btn-group">
                            <a href="<?= admin_url('purchases/invoice_add_purchase_order/' . $inv->id) ?>" 
                           target="_blank" class="tip btn btn-primary" title="<?= lang('email') ?>">
                                 <i class="fa fa-download"></i>
                        ...
                        <div class="btn-group">
                            <a href="<?= admin_url('purchases/pdf_order/' . $inv->id) ?>" class="tip btn btn-primary" title="<?= lang('download_pdf') ?>">
                                <i class="fa fa-download"></i>
						...
						<?php if($Owner || $Admin || $GP['purchases_order-edit']) { ?>
                            ...
                            <div class="btn-group">
                                <a href="<?= admin_url('purchases/edit_purchase_order/' . $inv->id) ?>" class="tip btn btn-warning sledit" title="<?= lang('edit') ?>">
