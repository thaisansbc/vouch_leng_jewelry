						...
						<th>
							<table>
								<tr>
									...
									<td class="text_center" style="width:20%">
										<?= $this->bpas->qrcode('link', urlencode(admin_url('attendances/department_time_card/' . $date.'/'.$department->id.((isset($group) && $group) ? '/'.$group->id:''))), 2); ?>
									</td>
								</tr>
							</table>
						</th>
							...
							<table>
								<tr>
									<td valign="bottom" style="width:45%"><hr class="hr_title"></td>
										<td class="text_center" style="width:30%"><span style="font-size:<?= $font_size+5 ?>px"><b><i><?= lang('department_time_card') ?></i></b></span></td>
					...
					<?php
						...
						if($attendances){
							foreach($attendances as $attendance){
								...
								$attedance_array[$employee_id][$attendance->timeshift][$attendance->check_type] = $attendance->time_only;
								if($attendance->check_type=='in' && $attendance->after_time > 0){
									$attedance_late_early[$employee_id]['late'] = $attedance_late_early[$employee_id]['late'] + $attendance->after_time;
								}else if($attendance->check_type=='out' && $attendance->before_time > 0){
									$attedance_late_early[$employee_id]['early'] = $attedance_late_early[$employee_id]['early'] + $attendance->before_time;
								}
							}
						}
						$tbody = '';
						...
						foreach($employees as $employees){
							$tbody .='<tr>
											...
											<td class="text_center">'.$employees->empcode.'</td>
											<td class="text_left">'.$employees->firstname.' '.$employees->lastname.'</td>
											<td class="text_center">'.(isset($attedance_array[$employees->employee_id]['one']['in']) ? $attedance_array[$employees->employee_id]['one']['in'] : "").'</td>
											<td class="text_center">'.(isset($attedance_array[$employees->employee_id]['one']['out']) ? $attedance_array[$employees->employee_id]['one']['out'] : "").'</td>
											<td class="text_center">'.(isset($attedance_array[$employees->employee_id]['two']['in']) ? $attedance_array[$employees->employee_id]['two']['in'] : "").'</td>
											<td class="text_center">'.(isset($attedance_array[$employees->employee_id]['two']['out']) ? $attedance_array[$employees->employee_id]['two']['out'] : "").'</td>
											<td class="text_center">'.(isset($attedance_late_early[$employees->employee_id]['late']) ? $this->bpas->secTotime($attedance_late_early[$employees->employee_id]['late']) : "") .'</td>
											<td class="text_center">'.(isset($attedance_late_early[$employees->employee_id]['early']) ? $this->bpas->secTotime($attedance_late_early[$employees->employee_id]['early']) : "") .'</td>
										</tr>';		
						}
