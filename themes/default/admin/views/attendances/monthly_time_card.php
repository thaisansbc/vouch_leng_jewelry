							...
							<table>
								<tr>
									<td class="text_center" style="width:20%">
										<?php
											echo '<img height="100px" src="'.base_url().'assets/uploads/logos/' . $biller->logo.'" alt="'.$biller->name.'">';
										?>
									</td>
									...
									<td class="text_center" style="width:20%">
										<?= $this->bpas->qrcode('link', urlencode(admin_url('attendances/montly_time_card/' . $id)), 2); ?>
									</td>
								</tr>
							</table>
						</th>
							...
							<table>
								<tr>
									<td valign="bottom" style="width:55%"><hr class="hr_title"></td>
										<td class="text_center" style="width:20%"><span style="font-size:<?= $font_size+5 ?>px"><b><i><?= lang('monthly_time_card') ?></i></b></span></td>
										...
										<fieldset>
											...
											<table>
												...
												<tr>
													...
													?>
													<td style="text-align:left"> : <?= $split_month[1].'/'.$split_month[0] ?></td>
												</tr>
											</table>
										</fieldset>
									</td>
					...
					<?php
						...
						if($attendances){
							foreach($attendances as $attendance){
								...
								$attedance_array[$attedance_date][$attendance->timeshift][$attendance->check_type] = $attendance->time_only;
								if($attendance->check_type=='in' && $attendance->after_time > 0){
									$attedance_late_early[$attedance_date]['late'] = $attedance_late_early[$attedance_date]['late'] + $attendance->after_time;
								}else if($attendance->check_type=='out' && $attendance->before_time > 0){
									$attedance_late_early[$attedance_date]['early'] = $attedance_late_early[$attedance_date]['early'] + $attendance->before_time;
								}
							}
						}
						...
						$tbody = '';
						$b = 1;
						$begin = new DateTime($month.'-01');
						$end   = new DateTime(date("Y-m-t", strtotime($month.'-01')));
						for($i = $begin; $i <= $end; $i->modify('+1 day')){
							$date = $i->format("Y-m-d");
							$tbody .='<tr>
											<td class="text_center">'.$b++.'</td>
											<td class="text_center">'.$this->bpas->hrsd($date).'</td>
											<td class="text_center">'.(isset($attedance_array[$date]['one']['in']) ? $attedance_array[$date]['one']['in'] : "").'</td>
											<td class="text_center">'.(isset($attedance_array[$date]['one']['out']) ? $attedance_array[$date]['one']['out'] : "").'</td>
											<td class="text_center">'.(isset($attedance_array[$date]['two']['in']) ? $attedance_array[$date]['two']['in'] : "").'</td>
											<td class="text_center">'.(isset($attedance_array[$date]['two']['out']) ? $attedance_array[$date]['two']['out'] : "").'</td>
											<td class="text_center">'.(isset($attedance_late_early[$date]['late']) ? $this->bpas->secTotime($attedance_late_early[$date]['late']) : "").'</td>
											<td class="text_center">'.(isset($attedance_late_early[$date]['early']) ? $this->bpas->secTotime($attedance_late_early[$date]['early']) : "").'</td>
										</tr>';		
						}
