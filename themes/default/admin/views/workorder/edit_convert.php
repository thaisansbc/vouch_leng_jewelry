...
<script type="text/javascript">
	<?php if ($convert) { ?>
        localStorage.setItem('cvdate', '<?= $this->bpas->hrld($convert->date) ?>');
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <?php
					...
					echo admin_form_open_multipart("workorder/edit_convert/".$convert->id, $attrib);
                ?>
					<?php if ($Owner || $Admin || $GP['workorder-converts-date']) { ?>
						<div class="col-md-4">
		...
		function get_bom_items(){
			...
			if(bom_id > 0){
				$.ajax({
					type: "get", 
					async: true,
					url: site.base_url + "workorder/get_bom_items/",
					data : { 
							bom_id : bom_id,
