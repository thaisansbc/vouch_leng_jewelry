...
<body>
	...
	<?php
		$this->db->select('bpas_categories_note.*');
		$this->db->join('delivery_items','deliveries.id = delivery_items.delivery_id', 'left');
		$this->db->join('bpas_products','bpas_delivery_items.product_id = bpas_products.id', 'left');
		$this->db->join('bpas_categories','bpas_products.category_id = bpas_categories.id', 'left');
		$this->db->join('bpas_categories_note','bpas_categories.categories_note_id = bpas_categories_note.id', 'left');
		$this->db->where('bpas_deliveries.id',$inv->id);
				...
				<div class="row">
					<div class="table-responsive">
						<table class="table "  border="1">
							...
							<tbody>
							<?php
								$this->db->select('bpas_deliveries.*,
						        					bpas_delivery_items.product_name, bpas_products.id as product_id, bpas_products.code,bpas_delivery_items.piece,bpas_delivery_items.wpiece,delivery_items.option_id,
						        					COALESCE(SUM(bpas_delivery_items.quantity_received)) as quantity_received,
						        					bpas_companies.name,
						        				...
						        				');
								$this->db->from('deliveries');
								$this->db->join('bpas_companies','deliveries.delivery_by = bpas_companies.id','left');
								$this->db->join('delivery_items','delivery_items.delivery_id = deliveries.id', 'left');
								$this->db->join('bpas_products','delivery_items.product_id = bpas_products.id', 'left');
								$this->db->join('bpas_product_variants','delivery_items.option_id = bpas_product_variants.id', 'left');
								$this->db->join('units','bpas_products.unit = units.id', 'left');
								$this->db->join('bpas_categories','bpas_products.category_id = bpas_categories.id', 'left');
								$this->db->join('bpas_categories_note','bpas_categories.categories_note_id = bpas_categories_note.id', 'left');
								$this->db->where('bpas_deliveries.id', $inv->id);
								$this->db->where_in('bpas_categories_note.id',$cnote->id);
								...
								<tr>
									...
									<td style="text-align:center; vertical-align:middle;">
										<?php
											if($row->piece != 0){ 
												echo  $str_unit;
												//$this->bpas->print_arrays($str_unit);
											}else{ 
												echo $row->unit;
											}
										?>
									...
									<td style=" text-align:center; vertical-align:middle;">
										<?php 
											if($row->piece != 0){ 
												echo $row->piece; 
												//$this->bpas->print_arrays($inv_item);
											}else{ 
												echo $this->bpas->formatQuantity($row->quantity_received);}
										?>
									</td>
									<td style=" text-align:center; vertical-align:middle;">
										<?php
