			...
			<td>
				<table width="100%">
					<tr>
						...
						<td width="30%" style="padding-left:0px;">: <?= $this->bpas->hrld($inv->date); ?> </td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr>
			<td colspan="2">
				<div class="table-responsive">
					<table class="table table-hover table-striped print-table order-table" style="'font-family:Khmer OS'; font-size:14px;">
						...
						?>
							<tr>
								...
								<td style="text-align:right; width:100px;"><?= $row->subtotal!=0?$this->bpas->formatMoney($row->unit_price):$free; ?></td>
								<?php
								if ($Settings->tax1) {
								...
								?>
								<td style="text-align:right; width:120px;"><?= $row->subtotal!=0?$this->bpas->formatMoney($row->subtotal):$free; ?></td>
							</tr>
							<?php
							$r++;
						endforeach;
						...
						<?php if ($inv->order_discount != 0) {
							echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px; font-weight:bold;">' . lang("បញ្ចុះតម្លៃ​រួម/Order_Discount") . '</td><td style="text-align:right; padding-right:10px; font-weight:bold; padding-top:20px;">' . $this->bpas->formatMoney($inv->order_discount) . '</td></tr>';
						}
						?>
						<?php if ($Settings->tax2 && $inv->order_tax != 0) {
							echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px; font-weight:bold;">' . lang("អាករលើតម្លៃបន្ថែម ".number_format($vattin->rate)."%/VAT(".number_format($vattin->rate)."%)") . '</td><td style="text-align:right; padding-right:10px; font-weight:bold; padding-top:20px;">' . $this->bpas->formatMoney($inv->order_tax) . '</td></tr>';
						}
						?>
						<?php if ($inv->shipping != 0) {
							echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("ការ​ដឹក​ជញ្ជូន/Shipping") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . abs($this->bpas->formatMoney($inv->shipping)) . '</td></tr>';
						}
						?>
						<tr>
							<td colspan="<?= $col; ?>"
			...
			<td colspan="2">
				<table width="100%" style="font-family:'Arial'; font-size:14px;">
					<tr>
						...
						<td width="85%">: <?= $this->bpas->convert_number_to_words($this->bpas->formatMoney($inv->paid)).' dolar'; ?> </td>
					</tr>
					<!--<tr>
