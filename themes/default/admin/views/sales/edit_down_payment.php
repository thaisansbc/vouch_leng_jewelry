<?php /* $this->bpas->print_arrays($customer); */ ?>
<?php foreach($loan as $lo){
		$total_sched = $this->bpas->formatMoney($lo->interest)+$this->bpas->formatMoney($lo->principle);
...
<div class="row">
        ...
        <div class="row">
            <div class="col-sm-12 text-center">
                <div style="max-width:200px; margin: 0 auto;">
					...
					//$this->bpas->print_arrays($inv->attachment);
					
                    if(!empty($inv->attachment)){ 
						echo  '<img alt="'.$inv->attachment .'" src="' .$this->config->admin_url().'assets/images/' .$inv->attachment .'" class="avatar">';
					}else{
						echo '<img alt="'.$inv->attachment .'" src="'.$this->config->admin_url() .'assets/images/male.png" class="avatar">'; 
					}
                    ?>
                </div>
    ...
    <div class="col-sm-10">
		 ...
		 <?php $attrib = array('data-toggle' => 'validator', 'role' => 'form');
                                echo admin_form_open('sales/edit_down_payment/'.$inv->id ,$attrib);
                                    ...
                                    <div class="col-md-12">
										<table width="100%" style="line-height:31px">
											...
											<tr>
												...
												<td width="15%" rowspan="2" style="padding-left:0px;">: <?= $this->bpas->hrld($inv->date); ?> </td>
											</tr>
											<tr>
								...
								<table class="table table-hover table-striped print-table order-table">
									...
									?>
										<tr>
											...
											?>
											<td style="text-align:right; width:120px;"><?= $row->subtotal!=0?$this->bpas->formatMoney($row->subtotal):$free; 
												$total += $row->subtotal;
												?></td>
										</tr>
										<?php
									...
									<?php if ($return_sale && $return_sale->surcharge != 0) {
										echo '<tr><td></td><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("return_surcharge") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->bpas->formatMoney($return_sale->surcharge) . '</td></tr>';
									}
									?>
									<?php if ($inv->order_discount != 0) {
										echo '<tr><td></td><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("order_discount") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->bpas->formatMoney($inv->order_discount) . '</td></tr>';
									}
									?>
									<?php if ($Settings->tax2 && $inv->order_tax != 0) {
										echo '<tr><td></td><td colspan="' . $col . '" style="text-align:right; padding-right:10px;">' . lang("order_tax") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->bpas->formatMoney($inv->order_tax) . '</td></tr>';
									}
									?>
									<?php if ($inv->shipping != 0) {
										echo '<tr><td></td><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("shipping") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->bpas->formatMoney($inv->shipping) . '</td></tr>';
									}
									?>
									<tr>
										<td></td>
									...
									<tr>
										...
										<td style="text-align:right; font-weight:bold;"><?= $this->bpas->formatMoney($inv->deposit); ?></td>
									</tr>
									<?php } ?>
									<tr>
										<td></td>
										...
										<td style="text-align:right; font-weight:bold;"><?= $this->bpas->formatMoney($inv->paid-$inv->deposit); ?></td>
									</tr>
									<tr>
										<td></td>
										<td colspan="<?= $col; ?>"
							...
							<div class="col-md-0">
								<div class="col-md-4">
									...
									<div class="form-group">
										...
										<input name="grand_total" type="text" id="grand_total" value="<?= $this->bpas->formatDecimal($inv->grand_total); ?>" style="pointer-events:none"
								...
								<div class="col-md-4">
									...
									<div class="form-group">
										...
										<input name="deposit" type="text" id="deposit" value="<?= (isset($inv->deposit)?$this->bpas->formatDecimal($inv->deposit):0)?>" style="pointer-events:none"
								...
								<div class="col-md-4">
									...
									<div class="form-group">
										...
										<input name="down_payment" type="text" id="down_payment"
											   class="form-control down_payment" value="<?= $this->bpas->formatDecimal($order_down->down_amount);?>"
							...
							<div class="col-md-0">
							   <div class="col-md-4">
									...
									<div class="form-group">
										...
										<input name="priciple_loan" type="text" id="priciple_loan" value="<?=$this->bpas->formatDecimal($order_down->principle_amount);?>"  class="form-control priciple_loan" placeholder="<?= lang('priciple_loan') ?>"/>
									</div>
								</div>
							</div>
							<div class="col-md-0">
							   ...
							   <div class="col-md-4">
									...
									<div class="form-group">
										...
										<input name="loan_amount" type="text" id="loan_amount" style="pointer-events:none"
											   class="form-control loan_amount" value="<?=$this->bpas->formatDecimal($inv->grand_total - ($inv->deposit+$order_down->down_amount+$order_down->principle_amount)) ?>"
							...
							<div class="depreciation_1" >
								...
								<div class="row">
									<div class="dep_tbl" style="display:none;">
										<table border="1" width="100%" class="table table-condensed tbl_dep" id="tbl_dep1">
											<tbody>
								...
								<div class="form-group">
									<div class="dep_tbl" style="display:none;">
										<table border="1" width="100%" class="table table-condensed tbl_dep" id="tbl_dep">
											<tbody>
...
$( document ).ready(function() {	
	...
	var site = <?= json_encode(array('base_url' => admin_url(), 'settings' => $Settings, 'dateFormats' => $dateFormats)) ?> 
	...
	function principal(principal_id,amount,p_type,start_date,rate)
	{
			...
			$.ajax({
					type: 'get',
					url: '<?= admin_url('sales/getPrinciple_id'); ?>',
					dataType: "json",
					data: {
	...
	$('#export_depre').click(function () {	
		...
		$.ajax({
				type: "get",
				url: "<?= admin_url('sales/getCustomerInfo'); ?>",
		...
		var myexport  ='<table width="95%" style="line-height:31px"> '+
									...
									'<tr>'+
										...
										'<td width="50%" rowspan="2" style="padding-left:0px;">: <?= $this->bpas->hrld($inv->date); ?> </td>'+
									'</tr>'+
									'<tr>'+
										'<td width="25%" style="font-family:Khmer OS; font-size:14px;"> <?= lang('លេខអត្តសញ្ញាណកម្ម អតប  (VATTIN)');?></td>'+
										'<td width="50%">: <?= $customer->vat_no; ?></td>'+
			...
			myexport +='<table border="2px" class="table table-condensed table_shape" style="width:95%; font-family:Verdana,Geneva,sans-serif; font-size:12px; border-collapse:collapse;">'+
									'<thead>'+
										 '<tr>'+
	...
	function PopupPayments() {
		...
		var mywindow = window.open('', 'bpas_pos_print', 'height=auto,max-width=480,min-width=250px');
		...
		mywindow.document.write('<div id="wrap" style="width: 794px; margin: 0 auto;">');
			mywindow.document.write('<div class="row">');
				mywindow.document.write('<div class="col-lg-12 col-xs-12"><br>');
					...
					mywindow.document.write('<table class="table" style="width: 95%;margin: 0 auto;">');
						mywindow.document.write('<tr>');
							...
							mywindow.document.write('<img src="<?= admin_url() ?>assets/uploads/logos/'+ biller_logo +'">');
							...
							mywindow.document.write('<td colspan="2"><p><b>: US$ <?php echo $this->bpas->formatMoney($rows->unit_price); ?></b></p></td>');
						mywindow.document.write('</tr>');
						mywindow.document.write('<tr>');
							mywindow.document.write('<td><p style="font-family: Moul, cursive;margin-top:8px;">ទឹកប្រាក់កក់</p></td>');
							mywindow.document.write('<td><p style="margin-top:8px;"><?php echo $this->bpas->formatMoney($payments->amount*100/$rows->unit_price); ?>%</p></td>');
							mywindow.document.write('<td><p style="margin-top:8px;">$<?php echo $this->bpas->formatMoney($payments->amount); ?></p></td>');
						mywindow.document.write('</tr>');
						mywindow.document.write('<tr>');
						...
						mywindow.document.write('<tr>');
							...
							mywindow.document.write('<td colspan="2"><p style="margin-top:8px;"><span>$</span >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span><?php echo $this->bpas->formatMoney($total_balance); ?></span></p></td>');
						mywindow.document.write('</tr>');
						mywindow.document.write('<tr>');
						...
						mywindow.document.write('<tr>');
							mywindow.document.write('<td><p style="font-family: Moul, cursive;margin-top:8px;">កាលបរិច្ឆេទ</p></td>'+
								                    '<td colspan="2"><p style="margin-top:8px;">: <?= $this->bpas->hrld($inv->date); ?></p></td>'+
								                    '<td style="border-bottom-color:white !important;"></td>'+
								                    '<td><p style="font-family: Moul, cursive;margin-top:8px;">ប្រាក់បង់ប្រចាំខែ</p></td>'+
								                    '<td colspan="2"><p style="margin-top:8px;">: US$ <?php echo $this->bpas->formatMoney($monthly_paid); ?></p></td>');
						mywindow.document.write('</tr>');
