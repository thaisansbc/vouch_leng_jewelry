 ...
 <?php
	//$sale->total = $this->bpas->convertCurrency($sale_item->currency_code, $setting->default_currency, $sale->total);
	//$this->bpas->print_arrays($services);
?>
 <style type="text/css">
    .container {
        width: 800px;
					...
					<?php
							...
							if(array($loan)) {
								foreach($loan as $pt){
										...
										$princ           = $this->bpas->formatMoney($pt->principle);
										$interest        = $this->bpas->formatMoney($pt->interest);									
										...
										$balance_moeny   = $this->bpas->formatMoney($pt->balance);
										
										$Principles		 = $this->bpas->formatMoney($pt->payment-$pt->interest);
										$interests 		 = $this->bpas->formatMoney($pt->interest);
									...
									echo '<tr class="row-data" '.(($pt->paid_amount > 0)? 'style="background-color:#B4D8E8;pointer-events:none;" disabled="disabled"':'').'>
										...
										<td class="t_c" style="padding-left:5px; padding-right:5px;">'. $this->bpas->hrsd($pt->dateline) .'</td>
										...
										$balances = str_replace(',', '', $this->bpas->formatMoney($balances));
										...
										$balances = (($pt->balance > 0)? $pt->balance : 0);
									echo '<td class="t_r" style="padding-left:5px;padding-right:5px;">'. $this->bpas->formatMoney($payment) .'</td>
										  <td class="t_r" style="padding-left:5px;padding-right:5px;">'. $this->bpas->formatMoney($balances) .'</td>
										 ';	
		...
		<div class="buttons">
			<div class="btn-group btn-group-justified no-print">
				<?php if ($this->Owner || $this->Admin || $this->permission['payment-add']) { ?> 
					<div class="btn-group">
						<a href="#" data-toggle="modal" data-backdrop="static" data-target="#myModal2" class="add_payment tip btn btn-primary" id="add_payment" title="<?= lang('add_payment') ?>">
				...
				<?php } ?>
				<div class="btn-group">
					<a href="#" data-toggle="modal" data-backdrop="static" data-target="#myModal2" class="change_date tip btn btn-primary" title="<?= lang('change_date') ?>">
						<i class="fa fa-edit"></i>
						<span class="hidden-sm hidden-xs"><?= lang('change_date') ?></span>
					</a>
				</div>
				<div class="btn-group">
					<a href="#" data-toggle="modal" data-backdrop="static" data-target="#myModal2" class="change_term tip btn btn-primary" title="<?= lang('change_term') ?>">
						<i class="fa fa-edit"></i>
						<span class="hidden-sm hidden-xs"><?= lang('change_term') ?></span>
					</a>
                </div>
				<div class="btn-group">
					<a href="#" data-toggle="modal" data-backdrop="static" data-target="#myModal2" class="pdf tip btn btn-primary" id="pdf" title="<?= lang('add_payment') ?>">
						<i class="fa fa-money"></i>
						<span class="hidden-sm hidden-xs"><?= lang('pdf') ?></span>
					</a>
				</div>
				<div class="btn-group">
					<a href="#" data-toggle="modal" data-backdrop="static" data-target="#myModal2" class="excel tip btn btn-primary" id="excel" title="<?= lang('add_payment') ?>">
						<i class="fa fa-money"></i>
	...
	$(document).ready(function() {
		...
		$('#add_payment').on('click', function() {
			...
			if($(".schedule .ch_check:checked").length > 0){
				...
				if(id){
					$(this).attr('href', "<?= admin_url('sales/add_payment_loans') ?>/"+total_payment+"/"+id+"/"+paid_amount+"/"+principle+"/"+interest+"/"+sale_id);
				}else{
					alert("Please check..");
					return false;
				}
		...
		$(".change_date").on('click',function(){
			...
			if($(".schedule .ch_check:checked").length > 0){
				...
				$(this).attr('href', "<?= admin_url('sales/changePaymentDate') ?>/"+id);
			}else {
				alert("Please check..");
				return false;
			}
		...
		$(".change_term").on('click',function(){
				var id = <?= $sale_id; ?>;
				$(this).attr('href', "<?= admin_url('sales/changeLoanTerm') ?>/"+id);
		});
