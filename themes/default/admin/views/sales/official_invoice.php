        ...
        <div class="modal-body print">
            ...
            <div class="row padding10">
                <div class="col-xs-12 text-center">
						<img style="width:120px;position:absolute;left:10px;top:0px;" src="<?=admin_url() . 'assets/uploads/logos/ptc.png'; ?>">
				...
				<table class="table receipt">
					...
					<tbody>
						...
						foreach($rows as $row){
							...
							echo '<td class="text-right">' . ($this->bpas->formatMoney($row->quantity*$row->real_unit_price)). '</td>';
							$no++;
					...
					<tfoot>
						<tr>
							...
							<td class="text-right"><?=$this->bpas->formatMoney($sub)?></td>
						</tr>	
						<tr>
							<?php
							$q = $this->db->get_where('bpas_tax_rates',array('id'=>$inv->order_tax_id),1);
							?>
							<td colspan="4" class="text-right">អាករលើតម្លៃបន្ថែម <?=$q->row()->rate?>%/VAT <?=$q->row()->rate?>%</td>
							<td class="text-right"><?=$this->bpas->formatMoney($inv->order_tax)?></td>
						</tr>
						<tr>
							<td colspan="4" class="text-right">សរុប/Total US$</td>
							<td class="text-right"><?=$this->bpas->formatMoney($sub-$inv->order_tax)?></td>
						</tr>
					</tfoot>
				</table>
