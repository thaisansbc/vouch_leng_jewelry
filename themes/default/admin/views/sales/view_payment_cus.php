...
<div id="wrapper">
        ...
        <div class="text-center">
				...
				<table style="width: 100%;">				
					<tbody style="font-size: 12px;">						
						<tr style="text-align:center;">	
							...
							<td style="vertical-align:middle;text-align:right;"><?= $this->bpas->hrsd($row->date) ?></td>	
						</tr>
						<tr style="text-align:center;">	
            ...
            <table class="table-condensed receipt no_border_btm" style="width:100%;">
                ...
                <?php
                	...
                	$this->db->select("sale_items.*, bpas_product_variants.name as variant")
						->from("sale_items")->where("sale_id",$row->sale_id)
						->join('bpas_product_variants','sale_items.option_id=bpas_product_variants.id','left')
					...
					foreach (($q2->result()) as $k=> $row2) {
						 ...
						 echo '	<td class="text-center">' . $this->bpas->formatQuantity($row2->quantity) . '</td>';
						
						 echo '	<td class="text-center"  style="text-align:right; width:65px !important">$ ' . $this->bpas->formatMoney($row2->unit_price) . '</td>';
						...
						if ($row->order_discount != 0 || $row2->item_discount != 0) {
						 ...
						 echo '<td class="text-right" style="text-align:right;">$ ' . ($this->bpas->formatMoney($row2->subtotal) == 0 ? $free:$this->bpas->formatMoney($row2->subtotal)) . '</td>';
						 ...
						 if($row2->product_type === 'combo')
						 {
						 	...
						 	$this->db->where('bpas_combo_items.product_id = "' . $row2->product_id . '"');
			...
			<table style="width: 100%; margin-top: 5px;">
				...
				<tr>
					...
					<td style="text-align:right;">$ <?=$this->bpas->formatMoney($invs->order_discount)?></td>
				</tr>
				<?php } ?>
				...
				<tr>
					...
					<td style="text-align:right;">$ <?=$this->bpas->formatMoney($invs->order_tax)?></td>
				</tr>
				<?php } ?>
				...
				<tr>
					...
					<td style="text-align:right;">$ <?= $this->bpas->formatMoney($invs->grand_total);?></td>
				</tr>
				<?php } ?>
				<?php if($old_paid > 0) { ?>
				<tr>
					...
					<td style="text-align:right;">$ <?= $this->bpas->formatMoney($old_paid);?></td>
				</tr>
				<tr>
					<td style="text-align:left;">នៅសល់</td>
					<td style="text-align:right;width:40%;">Grand Total (<?= $default_currency->code; ?>) :</td>
					<td style="text-align:right;">$ <?= $this->bpas->formatMoney(($invs->grand_total - $row->paid) + $row->amount);?></td>
				</tr>
				<?php } ?>
			</table>
			<br>
			...
			<table class="received" style="width:100%;margin-top: 5px;">
				<tr>
					...
					<th style="border-right:2px solid #000;border-top:2px solid #000;border-left:none; text-align: right;" class="text-right">$<?= $this->bpas->formatMoney($grand_total); ?></th>
				</tr>
				...
				<tr>
					...
					<th>:</th>
					<th style="border-right:2px solid #000;border-top:none;border-left:none; text-align: right;" class="text-right">$<?= $this->bpas->formatMoney($total_discount); ?></th>
				</tr>
				<?php } ?>
				...
				<tr>
					...
					<th>:</th>
					<th style="border-right:2px solid #000;border-top:none;border-left:none; text-align: right;" class="text-right">$<?= $this->bpas->formatMoney($total_cur_paid); ?></th>
				</tr>
				<?php } ?>
				...
				<tr>
					...
					<th style="border-right:2px solid #000;border-top:none; border-bottom:2px solid;border-left:none; text-align: right;" class="text-right">$<?= $this->bpas->formatMoney($grand_total - ($total_cur_paid + $total_discount)); ?></th>
				</tr>
					<?php }?>
			</table>
            ...
            <?= $row->note ? '<p class="text-left"><strong>'.lang("note").': '. $this->bpas->decode_html($row->note) . '</strong></p>' : ''; ?>
            <?= $row->staff_note ? '<p class="no-print"><strong>' . lang('staff_note') . ':</strong> ' . $this->bpas->decode_html($row->staff_note) . '</p>' : ''; ?>
        </div>
        <?php $this->bpas->qrcode('link', urlencode(admin_url('pos/view/' . $row->id)), 2); ?>
	        <div class="text-center">
		        <div style="clear:both;"></div>
		    </div>
    ...
    <a href="<?= admin_url('sales/view_customer_balance/'.$idd) ?>">
