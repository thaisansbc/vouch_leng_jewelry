	...
	<div class="container" style="width: 1024px;margin-top:8px;">
		<div class="row">
			<div class="col-sm-3 col-xs-3">
				<img style="width:120px; float: right;" src="<?=admin_url() . 'assets/uploads/logos/ptc.png'; ?>">
			</div>
				...
				<div class="table-responsive">
					<table class="table table-hover table-striped print-table order-table">
						...
						if($row->variant){
							...
							$product_unit = $row->product_unit_code;
						}
						...
						?>
							<tr>
								...
								<td style="vertical-align:middle;">
									<?= ($row->comment?$row->comment:$row->details) ?>
								</td>
								<td style="width: 150px; text-align:center; vertical-align:middle;"><?= $this->bpas->formatQuantity($row->quantity) .' x '.$this->bpas->formatQuantity($row->unit_quantity).' '.$row->product_unit_name; ?></td>
								<td style="width: 80px; text-align:center; vertical-align:middle;"><?php echo $product_unit ?></td>
								<!-- <td style="text-align:right; width:100px;"><?= $this->bpas->formatMoney($row->net_unit_price); ?></td> -->
								<td style="text-align:right; vertical-align:middle; width:100px;"><?= $row->subtotal!=0?$this->bpas->formatMoney($row->unit_price):$free; ?></td>
								<?php
								if ($Settings->tax1) {
								...
								?>
								<td style="text-align:right; vertical-align:middle; width:120px;"><?= $row->subtotal!=0?$this->bpas->formatMoney($row->subtotal):$free; ?></td>
							</tr>
							<?php
							$r++;
						endforeach;
						...
						<?php if ($inv->grand_total != $inv->total) { ?>
							<tr>
								...
								<?php
								if ($Settings->tax1) {
									echo '<td style="text-align:right; padding-top:20px;">' . $this->bpas->formatMoney($inv->product_tax) . '</td>';
								}
								if ($Settings->product_discount && $inv->product_discount != 0) {
									echo '<td style="text-align:right; vertical-align:middle;">' . $this->bpas->formatMoney($inv->product_discount) . '</td>';
								}
								?>
								<td style="text-align:right; padding-right:10px; vertical-align:middle;"><?= $this->bpas->formatMoney($inv->total + $inv->product_tax); ?></td>
							</tr>
						<?php } ?>
						<?php if ($inv->order_discount != 0) {
							echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px; font-weight:bold;">' . lang("បញ្ចុះតម្លៃ​រួម  <br/> Order_Discount") . '</td><td style="text-align:right; padding-right:10px; font-weight:bold; padding-top:20px;">' . $this->bpas->formatMoney($inv->order_discount) . '</td></tr>';
						}
						?>
						<?php if ($Settings->tax2 && $inv->order_tax != 0) {
							echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px; font-weight:bold;">' . lang("អាករលើតម្លៃបន្ថែម ".number_format($vattin->rate)."% <br/> VAT(".number_format($vattin->rate)."%)") . '</td><td style="text-align:right; padding-right:10px; font-weight:bold; padding-top:20px;">' . $this->bpas->formatMoney($inv->order_tax) . '</td></tr>';
						}
						?>
						<?php if ($inv->shipping != 0) {
							echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("ការ​ដឹក​ជញ្ជូន​ <br/> Shipping") . ' (' . $default_currency->code . ')</td><td style="text-align:right; vertical-align:middle; padding-right:10px;">' . $this->bpas->formatMoney($inv->shipping) . '</td></tr>';
						}
						?>
						<tr>
						<?php 
						...
						<td colspan="<?= $cols ?>"  rowspan="3"><?=$biller->invoice_footer;?></td>
							...
							<td style="text-align:right; padding-right:10px; font-weight:bold; padding-top:20px;"><?= $this->bpas->formatMoney($inv->grand_total); ?></td>
						</tr>
						<tr>
							<td colspan="2"
								style="text-align:right; font-weight:bold;"><?= lang("បានបង់​ <br/> Paid"); ?>
							</td>
							<td style="text-align:right; font-weight:bold; padding-top:20px;"><?= $this->bpas->formatMoney($inv->paid); ?></td>
						</tr>
						<tr>
							<td colspan="2"
								style="text-align:right; font-weight:bold;"><?= lang("សមតុល្យ​ទឹកប្រាក់ <br/> Balance"); ?>
							</td>
							<td style="text-align:right; font-weight:bold; padding-top:20px;"><?= $this->bpas->formatMoney($inv->grand_total - $inv->paid); ?></td>
						</tr>
