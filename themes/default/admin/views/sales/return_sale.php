...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        <?php if ($inv) {
        ?>
            //localStorage.setItem('redate', '<?= $this->bpas->hrld($inv->date) ?>');
        ...
        <?php if ($Owner || $Admin || $this->GP['change_date']) { ?>
        ...
        function loadItems() {
            ...
            if (localStorage.getItem('reitems')) {
                ...
                $.each(reitems, function() {
                    ...
                    tr_html = '<td><input name="sale_item_id[]" type="hidden" class="rsiid" value="' + sale_item_id + '"><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_expiry[]" type="hidden" class="rexpiry" value="' + (item_expiry != null ? item_expiry : '') + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + item_code + ')' + (item_expiry != null ? ' (' + item_expiry + ')' : '') + (sel_opt != '' ? ' (' + sel_opt + ')' : '') + '</span></td>';
                ...
                if ($inv->payment_status == 'paid' || $inv->payment_status == 'due') {
                    echo "$('#amount_1').val(formatDecimal(".$inv->paid."));";
                }
                ?>
                if (an > site.settings.bc_fix && site.settings.bc_fix != 0) {
                    $("html, body").animate({
                        scrollTop: $('#reTable').offset().top - 150
                    }, 500);
            ...
            <div class="col-lg-12">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <?php } ?>
                        <input type="hidden" name="project" value="<?= $inv->project_id ?>">
                        <div class="col-md-4">
                            <div class="form-group">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('order_discount', (isset($_POST['rediscount']) ? $_POST['rediscount'] : $inv->order_discount > 0 ? $this->bpas->formatDecimal($inv->order_discount_id) : ''), 'class="form-control input-tip" id="rediscount"'); ?>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <?= lang('shipping', 'shipping'); ?>
                                <?php echo form_input('shipping', (isset($_POST['shipping']) ? $_POST['shipping'] : $inv->shipping > 0 ? $this->bpas->formatDecimal($inv->shipping) : ''), 'class="form-control input-tip" id="shipping" required="required"'); ?>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <?php
                            if ($inv->payment_status == 'paid') {
                                echo '<div class="alert alert-success">' . lang('payment_status') . ': <strong>' . $inv->payment_status . '</strong> & ' . lang('paid_amount') . ' <strong>' . $this->bpas->formatMoney($inv->paid) . '</strong></div>';
                            } else {
                                echo '<div class="alert alert-warning">' . lang('payment_status_not_paid') . ' ' . lang('payment_status') . ': <strong>' . $inv->payment_status . '</strong> & ' . lang('paid_amount') . ' <strong>' . $this->bpas->formatMoney($inv->paid) . '</strong></div>';
                            }
                            ?>
                        </div>
                        <?php if (($Owner || $Admin || $GP['sales-payments']) && ($inv->paid >0 || $inv->payment_status == 'paid' || $inv->payment_status == 'partial')) {
                        ?>
                            <div id="payments">
                                <div class="col-md-12">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        <div class="input-group-addon" style="padding-left: 10px; padding-right: 10px; height:25px;">
                                                            <a href="#" id="sellGiftCard" class="tip" title="<?= lang('sell_gift_card') ?>"><i class="fa fa-credit-card"></i></a></div>
                                                    </div>
                                                </div>
