    ...
    <div id="receipt-data">
        <div class="text-center">
            ...
            <?php if ($Settings->system_management == 'project') { ?>
				<img src="<?= admin_url() . 'assets/uploads/logos/' . $Settings->logo2; ?>" alt="<?= $Settings->site_name; ?>" style="padding-right: 50px" >
			<?php } else { ?>
				<img src="<?= admin_url() . 'assets/uploads/logos/' . $biller->logo; ?>" alt="<?= $biller->company; ?>">
			<?php } ?>
			<!--<img style="width:100%;" src="<?= admin_url() . 'assets/Aylogo/logo501.jpg'; ?>"> -->
				...
				<table style="width: 100%;">				
					<tbody style="font-size: 12px;">						
						<tr style="text-align:center;">	
							...
							<td style="vertical-align:middle;text-align:right;"><?= $this->bpas->hrsd($inv->date) ?></td>	
						</tr>
						<tr style="text-align:center;">	
            ...
            <table class="table-condensed receipt no_border_btm" style="width:100%;">
                ...
                <?php
					...
					foreach ($rows as $row) {
						...
						echo '	<td class="text-center">' . $this->bpas->formatQuantity($row->quantity) . '</td>';
						
						echo '	<td class="text-center"  style="text-align:right; width:65px !important">$ ' . $this->bpas->formatMoney($row->unit_price) . '</td>';
						...
						echo '<td class="text-right">$ ' . ($this->bpas->formatMoney($row->subtotal) == 0 ? $free:$this->bpas->formatMoney($row->subtotal)) . '</td>';
						...
						{
							$this->db->select('*, (select name from bpas_products p where p.id = bpas_combo_items.product_id) as p_name ');
							$this->db->where('bpas_combo_items.product_id = "' . $row->product_id . '"');
							$comboLoop = $this->db->get('bpas_combo_items');
							$c = 1;
			...
			<table style="width: 100%; margin-top: 5px;">
				...
				<tr>
					...
					echo $this->bpas->formatMoney($inv->grand_total + $rounding); 
					}
					else { 
						 if ($inv->order_discount != 0 || $inv->order_tax != 0) {
							echo   $this->bpas->formatMoney($inv->grand_total); 
						}
						if ($inv->shipping != 0) { 
							echo $this->bpas->formatMoney($inv->shipping); 
						}			
					}*/
					echo $this->bpas->formatMoney($inv->grand_total);
					?>
				...
				if ($pos_paid < $inv->grand_total) { 
					if(count($payments) > 1){
						foreach($payments as $payment) {
							$us_paid=$this->bpas->formatMoney($payment->pos_paid);
							if($inv->other_cur_paid){
							$riel_paid=$inv->other_cur_paid . '  ៛' ; 
							}else{}
						}
				}else{
					$us_paid=$this->bpas->formatMoney($pos_paid); 
							if($inv->other_cur_paid){
					$riel_paid=$inv->other_cur_paid . '  ៛' ; 
							}else{}
					}
			...
			<tr style="display:none;">
				...
				<th style="display:none;">
					...
					?>
					<?= lang("qty"); ?>= (<?=$this->bpas->formatQuantity($total_quantity)?>)
				</th>
				<th  style="display:none;"colspan="<?=$total_col?>" class="text-right"><?= lang("total"); ?></th>
				<th  style="display:none;"class="text-right"><?= $this->bpas->formatMoney($inv->grand_total); ?></th>
			</tr>
				
                <?php
                if ($inv->order_tax != 0) {
                    echo '<tr  style="display:none;"><th colspan="' . $colspan . '" class="text-right">' . lang("tax") . '</th>';
					echo '<th style="display:none;" class="text-right">' . $this->bpas->formatMoney($inv->order_tax) . '</th></tr>';
                }
                ...
                if ($pos_settings->rounding){ 
                    $round_total = $this->bpas->roundNumber($inv->grand_total, $pos_settings->rounding);
                    $rounding = $this->bpas->formatMoney($round_total - $inv->grand_total);
                ?>
                    <tr style="display:none;">
                    ...
                    <tr style="display:none;">
                        ...
                        <th style="display:none;border-top:2px dotted #000" class="text-right"><?= $this->bpas->formatMoney($inv->grand_total + $rounding); ?></th>
                    </tr>
				...
				<?php if ($inv->order_discount != 0 || $inv->order_tax != 0) { ?>
                    <tr style="display:none;">
                        ...
                        <th style="border-top:2px dotted #000" class="text-right"><?= $this->bpas->formatMoney($inv->grand_total); ?></th>
                    </tr>
                <?php }
                        if ($inv->shipping != 0) { ?>
                    <tr style="display:none;">
                        <th style="border-top:2px dotted #000;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right"><?= lang("shipping"); ?></th>
                        <th style="border-top:2px dotted #000" class="text-right"><?= $this->bpas->formatMoney($inv->shipping); ?></th>
                    </tr>
                <?php }
				}
				$pos_paid = 0;
				...
				?>
						...
						<tr>
							...
							<th style="border-right:2px solid #000;border-top:2px solid #000;border-left:none;" class="text-right"><?= $this->bpas->formatMoney($payment->pos_paid); ?></th>
						</tr>
						<?php
							if($inv->other_cur_paid){
						?>
				...
				?>
					<tr>
                        ...
                        <th style="border-right:2px solid #000;border-top:2px solid #000;border-left:none;" class="text-right"><?= $this->bpas->formatMoney($pos_paid-$khr_paid); ?></th>
                    </tr>
                    <tr>
                        ...
                        <th style="border-bottom:2px solid #000;border-right:2px solid #000;border-left:none;" class="text-right"><?= $this->bpas->formatMoney($inv->other_cur_paid) ; ?></th>
                    </tr>
				...
				?>
					<tr>
						...
						<?php
							echo $this->bpas->formatMoney(($pay+$pay_kh) - $grand_totals);
							$total_us_b = $this->bpas->formatMoney(($pay+$pay_kh) - $grand_totals);
							$m_us = $this->bpas->fraction($total_us_b);
						?>
						</th>
                    </tr>
                    <tr>
				...
				<?php
					...
					if((($pos_paid+$amount_kh_to_us) - $grand_totals) != 0 || ($this->bpas->formatMoney((($pos_paid+$amount_kh_to_us) - $grand_totals)*$exchange_rate->rate )) != 0){;?>
					<tr>
                        <th style="border-top:2px dotted #000;padding-right: 12px;" class="text-right"><?= lang("change_amount_us"); ?></th>
                        <th style="border-top:2px dotted #000" class="text-right">
						<?php
							echo $this->bpas->formatMoney($payment->pos_balance);
							// echo $this->bpas->formatMoney(($pos_paid+$amount_kh_to_us) - $grand_totals);
							$total_us_b = $this->bpas->formatMoney(($pos_paid+$amount_kh_to_us) - $inv->grand_total);
							$m_us = $this->bpas->fraction($total_us_b);
						?>
						</th>
                    </tr>
				...
				?>
					<table style="width:100%;">
						...
						<tr>
							...
							<th style="border:2px solid #000;border-left:none;" class="text-right"><?= $this->bpas->formatMoney($payment->pos_paid); ?></th>
						</tr>
						<?php
							if($inv->other_cur_paid){
						?>
				...
				?>
					<tr>
                        ...
                        <th style="border:2px solid #000;border-left:none;" class="text-right"><?= $this->bpas->formatMoney($inv->paid); ?></th>
                    </tr>
					<?php
						if($inv->other_cur_paid){
					?>
				...
				?>
					<tr>
						...
						<?php
							...
							echo $this->bpas->formatMoney(abs(($pay+$money_kh) - $grand_totals));
							$total_us_b = $this->bpas->formatMoney(($pay+$money_kh) - $grand_totals);
							$m_us = $this->bpas->fraction($total_us_b);
						?>
						</th>
                    </tr>
				...
				<?php
					...
					if((($pos_paid+$amount_kh_to_us) - $grand_totals) != 0 || ($this->bpas->formatMoney((($pos_paid+$amount_kh_to_us) - $grand_totals)*(($pos_settings->in_out_rate) ? $outexchange_rate->rate:$exchange_rate->rate) )) != 0){ ?>
                    <tr>
                        <th style="border-top:2px dotted #000;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right"><?= lang("remaining_us"); ?></th>
                        <th style="border-top:2px dotted #000" class="text-right">
						<?php
							echo $this->bpas->formatMoney(abs(($pos_paid+$amount_kh_to_us) - $grand_totals));
							$total_us_b = $this->bpas->formatMoney(($pos_paid+$amount_kh_to_us) - $grand_totals);
							$m_us = $this->bpas->fraction($total_us_b);
						?>
						</th>
                    </tr>
			...
			<div style="text-align:center;font-size:11px;margin-top:10px;">
				...
				~ ~ ~ <b>SBC</b> &nbsp;&nbsp;&nbsp;&nbsp;<span style="font-size:12px;">www.SBC.com.kh</span> ~ ~ ~
			</div>
			
			<!--<div class="alert alert-success">
				<?php $rate = (($pos_settings->in_out_rate) ? $outexchange_rate->rate:$exchange_rate->rate); ?>
				<h4>US <?=$this->bpas->fraction($pos_paid)?> = <?=number_format($this->bpas->fraction($pos_paid)*$rate)?> ៛</h4>
			</div>-->
            <?php
            if ($Settings->invoice_view == 1) {
                if (!empty($tax_summary)) {
                    ...
                    foreach ($tax_summary as $summary) {
                        echo '<tr><td>' . $summary['name'] . '</td><td class="text-center">' . $summary['code'] . '</td><td class="text-center">' . $this->bpas->formatQuantity($summary['items']) . '</td><td class="text-right">' . $this->bpas->formatMoney($summary['amt']) . '</td><td class="text-right">' . $this->bpas->formatMoney($summary['tax']) . '</td></tr>';
                    }
                    echo '</tbody></tfoot>';
                    echo '<tr><th colspan="4" class="text-right">' . lang('total_tax_amount') . '</th><th class="text-right">' . $this->bpas->formatMoney($inv->product_tax) . '</th></tr>';
                    echo '</tfoot></table>';
                }
            }
            ?>

            <?= $inv->note ? '<p class="text-left"><strong>'.lang("note").': '. $this->bpas->decode_html($inv->note) . '</strong></p>' : ''; ?>
            <?= $inv->staff_note ? '<p class="no-print"><strong>' . lang('staff_note') . ':</strong> ' . $this->bpas->decode_html($inv->staff_note) . '</p>' : ''; ?>
           
        </div>
        <?php $this->bpas->qrcode('link', urlencode(admin_url('pos/view/' . $inv->id)), 2); ?>
        <div class="text-center">
		<?php 
		if($pos->display_qrcode) {
		?>
			<img
                src="<?= admin_url() ?>assets/uploads/qrcode<?= $this->session->userdata('user_id') ?>.png"
                alt="<?= $inv->reference_no ?>"/></div>
        <?php $br = $this->bpas->save_barcode($inv->reference_no, 'code39'); ?>
        <div class="text-center"><img
                src="<?= admin_url() ?>assets/uploads/barcode<?= $this->session->userdata('user_id') ?>.png"
                alt="<?= $inv->reference_no ?>"/></div>
		<?php } ?>
        <div style="clear:both;"></div>
    </div>
...
<?php } ?>
	<span class="pull-right col-xs-12">
        <a href="<?=admin_url()?>pos/view_cabon_bk/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("jones_the_grocer"); ?></a>
    </span>
    <span class="pull-right col-xs-12">
        <a href="<?=admin_url()?>pos/view_dragon_fly/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("dragon_fly"); ?></a>
    </span>
    <span class="pull-right col-xs-12">
        <a href="<?=admin_url()?>pos/view_cabon/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("tree_try"); ?></a>
    </span>
    ...
    <?php } ?>
    <span class="pull-right col-xs-12">
        <a href="<?=admin_url()?>sales/print_/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("print_a4"); ?></a>
    </span>
	<span class="pull-right col-xs-12">
        <a href="<?=admin_url()?>sales/p_invoice/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("print_invoice"); ?></a>
    </span>	
	<span class="pull-right col-xs-12">
        <a href="<?=admin_url()?>sales/print_rks/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("print_a4_rks"); ?></a>
    </span>
	<span class="pull-right col-xs-12">
        <a href="<?=admin_url()?>sales/invoice_landscap_a5/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("print_a5"); ?></a>
    </span>
    <!--<span class="pull-right col-xs-12">
        <a href="<?=admin_url()?>sales/tax_invoice/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("print_tax_invoice"); ?></a>
    </span>	-->
	<span class="pull-right col-xs-12">
        <a href="<?=admin_url()?>sales/print_receipt/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("print_receipt"); ?></a>
    </span>
	<?php if(isset($payments->id) != '') { ?>
	<span class="pull-right col-xs-12">
        <a href="<?=admin_url()?>sales/cash_receipt/<?=$payment->id?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("cash_receipt"); ?></a>
    </span>
	<?php } ?>
	<!--<span class="pull-right col-xs-12">
        <a href="<?=admin_url()?>sales/print_cabon/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("print_Cabon"); ?></a>
    </span>-->
...
<?php if ($pos_settings->java_applet) {
        ...
        <script type="text/javascript">
            ...
            <?php /*$image = $this->bpas->save_barcode($inv->reference_no);*/ ?>
            ...
            foreach ($rows as $row): ?>";
                ...
                receipt += "<?= printLine($this->bpas->formatQuantity($row->quantity)."x".$this->bpas->formatMoney($row->net_unit_price+($row->item_tax/$row->quantity)) . ":  ". $this->bpas->formatMoney($row->subtotal), ' ') . ""; ?>" + "\n";
                receipt += "<?php $r++;
            endforeach; ?>";
                receipt += "\x1B\x61\x31";
                receipt += "<?=drawLine();?>\r\n";
                receipt += "\x1B\x61\x30";
                receipt += "<?= printLine(lang("total") . ": " . $this->bpas->formatMoney($inv->total+$inv->product_tax)); ?>" + "\n";
                <?php if ($inv->order_tax != 0) { ?>
                receipt += "<?= printLine(lang("tax") . ": " . $this->bpas->formatMoney($inv->order_tax)); ?>" + "\n";
                <?php } ?>
                <?php if ($inv->total_discount != 0) { ?>
                receipt += "<?= printLine(lang("discount") . ": (" . $this->bpas->formatMoney($inv->product_discount).") ".$this->bpas->formatMoney($inv->order_discount)); ?>" + "\n";
                <?php } ?>
                <?php if($pos_settings->rounding) { ?>
                receipt += "<?= printLine(lang("rounding") . ": " . $rounding); ?>" + "\n";
                receipt += "<?= printLine(lang("grand_total") . ": " . $this->bpas->formatMoney($this->bpas->roundMoney($inv->grand_total+$rounding))); ?>" + "\n";
                <?php } else { ?>
                receipt += "<?= printLine(lang("grand_total") . ": " . $this->bpas->formatMoney($inv->grand_total)); ?>" + "\n";
                <?php } ?>
                <?php if($inv->paid < $inv->grand_total) { ?>
                receipt += "<?= printLine(lang("paid_amount") . ": " . $this->bpas->formatMoney($inv->paid)); ?>" + "\n";
                receipt += "<?= printLine(lang("due_amount") . ": " . $this->bpas->formatMoney($inv->grand_total-$inv->paid)); ?>" + "\n\n";
                <?php } ?>
                <?php
                if($payments) {
                    foreach($payments as $payment) {
                ...
                receipt += "<?= printLine(lang("amount") . ": " . $this->bpas->formatMoney($payment->pos_paid)); ?>" + "\n";
                receipt += "<?= printLine(lang("change") . ": " . ($payment->pos_balance > 0 ? $this->bpas->formatMoney($payment->pos_balance) : 0)); ?>" + "\n";
                ...
                receipt += "<?= printLine(lang("paid_by") . ": " . lang($payment->paid_by)); ?>" + "\n";
                receipt += "<?= printLine(lang("amount") . ": " . $this->bpas->formatMoney($payment->amount)); ?>" + "\n";
    ...
    ?>
                ...
                receipt += "<?= taxLine($summary['name'],$summary['code'],$this->bpas->formatQuantity($summary['items']),$this->bpas->formatMoney($summary['amt']),$this->bpas->formatMoney($summary['tax'])); ?>" + "\n";
                receipt += "<?php endforeach; ?>";
                receipt += "<?= printLine(lang("total_tax_amount") . ":" . $this->bpas->formatMoney($inv->product_tax)); ?>" + "\n";
                <?php
                    }
                }
                ?>
                receipt += "\x1B\x61\x31";
                receipt += "\n" + "<?= $biller->invoice_footer ? printText(str_replace(array('\n', '\r'), ' ', $this->bpas->decode_html($biller->invoice_footer))) : '' ?>" + "\n";
                receipt += "\x1B\x61\x30";
         ...
         <?php if (!$pos_settings->java_applet) { ?>
			$(window).load(function () {
				...
				if($Settings->auto_print){?>
					...
					document.location.href = "<?=admin_url()?>pos"; 
				<?php }	?>
			});
    <?php } ?>
            </script>
