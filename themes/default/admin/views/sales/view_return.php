            ...
            <div class="col-lg-12">
                ...
                <div class="col-xs-8 pull-right">
                    <div class="col-xs-12 text-right">
                        ...
                        <?php $this->bpas->qrcode('link', urlencode(admin_url('sales/view_return/' . $inv->id)), 2); ?>
                    ...
                    <table class="table table-hover table-striped order-table">
                        ...
                        foreach ($rows as $row):
                            ...
                            <tr>
                                ...
                                <td style="width: 120px; text-align:center; vertical-align:middle;"><?= $this->bpas->formatQuantity($row->quantity); ?></td>
                                <?php
                                if ($Settings->product_serial) {
                                    echo '<td>' . $row->serial_no . '</td>';
                                }
                                ?>
                                <td style="text-align:right; width:150px; padding-right:10px;"><?= $this->bpas->formatMoney($row->net_unit_price + ($row->item_tax / $row->quantity)); ?></td>
                                <!--<?php
                                if ($Settings->tax1) {
                                    echo '<td style="width: 150px; text-align:right; vertical-align:middle;">' . ($row->item_tax != 0 ? '<small>('.$row->tax.')</small>' : '') . ' ' . $this->bpas->formatMoney($row->item_tax) . '</td>';
                                }
                                ?>-->
                                <?php
                                if ($Settings->product_discount) {
                                    echo '<td style="width: 150px; text-align:right; vertical-align:middle;">' . ($row->item_discount != 0 ? '<small>('.$row->discount.')</small>' : '') . ' ' . $this->bpas->formatMoney($row->item_discount) . '</td>';
                                }
                                ?>
                        ...
                        <?php if ($inv->product_discount != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("product_discount") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->bpas->formatMoney($inv->product_discount) . '</td></tr>';
                        } ?>
                        <!--<?php if ($Settings->tax1 && $inv->product_tax != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("product_tax") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->bpas->formatMoney($inv->product_tax) . '</td></tr>';
                        } ?>-->
                        <?php if ($inv->order_discount != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("order_discount") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->bpas->formatMoney($inv->order_discount) . '</td></tr>';
                        } ?>
                        ...
                        <?php if ($inv->surcharge != 0) {
                            echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("surcharge") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->bpas->formatMoney($inv->surcharge) . '</td></tr>';
                        } ?>
                        <tr>
                            <td colspan="<?= $col; ?>"
						...
						<?php 
							$grand_total = 0;
							if ($inv->paid > 0) {
								echo '<tr><td colspan="' . $col . '" style="text-align:right; padding-right:10px;;">' . lang("paid") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->bpas->formatMoney($inv->paid) . '</td></tr>';
						...
						<tr>
                            ...
                            <td style="text-align:right; padding-right:10px; font-weight:bold;"><?= $this->bpas->formatMoney($grand_total); ?></td>
                        </tr>
						<?php
							} 
						?>
