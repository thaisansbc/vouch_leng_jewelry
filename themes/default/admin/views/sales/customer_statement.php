<script>
    $(document).ready(function (e) {
		...
		var oTable = $('#Loan_List').dataTable({
			...
			'bFilter': false,
            'sAjaxSource': '<?=admin_url('sales/list_house_data/'.$sale_id)?>',
        ...
        <div class="modal-body">
            ...
            <table class="table" style="width: 95%;margin: 0 auto;">
                ...
                <tr>
					...
					<b>: <?php echo $this->bpas->formatMoney($rows->order_discount);?></b></span></p>
					</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td colspan="2"><p style="margin-top:8px;"><b>: <?php echo $this->bpas->hrsd($date); ?></b></p></td>
                                ...
                                foreach($loan as $pt){
                                   	...
                                   	if($paid > 0){								 
										echo '<tr class="row-data" '.(($pt->paid_amount > 0)? 'style="pointer-events:none;" disabled="disabled"':'').'>
											...
											$balances = (($pt->balance > 0)? $pt->balance : 0);
										echo '<td class="t_r" style="padding-left:5px;padding-right:5px;text-align:center;">'. $this->bpas->formatMoney($pt->principle) .'</td>
											  <td>'.$this->bpas->formatMoney($pt->payment).'</td>
											  <td class="t_r" style="padding-left:5px;padding-right:5px;text-align:center;">'. $this->bpas->formatMoney($balances) .'</td>
											  <td>'.$this->bpas->formatMoney($payment_card).'</td>
											  <td>'.$this->bpas->formatMoney($pt->discount).'</td>
											  <td>'.$this->bpas->formatMoney($balance_pay_card).'</td>
											  <td>'.$this->bpas->hrsd($pt->paid_date).'</td>'; 
                                    }   
                                }
                            }
                        ?>
