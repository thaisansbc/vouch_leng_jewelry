	 ...
	 $(document).ready(function () {
		...
		$("#add_item").autocomplete({
            source: function (request, response) {
                ...
                $.ajax({
                    type: 'get',
                    url: '<?= admin_url('sales/suggesion_fuel_sale'); ?>',
                    dataType: "json",
                    data: {
                        term: request.term,
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <?php
					...
					echo admin_form_open_multipart("sales/edit_fuel_sale/".$id, $attrib);
                ?>
                <div class="row">
                    <div class="col-lg-12">
                        <?php if ($Owner || $Admin || $GP['sales-fuel_sale-date']) { ?>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <?= lang("date", "shdate"); ?>
                                    <?php echo form_input('date', (isset($_POST['date']) ? $_POST['date'] : $this->bpas->hrld($fuel_sale->date)), 'class="form-control input-tip datetime" id="shdate" required="required"'); ?>
                                </div>
                            </div>
                        <?php } ?>
									...
									<div class="form-group">
										...
										<div class="input-group">
											...
											<div class="input-group-addon no-print" style="padding: 2px 7px; border-left: 0;">
												<a href="<?= admin_url('system_settings/projects'); ?>" class="external" target="_blank">
													<i class="fa fa-eye" id="addIcon" style="font-size: 1.2em;"></i>
												</a>
											</div>
											<div class="input-group-addon no-print" style="padding: 2px 8px; border-left: 0;">
												<a href="<?= admin_url('system_settings/add_project'); ?>" class="external" data-toggle="modal" data-target="#myModal">
                            ...
                            <div class="col-md-12">
								...
								<div class="col-md-6">
									...
									<table class="count-money-table" style="width:100%">
										...
										<td><input value="<?= $this->bpas->formatDecimal($json_credit_amount && $json_credit_amount->USD->amount ? $json_credit_amount->USD->amount:0) ?>" name="credit_amount_usd" class="form-control credit_amount_usd text-right"/></td>
										<td class="text-center">៛</td>
										<td><input value="<?= $this->bpas->formatDecimal($json_credit_amount && $json_credit_amount->KHR->amount ? $json_credit_amount->KHR->amount:0) ?>" name="credit_amount_khr" class="form-control credit_amount_khr text-right"/></td>
									</table>
									</div>
								</div>
								<div class="col-md-6">
									<?= lang("bank_amount", "bank_amount"); ?>
									<div class="form-group">
									<?php
										$json_bank_amount = json_decode($fuel_sale->json_bank_amount);
									?>
									<table class="count-money-table" style="width:100%">
										<td class="text-center">$</td>
										<td><input value="<?= $this->bpas->formatDecimal($json_bank_amount && $json_bank_amount->USD->amount ? $json_bank_amount->USD->amount:0) ?>" name="bank_amount_usd" class="form-control bank_amount_usd text-right"/></td>
										<td class="text-center">៛</td>
										<td><input value="<?= $this->bpas->formatDecimal($json_bank_amount && $json_bank_amount->KHR->amount ? $json_bank_amount->KHR->amount:0) ?>" name="bank_amount_khr" class="form-control bank_amount_khr text-right"/></td>
									</table>
									</div>
								</div>
								<div class="col-md-6">
												...
												echo '<tr>';
													if($moneys_USD){
														...
														echo '<td class="text-center">$</td><td>
																<input name="change_usd" class="form-control change-money-usd text-right" autocomplete="off" value="'.$this->bpas->formatDecimal($change_usd).'" placeholder="'.lang("USD").'" />
															</td>';
													}
													if($moneys_KHR){
														$change_kh = ($json_total_cash_open->KHR->amount?$json_total_cash_open->KHR->amount:0);
														echo '<td class="text-center">៛</td><td>
																<input name="change_kh" class="form-control change-money-kh text-right" autocomplete="off" value="'.$this->bpas->formatDecimal($change_kh).'" placeholder="'.lang("KHR").'" />
															</td>';
													}
												echo '</tr>';
											?>
											...
											if($moneys_KHR){
												foreach($moneys_KHR as $k=> $money){
													echo '<tr>';
														if($moneys_USD){
															...
															if($moneys_USD[$k] <= 0){
																echo '<td style="width:120px;"><button style="width:100%;" class="text-right" value="'.$moneys_USD[$k].'" onClick="return false;">'.$this->bpas->formatDecimal(0).'</button></td>';
															}else{
																echo '<td style="width:120px;"><button style="width:100%;" class="btn-money-usd text-right" value="'.$moneys_USD[$k].'" onClick="return false;">'.$this->bpas->formatDecimal($moneys_USD[$k]).'</button></td>';
															}
												...
												echo '<tr>';
													if($moneys_USD){
														...
														echo '<td>$</td>';
														echo '<td colspan="2" style="font-size:18px;"><input type="text" name="total_USD" id="total_USD" class="form-control cash_submit_usd" value="'.$this->bpas->formatDecimal($total_cash_en).'" style="text-align:right;"/></td>';
													}
													if($moneys_KHR){
														$total_cash_kh = ($json_total_cash->KHR->amount?$json_total_cash->KHR->amount:0);
														echo '<td>៛</td>';
														echo '<td colspan="2" style="font-size:18px;"><input type="text" name="total_KHR" id="total_KHR" class="form-control cash_submit_khr" value="'.$this->bpas->formatDecimal($total_cash_kh).'" style="text-align:right;"/></td>';
													}
												echo '</tr>';
											}
										?>
