...
<script>
    $(document).ready(function() {
        oTable = $('#SLData').dataTable({
            ...
            'bProcessing': true,
            'bServerSide': true,
            'sAjaxSource': '<?= admin_url('sales/getSalesDeclareTax' . ($biller_id ? '/' . $biller_id : '') . '?v=1' . $v . ($this->input->get('shop') ? '&shop=' . $this->input->get('shop') : '') . ($this->input->get('attachment') ? '&attachment=' . $this->input->get('attachment') : '') . ($this->input->get('delivery') ? '&delivery=' . $this->input->get('delivery') : '')); ?>',
            ...
            'fnRowCallback': function(nRow, aData, iDisplayIndex) {
                ...
                nRow.className = "declare_invoice_link re" + aData[8];
                //if(aData[7] > aData[9]){ nRow.className = "product_link warning"; } else { nRow.className = "product_link"; }

                var action = $('td:eq(11)', nRow);              
                if (aData[5] == 'completed' && aData[9] == 'paid') {
