...
<div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel"><?php echo lang('return_deposit'); ?></h4>
        </div>
        <?php $attrib = array('data-toggle' => 'validator', 'role' => 'form');
        echo admin_form_open_multipart("rental/return_deposit/" . $deposit->id, $attrib); ?>
        <div class="modal-body">
            <p><?= lang('enter_info'); ?></p>

            <div class="row">
                <?php if ($Owner || $Admin || $GP['sale-date']) { ?>
                    <div class="col-sm-6">
                        <div class="form-group">
                            <?= lang("date", "date"); ?>
                            <?= form_input('date', (isset($_POST['date']) ? $_POST['date'] : $this->bpas->hrld($deposit->date)), 'class="form-control datetime" id="date" required="required"'); ?>
                        </div>
                    </div>
                <?php } ?>
                <div class="col-sm-6 <?= ((!$Owner && !$Admin && !$GP['reference_no']) ? 'hidden' : '') ?>">
                    <div class="form-group">
                        ...
                        <?= form_input('reference_no', (isset($_POST['reference_no']) ? $_POST['reference_no'] : $deposit->reference_no), 'class="form-control tip" id="reference_no" required="required" readonly="readonly"'); ?>
                    </div>
                </div>
                <input type="hidden" value="<?php echo $deposit->sale_id; ?>" name="sale_id"/>
            </div>
                    ...
                    <div class="col-md-12">
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="deposit">
                                    <div class="form-group">
                                        ...
                                        <input name="amount-paid" readonly value="<?= $this->bpas->formatDecimal($deposit->amount*-1); ?>" type="text"
							...
							?>
								<div class="col-sm-12">								
									<div class="form-group">
										...
										<input name="c_amount[]" rate="<?= $base_currency->rate ?>" type="text" value="<?= $amount ?>" class="form-control c_amount" readonly="readonly"/>
							...
							<div class="col-sm-6 hidden">
								<div class="form-group">
	                                <?= lang("deposit_account", "deposit_account"); ?>
	                                <?php
	                                $bank = array(
	                                	'' => lang('selected')
	                                );
	                                ...
	                                echo form_dropdown('deposit_account', $bank, ($deposit->account_code?$deposit->account_code:$this->accounting_setting->default_sale_deposit), 'class="form-control"');
	                                ?>
	                            </div>
							</div>
							<?php }?>
						...
						<div class="row cbank" style="display: none;">
							<div class="col-sm-6">
								<div class="form-group">
									...
									<input name="account_number" value="<?= $deposit->account_number ?>" type="text" id="account_number" class="form-control"/>
								</div>
							</div>
							<div class="col-sm-6">
								<div class="form-group">
									<?= lang("account_name", "account_name"); ?>
									<input name="account_name" value="<?= $deposit->account_name ?>" type="text" id="account_name" class="form-control"/>
								</div>
							</div>
						</div>
						<div class="row ccheque" style="display: none;">
							<div class="col-sm-12">
								<div class="form-group">
									...
									<input name="bank_name" value="<?= $deposit->bank_name ?>" type="text" id="bank_name" class="form-control"/>
								</div>
							</div>
							<div class="col-sm-6">
								<div class="form-group">
									<?= lang("cheque_number", "cheque_number"); ?>
									<input name="cheque_number" value="<?= $deposit->cheque_number ?>" type="text" id="cheque_number" class="form-control"/>
								</div>
							</div>
							<div class="col-sm-6">
								<div class="form-group">
									<?= lang("cheque_date", "cheque_date"); ?>
									<input name="cheque_date" value="<?= $this->bpas->hrsd($deposit->cheque_date && $deposit->cheque_date != "0000-00-00" ? $deposit->cheque_date : date("Y-m-d")) ?>" type="text" id="cheque_date" class="form-control date"/>
								</div>
							</div>
						</div>
                    </div>
        ...
        <div class="modal-footer">
            <?php echo form_submit('edit_deposit', lang('return_deposit'), 'class="btn btn-primary"'); ?>
        </div>
    </div>
    <?php echo form_close(); ?>
</div>
