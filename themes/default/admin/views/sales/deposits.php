                            ...
                            <tr class="row<?= $deposit->id ?>">
                                ...
                                <td>
                                    <div class="text-center">
                                        <a href="<?= admin_url('rental/deposit_note/' . $deposit->id) ?>"
                                           data-toggle="modal" data-target="#myModal2"><i class="fa fa-file-text-o"></i></a>
                                        <?php if ($deposit->paid_by != 'gift_card') { ?>
                                            <a href="<?= admin_url('rental/return_deposit/' . $deposit->id) ?>"
                                               data-toggle="modal" data-target="#myModal2"><i class="fa fa-reply"></i></a>
                                            <a href="<?= admin_url('rental/edit_deposit/' . $deposit->id) ?>"
                                               data-toggle="modal" data-target="#myModal2"><i class="fa fa-edit"></i></a>
                                                    
                                            <a href="#" class="po"
                                               title="<b><?= $this->lang->line("delete_deposit") ?></b>"
                                               data-content="<p><?= lang('r_u_sure') ?></p><a class='btn btn-danger' id='<?= $deposit->id ?>' href='<?= admin_url('rental/delete_deposit/' . $deposit->id) ?>'><?= lang('i_m_sure') ?></a> <button class='btn po-close'><?= lang('no') ?></button>"
