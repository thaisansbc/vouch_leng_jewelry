                ...
                <table id="CompTable" cellpadding="0" cellspacing="0" border="0"
                    ...
                    if (!empty($payments)) {
                        ...
                        foreach ($payments as $payment) { ?>
                            <tr class="row<?= $payment->id ?>">
                                ...
                                <td><?= $this->bpas->formatMoney($payment->amount) . ' ' . (($payment->attachment) ? '<a href="' . admin_url('assets/uploads/' . $payment->attachment) . '" target="_blank"><i class="fa fa-chain"></i></a>' : ''); ?></td>
                                ...
                                <td class="noprint">
                                    <div class="text-center">
										...
										<a href="<?= admin_url('sales/bill_reciept_form/' . $payment->id) ?>"
                                        ...
                                        <?php if ($payment->paid_by != 'gift_card') { ?>
                                            <a href="<?= admin_url('sales/edit_payment/' . $payment->id) ?>"
                                               data-toggle="modal" data-backdrop="static" data-target="#myModal2"><i
					...
					<tfoot>
						<tr>
							...
							<td colspan="2" class="text-left"><?php echo $this->bpas->formatMoney($total_amount) ?></td>
							<td></td>
						</tr>
					</tfoot>
                </table>
