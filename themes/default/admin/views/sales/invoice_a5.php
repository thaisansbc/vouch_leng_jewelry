		...
		<div class="container">
			...
			<div class="col-xs-12">
				...
				<div class="box2 col-xs-3">
					<div>
						<div>
							<span><b>Date</b></span><span>: <?=$this->bpas->hrsd($inv->date);?></span>
						</div>
						<div>
			...
			<table class="table table-hover table-striped" style="width: 100%;" >
				...
				<tbody style="font-size: 13px;">
					...
					<tr>
						...
						<td style="text-align:center;"><?= $this->bpas->formatQuantity($row->quantity); ?></td>
						<td style="text-align:center;">
							<?php if($row->unit_price == 0){echo "Free";}else{echo $this->bpas->formatMoney($row->unit_price);} ?>
						</td>
						<?php
							if ($Settings->product_discount){
									echo '<td class="border" style="width: 100px; text-align:center; vertical-align:middle;">' . ($rows[$i]->discount != 0 ? '<small>(' . $rows[$i]->discount . ')</small> ' : '') . $this->bpas->formatMoney($row->item_discount) . '</td>';
							}
						?>
						<td style="text-align:center;">
						<?php if($row->unit_price == 0){echo "Free";}else{ echo $row->subtotal!=0?$this->bpas->formatMoney($row->subtotal):$t; ?>&nbsp<?php echo $sym;}?> </td>		
					</tr>
					<?php
					$i++;
							...
							<div class="foot-right">
								<ul class="list-right">
									<li>
										...
										<span class="moneyshow"> <?=$this->bpas->formatMoney($total)?> $</span>
									</li>
									<li>
										<span class="frt"><?= lang("paid") ?> :</span>
										<span class="moneyshow"> <?=$this->bpas->formatMoney($inv->paid)?> $</span>
									</li>
									<li>
										<span class="frt"><?= lang("balance") ?> :</span>
										<span class="moneyshow"> <?=$this->bpas->formatMoney($inv->grand_total-$inv->paid);?> $</span>
									</li>
									<li>
										<span class="frt"><?= lang("amount_in_word") ?> :</span>
										<span class="moneyshow">
											<?= $this->bpas->convert_number_to_words($inv->grand_total-$inv->paid); ?>
										</span>
									</li>
								</ul>
