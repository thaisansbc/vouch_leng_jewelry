<?php
	//$this->bpas->print_arrays($user_name);
?>
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo admin_form_open_multipart("sales/save_edit_deliveries/".$delivery->id, $attrib)
                ?>
                <div class="row">
                    <div class="col-lg-12">
                        <?php if ($Owner || $Admin || $Settings->allow_change_date == 1) { ?>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <?= lang("date", "sldate"); ?>
                                    <?php echo form_input('date',$this->bpas->hrld($delivery->date), 'class="form-control input-tip datetime" id="sldate"'); ?>
								</div>
                            </div>
                        <?php } ?>
											...
											if (isset($quantity_recs)) {
												foreach($quantity_recs as $quantity_rec) {
													...
													echo   '<input type="hidden" class="product_name" name="product_name[]" value = "'.$quantity_rec['pname'].'" id="product_name" style="width: 	150px; height: 30px;text-align:center;">
															...
															<td id="quantity" style="text-align:center;">'.$this->bpas->formatDecimal($order_items->quantity).'</td>
															
															<td>
																<input type="text" class="piece" value ="'.$this->bpas->formatDecimal($quantity_rec['piece']).'"name="piece[]" style="width: 150px; height: 30px;text-align:center;">
																<input type="hidden" class="cur_piece" value ="'.$this->bpas->formatDecimal((($quantity_bal + $quantity_reci)/$quantity_rec['wpiece'])).'"name="cur_piece[]" style="width: 150px; height: 30px;text-align:center;">
															</td>
															<td>
																<input type="text" class="wpiece" value ="'.$this->bpas->formatDecimal($quantity_rec['wpiece']).'"name="wpiece[]" style="width: 150px; height: 30px;text-align:center; pointer-events: none;">
															</td>
															
															<td>
																<input type="hidden" value="'.$this->bpas->formatDecimal($qty).'" name="quantity[]" id="quantity-x">
																<input type="hidden" value="'.$this->bpas->formatDecimal($bqty).'" name="bquantity[]" id="bquantity">
																<input type="hidden" value="'.($unit_qty!=""?$this->bpas->formatDecimal($quantity_reci*$unit_qty->qty_unit):$this->bpas->formatDecimal($quantity_reci)).'" name="rquantity[]" id="rquantity">
																<input type="hidden" id="real_qty" value="'.$this->bpas->formatDecimal(isset($quantity_reci_)).'" name="real_qty">
																<input type="text" class="quantity_received" name="quantity_received[]" value = "'.$this->bpas->formatDecimal($quantity_reci).'" id="quantity_received" style="width: 150px; height: 30px;text-align:center;">
																<input type="hidden" class="totalQuantityReceived" name="totalQuantityReceived[]" value = "'.$this->bpas->formatDecimal($totalQtyReceived).'" id="totalQuantityReceived" style="width: 150px; height: 30px;text-align:center;">
																<input type="hidden" class="totalQtyRec" name="totalQtyRec[]" value = "'.$this->bpas->formatDecimal($totalQtyReceived - $quantity_reci).'" id="totalQtyRec" style="width: 150px; height: 30px;text-align:center;">
																<input type="hidden" class="CurrentQuantityReceived" name="CurrentQuantityReceived[]" value = "'.$this->bpas->formatDecimal($quantity_reci).'" id="CurrentQuantityReceived" style="width: 150px; height: 30px;text-align:center;">
