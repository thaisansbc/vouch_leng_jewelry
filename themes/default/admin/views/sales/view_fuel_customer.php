						...
						<th>
							<table>
								<tr>
									...
									<td class="text_center" style="width:20%">
										<?= $this->bpas->qrcode('link', urlencode(site_url('sales/view_fuel_customer/' . $fuel_customer->id)), 2); ?>
									</td>
								</tr>
							</table>
						</th>
									...
									<td style="width:60%">
										<fieldset>
											...
											<table>
												<tr>
													<td><?= lang('name') ?></td>
													<td> : <strong><?= ($customer->company !='-') ?$customer->company:$customer->name;?></strong></td>
												</tr>
												<tr>
													<td><?= lang('address') ?></td>
									...
									<td style="width:40%">
										<fieldset style="margin-left:5px !important">
											...
											<table>
												...
												<tr>
													<td><?= lang('date') ?></td>
													<td style="text-align:left"> : <?= $this->bpas->hrsd($fuel_customer->date) ?></td>
												</tr>		
												<tr>
													<td><?= lang('salesman') ?></td>
					...
					<?php
						...
						foreach ($fuel_customer_items as $fuel_customer_item){
							...
							$tbody .='<tr>
											...
											'.$truck.'
											<td class="text_center">'.$this->bpas->formatQuantity($fuel_customer_item->quantity).'</td>
											<td class="text_right">'.$this->bpas->formatMoney($fuel_customer_item->unit_price).'</td>
											<td class="text_right">'.$this->bpas->formatMoney($fuel_customer_item->subtotal).'</td>
										</tr>';		
							$i++;
						}
						$footer_colspan = 4;
						...
						$footer_rowspan = 1;
						$tfooter = '';
						$footer_note = '<td class="footer_des" rowspan="'.$footer_rowspan.'" colspan="'.$footer_colspan.'">'.$this->bpas->decode_html($fuel_customer->note).'</td>';
						$tfooter .= '<tr>
										'.$footer_note.'
										<td class="text_right"><b>'.lang("grand_total").'</b></td>
										<td class="text_right"><b>'.$this->bpas->formatMoney($fuel_customer->grand_total).'</b></td>
									</tr>';
					?>
					<tr>
						<td>
