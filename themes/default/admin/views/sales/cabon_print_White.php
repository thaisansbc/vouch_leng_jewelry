...
<div id="wrapper">
    ...
    <div id="receipt-data">
        ...
        <div class="text-center">
			...
			<div class="col-xs-4">
			<img src="<?= admin_url() . 'assets/uploads/logos/' . $biller->logo; ?>" alt="<?= $biller->company; ?>" style='float:left'> 
			</div>
			<div class="col-xs-8">
			...
			/*echo '<div class="col-sm-6">';
				...
				echo lang("time_in") . " : " . $this->bpas->hrld($inv->date) . "<br>";
				echo lang("time_out") . " : " . $this->bpas->hrld($inv->date);
			echo '</div>';*/
            ...
            <table class="table table-striped table-condensed receipt">
                ...
                <?php
					...
					foreach ($rows as $row) {
						...
						echo '<td class="text-center">' . $this->bpas->formatQuantity($row->quantity);
						
						echo '<td class="text-center">' . $this->bpas->formatMoney($row->real_unit_price) . '</td>';
						$colspan = 5;
						...
						echo '<td class="text-right">' . ($row->subtotal== 0 ? $free:$this->bpas->formatMoney($row->subtotal)) . '</td>';
						$r++;
                ...
                <tr>
					<th></th>
					<th><!--<?= lang("exchange_rate"); ?> : <?php echo $exchange_rate_kh_c->rate?number_format($exchange_rate_kh_c->rate):0 ?> ៛ | <?= lang("qty"); ?>= (<?=$this->bpas->formatQuantity($total_quantity)?>)--></th>
                    <th colspan="<?=$total_col?>" class="text-right"><?= lang("total"); ?></th>
                    <th class="text-right"><?= $this->bpas->formatMoney($inv->total + $inv->product_tax); ?></th>
                </tr>
				
                <?php
                if ($inv->order_tax != 0) {
                    echo '<tr><th colspan="' . $colspan . '" class="text-right">' . lang("tax") . '</th>';
					echo '<th class="text-right">' . $this->bpas->formatMoney($inv->order_tax) . '</th></tr>';
                }
                if ($inv->order_discount != 0) {
                    echo '<tr><th colspan="' . $colspan . '" class="text-right" style="border-top:2px dotted #ddd;padding-right: 12px;">' . lang("order_discount") . '(' . (strpos($inv->order_discount_id, '%')?$inv->order_discount_id:$inv->order_discount_id . '%') . ')</th><th style="border-top:2px dotted #ddd #ddd" class="text-right">'.$this->bpas->formatMoney($inv->order_discount) . '</th></tr>';
                }
                ...
                ?>
                    ...
                    <tr>
                        ...
                        <th style="border-top:2px dotted #000" class="text-right"><?= $this->bpas->formatMoney($inv->grand_total + $rounding); ?></th>
                    </tr>
				...
				if($pos_paid >= $inv->grand_total){
					...
					<tr>
                        ...
                        <th style="border:2px solid #000;border-left:none;" class="text-right"><?= $this->bpas->formatMoney($pos_paid); ?></th>
                    </tr>
					<!--
                    <tr>
					...
					<!--<tr>
                        ...
                        <th style="border-top:2px dotted #ddd" class="text-right"><?= $this->bpas->formatMoney($inv->other_cur_paid); ?></th>
                    </tr> -->
					<?php if((($pos_paid+$amount_kh_to_us) - $inv->grand_total) != 0 && ($this->bpas->formatMoney((($pos_paid+$amount_kh_to_us) - $inv->grand_total)*$exchange_rate->rate )) != 0){ ?>
					<tr>
						...
						<?php
							echo $this->bpas->formatMoney(($pos_paid+$amount_kh_to_us) - $inv->grand_total);
				...
				<!--	<tr>
                        ...
                        <th style="border-top:2px dotted #ddd" class="text-right"><?= $this->bpas->formatMoney((($pos_paid+$amount_kh_to_us) - $inv->grand_total)*$exchange_rate_th->rate ); ?></th>
                    </tr> -->
					<?php
					}
				}
			...
			<!--<div class="alert alert-success">
				...
				<h4>US <?=$this->bpas->fraction($m_us)?> = <?=number_format($this->bpas->fraction($m_us)*$rate)?> ៛</h4>
			</div>-->
            <?php
			/*
            if ($payments) {
                ...
                foreach ($payments as $payment) {
                    ...
                    if ($payment->paid_by == 'gift_card' && $payment->pos_paid) {
                        ...
                        echo '<td>' . lang("amount") . ': ' . $this->bpas->formatMoney($payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td>';
                        echo '<td>' . lang("balance") . ': ' . ($payment->pos_balance > 0 ? $this->bpas->formatMoney($payment->pos_balance) : 0) . '</td>';
                    }
			...
			*/
            ...
            <div class="well well-sm text-center">
                <?= $this->bpas->decode_html($biller->invoice_footer); ?>
            </div>
        </div>
...
<?php } ?>
	...
	<span class="pull-right col-xs-12">
        <a href="<?=admin_url()?>sales/invoice_landscap_a5/<?=$sid?>" target="_blank" class="btn btn-block btn-primary" ><?= lang("print_invoice"); ?></a>
    </span>
	<span class="pull-right col-xs-12">
