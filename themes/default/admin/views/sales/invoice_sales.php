<?php //$this->bpas->print_arrays($Settings);?>
                ...
                <table class="table table-striped" style="width: 100%;">
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                    ...
                                    <?php
										if ($Settings->product_discount && $inv->product_discount != 0) {
											 if (strpos($row->discount , '%')){
												echo  ($row->discount != 0 ? '<small>(' . $row->discount . ')</small> ' : '') . $this->bpas->formatMoney($row->item_discount);
											} else{
												echo $this->bpas->formatMoney($row->item_discount);
											}
										}else{
											echo $this->bpas->formatMoney($inv->product_discount);
										}
                                ...
                                <td class="text-right" style="border-top:none !important;border-bottom:none !important;">
                                    <?= "$ ".$this->bpas->formatMoney(($row->quantity)*($row->unit_price)) ?>
                                </td>
                        ...
                        <tr>
								...
								<td colspan="7" rowspan="<?= $rows; ?>" style="border-left: 1px solid #FFF !important;​border-bottom: 1px solid #FFF !important">
									...
									if ($inv->note || $inv->note != "") { ?>
										<div>
											...
											<div><?= $this->bpas->decode_html(nl2br($inv->note)); ?></div>
										</div>
									<?php
									}
									?>
                            ...
                            <td colspan="2" style="text-align:right; padding-right:10px; font-weight:bold;"><?= "$ ".$this->bpas->formatMoney($grand_total); ?></td>
                        </tr>
						<?php if($inv->order_discount != 0){ ?>
							<tr>
								<td colspan="2" style="text-align:right; font-weight:bold;"><?= lang("discount_kh"); ?>
								</td>
								<td colspan="2" style="text-align:right; padding-right:10px; font-weight:bold;"><?= "$ ".$this->bpas->formatMoney($inv->order_discount); ?></td>
							</tr>
						<?php } ?>
                        <tr>
                            <td colspan="2" style="text-align:right; font-weight:bold;"><?= lang("totalpaid_kh"); ?>
                            </td>
                            <td colspan="2" style="text-align:right; padding-right:10px; font-weight:bold;"><?= "$ ".$this->bpas->formatMoney($grand_total-$inv->total_discount); ?></td>
                        </tr>
                        <tr>
                            <td colspan="2" style="text-align:right; font-weight:bold;"><?= lang("deposit_kh"); ?>
                            </td>
                            <td colspan="2" style="text-align:right; padding-right:10px; font-weight:bold;"><?= "$ ".$this->bpas->formatMoney($inv->paid); ?></td>
                        </tr>
                        <tr>
                            <td colspan="2" style="text-align:right; font-weight:bold;"><?= lang("balance_kh"); ?>
                            </td>
                            <td colspan="2" style="text-align:right; padding-right:10px; font-weight:bold;"><?= "$ ".$this->bpas->formatMoney(($grand_total-$inv->total_discount)-$inv->paid); ?></td>
                        </tr>
                    </tbody>
                </table>
            </div>
