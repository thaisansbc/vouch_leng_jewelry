        ...
        <div class="col-lg-12">
            ...
            <div class="row padding10">
                ...
                <div class="col-xs-5 text-right"  style="float: right;font-size:14px">
					...
					<?= lang("inv_date"); ?> : <?= $this->bpas->hrld($inv->date); ?><br/>
                </div>
            </div>
            <div class="clearfix"></div>
                ...
                <table class="table table-hover table-striped" style="width: 100%;">
					...
					if($pos->show_product_code == 0) {
                        ...
                        <tr>
                            ...
                            <td style="width: 10%; text-align:center; vertical-align:middle;"><?= $this->bpas->formatQuantity($row->quantity); ?></td>
                            <?php
                            if ($Settings->product_serial) {
                                echo '<td>' . $row->serial_no . '</td>';
                            }
                            ?>
                            <td style="text-align:center; width:15%;vertical-align:middle;"><?= $row->subtotal!=0?$this->bpas->formatMoney($row->unit_price):$free; ?></td>
                            <?php
                            if ($Settings->tax1) {
                                echo '<td style="width: 8%; text-align:right; vertical-align:middle;">' . ($row->item_tax != 0 && $row->tax_code ? '<small>(' . $row->tax_code . ')</small> ' : '') . $this->bpas->formatMoney($row->item_tax) . '</td>';
                            }
                            if ($Settings->product_discount) {
                                echo '<td style="width: 7%; text-align:right; vertical-align:middle;">' . ($row->discount != 0 ? '<small>(' . $row->discount . ')</small> ' : '') . $this->bpas->formatMoney($row->item_discount) . '</td>';
                            }
                            ?>
                            <td style="vertical-align:middle; text-align:right; width:20%;"><?= $row->subtotal!=0?$this->bpas->formatMoney($row->subtotal):$free; 
                                $total += $row->subtotal;
                                ?></td>
                        </tr>
                        <?php
                    ...
                    <?php if ($inv->grand_total != $inv->total) { ?>
                        <tr>
                            ...
                            <td style="text-align:right;"><?= $this->bpas->formatMoney($total); ?></td>
                        </tr>
                    <?php } ?>
