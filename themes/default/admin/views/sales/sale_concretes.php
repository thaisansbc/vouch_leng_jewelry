...
<script>
    $(document).ready(function () {
        oTable = $('#CSData').dataTable({
            ...
            'sAjaxSource': '<?= admin_url('sales/getSaleConcretes/'.($warehouse ? $warehouse->id : 0).'/'.($biller ? $biller->id : '')) ?>',
            ...
            "aoColumns": [{"mRender": checkbox}, {"mRender": fld}, null, null, null, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat},{"mRender": pay_status, "bSortable" : false}, {"bSortable": false,"mRender": attachment}, {"bSortable": false}],
			"fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
               var gtotal = 0, rtotal = 0, paid = 0, discount=0, balance = 0;
                for (var i = 0; i < aaData.length; i++) {
                    gtotal += parseFloat(aaData[aiDisplay[i]][5]);
					rtotal += parseFloat(aaData[aiDisplay[i]][6]);
				...
				nCells[6].innerHTML = currencyFormat(parseFloat(rtotal));
			...
			{column_number: 4, filter_default_label: "[<?=lang('location');?>]", filter_type: "text", data: []},
            ...
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right" class="tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li>
                            <a href="<?= admin_url('sales/add_sale_concrete') ?>"><i class="fa fa-plus-circle"></i> <?= lang('add_sale_concrete') ?>
                            </a>
                        </li>
                        <li>
