...
border: 1px solid #000000;">
				<table id="tb3" style=" width:100%;">
					...
					<tr>
						<td>Date</td>
						<td>: <?=$this->bpas->hrsd($bill->date);?></td>
					</tr>
					<!--<tr>
						<td>lnv. No</td>
			...
			<table id="tb" style="border-collapse: collapse;text-align:center;" border="1" >
				...
				$tqty = 0;
					...
					<tr>
						<td><?=$i?></td>
						<td><?=$this->bpas->hrsd($row->date);?></td>
						<td style="text-align:left;"><?=$row->product_name?></td>
						<td><?=$this->bpas->formatDecimal($row->quantity_received);?></td>
						<td><?=$this->bpas->formatMoney($unit_price->unit_price)?> $</td>
						<td style="text-align:right;"><?=$this->bpas->formatMoney($row->quantity_received*$unit_price->unit_price)?> $</td>	
						</tr>
						<?php
						$i++;
				...
				<tr>
					...
					<td ><?=$this->bpas->formatDecimal($tqty);?></td>
					<td></td>
					
					<td  style="text-align:right;"><?=$this->bpas->formatMoney($stotal);?> $</td>
