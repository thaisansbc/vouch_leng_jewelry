        ...
        <div class="container">
            ...
            <div class="header-nav">
                ...
                <ul class="nav navbar-nav pull-right">
                    ...
                    <li class="dropdown">
                        <a class="btn borange pos-tip" id="close_register" title="<span><?=lang('close_register')?></span>" data-placement="bottom" data-html="true" data-backdrop="static" href="<?=admin_url('pos/close_register')?>" data-toggle="modal" data-backdrop="static" data-target="#myModal">
                            <i class="fa fa-times-circle"></i>
                        </a>
                    </li>
                    <li class="dropdown">
                        <a class="btn borange pos-tip" id="add_expense" title="<span><?=lang('add_expense')?></span>" data-placement="bottom" data-html="true" href="<?=admin_url('purchases/add_expense')?>" data-toggle="modal" data-backdrop="static" data-target="#myModal">
				...
				foreach($kitchen_note as $note_order){
					...
					echo '<div class="table_display">';
						echo '<div class="'.$class.' sus_sale" room="'.$note_order->note_id.'" room_name="'.$note_order->name.'"	sid="'.$note_order->id.'" price="'.$note_order->price.'" discount="'.$note_order->amount.'">';
							...
							if($note_order->suspend_note){
								if($default_img =="BAHT"){
									echo '<img alt="" src="'.base_url().'assets/uploads/bungalow-close.png">';
								}else{
									if($pos_type =="room"){
										echo '<img alt="" src="'.base_url().'assets/uploads/room_close.png">';
									}else{
										echo '<img alt="" src="'.base_url().'assets/uploads/table_close.png">';
									}
								}
							}else{
								if($default_img =="BAHT"){
									echo '<img alt="" src="'.base_url().'assets/uploads/bungalow.png">';
								}else{
									if($pos_type =="room"){
										echo '<img alt="" src="'.base_url().'assets/uploads/room.png">';
									}else{
										echo '<img alt="" src="'.base_url().'assets/uploads/table.png">';
									}
								}
							}
					echo '</div>';
						...
						//	echo '<span style="float:right;">'.$note_order->type.'</span>';
							echo '<a href="#" cu_id="'.$note_order->id.'" class="r_change"> <i class="fa fa-random"></i></a> | 
								<a href="#" cu_id="'.$note_order->id.'" class="sus-delete"><i class="fa fa-trash-o"></i></a>
							</div>';
						}else{
							echo '<div class="sub_avail">';
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i
                        class="fa fa-2x">&times;</i></button>
                <h4 class="modal-title" id="changeg_susModalLabel"><?=lang('Change Room');?></h4>
            </div>
            <div class="modal-body">
			...
			if (sid ==""){
				$.ajax({
                    ...
                    success: function (data) {
						...
						window.location.href = "<?= admin_url('table/redirect_room') ?>/" + room;
                    }
                });
...
<?php if ($this->input->get('customer')) {?>
    ...
    <?php if ($pos_settings->remote_printing == 1) { ?>
    function Popup(data) {
        var mywindow = window.open('', 'bpas_pos_print', 'height=500,width=480');
